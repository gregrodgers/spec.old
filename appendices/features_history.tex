...
\item Support was added for specifying a \code{simd} construct with C++ 
      attribute specifiers (see \specref{subsubsec:simd Construct}).

\item Support was added for specifying a declare SIMD directive with C++ 
      attribute specifiers (see \specref{subsubsec:Declare SIMD Directive}).

\item Support was added so that iterators may be defined and used in a 
      motion clause on a \code{target}~\code{update} directive (see 
      \specref{subsec:target update Construct}) or in a \code{map} clause 
...
\item Support for \code{private} and \code{firstprivate} as an argument to 
      the \code{default} clause in C and C++ was added (see  
      \specref{subsubsec:default clause}).

\item New \code{defaultmap} \plc{implicit-behavior} type of \plc{present} 
      was added, (see \specref{subsubsec:defaultmap clause}).
