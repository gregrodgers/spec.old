...
\item Iterators (see \specref{sec:iterators}) were added to express that an
      expression may expand to several expressions.

\item The \code{depend} clause was extended to support iterators (see \specref{subsec:depend Clause}).
...
\item The \code{nontemporal} clause was added to the \code{simd} construct (see \specref{subsec:simd Construct}).
...
loop (see \specref{subsec:Loop Construct}), \code{simd}
(see \specref{subsec:simd Construct}), \code{taskloop} (see
\specref{subsec:taskloop Construct}) and \code{distribute} (see
...
clause (see \specref{subsubsec:in_reduction clause}) was added to the
\code{task} (see \specref{subsec:task Construct}) and \code{target} (see
\specref{subsec:target Construct}) constructs.

\item The description of the \code{map} clause was modified to clarify how structure
members are mapped. (see \specref{subsec:map Clause}).

\item The capability to map pointer variables (C/C++) and assign them the address of
device memory that is mapped by an array section was added (see \specref{subsec:map Clause}).

\item To support getting device number, the
  \code{omp\_get\_device\_num} runtime routine (see \specref{subsec:omp_get_device_num}) was added.
...
\item Support for controlling the offloading behavior using the 
