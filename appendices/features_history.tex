...
\item The \code{if}, \code{nontemporal} and \code{order(concurrent)} clauses were added to the
...
\item New combined constructs \code{master}~\code{taskloop}
     ...
     \code{parallel}~\code{master}~\code{taskloop} (see \specref{subsec:parallel master taskloop Construct}),
     \code{master}~\code{taskloop}~\code{simd} (see \specref{subsec:master taskloop simd Construct}),
