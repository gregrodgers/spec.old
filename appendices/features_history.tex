...
\item The collapse of associated loops that are imperfectly nested loops
      was defined for the loop (see \specref{subsec:Worksharing Loop Construct}),
...
\item SIMD constructs (see \specref{subsec:SIMD Constructs}) were extended
...
\item The \code{inscan} modifier for the \code{reduction} clause (see
    \specref{subsubsec:reduction clause}) and the \code{scan} directive (see
    \specref{sec:scan Directive}) were added to support inclusive and
    exclusive scan computations.
...
\specref{subsec:SIMD Constructs}).
...
\specref{subsec:declare reduction Directive}) was added to
...
\item New reduction operators \code{min} and \code{max} were added for C and C++ (see \specref{subsec:Reduction Clauses}).
