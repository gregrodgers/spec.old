...
\item Support for array shaping (see \specref{sec:Array Shaping}) and 
      support for array sections with non-unit strides  in C and C++ 
      (see \specref{sec:Array Sections}) were added to facilitate 
      specification of discontiguous storage.

\item The \code{target update} construct (see \specref{subsec:target update
    Construct}), the \code{depend} clause (see \specref{subsec:depend Clause})
    and the \code{affinity} clause (see \specref{subsec:task Construct}) were
    extended to allow the use of shape-operators (see \specref{sec:Array
    Shaping}).

\item The \code{target update} construct (see \specref{subsec:target update
    Construct}) was modified to allow array sections that specify discontiguous
    storage.
