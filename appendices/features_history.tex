...
\item The \plc{nest-var} ICV (see \specref{subsec:ICV Descriptions}), the
...
\item The default value of the \plc{nest-var} ICV is now determined depending on
the setting of the \plc{max-active-levels-var} ICV and the default value of the
\plc{max-active-level-var} is now determined depending on the setting of the
\plc{nthreads-var} and \plc{bind-var} ICVs (see \specref{subsec:ICV
Initialization}).
...
\item The \code{omp_get_max_active_levels_supported} routine was added to query the
      number of nesting levels of parallel regions supported by the
      implementation (see \specref{subsec:omp_get_max_active_levels_supported}).
