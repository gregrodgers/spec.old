...
\item the C/C++ type \code{omp\_lock\_hint\_t} and the corresponding Fortran kind \code{omp\_hint\_hint\_kind};
\item and the lock hint constants \code{omp\_lock\_hint\_none}, \code{omp\_lock\_hint\_uncontended}, \code{omp\_lock\_hint\_contended}, \code{omp\_lock\_hint\_nonspeculative}, and \code{omp\_lock\_hint\_speculative}.
\end{itemize}
...
\item The default value of the \plc{nest-var} ICV was changed from \plc{false} 
      ...
      \code{OMP\_NESTED} environment variable (see \specref{sec:OMP_NESTED}), 
      and the \code{omp\_set\_nested} and \code{omp\_get\_nested} routines 
...
\item Predefined memory allocators were added to support different kinds of memories (see \specref{sec:Memory Allocators}). Directives, clauses and API routines to use them were also added (see \specref{sec:Memory Management Directives} and \specref{sec:Memory Management Routines}).
...
\item The \code{nontemporal} clause was added to the \code{simd} 
      construct (see \specref{subsec:simd Construct}).

\item The \code{concurrent} construct was added to enable programmers to assert
  that the iterations of a loop may be executed concurrently in any order (see
    \specref{sec:concurrent Construct}).
...
\item To support task reductions, the \code{task} (see 
      ...
      accept the the \code{in\_reduction} clause (see 
      ...
      to accept the \code{task\_reduction} clause 
...
\item To support taskloop reductions, the \code{taskloop} (see
      ...
      and \code{in\_reduction} (see \specref{subsubsec:in_reduction clause}) 
      clauses.
...
\item Support for nested \code{declare}~\code{target} constructs was added
...
\item The \code{omp\_get\_device\_num} runtime routine 
      (see \specref{subsec:omp_get_device_num}) was added.

\item The \code{omp\_set\_affinity\_format}, \code{omp\_get\_affinity\_format},
\code{omp\_display\_affinity}, and \code{omp\_capture\_affinity} routines
(see \specref{subsec:omp_set_affinity_format}, \specref{subsec:omp_get_affinity_format}, 
\specref{subsec:omp_display_affinity}, and \specref{subsec:omp_capture_affinity}),
and the \code{OMP\_DISPLAY\_AFFINITY} and \code{OMP\_AFFINITY\_FORMAT} environment
variables (see \specref{sec:OMP_DISPLAY_AFFINITY} and \specref{sec:OMP_AFFINITY_FORMAT}) 
were added to provide OpenMP thread affinity information.
...
\item Support for a tool and a debugging interface was added (see \specref{chap:ToolsSupport}).

\item Support for controlling offloading behavior with the 
      \code{OMP\_TARGET\_OFFLOAD} environment variable was added 
