...
\item Support for array shaping (see \specref{subsec:Array Shaping}) and 
      ...
      of shape-operators (see \specref{subsec:Array Shaping}); further, the 
...
\item The \code{declare}~\code{variant} directive (see
      \specref{subsec:declare variant Directive}) and the metadirective
      meta-directive (see \specref{subsec:Metadirective Meta-Directive}) were
...
\item The \textit{canonical loop form} was defined for Fortran and, for all base
...
\item SIMD directives (see \specref{subsec:SIMD Directives}) were extended
...
\item The \code{loop} construct was added to support compiler
