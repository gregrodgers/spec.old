...
\item \textbf{OpenMP context}: for the \plc{isa(isa-name-list)} trait, the accepted \plc{isa-name} values are implementation defined; 
for the \plc{arch(arch-name-list)} trait, the accepted \plc{arch-name} values are implementation defined;
for the \plc{extension(extension-name-list)} trait, the accepted \plc{extension-name} values are implementation defined
 (see \specref{subsec:OpenMP Context}).
...
\item \code{teams} \textbf{construct}: the assignment of initial threads to places and the values of the \plc{place-partition-var} and \plc{default-device-var} ICVs for each initial thread (see \specref{sec:teams Construct}) are implementation defined.
...
the order of \plc{place_nums} returned in the array is implementation defined (see \specref{sec:ompt_get_partition_place_nums_t}).
