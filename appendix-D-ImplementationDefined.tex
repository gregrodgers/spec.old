...
\plc{default-device-var} are implementation defined (see \specref{subsec:ICV Initialization}).
...
\item \textbf{Thread affinity}: With $T\leq P$, when $T$ does not divide $P$ evenly, the assignment of the 
...
implementation defined. The \plc{simd\_width} used when a \code{simd} schedule modifier is specified is implementation defined. See \specref{subsec:Loop Construct}. 
...
\item \textbf{simd construct}: the integer type (or kind, for Fortran) used to compute the iteration 
...
\code{aligned} clause is not specified, the assumed alignment is implementation defined 
(see \specref{subsec:simd Construct}). 

\item \textbf{declare simd construct}: if the \code{simdlen} clause is not specified, the number of 
concurrent arguments for the function is implementation defined. If the \code{aligned} 
clause is not specified, the assumed alignment is implementation defined (see 
\specref{subsec:declare simd Construct}). 

\item The number of loop iterations assigned to a task created from a \code{taskloop} construct is implementation defined, unless the \code{grainsize} or \code{num\_tasks} clauses are specified (see \specref{subsec:taskloop Construct}.

\item \verb#device_ptr()# \textbf{clause}: 
...
\item \textbf{teams construct}: the number of teams that are created is implementation defined but 
...
\item If no \code{dist\_schedule} clause is specified then the schedule for the \code{distribute} 
...
construct is implementation defined (see \specref{subsec:critical Construct}). 
...
\item \verb#omp_target_memcpy_rect# \textbf{routine}: 
...
\item \code{OMP\_SCHEDULE} \textbf{environment variable}: if the value of the variable does not 
...
\item \code{OMP\_PROC\_BIND} \textbf{environment variable}: if the value is not \code{true}, false, or a 
...
\item \textbf{Thread affinity policy}: if the affinity request for a \code{parallel} construct cannot be 
