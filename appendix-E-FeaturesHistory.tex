...
\item The lock API was extended with lock routines that support attaching a hint
      to a lock to select a proper lock implementation for a lock's intended usage
      by the application code (see \specref{subsec:omp_init_lock_hinted and omp_init_nest_lock_hinted}).
\end{itemize}
