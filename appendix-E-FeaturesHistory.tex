...
\item The \code{ref} and \code{val} modifiers were added to the \code{linear} clause.

\item The \verb#get_device_ptr# clause was added to the \verb#target data#
  construct, \specref{subsec:target data Construct}, and \verb#device_ptr#
  clause to the \verb#target# construct, \specref{subsec:target Construct}.

\item Target memory management routines have been added to allow explicit
  allocation, deallocation, memory transfers and memory associations (see
  \specref{sec:device_memory}).
