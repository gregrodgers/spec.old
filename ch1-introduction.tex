...
The interface covers three aspects: a) the ability to query the state of the OpenMP runtime; b) mechanisms to recreate stack traces as they were written by the users; and c) a series of callbacks that get invoked when matching events occur in the OpenMP runtime. 
...
While the ability to provide state queries and the ability to track stack traces must be implemented in any OpenMP API implementation, an implementation of the OMPT interface can provide a varying set of functionality in terms of supported callbacks. While a core set of callbacks (as described in Section~\ref{tools}) must be guaranteed, i.e., an OpenMP implementation must 
guarantee that such a callback will always occur when its corresponding 
event occurs, the support for all other callbacks is implementation defined.
%
%
...
For cases where OMPT callbacks are practical in an OpenMP implementation
without a significant performance penalty, OpenMP implementations are 
encouraged to provide callbacks where implementation-defined behaviors
are permitted.

Specifically, to support the OMPT interface, an implementation of the OpenMP API 
must

\begin{itemize}
\item provide a mapping from each 
OpenMP thread, parallel region, and task region concurrently live on the host
device to a unique 64-bit data word that can be used by a tool to store its own information
...
\item support required event callbacks for tools 
...
\item implement several async signal safe inquiry functions to retrieve information from an OpenMP runtime, and
\item initiate a callback to a tool initialization routine 
as directed by an internal control variable
