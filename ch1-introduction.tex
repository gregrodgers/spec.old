...
associated with the \code{task} \emph{directive} is not unless it is an
...
\glossarydefstart
An \emph{OpenMP thread} that that has  \emph{thread} number 0. A \emph{master
...
\emph{implicit tasks}, then executes one of those \emph{tasks} as
...
\code{parallel} construct. \emph{Implicit parallel regions} surround the whole
...
enclosing \code{teams} region, the \emph{current team}, or the \emph{encountering thread}.
