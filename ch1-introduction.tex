...
To facilitate development of high-quality, first-party\footnote{A {\em first-party}
tool runs within the address space of an application process. This
differs from a {\em third-party} tool, e.g., a debugger, which runs
as a separate process.} tools that support monitoring and performance
analysis of OpenMP programs developed using any implementation of 
the OpenMP API, the OpenMP API includes a tool interface 
known as OMPT.
The OMPT interface can be implemented entirely by a compiler,
entirely by an OpenMP runtime system, or with a hybrid strategy
that employs a mixture of compiler and runtime support.

The design of the OMPT interface provides the following: a) a tool initialization interface, b) an introspection interface that enables a tool to 
examine the state of OpenMP runtime, 
c) mechanisms to help a tool reconstruct user-level call stacks from implementation-level state, 
d) a callback interface that enables a tool to receive notification of OpenMP events, and e) an interface for tracing OpenMP events on devices.
...
With the exception of one routine for tool control (described in Section~\ref{sec:tool_tcontrol}),
all functions in the OMPT interface are intended for use only by tools rather than by
applications.  For that reason, a Fortran binding is provided only 
for the application-facing tool control routine;
all other OMPT functionality is described with C syntax only.
%functions have only a C binding. 
...
While all implementations of the OpenMP API must support the OMPT
interface, OpenMP implementations may differ with respect to which
...
% the value of an OpenMP environment variable (\code{OMP\_TOOL}) 
and provide a function to register tool callbacks.

\item support required tool callbacks for events 
associated with threads, parallel regions, tasks, target regions, 
invocation of the user-level tool control routine, and runtime 
shutdown,
...
\item implement several async signal safe inquiry functions to retrieve information from an OpenMP runtime, 
...
device to a unique 64-bit data word that can be used by a tool to store its own information, and
%,\footnote{\color{red}Does this adequately reflect the data design? Notice that {\em unique} 
%is different for this bullet and next.}
\item associate a unique numerical ID with each OpenMP thread, 
parallel region, and task region that executes on any device 
other than the host, 
% A minimal implementation may reuse the task ID required by OpenMP for nested locks,
\end{itemize}
