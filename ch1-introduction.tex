...
With the exception of one routine for tool control (described in Section~\ref{sec:tool_control}),
...
all other OMPT functions have only a C binding. 
...
The OMPT interface provides three tiers of support. 
...
device to a unique 64-bit data word that can be used by a tool,\footnote{\color{red}Does this adequately reflect the data design? Notice that {\em unique} 
...
\item maintain pointers into the stack for each OpenMP thread to distinguish frames for user procedures from frames for OpenMP runtime routines,\footnote{\color{red}Do we need frame object in target trace records? I think not.}
...
\item Chapter \ref{chap:ToolsSupport} ``Tools Support''
