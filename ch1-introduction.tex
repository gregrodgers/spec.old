...
\glossarydefstart
A task that is generated by a \code{target}, \code{target enter data}, \code{target exit data} or \code{target update} construct.
\glossarydefend
...
The \emph{target task} region encloses the implicit \code{parallel} region that surrounds the
\code{target} region.  The \emph{target task} is complete after the execution of the 
\code{target} region is complete.

If the target device does not exist or the implementation does not support the target device, 
all \code{target} regions associated with that device are executed by the host device.
