...
the OpenMP API, the OpenMP API includes an interface for tools 
known as OMPT.
...
The OMPT interface provides three tiers of feature sets.
The first tier is a minimal set of features needed to support tools that
employ asynchronous sampling to measure application performance.
A second tier of callback interfaces can be used in conjunction with 
a performance tool that employs asynchronous sampling to help shift 
blame for runtime costs from symptoms (e.g., idle threads) to causes (e.g., serial work executed by a 
single thread).
A third tier of callback interfaces provides support for trace-based monitoring of 
runtime events.  OMPT can be implemented entirely by a compiler,
entirely by an OpenMP runtime system, or with a hybrid strategy
that employs a mixture of compiler and runtime support.

With the exception of one routine for tool control (described in Section~\ref{sec:tconctrol}),
...
for the single application-facing tool control routine;
all other OMPT functions have only a C binding. 

In some cases, OMPT may enable a tool to observe performance
...
of tools using OMPT to observe such differences does not constrain
implementations of the OpenMP API in any way. 

OMPT has a small set of mandatory features that provide a common foundation for all performance tools. In addition, OMPT defines a set of optional features that can be used tools to gather extra information about a program execution. All optional features in OMPT are explicitly marked. Implementations are encouraged to implement these features where feasible to support richer tools, but are considered compliant if they provide only only the mandatory features.

A minimally compliant implementation of OMPT in the OpenMP API must 

\begin{itemize}
\item maintain a unique numerical ID per OpenMP thread, parallel region, and task region, 
...
\item support callbacks for tools when the following events occur:  thread begin/end, parallel region begin/end, task region begin/end, invocation of the user-level tool control routine, and runtime shutdown,
...
as directed by the value of an OpenMP environment variable (\code{OMP\_TOOL}) and provide a function to register tool callbacks.
\end{itemize}
...
Chapters 1, 2, 3, 4 and 5. Note, though, that \specref{chap:ToolsSupport} contains both
mandatory and optional features; the OpenMP API is compliant if at least the
mandatory features are provided.
