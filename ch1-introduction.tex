...
\glossaryterm{league}
\glossarydefstart
The set of \emph{thread teams} created by a \code{target} construct or a \code{teams} construct.
\glossarydefend
...
The \emph{binding region} for \code{cancel} \emph{region} is the innermost enclosing \emph{region} that matches the specified in the \plc{construct-type-clause} of the \code{cancel} construct.

The \emph{binding region} for \code{cancellation point} \emph{region} is the innermost enclosing \emph{region} that matches the specified in the \plc{construct-type-clause} of the \code{cancellation point} construct.
...
\begin{quote}
COMMENT: The loop directive with an \code{ordered} clause with the parameter identifies the loop(s) associated for the \emph{doacross loop nest}.
\end{quote}
\glossarydefend
...
\glossarydefstart
A set of tasks that are logically grouped by a \code{taskgroup} construct.
\glossarydefend
...
The initial thread that executes the implicit parallel region that surrounds the \code{target}
...
implicit inactive \code{parallel} region that surrounds the entire \code{target} region.

When a \code{target} construct is encountered, the \code{target} region is executed by the
implicit device task. The task that encounters the \code{target} construct waits at the end of
the construct until execution of the region completes.  If the target device does not exist or the implementation does not support the target device, all \code{target} regions associated with that device are executed by the host device.
