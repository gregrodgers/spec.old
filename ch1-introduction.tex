...
\glossarydefstart
A first-party tool is considered activated if its initialization is successful.
\glossarydefend
...
known as OMPT.
% The OMPT interface can be implemented entirely by an OpenMP runtime system, entirely by a compiler, or with a hybrid strategy that employs a mixture of compiler and runtime support.

The OMPT interface provides the following: 
\begin{itemize}
\item a mechanism to intialize a first-party tool,
...
OpenMP implementations may differ with respect the thread states that
...
event. For other OpenMP events, OpenMP implementations may invoke a registered callback for some
or no occurrences of an event; for such
OpenMP events,  
OpenMP implementations are encouraged to support tool callbacks if it
...
With the exception of a runtime library routine for tool control, 
