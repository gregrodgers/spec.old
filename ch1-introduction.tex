...
\begin{quote}
COMMENT: If the \emph{loop directive} contains a \code{collapse} clause then there may be 
...
\glossarydefstart
A \emph{data environment} defined by a \code{target}~\code{data} or \code{target} construct.
\glossarydefend
\bigskip
...
The implementation must ensure that the target region executes as if it were executed in the data environment of the target device unless an \code{if} clause is present and the \code{if} clause expression evaluates to \plc{false}.
...
device data environment uses the corresponding variable from the enclosing device
...
\specref{sec:Master and Synchronization Constructs} and in \specref{sec:Lock Routines}, 
