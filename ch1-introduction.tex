...
\section{OpenMP Tools Support}
\label{subsec:OpenMP Tools Support}

The OpenMP specification includes an APIs that allow tools to access OpenMP specific information in a OpenMP implementation agnostic way. This enables 
quality tools that support for monitoring and performance analysis of OpenMP programs without having them tightly integrate tightly with a specific OpenMP runtime implementation. 
In particular, this API---named OMPT---is an application programming interface (API) for first-party performance tools.\footnote{A {\em first-party} tool runs within the address space of an application process. This differs from a {\em third-party} tool, e.g., a debugger, which runs as a separate process.}  
...
With the exception of one routine for tool control, all functions in the OMPT API are intended for use only by tools rather than by applications. All OMPT API functions, therefore, only have a C binding. A Fortran binding is  provided only for the single application-facing tool control function described in Section~\ref{sec:tconctrol}.
...
OMPT has a small set of mandatory features that provide a common foundation for all performance tools. Beyond that, the OpenMP specification defines a set of additional, optional, OMPT features that can be used tools to gather extra information about a program execution. These optional features are explicitly marked as optional. Implementations are encouraged to implement these features where feasible, as they allow a greater number of tools, but are considered compliant if only the mandatory features are provided.
