...
\glossarydefstart
A \code{task} or a \code{taskloop} \emph{construct}.
\glossarydefend
\bigskip
...
An async signal safe \emph{entry point} is safe to call from a \emph{signal handler}.
\glossarydefend
...
\glossaryterm{callback} 
\glossarydefstart
A function provided by a tool to an OpenMP implementation to be invoked when a particular \emph{event} occurs.
\glossarydefend

\glossaryterm{canonical frame address} 
\glossarydefstart
An address associated with a procedure frame on a call stack defined as the value of the stack pointer immediately prior 
to calling the procedure whose invocation the frame represents.
\glossarydefend

\glossaryterm{degenerate task} 
\glossarydefstart
A sequence of one or more procedure frames from OpenMP runtime
routines between a pair of frames for user procedures that
belong to the same task.
\glossarydefend

\glossaryterm{entry point} 
\glossarydefstart
A function interface provided by an OpenMP implementation for use by a tool that is
not associated with a global function symbol.
\begin{comment}
The address of an entry point 
can only be obtained using a special interface provided by an OpenMP implemenation 
as OpenMP is initialized for a device.
\end{comment}
\glossarydefend

\glossaryterm{event} 
\glossarydefstart
A particular point in a thread's execution that occurs as 
it executes a call to an OpenMP runtime library routine or
participates in the execution of an OpenMP directive. 
\glossarydefend

\glossaryterm{first-party tool} 
\glossarydefstart
A tool that executes code within the address space of the program it is monitoring.
\glossarydefend

\glossaryterm{inquiry function} 
\glossarydefstart
A tool \emph{entry point} used to inquire about the states or mutual exclusion implementations supported by 
an OpenMP implementation or to inquire about OpenMP state associated with the current thread.
\glossarydefend

\glossaryterm{OMPT} 
\glossarydefstart
An interface that a performance or correctness tool can use to monitor, interpret, and 
analyze an execution of an OpenMP program.
\glossarydefend

\glossaryterm{native trace record} 
\glossarydefstart
An trace record for an OpenMP device that is in a device-specific format defined by a third party.
\glossarydefend
...
\glossaryterm{spin wait} 
\glossarydefstart
Repeatedly evaluate a condition until it becomes true.
\glossarydefend

\glossaryterm{stack frame} 
\glossarydefstart
A storage area on a thread's stack associated with a function invocation. A stack frame includes space for 
one or more saved registers and often also includes space for saved arguments and/or local variables.
\glossarydefend

\glossaryterm{third-party tool} 
\glossarydefstart
A tool that executes in an address space distinct from the program it is monitoring.
\glossarydefend
...
\glossaryterm{type signature} 
\glossarydefstart
Definition of a function interface that specifies the type of each function argument and the function's return value.
\glossarydefend
...
to enforce mutual exclusion that may cause a thread to \emph{spin wait} or \emph{block}.
\glossarydefend
...
To facilitate development of high-quality, \emph{first-party}
...
known as OMPT.
The OMPT interface can be implemented entirely by an OpenMP runtime system, 
entirely by a compiler, or with a hybrid strategy
that employs a mixture of compiler and runtime support.

The OMPT interface provides the following: 
\begin{itemize}
\item a mechanism to invoke a tool-provided initializer, 
\item routines that enable a tool to determine what OpenMP thread states and mutual exclusion mechanisms an OpenMP API implementation employs,
\item routines that enable a tool to examine state information associated with an OpenMP thread,
\item mechanisms that help a tool to reconstruct user-level call stacks for OpenMP work,
\item a callback interface that enables a tool to receive notification of OpenMP \emph{events}, 
...
\begin{comment}
The design of OMPT was influenced by two prior efforts to define a
standard interface for OpenMP tools: POMP~\cite{Mohr:EWOMP02} and
the Sun/Oracle Collector~\cite{SunCollector,Jost:2005:AND:1892830.1892858}.
POMP provides support for instrumentation-based measurement. A
drawback of POMP is that its overhead can be significant because
an operation, e.g., an iteration of an OpenMP worksharing loop, may
take less time than tool callbacks monitoring its execution.  To avoid this drawback,
the Sun/Oracle Collector was designed to support
low-overhead performance measurement using asynchronous sampling.
Sampling-based tools can measure and attribute execution costs
without the overhead and intrusion of pervasive instrumentation. However,
the Collector interface doesn't include 
capabilities needed to provide full tool support for statically-linked
executables.  OMPT builds upon ideas from both POMP and Collector.
\end{comment}

OpenMP implementations may differ with respect to which
OpenMP events support tool callbacks. For some OpenMP events, an
OpenMP implementation must allow a tool to register a tool callback
and guarantee that it will be invoked for each occurrence of the
event. For other OpenMP events, support for callbacks is
implementation-defined; namely, when a tool attempts to register a
callback for such an event, an OpenMP implementation may indicate
that the event never occurs or that although the event may occur,
callbacks are possible only sometimes or never.
For OpenMP events where tool callbacks are practical 
without a significant performance penalty, OpenMP implementations are 
encouraged to support tool callbacks where implementation-defined behaviors
are permitted.

In some cases, the OMPT interface may enable a tool to observe performance
implications and infer details about an implementation of the OpenMP
API.  An implementation of the OpenMP API may differ from the
...
all other routines in the OMPT interface are only intended for use only by tools and
are not visible to applications.
For that reason, a Fortran binding is provided only 
for the application-facing tool control routine;
all other OMPT functionality is described with C syntax only.

%
%
% three tiers of support. 
%The first tier is a required, minimal set of features 
%needed to support tools that
%employ asynchronous sampling to measure application performance.
%A second tier of callback interfaces enables 
%performance tools that employ asynchronous sampling to shift 
%blame for runtime costs from symptoms (e.g., idle threads) 
%to apparent causes (e.g., serial work executed by a single thread).
%A third tier of callback interfaces supports comprehensive monitoring of 
%runtime events; such support is needed by correctness tools and desired
%by any tool that supports analysis and/or visualization of event traces.
%Support for features in the second and third tiers is implementation-defined.
%% An OpenMP implementation need not support invocation of all OMPT callbacks described in this document. 
%
%For all features in the first tier, 
%
%Support for any functionality beyond the first tier is implementation
%defined. 
%Table~\ref{table:ToolsSupport_set_rc} specifies the permissible
%return codes by an OpenMP implementation 
%when a tool attempts to register an OMPT callback within the tool's 
%initializer.
%Unless the table indicates that an OpenMP implementation must 
%guarantee that a callback will always occur when its corresponding 
%event occurs, it is implementation-defined whether an OpenMP
%implementation will provide callbacks in some or all circumstances when
%its corresponding event occurs.

\begin{comment}
% perhaps it is best to not try to enumerate these points here. we summarize them in the second paragraph
To support the OMPT interface, an implementation of the OpenMP API 
must

\begin{itemize}
\item call a tool initialization routine as directed by an internal control variable
and provide a function to register tool callbacks,
\item support required tool callbacks associated with OpenMP \emph{events}, 
% associated with threads, parallel regions, tasks, target regions, invocation of the user-level tool control routine, and runtime shutdown,
%\footnote{Table~\ref{table:ToolsSupport_set_rc} indicates exactly what event callbacks are required.}
% , including thread begin/end, parallel region begin/end, task region begin/end, target region begin/end, target region operations, invocation of the user-level tool control routine, and runtime shutdown,
\item maintain a state and a wait identifier for each OpenMP thread,
% . Mandatory states are idle, work serial, work parallel, and undefined,
\item implement several \emph{async signal safe} inquiry functions to retrieve state information associated with an OpenMP thread, 
\item maintain pointers into the stack associated with each OpenMP thread 
so that a tool can distinguish frames for user procedures from frames for OpenMP runtime routines
%,\footnote{\color{red}Do we need frame object in target trace records? I think not.}
\item provide a mapping from each 
OpenMP thread, parallel region, and task region concurrently live on the host
device to a distinct 64-bit data word that can be used by a tool to store its own information, and
\item associate a unique numerical ID with each OpenMP thread, 
parallel region, and task region that executes on any device 
other than the host, 
\end{itemize}
\end{comment}
...
executes all conforming programs and supports all required tool interfaces 
...
Appendices A, B, C and D and sections designated as Notes 
