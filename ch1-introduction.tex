...
\glossarydefstart
A sequence of one or more procedure \emph{frames} from OpenMP runtime
...
\glossaryterm{event} 
\glossarydefstart
A point in a thread's execution where a callback to a tool is performed as 
the thread executes a call to an OpenMP runtime library routine or
...
so that a tool can distinguish \emph{frames} for user procedures from frames for OpenMP runtime routines
