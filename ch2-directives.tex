...
If a \code{reduction} or \code{in\_reduction} clause is present on the \code{taskloop simd} construct, the clause is applied as follows:
\begin{itemize}
\item The clause is applied in a manner consistent with its use on the \code{taskloop} construct.
\item A \code{reduction} clause with the same reduction operator and the same list items is applied in a manner consistent with its use on the \code{simd} construct.
\end{itemize}
