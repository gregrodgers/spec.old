...
\begin{itemize}
\item The loop region to which an \code{ordered} region without any 
clause or with a \code{threads} clause binds must have an \code{ordered} 
clause without the parameter specified on the corresponding loop directive. 

\item The loop region to which an \code{ordered} region with any \code{depend}
...
\item An \code{ordered} construct with the \code{simd} clause specified must 
be closely nested inside a \code{simd} or loop SIMD construct. 

\item During execution of an iteration of a loop or a loop nest within a loop 
region, a thread must not execute more than one ordered region that arises 
from an \code{ordered} construct with either a \code{threads} or \code{simd}
clause. 
\end{itemize}
\cppspecificstart
\begin{itemize}
