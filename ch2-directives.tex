...
%EJS1
\ccppspecificstart
\item If the type of a variable is pointer, reference to array, or reference to pointer, then the variable is implicity treated as if it had appeared in a \code{map} clause as a zero-length array section.

\item If a variable is an array section and the type of the variable appearing in that array section is pointer, reference to array, or reference to pointer then the variable cannot be modfied in the region.
\ccppspecificend
...
If a new list item is created for an array section and the type of the variable appearing in that array section is pointer, reference to array, or reference to pointer then if the variable appears in a \code{to} or \code{from} clause in a \code{target}~\code{update} region during the lifetime of the new list item but not as part of the specification of an array section, the behavior is unspecified. 

What about addressing outside the bounds of a mapped variable?

What about accessing memory via a zero-length array section - see above.  Surrounding data section covers us?
\ccppspecificend
...
%EJS-FIXME
%/ CASE 2: Behavior is unspecified
%int *p = malloc(100);
%#pragma omp target map(p[:0])
%{
%     = p[10];
%}
\item If a list item is an array section where the the variable in the array section is a pointer (etc..) then the value of the pointer must have been previously mapped? 
\end{itemize}
