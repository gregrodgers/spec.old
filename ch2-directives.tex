...
\item in either the \code{omp\_target\_memcpy} or the
  \code{omp\_target\_memcpy\_rect} routine
...
outside of the \code{omp\_target\_alloc} routine and the \code{is\_device\_ptr} clause,
is implementation defined.
...
all references to elements of that structure in the \code{target} construct must be declared in the \code{target} construct.
...
\item A \plc{map-type} in a \code{map} clause must be \code{to}, \code{from}, \code{tofrom} or \code{alloc}.
  \item A list item that appears in an \code{is\_device\_ptr} must be a valid
