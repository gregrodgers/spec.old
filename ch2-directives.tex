...
% 383
\item A variable that is a member of a structure and that is both referenced in a \code{target} construct and not declared in the construct 
is implicitly treated as if a reference to the whole structure had appeared in a \code{map} clause with a \plc{map-type} of 
\code{tofrom}. 

%383
\item If one or more elements of a structure have been explicitly mapped in the encountered construct, 
no elements of this structure may be implicitly mapped in the target region.
