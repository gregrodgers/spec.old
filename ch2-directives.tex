...
times \plc{linear-step}. If the \code{ref} modifier is used, the value of the new list item on each iteration of the associated loop(s) corresponds to the value of applying  the \plc{linear-step} times the logical number of the iteration as a subscript to the original list item. If no modifier is specified it is assumed to be \code{val}. If \plc{linear-step} is not specified it is assumed to be 1. The value 
