...
\item An OpenMP program with orphaned \code{cancel}~\code constructs is 
non-conforming. 
\end{itemize}
...
\begin{itemize}
\item In a \code{parallel}, \code{task}, \code{taskloop}, or \code{teams} construct, the data-sharing attributes of these variables are
...
clause in a \code{task} construct may also appear in a \code{private}
clause in an enclosed \code{parallel}, \code{task}, \code{simd}, or
\code{target}construct.

List items that appear in a \code{private} or \code{firstprivate}
clause in a \code{taskloop} construct may also appear in a \code{private}
clause in an enclosed \code{parallel}, \code{task}, \code{simd}, or
\code{target}construct.
...
List items that appear in a \code{private} or \code{lastprivate}
clause in a \code{simd} construct may also appear in a \code{private}
clause in an enclosed \code{parallel}, \code{task}, or \code{target}
construct.
...
not appear in a \code{firstprivate} clause on a worksharing or
\code{taskloop} construct if any of the worksharing or task regions arising
from the worksharing or \code{taskloop} construct ever bind to any of the
...
\ccppspecificstart
\item A variable that appears in \code{firstprivate} clause must not have an incomplete C/C++ type or be a reference to an incomplete type.
\item If a list item is a reference type, then it must bind to the same object for all threads of the team.
\ccppspecificend
...
worksharing or \code{taskloop} construct if any of the corresponding
...
to pointer is treated as if it had appeared in a \code{map} clause as 
