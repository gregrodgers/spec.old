...
\item If a variable declared in a \code{target} construct is an element of a structure,
all references to entries of that structure in the \code{target} construct must be declared in the \code{target} construct.
...
\item If the variable of a list item is an element of a structure, and one or more elements of this structure are already mapped to the device prior to encountering this construct, then the variable must already be mapped to the device prior to encountering this construct.

% 383: map(t.z[10:20]),  
%         but a different structure map(t[0:20].x), or map(t[10:20].z[10:20]) or u.a.b[0:100].c.d is not allowed
\item If the variable of a list item is an element of a structure, only the rightmost  symbol of the variable reference can be an array section.

\item If variables that share storage are mapped, the behavior is unspecified. %, with the exceptions of a structure sharing storage with its own structure elements.
...
\descr
The \code{defaultmap(tofrom:scalar)} clause causes all scalar variables referenced in the construct that 
have implicitly determined data-mapping attributes to be map(tofrom).

\restrictions
The restrictions to the \code{defaultmap} clause are as follows:

\begin{itemize}
\item Only a single default clause may be specified on a \code{target} 
directive. 
\end{itemize}
