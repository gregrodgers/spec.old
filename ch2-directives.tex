...
The schedule \plc{modifier} can be one of those specified in Table~\ref{tab:Schedule Clause Modifier Values}. If neither the \code{monotonic} modifier nor the \code{nonmonotonic} modifier are specified it will be as if the \code{nonmonotonic} modifier was specified for the \code{dynamic}, \code{guided} and \code{auto} schedule kinds and it will be as if the \code{monotonic} modifier was specified for the \code{static} schedule kind.
...
chunks remain to be distributed. No ordering of chunk distribution is
required unless the \code{monotonic} modifier is specified.\\
...
No ordering of chunk distribution is required unless the \code{monotonic} modifier is specified.\\
...
in any order and the behavior of an application that depends on the execution order of the chunks is unspecified. This modifier can only be specified for the \code{dynamic} and \code{guided} schedule kinds. This modifier cannot be specified in loops where the \code{ordered} clause is specified.\\
...
\item The \code{monotonic} modifier can ony be specified with \code{schedule(dynamic)} or \code{schedule(guided)}.
