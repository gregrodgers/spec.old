...
\begin{itemize}
  ...
  \item The list-items that appear in a \code{use\_device} clause must be
    valid pointers in the memory of the target device.  Pointers returned from
    \code{omp\_device\_alloc} using the same \verb#device_num# may be used,
    support for other pointers and allocation types are implementation
    defined.
\end{itemize}
