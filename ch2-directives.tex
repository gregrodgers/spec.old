...
The data environment of the \plc{target task} is created according to the data-sharing attribute clauses on the \code{target} construct, per-data environment ICVs, and any default data-sharing attribute rules that apply to the \code{target} construct.

All clauses are evaluated when the \code{target} construct is encountered.  The mapping of variables occurs when the \plc{target task} executes.
