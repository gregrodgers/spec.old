...
All references to a list-items in a \code{use\_device} clause will refer to an
implementation-defined reference to the data in the device data environment
for the duration of the \code{target}~\code{data} region.
...
\item The list-items that appear in a \code{use\_device} must either resolve
  to true when passed to \code{omp\_is\_device\_ptr} or have been returned from
...
All references to a list-item in a \code{device\_ptr} clause will refer to the device
pointer specified by the list-item.
...
\item A \plc{map-type} in a \code{map} clause must be \code{to}, \code{from}, \code{tofrom} or \code{alloc}.
  \item The list-items that appear in a \code{device\_ptr} must be pointers or
    references mapped into the device data environment, returned from
    \code{omp\_device\_alloc} or \code{omp\_register\_ptr}, or that return
    success from \code{omp\_is\_device\_ptr()}. Anything else will
    result in unspecified behavior.
\ccppspecificstart
...
\item The list-items that appear in a \code{get\_device\_ptr} must have a
