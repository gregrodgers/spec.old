...
itereration encounters an \code{ordered} construct with a \code{depend(source)} clause.
...
\item Either \code{depend(sink:\plc{vec})} clause(s) or \code{depend(source)}
clause may appear on an \code{ordered} construct, but not both.
