...
\item List items of \code{map} clauses in the same construct must not share original storage, with the exceptions of structures sharing storage with their structure elements.
...
\item If a variable is an element of a structure, only the leftmost symbol of the variable reference, the rightmost symbol of the variable reference, or both can be array sections.
...
% 383: map(s.x) firstprivate(s.y) is not allowed: do it once the private/firstprivate is in
%\item A variable, or the structure of which the variable is a member, cannot be at the in a private or firstprivate clause in the same construct 
