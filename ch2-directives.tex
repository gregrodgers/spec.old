...
use\_native\_ptr(\plc{list})
use\_target\_ptr(\plc{list})
if(\plc{scalar-expression})
\end{indentedcodelist}
\ccppspecificend
\medskip
...
All references to a list-items in a \code{use\_native\_ptr} clause will be
privatized and set to the value of a native pointer pointing to the base
address of the device buffer associated with the list item.  The
\code{use\_target\_ptr} clause has the same semantics as
\code{use\_native\_ptr} except that the value will be in the form of an OpenMP
target pointer.
...
\item The list-items that appear in either a \code{use\_native\_ptr} or a
  \code{use\_target\_ptr} clause must be present in the device data
  environment.  If the value is mapped in by another clause on the same
  construct, the mapping will be considered to happen before the \code{use\_*}
  clause.
\end{itemize}
...
behavior is unspecified  if that variable is modified in the region.
  \item The list-items that appear in a \code{device\_ptr} clause must be
    device pointers
\cppspecificstart
