...
\tools
On entry to the \code{parallel} region, the master thread
shall invoke the callback \code{ompt\_event\_parallel\_begin} if registered.
The encountering thread invokes this callback after a \code{task} encounters a 
\code{parallel} construct but before any implicit task starts to execute the
parallel region's work. 
This callback has type signature \code{ompt\_parallel\_begin\_callback\_t}.
