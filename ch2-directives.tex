...
A \plc{cancellation} event occurs if cancellation is activated at an implicit cancellation point in an implicit barrier region.
...
The \plc{implicit-barrier-wait-begin} event occurs when a task starts spin waiting or suspends
...
terminates spin waiting or suspension and resumes executing an implicit barrier at the end of a 
\code{parallel} region.
...
The \plc{#1-wait-begin} event occurs when a task starts spin waiting or suspends
in a \code{#1} region. 

The \plc{#1-wait-end} event occurs when a task
finishes spin waiting or suspension and resumes execution in a \code{#1} region. 
