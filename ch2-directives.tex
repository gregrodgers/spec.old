...
%If a new list item is created for an array section and the type of the variable appearing in that array section is pointer then if that variable appears in a \code{to} or \code{from} clause in a \code{target}~\code{update} region during the lifetime of the new list item but not as part of the specification of an array section, the behavior is unspecified. 
\ccppspecificend
