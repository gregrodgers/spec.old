...
\def\omptTaskEventForTaskOrTarget#1{
  Upon encountering a \code{#1} construct that causes a \code{task} to be created, the runtime shall trigger the callback \code{ompt\_event\_task\_create}  in the context of the \code{task} that encountered the \code{#1} construct.
