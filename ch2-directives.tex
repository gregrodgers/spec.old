...
may invoke the callback \code{ompt\_callback\_work}, if registered, with
...
the callback \code{ompt\_callback\_work}, if registered, with
...
the encountering thread shall invoke the \code{ompt\_callback\_task\_schedule} 
...
% \code{#2} as its \plc{wstype} argument.
% }
\omptWorksharing{taskloop}{ompt\_work\_taskloop}
...
the scheduling thread shall invoke the \code{ompt\_callback\_task\_schedule} 
...
If the \code{ompt\_callback\_device\_initialize} callback is registered,
...
\code{ompt\_callback\_target\_submit}. This callback has type
...
\code{task}, the encountering thread may invoke the \code{ompt\_callback\_task\_dependences} 
...
The OpenMP encountering thread may invoke the \code{ompt\_callback\_task\_dependence\_pair} callback 
...
The \plc{cancel} event occurs after an implicit task encounters a 
\code{cancel} construct.

\tools

A thread invokes a registered \code{ompt\_callback\_cancel}
callback for each occurrence of a \plc{cancel} event in that thread. 
The callback occurs in the context of the implicit task.  The callback has type signature
...
The \plc{cancellation} event occurs if a thread encounters a 
\code{cancellation point} and detected the activation of cancellation.
