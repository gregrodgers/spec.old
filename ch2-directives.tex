...
are referenced in a \code{parallel}, \code{task}, \code{taskloop} or \code{teams} construct
...
\begin{itemize}
\item Only a single default clause may be specified on a \code{parallel}, \code{task},
\code{taskloop} or \code{teams} directive.
\end{itemize}
...
a \code{parallel}, \code{task}, \code{taskloop} or \code{teams} construct.
...
from the \code{parallel}, \code{task}, \code{taskloop} or \code{teams} construct if it
is associated with a target or a  subobject of a target that is in a \code{private},
\code{firstprivate}, \code{lastprivate}, or \code{reduction} clause inside the construct.
