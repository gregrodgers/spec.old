...
The iterations of the associated loop(s) are divided into contiguous, non-empty subsets, called SIMD chunks.
Iterations within a SIMD chunk execute concurrently using SIMD instructions and
each concurrent iteration will be executed by a different SIMD lane.
Lexical forward dependencies in the iterations of the original loop must be preserved within each SIMD chunk.
The number of iterations executed concurrently within a SIMD chunk is implementation defined.

The \code{safelen} clause defines the maximum number of consecutive logical loop iterations that can be executed concurrently within a SIMD chunk.
