...
\item If a list item in a \code{map} clause is an array section, and the array
    section is derived from a variable for which the type is pointer,
    reference to array, or reference to pointer, then the behavior is unspecified if that variables's corresponding private variable is modified in the region.
\ccppspecificend
...
of \plc{expr} or \plc{expr\_list}, nor the write to the location designated by
\plc{v}, need be atomic with 
...
\code{atomic} constructs executed in the same contention group
enforces mutually exclusive access to the locations designated by \plc{x}. 
...
\summary
The \code{ordered} construct specifies a structured block in a loop,
\code{simd}, or loop SIMD region that will be 
...
\code{simd} clause is present, and
otherwise it binds to the innermost enclosing loop region. \code{ordered} regions that bind to different 
...
\item An \code{ordered} construct with the \code{simd} clause specified must be closely nested inside a \code{simd} or loop SIMD construct. 
...
generated sibling tasks that contain at least one of the list items in an \code{out} or \code{inout} 
\plc{dependence-type} list.

The \code{out} and \code{inout} \plc{dependence-types}. The generated task will be a dependent task of 
all previously generated sibling tasks that contain at least one of the list
items in an \code{in}, \code{out}, or \code{inout} \plc{dependence-type} list.
...
Loop iteration dependences are derived from the \plc{dependence-type} of a \code{depend} clause and the iteration vector \plc{vec}.
...
y$_{1}$, y$_{2}$, ... , y$_{n}$, where y$_{\plc{i}}$ is either x$_{\plc{i}}$
or  x$_{\plc{i}}$ $\pm $ d$_{\plc{i}}$,
\plc{n} is the value specified by the \code{ordered} clause in the loop directive, 
