...
\def\omptTaskCreateEvents#1#2#3{
  The \plc{#3} event  occurs when the executing thread encounters a \code{#1} construct 
  that causes a \code{task} to be created.
}

\def\omptTaskCreateTools#1#2#3{
A thread invokes a registered \code{ompt\_callback\_task\_create}
callback for each occurrence of a \plc{#3} event in that
thread.  The callback occurs in the task encountering
the \code{#1} construct. This callback has the type signature
\code{ompt\_callback\_task\_create\_t}.
}

\def\omptTaskCreate#1#2#3{
  \events
  \omptTaskCreateEvents{#1}{#2}{#3}
  \tools
  \omptTaskCreateTools{#1}{#2}{#3}  
  }
\omptTaskCreate{task}{ompt\_explicit\_task}{task}
...
\omptTaskCreateEvents{#1}{ompt\_task\_target}{#3}
\omptTargetRegionEvents{#1}{#2}{#3}

\tools

\omptTaskCreateTools{#1}{ompt\_task\_target}{#3}
\omptTargetRegionTools{#1}{#2}{#3}
}
...
block of the \code{master} region.
This callback has type signature \code{ompt\_callback\_master\_t}.
...
This callback has type signature \code{ompt\_callback\_flush\_t}.
