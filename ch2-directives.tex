...
private copy of the list item is associaed with a private target.
\fortranspecificend
...
\item An original list item with the \code{POINTER} attribute or any
  pointer component of an original list item that participates in the
  reduction computation (referenced in \plc{combiner}) must be associated at
  entry to the construct that contains the reduction
  clause. Additionally, the list item or the pointer component of the
  list item must not be deallocated, allocated, or pointer assigned
  within the region.

\item An original list item with the \code{ALLOCATABLE} attribute or
  any allocatable component of an original list item that participates
  in the reduction computation (referenced in \plc{combiner}) must be in the
  allocated state at entry to the construct that contains the reduction
  clause. Additionally, the list item or the allocatable component of
  the list item must neither be deallocated nor allocated within the
  region.
