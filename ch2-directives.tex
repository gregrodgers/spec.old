...
By default, the \code{taskloop} construct executes as if it was enclosed in a \code{taskgroup} construct with no statements or directives outside of the \code{taskloop} construct. Thus, the \code{taskloop} construct creates an implicit \code{taskgroup} region. 

A \code{reduction} clause that is present on the \code{taskloop} construct behaves as if a \code{task\_reduction} clause was applied to the implicit \code{taskgroup} construct enclosing the \code{taskloop} construct.
...
For any \code{in\_reduction} clause present on the \code{taskloop} construct, an \code{in\_reduction} clause with the same reduction operator and the same list items that appear in the \code{in\_reduction} clause is applied to the generated tasks.
...
If the \code{nogroup} clause is present, no implicit \code{taskgroup} region is created.
...
list item can appear only once in reduction clauses for that directive.
