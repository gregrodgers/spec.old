...
either be closely nested inside a \code{simd} or loop SIMD construct, or it
must appear in a function associated with a \code{declare}~\code{simd}
construct.
\ccppspecificend
...
\item An \code{ordered} construct with the \code{simd} clause specified must 
either be closely nested inside a \code{simd} or loop SIMD construct, or it
must appear in a function or subroutine specified by a \code{declare}~\code{simd}
directive.
\fortranspecificend
