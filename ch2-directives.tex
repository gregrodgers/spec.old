...
The runtime invokes this callback after a \code{task} encounters a 
...
The runtime invokes this callback 
...
The runtime invokes this callback after an \code{implicit task} encounters a 
...
\def\omptTaskCreate#1#2{
  ...
  the runtime shall invoke the callback \code{ompt\_event\_task\_create}, if registered,
  with \code{type=#2}
...
the runtime shall invoke the \code{ompt\_event\_task\_schedule} 
...
The thread invokes this callback after an \code{implicit task} encounters a 
...
On exit from the \code{#1} region, the executing thread may invoke the 
...
On exit from the \code{#1} region, the thread
may invoke the callback \code{ompt\_event\_sync\_region}, if registered, with
...
\code{task}, the runtime may invoke the \code{ompt\_event\_task\_dependences} 
...
The OpenMP runtime may invoke the \code{ompt\_event\_task\_dependence\_pair} callback 
...
If the runtime invokes the callback, it will occur before the consumer knows 
