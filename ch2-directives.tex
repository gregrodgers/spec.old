...
\item The loop iteration variable(s) in the associated \plc{for-loop(s)} of a \code{taskloop} construct is (are) private. 
...
\item The loop iteration variable in the associated \plc{do-loop(s)} of a \code{taskloop} construct is (are) private.
...
\item The loop iteration variable(s) in the associated \plc{for-loop(s)} of a \code{taskloop} construct may be listed in a \code{private} or \code{lastprivate} clause.
...
\item The loop iteration variable(s) in the associated \plc{do-loop(s)} of a \code{taskloop} construct may be listed in a \code{private} or \code{lastprivate} clause.
...
\begin{itemize}
\item In a \code{parallel}, task generating, or \code{teams} construct, the data-sharing attributes of these variables are
...
\item For constructs other than task generating constructs, or \code{target}, if no \code{default} clause is present, these variables reference the variables with the same names that exist in the enclosing context.

\item In a task generating construct, if no \code{default} clause is present, a variable that in the 
...
are referenced in a \code{parallel}, task generating, or \code{teams} construct
...
a \code{parallel}, task generating, or \code{teams} construct.
...
from the \code{parallel}, task generating, or \code{teams} construct if it
