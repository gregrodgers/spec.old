...
priority clause of the task construct. There is one copy of the ICV for the whole program (the scope
is global).
...
\plc{max-task-priority-var} & device\\
\end{supertabular}
...
priority(\plc{non-negative-scalar-expression})
\end{indentedcodelist}
\ccppspecificend
\end{samepage}
...
shared(\plc{list})
depend(\plc{dependence-type }:\plc{ list})
priority(\plc{non-negative-scalar-numerical-expression})
\end{indentedcodelist}
\fortranspecificend
...
The \code{priority} clause is a hint for the priority of the generated task. The priority-value is a
...
priority-value when no \code{priority} clause is specified is zero (the lowest priority). If a value is
specified in the \code{priority} clause that is higher than the max-task-priority-var ICV then the
