...
\code{cancel} construct must be nested inside a \code{taskgroup} region. If
...
construct must be nested inside a \code{taskgroup} region. A
...
\subsection{Device Pointer Clauses}
\label{subsec:device_ptr}
\index{device_ptr@{\code{device\_ptr}}}
\index{use_device@{\code{use\_device}}}
\index{clauses!device_ptr@{\code{device\_ptr}}}
\index{clauses!use_device@{\code{use\_device}}}

\summary
This section describes clauses used to specify the translation of pointers
in the data environment, specifically the \code{use\_device} clause and the
\code{device\_ptr} clause.

These clauses are used to access a target device address on the host device
and to pass a registered device pointer to the target without translation.
The clauses accept a comma-separated list of list items. All list items
appearing in a clause must be visible, according to the scoping rules of the
base language. Clauses may be repeated as needed, but a list item that
specifies a given variable may not appear in more than one clause on the same
directive.

\subsubsection{\code{use\_device} Clause}

\summary
The \code{use\_device} clause declares that in the construct all references to
the list items from the encountering thread will refer to the corresponding
list items on the device.

\syntax
The syntax of the \code{use\_device} clause is as follows:
\begin{boxedcode}
use\_device(\plc{list})
\end{boxedcode}

\descr All references to a list-item inside a region will refer to an
implementation-defined reference to the data in the device data environment.
The \code{use\_device} clause can only appear on the target data construct.

\restrictions
\begin{itemize}

  \item The list-items that appear in a \code{use\_device} must be pointers or
    references mapped into the device data environment or returned from
    \code{omp\_device\_alloc} or \code{omp\_register\_ptr}, anything else will
    result in unspecified behavior.

  \item List items must not appear in any other data environment clause on the
    same construct.

\end{itemize}

\subsubsection{\code{device\_ptr} Clause}

\summary
The \code{device\_ptr} clause declares that each list-item is already a valid
OpenMP device pointer, and should be used without translation.

\syntax
The syntax of the \code{device\_ptr} clause is as follows:
\begin{boxedcode}
device\_ptr(\plc{list})
\end{boxedcode}

\descr All references to a list-item within a region will refer to the device
pointer specified by the list-item.  The \code{device\_ptr} clause
can appear on the target data, target enter data, target exit data and target
constructs.

\restrictions
\begin{itemize}

  \item The list-items that appear in a \code{use\_device} must have been returned from
    \code{omp\_device\_alloc} or \code{omp\_register\_ptr}, anything else will
    result in unspecified behavior.
