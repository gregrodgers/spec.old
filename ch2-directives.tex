...
\cppspecificstart
\item The type of the variable appearing in an array section must be array, pointer, reference to array, or reference to pointer.
...
\item If an array section is a list item in a \code{map} clause on the \code{target} construct, and the array section is derived from a variable for which the type is pointer or reference to pointer then that variable is firstprivate. 
\end{itemize}
\ccppspecificend
...
\item If the \plc{map-type} is \code{tofrom}, \code{from} or \code{release} the corresponding list item�s reference count is decremented by one.
...
% Remove this restriction.  It is specified.
If a new list item is created for an array section and the type of the variable appearing in that array section is pointer or reference to pointer then if the variable appears in a \code{to} or \code{from} clause in a \code{target}~\code{update} region during the lifetime of the new list item but not as part of the specification of an array section, the behavior is unspecified. 
\ccppspecificend
...
\cppspecificstart
\item If the type of a list item is a reference to a type \plc{T} then the type will be considered to be 
\plc{T} for all purposes of this clause.
\cppspecificend
...
% Do we still need these three resitrictions?
...
%\item A variable for which the type is pointer must not appear as a list item if the device data environment already contains an array section derived from that variable.

%\item An array section derived from a variable for which the type is pointer or must not appear as a list item if the device data environment already contains that variable. 
