...
<<<<<<< HEAD
A \code{reduction} clause that is present at the \code{taskloop} construct behaves as if it was applied to the implicit \code{taskgroup} construct enclosing the \code{taskloop} construct.
For each task that is generated by the \code{taskloop} construct an \code{in\_reduction} clause with the same reduction operator and the same list items of the \code{reduction} clause is applied to the task.
=======
A \code{reduction} clause that is present on the \code{taskloop} construct behaves as if a \code{task\_reduction} clause with the same reduction operator and the same list items was applied to the implicit \code{taskgroup} construct enclosing the \code{taskloop} construct.
For each task that is generated by the \code{taskloop} construct an \code{in\_reduction} clause with the same reduction operator and the same list items that appear in the \code{reduction} clause is applied to the task.
>>>>>>> Fixing some issues discussed during the last call

For any \code{in\_reduction} clause present on the \code{taskloop} construct, they behave as if they were applied to the generated tasks.
...
\item If a \code{reduction} clause is present at the \code{taskloop} directive, the \code{nogroup} clause must not be specified.
...
\descr
<<<<<<< HEAD
The \code{taskloop}~\code{simd} construct will first distribute the iterations of the associated loop(s) across tasks in a manner consistent with any clauses that apply to the \code{taskloop} construct. The resulting tasks will then be converted to a SIMD loop in a manner consistent with any clauses that apply to the \code{simd} construct. The effect of any clause except \code{reduction} and \code{in\_reduction} that applies to both constructs is as if it were applied to both constructs separately except the \code{collapse} clause, which is applied once.

If a \code{reduction} or \code{in\_reduction} clause is present on the \code{taskloop simd} construct, the clause is implicitly applied to the \code{taskloop} construct and a \code{reduction} clause with the same reduction operator and the same list items that appear in the \code{reduction} or \code{in\_reduction} clause is applied to the \code{simd} construct.
=======
The \code{taskloop}~\code{simd} construct will first distribute the iterations of the associated loop(s) across tasks in a manner consistent with any clauses that apply to the \code{taskloop} construct. The resulting tasks will then be converted to a SIMD loop in a manner consistent with any clauses that apply to the \code{simd} construct, except for the \code{collapse} clause which is ignored. For the purposes of each task's conversion to a SIMD loop, the effect of any \code{in\_reduction} clause is as if a \code{reduction} clause with the same reduction operator and the same list items was applied to the \code{simd} construct.
>>>>>>> Fixing some issues discussed during the last call
