...
\item A variable referenced in a \code{target} construct that does not appear in a data-sharing attribute clause 
is implicitly treated as if it had appeared in a \code{map} clause with a \plc{map-type} of \code{tofrom}.

%EJS1
\ccppspecificstart
\item If the type of a variable is pointer, reference to array or reference to pointer, and the variable does not appear in a data-sharing attribute clause, then the variable is implicitly treated as if it had appeared in a \code{map} clause on the construct as a zero-length array section.

\item If an array section is derived from a variable for which the type is pointer, reference to array or reference to pointer then that variable cannot be modified in the region.
\ccppspecificend
