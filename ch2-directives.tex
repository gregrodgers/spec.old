...
If a \code{use\_device\_ptr} clause appears with one or more data-mapping
clauses, this conversion will occur after all variables are mapped according to
the data-mapping clauses.

\restrictions
\begin{itemize}
    \item A program must not depend on any ordering of the evaluations of the \code{map} clauses appearing on the
    \code{target}~\code{data} directive, or on any side effects of the evaluations of these clauses.
