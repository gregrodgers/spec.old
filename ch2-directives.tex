...
between sibling tasks whose lifetimes overlap.  This event will occur in a
thread that observes the dependence before it is satisfied.
...
A thread invokes the \code{ompt\_callback\_task\_dependence\_pair}
callback for a \plc{task-dependence-pair} event to to report a
dependence between a producer (\plc{src\_task\_data}) and a consumer
(\plc{sink\_task\_data}) that blocked execution of the consumer.  This
