...
\ccppspecificstart
If a variable or function is treated as if it appeared as a list item in a
\code{to} clause on a \code{declare}~\code{target} directive in the same
translation unit in which the definition of the variable or function occurs
then a device-specific version of the variable or function is created.
\ccppspecificend

\fortranspecificstart
If a variable or procedure that is host or use associated is treated as if it
appeared as a list item in a \code{to} clause on a \code{declare}~\code{target}
directive then a device-specific version of the variable or procedure is
created.
\fortranspecificend
