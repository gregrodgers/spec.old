...
regions in the order of the loop iterations.  If any \code{depend} clauses are
specified then those clauses specify the order in which the threads in the
team execute \code{ordered} regions. An \code{ordered} region arising from an
\code{ordered} construct with a \code{depend} clause having the \code{source}
\plc{dependence-type} is a \plc{doacross dependence source}.
...
one or more \code{depend} clauses having the \code{sink}
\plc{dependence-type}, it waits until a doacross dependence source, if encountered, completes
execution in all valid iterations specified by the \code{depend} clauses
before completing execution of the \code{ordered} region.
...
The \code{source} \plc{dependence-type} specifies the source of
cross-iteration dependences that arise from the current iteration.

The \code{sink} \plc{dependence-type} specifies the sink of a cross-iteration
dependence, where the iteration vector \plc{vec} indicates the iteration
containing the source of the dependence.
