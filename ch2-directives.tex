...
\index{affinity}
When creating a team for a parallel region, the \code{proc\_bind} clause specifies a policy 
for assigning OpenMP threads to places within the current place partition, that is, the 
places listed in the \plc{place-partition-var} ICV for the implicit task of the encountering 
thread. Once a thread is assigned to a place, the OpenMP implementation should not 
move it to another place.

The \code{master} thread affinity policy instructs the execution environment to assign every 
thread in the team to the same place as the master thread. The place partition is not 
changed by this policy, and each implicit task inherits the \plc{place-partition-var} ICV of the 
parent implicit task.

The \code{close} thread affinity policy instructs the execution environment to assign the 
...
not changed by this policy, and each implicit task inherits the \plc{place-partition-var} ICV of 
...
$P$, or $P$ subpartitions if $T$ is greater than $P$. Then it assigns one thread $(T\leq P)$ or a set of 
threads $(T>P)$ to each subpartition. The \plc{place-partition-var} ICV of each implicit task is 
...
\begin{itemize}
\item $T\leq P$. The parent's partition is split into $T$ subpartitions, where each subpartition 
...
\item $T>P$. The parent's partition is split into $P$ unit-sized subpartitions. Each place is 
...
For the \code{close} and \code{spread} thread affinity policies, the threads with the smallest thread 
...
\code{lastprivate} on the loop construct, their values after the loop are unspecified.
\fortranspecificend
...
where \plc{motion-clause} is one of the following:
...
and where \plc{clause} is \plc{motion-clause} or one of the following:
...
\item If any procedure is declared via a procedure declaration statement, any 
\code{declare}~\code{target} directive with the procedure name must appear in the same specification 
part.
...
\crossreferences
\begin{itemize}
\item \plc{num\_teams\_var}, see 
\specref{subsec:ICV Override Relationships}. 
...
All associated for-loops must have the canonical form described in 
\specref{sec:Canonical Loop Form}.
\ccppspecificend
\bigskip
...
is assumed at the end of the \plc{do-loop}.
...
master threads of the \code{teams} region and executed concurrently using SIMD instructions.
...
\code{simd} construct. The effect of any clause that applies to both constructs is as if it were 
...
loop(s) according to the semantics of the \code{distribute} construct and any clauses that 
apply to the \code{distribute} construct. The resulting loops will then be distributed across 
the threads contained within the \code{teams} region to which the \code{distribute} construct 
binds in a manner consistent with any clauses that apply to the parallel loop construct. 
The effect of any clause that applies to both the \code{distribute} and parallel loop 
constructs is as if it were applied to both constructs separately.

\restrictions
The restrictions for the \code{distribute} and parallel loop constructs apply.
...
The effect of any clause that applies to both the \code{distribute} and parallel loop SIMD 
constructs is as if it were applied to both constructs separately.

\restrictions
The restrictions for the \code{distribute} and parallel loop SIMD constructs apply.
...
assumed at the end of the \plc{do-loop}. \code{nowait} may not be specified on an 
...
\plc{do-loop}. \code{nowait} may not be specified on 
...
\item The expression \plc{expr operator x} must be mathematically equivalent to \plc{(expr) operator 
...
\item At entry to and exit from \code{parallel}, \code{critical}, and \code{ordered} regions.
...
\descr
The threads in the team executing the loop region execute \code{ordered} regions 
...
all the \code{ordered} regions belonging to all previous iterations have completed.

\restrictions
Restrictions to the \code{ordered} construct are as follows:

\begin{itemize}
\item The loop region to which an \code{ordered} region binds must have an \code{ordered} clause 
specified on the corresponding loop (or parallel loop) construct.

\item During execution of an iteration of a loop or a loop nest within a loop region, a 
thread must not execute more than one \code{ordered} region that binds to the same loop 
region.
\end{itemize}
...
specified in the directive (that is, the innermost \code{parallel}, \code{sections}, \code{do}, or 
...
active, the task continues execution at the end of its \code{taskgroup} region, which implies 
its completion. Any task that belongs to the innermost enclosing \code{taskgroup} and that 
...
\notestart
\noteheader – The user is responsible for releasing locks and similar data structures that might 
cause a deadlock when a \code{cancel} construct is encountered and blocked threads cannot 
be canceled.
\noteend
...
\item Cray pointees inherit the data-sharing attribute of the storage with which their Cray 
...
\item For constructs other than \code{task}, if no \code{default} clause is present, these variables 
...
\item Variables with \code{const}-qualified type having no mutable member, and that are 
...
\item Formal arguments of called routines in the region that are passed by reference inherit 
...
\item Dummy arguments of called routines in the region that are passed by reference inherit 
the data-sharing attributes of the associated actual argument. 
...
\begin{itemize}
\item A variable that is part of another variable (as an array or structure element) cannot 
appear in a \code{shared} clause unless it is a static data member of a C++ class.
\end{itemize}
\ccppspecificend
...
\begin{itemize}
\item A variable that is part of another variable (as an array or structure element) cannot 
appear in a \code{shared} clause.
\end{itemize}
\fortranspecificend
...
\item A variable that appears in a \code{private} clause must not have an incomplete type or a 
reference type. 
\ccppspecificend
...
\ccppspecificstart
\item A variable that appears in a \code{firstprivate} clause must not have an incomplete 
...
\item A variable that appears in a \code{lastprivate} clause must not have an incomplete type 
or a reference type. 
\ccppspecificend
...
\item Arrays may not appear in a \code{reduction} clause.
...
\code{declare}~\code{reduction} directive, the \plc{reduction-identifier} of that directive can be used in a 
\code{reduction} clause with any variable of the same type, regardless of the length type 
...
\item If a character type with deferred or assumed length parameter is specified in a 
\code{declare} \code{reduction} directive, no other \code{declare}~\code{reduction} directives with 
character type and the same \plc{reduction-identifier} are allowed in the same scope.
\fortranspecificend
\end{itemize}
