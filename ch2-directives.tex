...
\begin{boxedcode}
\#pragma omp critical \plc{[}(\plc{name})\plc{]} \plc{[} hint(\plc{hint-kind}) \plc{] new-line}
   \plc{structured-block}
\end{boxedcode}

where \plc{hint-kind} is one of the following: 
...
\begin{boxedcode}
!\$omp critical \plc{[}(\plc{name})\plc{]} \plc{[} hint(\plc{hint-kind}) \plc{] new-line}
   \plc{structured-block}
...
The threads of a contention group execute the \code{critical} region as if only one thread of the contention group is entering the \code{critical} region at a time.
...
If the \code{hint} clause is present, the effect of the \code{critical} construct is implementation-defined, but retains the mutual exclusion semantics.
If no \code{hint} clause is specified, the default is as if \code{hint(none)} has been specified, 

\restrictions
\begin{itemize}
\item If the \code{hint} clause is specified, the \code{critical} construct must have a \plc{name}.
\item All \code{critical} constructs with the same \plc{name} must have the same \code{hint} clause. 
\end{itemize}
