...
\code{ordered} regions sequentially in the order of the loop iterations. If
any \code{depend} clauses are specified then those clauses specify the order
in which the threads in the team execute \code{ordered} regions. An
\code{ordered} region arising from an \code{ordered} construct with a
\code{depend} clause having the \code{source} \plc{dependence-type} is a
\plc{doacross dependence source}.
...
regions belonging to all previous iterations have completed. When a thread
executing any subsequent iteration encounters an \code{ordered} construct with
one or more \code{depend} clauses having the \code{sink} \plc{dependence-type},
it waits at the beginning of the \code{ordered} region until a doacross
dependence source completes execution in all iterations specified by the
\code{depend} clauses.
