...
When a \code{linear} clause is specified on the \code{simd} construct or a combined or composite construct that 
includes it, the \code{linear} clause provides a superset of
the functionality provided by the \code{private} clause. A list item that appears in a \code{linear} clause is subject 
to the \code{private} clause semantics described in \specref{subsubsec:private clause} except as noted. The value of 
the new list item on each iteration of the associated loop(s) corresponds to the value of the original list item 
before entering the construct plus the logical number of the iteration times \plc{linear-step}. If \plc{linear-step} 
is not specified it is assumed to be 1. The value corresponding to the sequentially last iteration of the associated 
loops is assigned to the original list item.

When a \code{linear} clause is specified on the \code{declare simd} directive, if a list item that is a C/C++ formal 
parameter (or Fortan dummy argument) specified in the \code{linear} clause without a modifier, the value of the list item on each lane of the associated function corresponds to the value of the value of the list item before entering the 
function plus the logical number of the lane times \plc{linear-step}. If \plc{linear-step} is not specified it is 
assumed to be 1. 

\begin{itemize}
\item If the list item appears in the \code{linear} clause with \code{ref} modifier, the value on each 
lane corresponds the location of the list item, and computed as above stated for read/write of data through memory 
dereferencing.  

\item If the list item appears in the \code{linear} clause with \code{uval} modifier, the location of 
the list item is same for all lanes, and the value of the list item is computed as above stated for each lane.
The corresponding memory of the list item is updated with the value of the last lane.

\item If the list item appears in the \code{linear} clause with \code{val} modifier, the location 
of the list item may be different for each lane. The value of the list item is computed as above stated for each lane. 
The corresponding memory of the list item for each lane may be updated, and any read of the variable within a lane 
must occur before any write to the variable within that lane.
\end{itemize}
...
\item An ordered construct with the \code{simd} clause is the only OpenMP
construct that can be encountered during execution of a \code{simd}
region.
