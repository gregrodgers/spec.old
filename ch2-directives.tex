...
The schedule \plc{modifier} can be one of those specified in Table~\ref{tab:Schedule Clause Modifier Values}. If neither the \code{monotonic} modifier nor the \code{nonmonotonic} modifier are specified, the effect will be as if the \code{monotonic} modifier was specified for the \code{static} schedule kind or if the \code{ordered} clauses is specified. Otherwise, the effect will be as if the \code{nonmonotonic} modifier was specified.
...
in any order and the behavior of an application that depends on the execution order of the chunks is unspecified.\\
...
\item The \code{nonmonotonic} modifier cannot be specified if an \code{ordered} clause is present.

\item Only one of modifier of \code{monotonic} or \code{nonmonotonic} can be specified.
