...
\item Details of each OpenMP directive, including events and tool callbacks associated
...
The \plc{single-end} event occurs after a \code{single} region finished execution of the structured block 
...
A thread invokes a registered \code{ompt\_callback\_work}
callback for each occurrence of a \plc{single-begin} and
\plc{single-end} events in that thread. The callback has type signature
\code{ompt\_callback\_work\_t}. The callback receives
\code{ompt\_scope\_begin} or \code{ompt\_scope\_end} 
as its \plc{endpoint} argument, as appropriate, and
\code{ompt\_work\_single\_executor} as its \plc{wstype} argument.
