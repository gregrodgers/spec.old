...
The \plc{#1{-end}} event occurs after a \code{#1} region finished execution 
...
\restrictions
No thread may offload execution of an OpenMP construct to a device until any  
callback associated with a \plc{device-initialize} event completes.
...
\tools
\omptTargetRegionTools{\code{target}}{ompt\_target}{target}

A thread invokes a registered \code{ompt\_callback\_target\_submit}
...
The \plc{master-begin} event occurs in the master thread of 
a team encountering the \code{master} construct on entry to the
\code{master} region.

The \plc{master-end} event occurs in the master thread of a 
...
master thread of the team encountering
...
\item \code{ompt\_scope\_begin} and \code{ompt\_scope\_end} are
  members of enumeration type \code{ompt\_scope\_endpoint\_t}, see 
...
the #1 construct.  Ideally, the callbacks should receive \code{ompt\_mutex\_#1}
...
\item \code{ompt\_mutex\_critical} is a member of enumeration type
  \code{ompt\_mutex\_kind\_t}, see 
...
\item \code{ompt\_sync\_region\_barrier} is a
  member of enumeration type \code{ompt\_sync\_region\_kind\_t}, see 
...
\item \code{ompt\_sync\_region\_taskwait} is a
  member of enumeration type \code{ompt\_sync\_region\_kind\_t}, see 
\specref{sec:ompt_sync_region_kind_t}.
...
begin and end callbacks, see 
\specref{sec:ompt_callback_sync_region_t}.

\item \code{ompt\_sync\_region\_taskgroup} is a
  member of enumeration type \code{ompt\_sync\_region\_kind\_t}, see 
\specref{sec:ompt_sync_region_kind_t}.
...
\item \code{ompt\_mutex\_atomic} is a member of enumeration type
  \code{ompt\_mutex\_kind\_t}, see 
\specref{sec:ompt_mutex_kind_t}.
\end{itemize}
...
\item \code{ompt\_mutex\_ordered} is a member of enumeration type
  \code{ompt\_mutex\_kind\_t}, see 
\specref{sec:ompt_mutex_kind_t}.
\end{itemize}
...
The \plc{task-dependence} event occurs in a thread encountering a
tasking construct with a \code{depend} clause immediately after the
\plc{task-create} event for the new task.  

The \plc{task-dependence-pair} event indicates a blocking dependence
between sibling tasks whose lifetimes overlap.  This event occurs in a
thread consuming a dependence before consumer knows that the
dependence is satisfied. A dependence will not cause an event if the
dependence producing task finishes before a dependence consuming task
is created.

\tools

A thread invokes the \code{ompt\_callback\_task\_dependences} callback
for each occurrence of the \plc{task-dependence} event to 
...
A thread invokes the \code{ompt\_callback\_task\_dependence\_pair} callback 
...
A thread invokes a registered \code{ompt\_callback\_target\_map}
...
A thread invokes a registered \code{ompt\_callback\_target\_transfer}
