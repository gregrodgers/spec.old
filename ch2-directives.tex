...
The schedule \plc{modifier} can be one of those specified in Table 2-XX.
\begin{supertabular}{ p{0.8in} p{4.3in} }
\code{simd} & When the \code{simd} modifier is specified and the loop is executed using more than one SIMD lane, the \plc{chunk\_size} for all chunks except the first and the last is adjusted in to be  $new\_chunk\_size =  \lceil chunk\_size / simd\_width \rceil * simd\_width $ where \plc{simd\_width} is an implementation defined value. The first chunk will have at least \plc{new\_chunk\_size} iterations but it is unspecified if it has more. The last chunk may have fewer iterations. \\
\end{supertabular}

\notestart
\noteheader - The use of a \plc{modifier} in the schedule clause may result in some threads not being assigned any iteration.
\noteend
