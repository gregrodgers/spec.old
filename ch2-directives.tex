...
of the device that will execute the region. If a \code{teams} construct with a \code{thread\_limit}
clause is encountered, the \plc{thread-limit-var} ICV of the new device data environment is 
...
\item No OpenMP construct can appear in the \code{simd} region.
...
\item The execution of the function or subroutine, when called from a SIMD loop, cannot 
...
\descr
When a \code{target}~\code{data} construct is encountered, a new device data environment is 
created, and the encountering task executes the \code{target}~\code{data} region. If there is no 
\code{device} clause, the default device is determined by the \plc{default-device-var} ICV. The 
...
construct. When an \code{if} clause is present and the \code{if} clause expression evaluates to \plc{false}, 
...
\descr
The \code{target} construct provides a superset of the functionality and restrictions provided 
by the \code{target}~\code{data} directive. The functionality added to the \code{target} directive is the 
inclusion of an executable region to be executed by a device. That is, the \code{target} 
directive is an executable directive. The encountering task waits for the device to 
complete the target region. When an \code{if} clause is present and the \code{if} clause expression 
evaluates to \plc{false}, the target region is executed by the host device.

\restrictions
\begin{itemize}
\item If a \code{target}, \code{target}~\code{update}, or \code{target}~\code{data} construct appears within a target 
...
\plc{declarations-definition-seq}
...
\descr
\ccppspecificstart
Variable and routine declarations that appear between the \code{declare}~\code{target} and 
\code{end}~\code{declare}~\code{target} directives form an implicit list where each list item is the variable 
or function name.
\ccppspecificend
...
\ccppspecificstart
\item A variable declared in a \code{declare}~\code{target} directive must be at file or namespace 
scope.

\item A function declared in a \code{declare}~\code{target} directive must be at file, namespace, or 
class scope.
...
\item If any procedure is declared via a procedure declaration statement that is not in the type-bound procedure part of a derived-type definition, any declare \code{declare}~\code{target} with the procedure name must appear in the same specification part.
...
\item A list item may appear in a linear or firstprivate clause but not both.
\item A list item may appear in a linear or lastprivate clause but not both.
\end{itemize}
...
\item \plc{x} must not be an allocatable variable.
...
\item Variables belonging to common blocks, or declared in modules, and referenced in 
called routines in the region are shared unless they appear in a \code{threadprivate} 
directive.
...
common block appear in a \code{private}, \code{firstprivate}, \code{lastprivate}, or 
\code{reduction} clause of a directive, the storage of the specified variables is no longer 
...
\item An original list item with the \code{ALLOCATABLE} attribute in the sequentially last 
...
\item If a list item is an array section, accesses to the elements of the array outside the specified array section result in unspecified behaviour.
\end{itemize}
...
\summary
The \code{map} clause maps a variable from the current task's data environment to the device
...
For list items that appear in a \code{map} clause, corresponding new list items are created in
...
the \plc{map-type} that is specified.
...
assignment are performed if specified by the \plc{map-type}.
...
reference to pointer then the variable is implicitly treated as if it had appeared in a \code{map}
clause with a \plc{map-type} of \code{alloc}. The corresponding variable is assigned the address of
the storage location of the corresponding array section in the new device data
environment. If the variable appears in a \code{to} or \code{from} clause in a \code{target}~\code{update}
...
\needspace{11\baselineskip}\begin{samepage}
\fortranspecificstart
...
The \code{alloc} \plc{map-type} declares that on entry to the region each new corresponding list
item has an undefined initial value.

The \code{to} \plc{map-type} declares that on entry to the region each new corresponding list item
is initialized with the original list item's value.

The \code{from} \plc{map-type} declares that on exit from the region the corresponding list item's
value is assigned to each original list item.

The \code{tofrom} \plc{map-type} declares that on entry to the region each new corresponding list
...
\item If any part of the original storage of a list item has corresponding storage in the 
...
\item A variable for which the type is pointer, reference to array, or reference to pointer 
...
\item An array section derived from a variable for which the type is pointer, reference to 
...
\item If the \code{declare}~\code{reduction} directive appears in a module and the corresponding 
\code{reduction} clause does not appear in the same module, the \plc{reduction-identifier} 
must be a user-defined operator, one of the allowed operators or one of the allowed 
intrinsic procedures. 

\item If the \plc{reduction-identifier} is a user-defined operator or an extended operator, the 
interface for that operator must be defined in the same subprogram, or must be 
accessible by host or use association. 

\item If the \code{declare}~\code{reduction} directive appears in a module, any user-defined 
...
\code{reduction} clause appears. 

\item Any subroutine or function used in the \code{initializer} clause or \plc{combiner} 
...
\item If the length type parameter is specified for a character type, it must be a constant, a 
colon or an~\code{*}. 
