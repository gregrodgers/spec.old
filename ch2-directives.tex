...
When a \code{priority} clause is present on a \code{taskloop} construct the generated tasks will have the \plc{priority-value} as if it was specified per each individual task.

The tasks generated by the \code{taskloop} construct have default task priority (zero).
...
where \plc{write-statement} has the following form (if \plc{atomic-clause} is \code{write}):

\begin{quote}
\code{\plc{x} = \plc{expr}}
\end{quote}

where \plc{capture-statement} has the following form (if \plc{atomic-clause} is \code{capture} or \code{read}):
...
\item In an orphaned \code{taskloop} construct, if no \code{default} clause is present, dummy arguments 
are firstprivate.
\end{itemize}
\fortranspecificend
\end{samepage}
...
\item In a \code{taskloop} construct, if no \code{default} clause is present, a variable whose data-sharing 
attribute is not determined by the rules above is firstprivate.
\end{itemize}
