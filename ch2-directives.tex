...
\ccppspecificstart
A \plc{list item} is a variable, array section, or \plc{lvalue} expression. An
\plc{extended list item} is a \plc{list item} or a function name.  An
\plc{lvalue} expression can appear only in clauses where it is explicitly
allowed.
\ccppspecificend
...
sections or \plc{lvalue} expressions.
\ccppspecificend
...
\ccppspecificstart
\item A list item cannot be a variable that is a member of a structure with a
    union type.
...
\fortranspecificstart
\item A list item must not be an element of a common block or appear in an
