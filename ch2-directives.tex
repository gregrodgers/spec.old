...
\item Dummy arguments of called routines in the region that have the \code{VALUE} attribute are private.

\item Data-sharing attributes of dummy arguments of called routines in the region
that do not have the \code{VALUE} attribute depend on whether temporary storage
is used in place of the associated actual arguments on the procedure reference. They have
the same data-sharing attributes as the actual arguments if
temporary storage is not used, and they are private if temporary storage is
used.  The use of temporary storage is implementation-defined, except:
\begin{enumerate}
\item If the associated actual argument is an array section with a vector subscript,
temporary storage will be used.
\item If the associated actual argument is not an array section with a vector
subscript, and the actual argument has contiguous storage or the dummy argument
is a scalar variable or structure, temporary storage will not be used.
\end{enumerate}
...
\notestart
\noteheader – If a shared variable is an actual argument to a non-intrinsic
procedure, it may be modified on exit from the procedure if the following conditions are satisfied:
  
\begin{enumerate}%%[label=\alph*] % L0 vvvvvvvvvvv
\item Temporary storage is or may be used in place of the actual argument (See
\specref{subsubsec:Data-sharing Attribute Rules for Variables
Referenced in a Region but not in a Construct}).
\item The associated dummy argument does not have an \code{INTENT(IN)} attribute.
\end{enumerate}

Under these conditions, any references to (or definitions of) the shared 
storage that is associated with the dummy argument by any other task must be 
synchronized with the procedure reference to avoid a possible data race.
\noteend
\medskip
\fortranspecificend
