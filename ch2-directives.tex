...
An \emph{implicit parallel region} executing on the \emph{host device} surrounds the whole program. 
An \emph{initial task} is an \emph{implicit task} associated with an \emph{implicit parallel region}.
A thread that executes an \emph{implicit parallel region} is known as an \emph{initial thread}.

\events
No events are associated with an \emph{implicit parallel region}.

The \plc{initial-thread-begin} event occurs when an \emph{initial thread} first encounters an OpenMP \emph{region}, 
before the \emph{region} begins to execute.

The \plc{initial-task-create} event occurs immediately after an \plc{initial-thread-begin} event 
before the first OpenMP \emph{region} begins to execute.

The \plc{initial-thread-end} event occurs in an \emph{initial thread} immediately prior to invocation of the tool finalizer 
prior to shutdown of the OpenMP runtime.

\tools

A thread dispatches a registered \code{ompt\_callback\_thread\_begin}
callback for the \plc{initial-thread-begin} event in an \emph{initial thread}. 
The callback occurs in the context of the \emph{initial thread}.  
...
\pagebreak
\ccppspecificstart
The form of the \code{declare}~\code{target} directive that has no clauses and
requires a matching \code{end}~\code{declare}~\code{target} directive defines
an implicit \plc{extended-list} to an implicit \code{to} clause.  The implicit
\plc{extended-list} consists of the variable names of any variable declarations
that have static storage duration between the two directives and of the
function names of any function declarations that appear between the two
directives.
\ccppspecificend

\fortranspecificstart
If a \code{declare}~\code{target} does not have any clauses then an implicit
\plc{extended-list} to an implicit \code{to} clause is formed from the name of
the enclosing procedure or interface body to which it applies and, if it
applies to a subprogram, any procedure declared in that subprogram.
\fortranspecificend
