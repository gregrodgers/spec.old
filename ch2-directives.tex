...
All clauses are evaluated when the \code{target} construct is encountered.  The data environment of the \plc{target task} is created according to the data-sharing attribute clauses on the \code{target} construct, per-data environment ICVs, and any default data-sharing attribute rules that apply to the \code{target} construct.  A variable that is mapped in the target construct has a default data-sharing attribute of shared in the data environment of the \plc{target task}.

%Now since the language of "merged task" was deleted for the target task, I was wondering if we need say something about the default data-sharing attribute rules for mapped variables.  Maybe it is obvious, but I just missed.  The only thing I can find is on page 178, lines 17-18: In a target construct, variables that are not mapped after applying data-mapping attribute rules (see Section 2.15.5 on page 212) are firstprivate.

Assignment operations associated with mapping a variable ({see \specref{subsec:map Clause}) occur when a \plc{target task} executes.

%The steps for "mapping a variable" appear in section 2.15.5.1. I think we only need to say that "entry" steps 3.a and 4.a and "exit" steps 2.b.i and 2.c occur when the target task executes -- the other steps may occur at an implementation-defined point between when the construct is encountered and when the target task executes.

A \plc{target task} waits at a task scheduling point for the device to complete the \code{target} region. The encountering thread becomes available to execute other tasks at that task scheduling point. If the \plc{target task} is undeferred then the encountering thread must suspend the current task region, for which execution cannot be resumed until the \plc{target task} is complete.

The \plc{target task} is mergeable.  If the \code{nowait} clause is not present, the \plc{target task} is an included task.  When a \code{nowait} clause is present, the \plc{target task} is not an included task, and the current task may resume execution before the \plc{target task} is complete.

If a \code{depend} clause is present, then it is associated with the \plc{target task}.
