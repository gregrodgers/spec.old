...
\begin{itemize}
\item OpenMP directives may not appear in \code{PURE} or \code{ELEMENTAL} procedures.
\end{itemize}
\fortranspecificend
\end{samepage}
...
data copying clauses (\specref{subsec:Data Copying Clauses}), the \code{threadprivate} directive 
(\specref{subsec:threadprivate Directive}) and the \code{flush} directive (\specref{subsec:flush Construct}) 
...
\plc{dyn-var} & \code{OMP\_DYNAMIC} & See comments below\\
...
\plc{max-active-levels-var} & \code{OMP\_MAX\_ACTIVE\_LEVELS} & See comments below\\
...
\plc{active-levels-var} & (none) & \code{omp\_get\_active\_levels()}\\

\plc{levels-var} & (none) & \code{omp\_get\_level()}\\

\plc{place-partition-var} & (none) & (none)\\
...
first elements of the \plc{bind-var} ICV; otherwise, the \code{proc\_bind} clause has no effect. 
\end{itemize}
...
The \plc{lower-bound} and \plc{length} must evaluate to non-negative integers.
...
Note that using a variable in an \code{if} or \code{num\_threads} clause expression of a 
...
\code{for (\plc{init-expr}; \plc{test-expr}; \plc{incr-expr}) \plc{structured-block}}\\[-2ex]
\line(1,0){400}\linenumbers
...
\plc{var} & One of the following:\\
 ...
 & If this variable would otherwise be shared, it is implicitly made private in the loop 
  ...
  other than in \plc{incr-expr}. Unless the variable is specified \code{lastprivate} on the
...
\index{clauses!collapse@{\code{collapse}}}
The \code{collapse} clause or the \code{ordered} clause with the parameter may be used to specify how many loops are associated with the loop construct. An \code{ordered} clause without a parameter is equivalent to the \code{ordered} clause with a parameter value of one specified. The number of loops associated is determined by the parameters of the \code{collapse} clause and the \code{ordered} clause, which must be constant positive integer expressions.

If neither the \code{collapse} nor the \code{ordered} clause is present, the only loop that is associated with the loop construct is the one that immediately follows the loop directive. If the value of the parameter in the \code{collapse} or \code{ordered} clause is larger than the number of nested loops following the construct, the behavior is unspecified.

If both the \code{collapse} clause and the \code{ordered} clause with a parameter are specified, the \code{collapse} clause applies to the loops that immediately follow the directive. The \code{ordered} clause with the parameter then applies to the resulting loop nest (that is, the collapsed loop and the remaining loops). If the value of the parameter in the \code{ordered} clause is larger than the number of the loops in the resulting loop nest, the behavior is unspecified.

\index{clauses!schedule@{\code{schedule}}}
If more than one loop is associated with the loop construct and a \code{collapse} clause is specified, then the iterations of all associated loops are collapsed into one larger iteration space that is then divided according to the schedule clause.

If an \code{ordered} clause with the parameter is specified for the loop construct, then the associated loops form a \emph{doacross loop nest}. 
...
\code{schedule} clause specifies how iterations of the associated loops are divided into 
...
its implicit task. The \plc{chunk\_size} expression is evaluated using the original list items of 
...
occur. The use of a variable in a \code{schedule} clause expression of a loop construct 
...
exception is for the \code{static} schedule as specified in Table 2-1. 
...
The schedule \plc{kind} can be one of those specified in Table 2-1.

The schedule \plc{modifier} can be one of those specified in Table 2-2.

%% \pagebreak
\textsc{table 2-1} \code{schedule} clause \plc{kind} values
...
value of the \code{schedule} clause determines the schedule. Figure 2-1 describes how the 
...
All associated \plc{do-loops} must be \plc{do-constructs} as defined by the Fortran standard. If an 
\code{end}~\code{simd} directive follows a \plc{do-construct} in which several loop statements share a \code{DO} 
...
\item An ordered construct with the \code{simd} clause is the only OpenMP construct that can appear in the \code{simd} region. 
...
If a \code{declare}~\code{simd} directive contains multiple SIMD declarations, then one or more 
...
The \code{inbranch} clause specifies that the function will always be called from inside a 
...
\item When a \plc{constant-linear-step} expression is specified in a \code{linear} clause it must be a 
...
\item The function cannot contain calls to the \plc{longjmp} or \plc{setjmp} functions. 
\ccppspecificend
...
be guaranteed using task synchronization constructs. A \code{task} construct may be nested 
inside an outer task, but the \code{task} region of the inner task is not a part of the \code{task} 
...
generated task is completed. Note that the use of a variable in an \code{if} clause expression 
...
temporarily suspend the \code{task} region. By default, a task is tied and its suspended task 
...
\notestart
\noteheader – When storage is shared by an explicit \code{task} region, it is the programmer's 
...
num_tasks(\plc{num-tasks})
collapse(\plc{n})
if(\plc{scalar-logical-expr})
final(\plc{scalar-logical-expr})
untied
mergeable
\end{indentedcodelist}

If \code{end}~\code{taskloop} directive is not specified, an \code{end}~\code{taskloop} directive is assumed at the end of the \plc{do-loops}.

All associated \plc{do-loops} must be \plc{do-constructs} as defined by the Fortran standard. If an \code{end}~\code{do} directive follows a \plc{do-construct} in which several loop statements share a \code{DO} termination statement, then the directive can only be specified for the outermost of these \code{DO} statements.
...
\binding
The binding thread set of the \code{taskloop} construct is the current team. A \code{taskloop} region binds to the innermost enclosing \code{parallel} region.
...
If a \code{grainsize} clause is present on the \code{taskloop} construct, the number of logical loop iterations assigned to each created task is larger than or equal to the value of the \plc{grain-size} expression, but less than two times the value of the \plc{grain-size} expression and less than or equal to the number of logical loop iterations. If \code{num\_tasks} is specified, the \code{taskloop} construct creates as many tasks as the minimum of the \plc{num-tasks} expression and the number of logical loop iterations. If neither a \code{grainsize} nor \code{num\_tasks} clause is present, the number of loop tasks created and the number of logical loop iterations assigned to these tasks is implementation defined.
...
\summary
The \code{taskloop}~\code{simd} construct specifies a loop that can be executed concurrently using SIMD instructions and that those iterations will also be executed in parallel using OpenMP tasks.
\syntax
\ccppspecificstart
...
If \code{end}~\code{taskloop}~\code{simd} directive is not specified, an \code{end}~\code{taskloop}~\code{simd} directive is assumed at the end of the \plc{do-loops}.
\fortranspecificend

\binding
The binding thread set of the \code{taskloop}~\code{simd} construct is the current team. A \code{taskloop}~\code{simd} region binds to the innermost enclosing parallel region.
...
\binding
The binding task region for a \code{target}~\code{data} construct is the encountering task. The 
...
\binding
The binding task for a \code{target} construct is the encountering task. The target region 
binds to the enclosing parallel or task region.

\descr
The \code{target} construct provides a superset of the functionality and restrictions provided by the \code{target}~\code{data} directive.
...
\restrictions
\begin{itemize}
\item If a \code{target}, \code{target}~\code{update}, \code{target}~\code{data}, \code{target}~\code{enter}~\code{data}, or \code{target}~\code{exit}~\code{data} construct appears within a \code{target} region the behavior is unspecified.

\item The result of an \code{omp\_set\_default\_device}, \code{omp\_get\_default\_device}, 
or \code{omp\_get\_num\_devices} routine called within a target region is unspecified.

\item The effect of an access to a \code{threadprivate} variable in a target region is 
unspecified.

\item A variable referenced in a \code{target} construct that is not declared in the construct 
is implicitly treated as if it had appeared in a \code{map} clause with a \plc{map-type} of 
\code{tofrom}.

\item A variable referenced in a \code{target} region but not the target construct that is not 
declared in the target region must appear in a \code{declare}~\code{target} directive. 
...
\binding
The binding task for a \code{target}~\code{update} construct is the encountering task.The 
\code{target}~\code{update} directive is a stand-alone directive.
...
\code{target}~\code{update} construct. 
...
\summary
The \code{declare}~\code{target} directive specifies that variables, functions (C, C++ and 
Fortran), and subroutines (Fortran) are mapped to a device. The \code{declare}~\code{target} 
...
\begin{boxedcode}
!\$omp declare target(\plc{list})
\end{boxedcode}

where \plc{list} is a comma-separated list of named variables, procedure names and named 
...
\descr
\ccppspecificstart
Variable declarations at file or namespace scope that appear between the \code{declare}~\code{target} and \code{end}~\code{declare}~\code{target} directives form an implicit list where each list item is the variable name.

Function declarations at file, namespace or class scope that appear between the \code{declare}~\code{target} and \code{end}~\code{declare}~\code{target} form an implicit list where each list item is the function name.
\ccppspecificend

\fortranspecificstart
If a \code{declare}~\code{target} does not have an explicit list, then an implicit list of one item is 
...
If a list item is a variable then the original variable is mapped to a corresponding variable in the device data environment of all devices as if it had appeared in a \code{map} clause with a \plc{map-type} to on the implicit \code{target}~\code{data} construct for each device. The list item is never removed from those device data environments as if its reference count is initialized to positive infinity.

\restrictions
\begin{itemize}
\item A threadprivate variable cannot appear in a \code{declare}~\code{target} directive.

\item A variable declared in a \code{declare}~\code{target} directive must have a mappable type

\ccppspecificstart
\item All declarations and definitions for a function must have a \code{declare}~\code{target}
...
\item Any \code{declare}~\code{target} directive with a list can only appear in a specification part 
of a subroutine subprogram, function subprogram, program or module.

\item Any \code{declare}~\code{target} directive without a list can only appear in a specification 
part of a subroutine subprogram, function subprogram or interface body to which it 
applies.

\item If a \code{declare}~\code{target} directive is specified in an interface block for a procedure, it 
must match a \code{declare}~\code{target} directive in the definition of the procedure.
\item If an external procedure is a type-bound procedure of a derived type and a \code{declare}~\code{target} directive is specified in the definition of the external procedure, such a directive must appear in the interface block that is accessible to the derived type definition.
\item If any procedure is declared via a procedure declaration statement that is not in the type-bound procedure part of a derived-type definition, any \code{declare}~\code{target} with the procedure name must appear in the same specification part.

\item A variable that is part of another variable (as an array or structure element) cannot 
appear in a \code{declare}~\code{target} directive.

\item The \code{declare}~\code{target} directive must appear in the declaration section of a scoping 
...
\code{declare}~\code{target} directive must be declared to be a common block in the same scoping unit 
in which the \code{declare}~\code{target} directive appears.

\item If a \code{declare}~\code{target} directive specifying a common block name appears in one 
...
\item If a \code{declare}~\code{target} variable or a \code{declare}~\code{target} common block is declared 
...
\item A blank common block cannot appear in a \code{declare}~\code{target} directive.

\item A variable can only appear in a \code{declare}~\code{target} directive in the scope in which it 
is declared. It must not be an element of a common block or appear in an 
\code{EQUIVALENCE} statement.

\item A variable that appears in a \code{declare}~\code{target} directive must be declared in the 
Fortran scope of a module or have the \code{SAVE} attribute, either explicitly or implicitly. 
\end{itemize}
\fortranspecificend
...
encounters a \code{parallel} region.
...
\item \code{distribute}, \code{parallel}, \code{parallel}~\code{sections}, \code{parallel}~\code{workshare}, 
...
All associated \plc{do-loops} must be \plc{do-constructs} as defined by the Fortran standard. If an 
\code{end}~\code{do} directive follows a \plc{do-construct} in which several loop statements share a \code{DO} 
termination statement, then the directive can only be specified for the outermost of these 
\code{DO} statements.
\fortranspecificend

\binding
The binding thread set for a \code{distribute} region is the set of master threads created by 
a \code{teams} construct. A \code{distribute} region binds to the innermost enclosing \code{teams} 
region. Only the threads executing the binding \code{teams} region participate in the 
...
\item A list item may appear in a \code{linear} or \code{firstprivate} clause but not both.
\item A list item may appear in a \code{linear} or \code{lastprivate} clause but not both.
...
\binding
The binding task for a \code{target}~\code{enter}~\code{data} construct is the encountering task. 
...
\item \code{target}~\code{exit}~\code{data}, see \specref{subsec:target exit data Construct}.
...
\item \code{task}~\code{scheduling}~\code{constraints}, see \specref{subsec:Task Scheduling}. 
\end{itemize}
...
\fortranspecificend
\binding
The binding task for a \code{target}~\code{exit}~\code{data} construct is the encountering task. 
...
\begin{itemize}
\item \code{target}~\code{data}, see \specref{subsec:target data Construct}.
\item \code{target}~\code{enter}~\code{data}, see \specref{subsec:target enter data Construct}.
\item \code{map} clause, see \specref{subsec:map Clause}.
\item \plc{default-device-var}, see \specref{subsec:ICV Descriptions}.
\item \code{task}, see \specref{subsec:task Construct}.
\item \code{task}~\code{scheduling}~\code{constraints}, see \specref{subsec:Task Scheduling}. 
...
followed by a \code{for} directive.
\ccppspecificend

\fortranspecificstart
The semantics are identical to explicitly specifying a \code{parallel} directive immediately 
followed by a \code{do} directive, and an \code{end}~\code{do} directive immediately followed by an 
...
where \plc{clause} can be any of the clauses accepted by the \code{parallel}, \code{for} or \code{simd} 
directives, except the \code{nowait} clause, with identical meanings and restrictions.
\ccppspecificend
\end{samepage}
...
where \plc{clause} can be any of the clauses accepted by the \code{parallel}, \code{do} or \code{simd} 
directives, with identical meanings and restrictions.
...
\summary
The target parallel construct is a shortcut for specifying a \code{target} 
construct containing a \code{parallel} construct.
...
\code{parallel} directives with identical meanings and restrictions.

The optional \code{if} clause directive-name modifier, \plc{targetparallel:},
does not change the behavior of the clause. It provides compatibility
with the syntax of the multi-\code{if} clause of combined constructs,
as described in \specref{subsec:Multi-if Clause}.
\ccppspecificend
...
!\$omp target parallel \plc{[clause[ [},\plc{] clause] ... ]}
    \plc{structured-block}
\plc{[}!\$omp end target parallel\plc{]}
\end{boxedcode}
\end{samepage}
...
construct containing a parallel loop construct.
...
where \plc{clause} can be any of the clauses accepted by the \code{target} or
\code{parallel}~\code{for} directives with identical meanings and restrictions.


Conditional execution of the target and/or conditional multi-threading of the \code{parallel} \code{for} can
be specified with the \plc{target:} and \plc{parallelfor:} \plc{directive-name} modifiers in the code{if} clause.
See Multi-if Clause in \specref{subsec:Multi-if Clause}.
\ccppspecificend
...
where \plc{clause} can be any of the clauses accepted by the \code{target} or 
\code{parallel}~\code{do} directives with identical meanings and restrictions.

Conditional execution of the target and/or conditional multi-threading of the \code{parallel} \code{do} can
be specified with the \plc{target:} and \plc{paralleldo:} \plc{directive-name} modifiers in the code{if} clause.
See Multi-if Clause in \specref{subsec:Multi-if Clause}.
...
\restrictions
The restrictions for the \code{target} and parallel loop constructs apply.
...
\summary
The \code{target} \code{parallel} \code{loop} \code{SIMD} construct is a shortcut for specifying a \code{target} 
construct containing a parallel loop SIMD construct.

\syntax
The syntax of the \code{target} \code{parallel} \code{loop} \code{SIMD} construct is as follows:
...
where \plc{clause} can be any of the clauses accepted by the \code{target} or
\code{parallel}~\code{for}~\code{simd} directives with identical meanings and restrictions.

Conditional execution of the target and/or conditional multi-threading of the \code{parallel} \code{for} \code{simd} can
be specified with the \plc{target:} and \plc{parallelforsimd:} \plc{directive-name} modifiers in the code{if} clause.
See Multi-if Clause in \specref{subsec:Multi-if Clause}.
\ccppspecificend
...
where \plc{clause} can be any of the clauses accepted by the \code{target} or 
\code{parallel}~\code{do}~\code{simd} directives with identical meanings and restrictions.

Conditional execution of the target and/or conditional multi-threading of the \code{parallel} \code{do} \code{simd} can
be specified with the \plc{target:} and \plc{paralleldosimd:} \plc{directive-name} modifiers in the code{if} clause.
See Multi-if Clause in \specref{subsec:Multi-if Clause}.
...
\restrictions
The restrictions for the \code{target} and parallel loop SIMD constructs apply.
...
\summary
The target parallel construct is a shortcut for specifying a \code{target} 
construct containing a \code{SIMD} construct.
...
The optional \code{if} clause directive-name modifier, \plc{targetsimd:},
does not change the behavior of the clause. It provides compatibility
with the syntax of the multi-\code{if} clause of combined constructs,
as described in \specref{subsec:Multi-if Clause}.
\ccppspecificend
...
The effect of an \code{if} clause is as \code{if} it were applied to the 
\code{parallel} construct and not to the \code{target} construct.
...
\fortranspecificstart
The semantics are identical to explicitly specifying a \code{target} diretive 
immediately followed by a \code{simd} directive, 
and an \code{end} \code{simd} directive immediately followed by an \code{end} \code{target} directive.
\fortranspecificend
...
\item Multi-if Clause, \specref{subsec:Multi-if Clause}.
\end{itemize}
...
containing a \code{teams} construct.
...
followed by a \code{teams} directive.
\ccppspecificend

\fortranspecificstart
The semantics are identical to explicitly specifying a \code{target} directive immediately 
followed by a \code{teams} directive, and an \code{end}~\code{teams} directive immediately followed by 
an \code{end}~\code{target} directive.
\fortranspecificend
...
containing a \code{distribute} construct.
...
followed by a \code{distribute} directive. Some clauses are permitted on both constructs.
...
containing a \code{distribute}~\code{simd} construct.
...
\descr
The semantics are identical to explicitly specifying a \code{teams} directive immediately 
followed by a \code{distribute}~\code{simd} directive. Some clauses are permitted on both constructs.
...
containing a \code{teams}~\code{distribute} construct.
...
containing a \code{teams}~\code{distribute}~\code{simd} construct.
...
where \plc{clause} can be any of the clauses accepted by the \code{target} or 
\code{teams}~\code{distribute}~\code{parallel}~\code{do} directives with identical 
meanings and restrictions.

If an \code{end}~\code{target}~\code{teams}~\code{distribute}~\code{parallel}~\code{do} directive is not specified, an 
\code{end}~\code{target}~\code{teams}~\code{distribute}~\code{parallel}~\code{do} directive is assumed at the end of 
the \plc{do-loops}.
\fortranspecificend
...
\restrictions
The restrictions for the \code{target} and teams distribute parallel loop constructs apply.
...
\restrictions
The restrictions for the \code{target} and teams distribute parallel loop SIMD constructs apply.
...
\begin{boxedcode}
\#pragma omp critical \plc{[}(\plc{name})\plc{] new-line}
   \plc{structured-block}
\end{boxedcode}
\ccppspecificend
...
waits at the beginning of a \code{critical} region until no thread in the contention group is 
executing a \code{critical} region with the same name. The \code{critical} construct enforces 
exclusive access with respect to all \code{critical} constructs with the same name in all 
...
\binding
A \code{taskgroup} region binds to the current task region. The binding thread set of the 
\code{taskgroup} region is the current team.

\descr
When a thread encounters a \code{taskgroup} construct, it starts executing the region. There 
is an implicit task scheduling point at the end of the \code{taskgroup} region. The current 
task is suspended at the task scheduling point until all child tasks that it generated in the 
\code{taskgroup} region and all of their descendent tasks complete execution.
...
\code{atomic} regions enforce exclusive access with respect to other \code{atomic} regions that 
access the same storage location \plc{x} among all threads in the contention group without 
...
For all forms of the \code{atomic} construct, any combination of two or more of these 
\code{atomic} constructs enforces mutually exclusive access to the locations designated by \plc{x}. 
To avoid race conditions, all accesses of the locations designated by \plc{x} that could 
potentially occur in parallel must be protected with an \code{atomic} construct. 
...
\item At entry to and exit from \code{parallel, critical, ordered, target} and \code{target data} regions.
...
\summary
The \code{ordered} construct specifies a structured block in a loop region that will be 
executed in the order of the loop iterations. This sequentializes and orders the code 
within an \code{ordered} region while allowing code outside the region to run in parallel. 
...
where \plc{clause} is:
\begin{indentedcodelist}
depend(\plc{dependence-type} \plc{[}: \plc{vec]})
\end{indentedcodelist}
...
binds to the innermost enclosing loop region. \code{ordered} regions that bind to different 
loop regions execute independently of each other.

\descr
If no clause is specified, the \code{ordered} construct behaves as if the \code{threads} clause had been specified. If the \code{threads} clause is specified, the threads in the team executing the loop region execute \code{ordered} regions sequentially in the order of the loop iterations. If any \code{depend} clauses are specified then those clauses specify the order in which the threads in the team execute \code{ordered} regions. When the thread executing the first iteration of the loop encounters an \code{ordered} construct, it can enter the \code{ordered} region without waiting. When a thread executing any subsequent iteration encounters an \code{ordered} region, it waits at the beginning of that ordered region until execution of all \code{ordered} regions belonging to all previous iterations or iterations specified by the \code{depend} clauses have completed. If the \code{simd} clause is specified, the \code{ordered} regions encountered by any thread will use only a single SIMD lane to execute the \code{ordered} regions in the order of the loop iterations.

\restrictions
Restrictions to the \code{ordered} construct are as follows:

\begin{itemize}
\item The loop region to which an \code{ordered} region without any clause or with a \code{threads} clause binds must have an \code{ordered} clause without the parameter specified on the corresponding loop directive. 

\item The loop region to which an \code{ordered} region with any \code{depend} clauses binds must have an \code{ordered} clause with the parameter specified on the corresponding loop directive. 

\item An \code{ordered} construct with the \code{depend} clause specified must be closely nested inside a loop (or parallel loop) construct. 

\item An \code{ordered} construct with the \code{depend} clause specified must not be closely nested in a \code{simd} construct. 

\item During execution of an iteration of a loop or a loop nest within a loop region, a thread must not execute more than one ordered region that arises from an \code{ordered} clause with no parameter and binds to the same loop region. 
\end{itemize}
\cppspecificstart
\begin{itemize}
\item A throw executed inside a \code{ordered} region must cause execution to resume within 
the same \code{ordered} region, and the same thread that threw the exception must catch 
it.
\end{itemize}
\cppspecificend
...
where \plc{vec} is the iteration vector.

\descr
Task dependences are derived from the \plc{dependence-type} of a \code{depend} clause and its list 
items, where \plc{dependence-type} is one of the following:

The \code{in} \plc{dependence-type}. The generated task will be a dependent task of all previously 
generated sibling tasks that reference at least one of the list items in an \code{out} or \code{inout} 
\plc{dependence-type} list.

The \code{out} and \code{inout} \plc{dependence-types}. The generated task will be a dependent task of 
all previously generated sibling tasks that reference at least one of the list items in an 
\code{in}, \code{out}, or \code{inout} \plc{dependence-type} list.
...
Loop dependences are derived from the \plc{dependence-type} of a \code{depend} clause and the iteration vector \plc{vec}.

The \code{source} \plc{dependence-type} specifies the completion of cross-iteration dependences that arise from the current iteration.

The \code{sink} \plc{dependence-type} specifies a sink of cross-iteration dependences. The current iteration is blocked until all source iterations specified complete execution.

The iteration vector \plc{vec} must have the form of 
(x$_{1}$ $\pm $ d$_{1}$, x$_{2}$ $\pm $ d$_{2}$ , ... , x$_{n}$ $\pm $ d$_{n}$), 
where \plc{n} is the value specified by the \code{ordered} clause in the loop directive, 
x$_{\plc{i}}$ denotes the loop iteration variable of the \plc{i}-th nested loop associated with the loop directive, and 
d$_{\plc{i}}$ is a constant non-negative integer. 
If the iteration vector \plc{vec} indicates a lexicographically later iteration, it can cause a deadlock. If the iteration vector \plc{vec} indicates an invalid iteration, the \code{ordered} construct with the \code{depend} clause is ignored.
...
\item If the \plc{dependence-type} is \code{source}, the iteration vector \plc{vec} must not be specified.  
\end{itemize}
...
\binding
The binding thread set of the \code{cancel} region is the current team. The \code{cancel} region 
binds to the innermost enclosing construct of the type corresponding to the \plc{type-clause} 
specified in the directive (that is, the innermost \code{parallel}, \code{sections}, loop, or 
\code{taskgroup} construct).

\descr
The \code{cancel} construct activates cancellation of the binding construct only if \plc{cancel-var} 
is \code{true}, in which case the construct causes the encountering task to continue execution 
at the end of the canceled construct. If \plc{cancel-var} is \code{false}, the \code{cancel} construct is 
ignored.
...
When a thread reaches one of the above cancellation points and if \plc{cancel-var} is \plc{true}, 
the thread immediately checks for active cancellation (that is, if cancellation has been 
activated by a \code{cancel} construct). If cancellation is active, the encountering thread 
...
When cancellation of tasks is activated through the \code{cancel}~\code{taskgroup} construct, the 
innermost enclosing \code{taskgroup} will be canceled. The task that encountered the 
\code{cancel}~\code{taskgroup} construct continues execution at the end of its \code{task} region, 
which implies completion of that task. Any task that belongs to the innermost enclosing 
\code{taskgroup} and has already begun execution must run to completion or until a 
cancellation point is reached. Upon reaching a cancellation point and if cancellation is active, the task continues execution at the end of its \code{task} region, which implies the task's completion. Any task that belongs to the innermost enclosing \code{taskgroup} and that 
has not begun execution may be discarded, which implies its completion.

When cancellation is active for a \code{parallel}, \code{sections}, \code{for}, or \code{do} region, each 
...
region is a \code{sections}, \code{for}, or \code{do} region, no task cancellation occurs.
...
\notestart
\noteheader – The programmer is responsible for releasing locks and other synchronization data structures that might cause a deadlock when a \code{cancel} construct is encountered and blocked threads cannot be canceled. The programmer is also responsible for ensuring proper synchronization to avoid deadlocks that might arise from cancellation of OpenMP regions that contain OpenMP synchronization constructs.
\noteend
...
\item If \plc{construct-type-clause} is \code{taskgroup}, the \code{cancel} construct must be closely 
nested inside a \code{task} construct. Otherwise, the \code{cancel} construct must be closely 
...
\item If \plc{construct-type-clause} is \code{taskgroup} and the \code{cancel} construct is not nested 
inside a \code{taskgroup} region, then the behavior is unspecified.
...
\binding
A \code{cancellation}~\code{point} region binds to the current task region.
...
\plc{cancel-var} is \code{true} the task immediately checks whether cancellation of the region 
...
\restrictions
\begin{itemize}
\item A \code{cancellation}~\code{point} construct for which \plc{construct-type-clause} is 
\code{taskgroup} must be closely nested inside a \code{task} construct. A 
\code{cancellation}~\code{point} construct for which \plc{construct-type-clause} 
is not \code{taskgroup} must be closely 
nested inside an OpenMP construct that matches the type specified in 
\plc{construct-type-clause}.

\item An OpenMP program with orphaned \code{cancellation}~\code{point} constructs is 
non-conforming. 
\end{itemize}
...
during the execution of \code{parallel}, \code{task}, \code{simd}, and worksharing regions.

\begin{itemize}
\item \specref{subsec:Data-sharing Attribute Rules} 
describes how the data-sharing attributes of variables 
referenced in \code{parallel}, \code{task}, \code{simd}, and worksharing regions are determined.
...
variables referenced in \code{parallel}, \code{task}, \code{simd} or worksharing constructs are 
described in \specref{subsec:Data-Sharing Attribute Clauses}
...
\code{parallel}, \code{task}, \code{simd}, and worksharing regions are determined. The following two 
...
\item The loop iteration variable(s) in the associated \plc{for-loop(s)} of a \code{for} or 
\code{parallel}~\code{for} construct is (are) private. 

\item The loop iteration variable in the associated \plc{for-loop} of a \code{simd} construct with just 
one associated \plc{for-loop} is linear with a \plc{constant-linear-step} that is the increment of 
...
\item The loop iteration variable(s) in the associated \plc{do-loop(s)} of a \code{do} or \code{parallel}~\code{do} 
...
one associated \plc{do-loop} is linear with a \plc{constant-linear-step} that is the increment of 
...
\item A loop iteration variable for a sequential loop in a \code{parallel} or \code{task} construct is 
...
\begin{itemize}
\item The loop iteration variable(s) in the associated \plc{for-loop(s)} of a \code{for} or 
\code{parallel}~\code{for} construct may be listed in a \code{private} or \code{lastprivate} clause. 

\item The loop iteration variable in the associated \plc{for-loop} of a \code{simd} construct with just 
one associated \plc{for-loop} may be listed in a \code{linear} clause with a \plc{constant-linear-step} 
...
\fortranspecificstart
\begin{itemize}
\item The loop iteration variable(s) in the associated \plc{do-loop(s)} of a \code{do} or 
\code{parallel}~\code{do} 
...
one associated \plc{do-loop} may be listed in a \code{linear} clause with a \plc{constant-linear-step} 
that is the increment of the associated loop.

\item The loop iteration variables in the associated \plc{do-loops} of a \code{simd} construct with 
multiple associated \plc{do-loops} may be listed in a \code{lastprivate} clause. 

\item Variables used as loop iteration variables in sequential loops in a \code{parallel} or 
\code{task} construct may be listed in data-sharing clauses on the construct itself, and on 
...
\begin{itemize}
\item In a \code{parallel} or \code{task} construct, the data-sharing attributes of these variables are 
...
\item For constructs other than \code{task}, if no \code{default} clause is present, these variables reference the variables with the same names that exist in the enclosing context.

\item In a \code{task} construct, if no \code{default} clause is present, a variable that in the 
...
\fortranspecificstart
\begin{itemize}
\item In an orphaned \code{task} construct, if no \code{default} clause is present, dummy arguments 
are firstprivate.
\end{itemize}
\fortranspecificend
\end{samepage}
...
\begin{itemize}
\item In a \code{task} construct, if no \code{default} clause is present, a variable whose data-sharing 
...
implicitly determined in a \code{task} construct are described in 
...
persist between two consecutive active \code{parallel} regions only if all the following 
conditions hold:
...
copy of a \code{threadprivate} variable or a variable in a \code{threadprivate} common 
...
If a \code{threadprivate} variable or a variable in a \code{threadprivate} common block is 
...
\item If a \code{threadprivate} variable or a \code{threadprivate} common block is declared 
with the \code{BIND} attribute, the corresponding C entities must also be specified in a 
...
are referenced in a \code{parallel}, \code{task} or \code{teams} construct and would otherwise be 
...
\begin{itemize}
\item Only a single default clause may be specified on a \code{parallel}, \code{task}, or \code{teams} 
directive. 
\end{itemize}
...
a \code{parallel}, \code{task} or \code{teams} construct. 
...
from the \code{parallel}, \code{task} or \code{teams} construct if it is associated with a target or a 
subobject of a target that is in a \code{private}, \code{firstprivate}, \code{lastprivate}, or 
\code{reduction} clause inside the construct.
...
List items that appear in a \code{private}, \code{firstprivate}, or \code{reduction} clause in a 
\code{parallel} construct may also appear in a \code{private} clause in an enclosed \code{parallel}, 
\code{task}, or worksharing, or \code{simd} construct. 

List items that appear in a \code{private} or \code{firstprivate} clause in a \code{task} construct 
may also appear in a \code{private} clause in an enclosed \code{parallel} or \code{task} construct. 

List items that appear in a \code{private}, \code{firstprivate}, \code{lastprivate}, or 
\code{reduction} clause in a worksharing construct may also appear in a \code{private} clause 
in an enclosed \code{parallel} or \code{task} construct. 
...
and type parameters is allocated: once for each implicit task in the \code{parallel} 
construct; once for each task generated by a \code{task} construct; and once for each SIMD 
lane used by a \code{simd} construct. The initial value of the new list item is undefined. 
Within a \code{parallel}, \code{worksharing}, \code{task}, \code{teams}, or \code{simd} region, the initial status 
...
in a \code{private} clause and \plc{B} is a variable that is storage-associated with \plc{A}, then:

\begin{itemize}
\item The contents, allocation, and association status of \plc{B} are undefined on entry to the 
\code{parallel}, \code{task}, \code{simd}, or \code{teams} region.
...
\cppspecificstart
\item A variable that is part of another variable (as an array or structure element) cannot 
appear in a \code{private} clause except if the \code{private} clause is associated with a construct within a class member function and the variable is a member of the same class or the variable is an accessible member of a base class.
...
For a \code{firstprivate} clause on a \code{parallel}, \code{task}, or \code{teams} construct, the initial 
...
\code{firstprivate} clause on a worksharing construct, the initial value of the new list 
...
\begin{itemize}
\item A list item that is private within a \code{parallel} region must not appear in a 
\code{firstprivate} clause on a worksharing construct if any of the worksharing 
regions arising from the worksharing construct ever bind to any of the \code{parallel} 
...
\item A list item that appears in a \code{reduction} clause of a \code{parallel} construct must not 
appear in a \code{firstprivate} clause on a worksharing or \code{task} construct if any of 
the worksharing or \code{task} regions arising from the worksharing or \code{task} construct 
ever bind to any of the \code{parallel} regions arising from the \code{parallel} construct.
...
\item A list item that appears in a \code{reduction} clause in a worksharing construct must not 
appear in a \code{firstprivate} clause in a task construct encountered during execution 
...
\begin{itemize}
\item A list item that is private within a \code{parallel} region, or that appears in the 
\code{reduction} clause of a \code{parallel} construct, must not appear in a \code{lastprivate} 
clause on a worksharing construct if any of the corresponding worksharing regions 
ever binds to any of the corresponding \code{parallel} regions.
...
The following table lists each \plc{reduction-identifier} that is implicitly declared at every 
...
\tablelasttail{\hline}
\begin{supertabular}{ p{0.1\textwidth} p{0.3\textwidth} p{0.45\textwidth}}
...
where \code{omp\_in} and \code{omp\_out} correspond to two identifiers that refer to storage of the 
type of the list item. \code{omp\_out} holds the final value of the combiner operation.
\ccppspecificend % L0 ^^^^^
\bigskip
...
The following table lists each \plc{reduction-identifier} that is implicitly declared for numeric 
...
\tablelasttail{\hline}
\begin{supertabular}{ p{0.1\textwidth} p{0.30\textwidth} p{0.60\textwidth}}
\code{+} & \code{omp\_priv = 0} & \code{omp\_out = omp\_in + omp\_out}\\
\code{*} & \code{omp\_priv = 1} & \code{omp\_out = omp\_in * omp\_out}\\
\code{-} & \code{omp\_priv = 0} & \code{omp\_out = omp\_in + omp\_out}\\
...
specified by the \plc{initializer-clause} and the combiner in the \code{declare}~\code{reduction} 
directive.
...
If the list item is an array or an array section it will be treated as if a \code{reduction} clause would be applied to each separate element of the array section. The elements of each private array section will be allocated contiguously.
\ccppspecificend
...
\item An original list item with the \code{POINTER} attribute must be associated at entry to the 
construct containing the \code{reduction} clause. Additionally, the list item must not be 
deallocated, allocated, or pointer assigned within the region.

\item An original list item with the \code{ALLOCATABLE} attribute must be in the allocated state 
at entry to the construct containing the \code{reduction} clause. Additionally, the list 
...
times \plc{linear-step}. If \plc{linear-step} is not specified it is assumed to be 1. The value 
...
\ccppspecificstart
\item A \plc{list-item} that appears in a \code{linear} clause must be of integral or pointer type, or must be a reference to an integral or pointer type. 
\ccppspecificend

\fortranspecificstart
\item A \plc{list-item} that appears in a \code{linear} clause must be of type \code{integer}.
...
If a new list item is created for an array section and the type of the variable appearing in that array section is pointer, reference to array, or reference to pointer then the variable is implicitly treated as if it had appeared in a map clause with a \plc{map-type} of \code{alloc}. The corresponding variable is assigned the address of the storage location of the corresponding array section in the device data environment. If the variable appears in a \code{to} or \code{from} clause in a \code{target}~\code{update} region during the lifetime of the new list item but not as part of the specification of an array section, the behavior is unspecified. 
\ccppspecificend
...
\item A variable that is part of another variable (such as a field of a structure) but is not an 
array element or an array section cannot appear in a \code{map} clause. 
...
an array section derived from that variable must not appear as list items of \code{map}
clauses of the same construct. 
...
\item \plc{typename-list} is list of type names 
...
% Is there an equal sign missing in the original spec after omp_priv?
\item \plc{initializer-clause} is \code{initializer(}\plc{initializer-expr}\code{)} 
where \plc{initializer-expr} is\linebreak
\code{omp\_priv} \plc{initializer} or \plc{function-name}\code{(}\plc{argument-list}\code{)} 
\end{itemize}
...
must be specified for any function that can be accessed through \plc{combiner} and 
...
\fortranspecificstart
If \plc{reduction-identifier} is used in a \code{target} region then a \code{declare}~\code{target} construct 
must be specified for any function or subroutine that can be accessed through \plc{combiner} 
...
\item If a character type with deferred or assumed length parameter is specified in a \code{declare}~\code{reduction} directive, no other \code{declare}~\code{reduction} directives with Fortran character type of the same kind parameter and the same \plc{reduction-identifier} are allowed in the same scope.
...
\begin{itemize}
\item A worksharing region may not be closely nested inside a worksharing, explicit \code{task}, 
\code{critical}, \code{ordered}, \code{atomic}, or \code{master} region.

\item A \code{barrier} region may not be closely nested inside a worksharing, explicit \code{task}, 
\code{critical}, \code{ordered}, \code{atomic}, or \code{master} region.

\item A \code{master} region may not be closely nested inside a worksharing, \code{atomic}, or 
explicit \code{task} region.

\item An \code{ordered} region may not be closely nested inside a \code{critical}, \code{atomic}, or 
explicit \code{task} region. 

\item An \code{ordered} region must be closely nested inside a loop region (or parallel loop 
region) with an \code{ordered} clause.
...
\item OpenMP constructs may not be nested inside a \code{simd} region.

\item If a \code{target}, \code{target}~\code{update}, or \code{target}~\code{data} construct appears within a 
\code{target} region then the behavior is unspecified. 

\item If specified, a \code{teams} construct must be contained within a \code{target} construct. That 
\code{target} construct must contain no statements or directives outside of the \code{teams} 
construct. 

\item \code{distribute}, \code{parallel}, \code{parallel}~\code{sections}, \code{parallel}~\code{workshare}, and 
...
\item If \plc{construct-type-clause} is \code{taskgroup}, the \code{cancel} construct must be closely 
nested inside a \code{task} construct and the \code{cancel} construct must be nested inside a 
\code{taskgroup} region. Otherwise, the \code{cancel} construct must be closely nested inside 
an OpenMP construct that matches the type specified in \plc{construct-type-clause} of the 
\code{cancel} construct. 

\item A \code{cancellation}~\code{point} construct for which \plc{construct-type-clause} is 
\code{taskgroup} must be nested inside a \code{task} construct. A \code{cancellation}~\code{point} 
construct for which \plc{construct-type-clause} is not \code{taskgroup} must be closely nested 
inside an OpenMP construct that matches the type specified in \plc{construct-type-clause}. 
\end{itemize}
