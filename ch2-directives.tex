...
The \plc{parallel-begin} event occurs in the thread encountering the
\code{parallel} construct before any implicit task starts to execute
the parallel region's work.

If the parallel region activates an idle thread, an \plc{idle-end}
event occurs in the newly activated thread.

If the parallel region creates a thread, a \plc{thread-begin}
event occurs in the context of the new thread as the first event in
that thread.

On creation of the implicit task, an \plc{implicit-task-begin} event
occurs in the thread executing the implicit task after the implicit
task is fully initialized but before the task begins to execute the
structured block of the \code{parallel} construct.

An \plc{implicit-task-begin} event in a thread for a parallel region 
occurs after an \plc{idle-end} or \plc{thread-begin} event in the
thread for the parallel region. 

When finishing an implicit task, an \plc{implicit-task-end} event
occurs in the executing thread after the implicit task executes its
closing synchronization barrier but before the task is destroyed.

The \plc{parallel-end} event occurs in the thread encountering the
\code{parallel} construct but before any implicit task starts to
execute the parallel region's work.

If a thread is destroyed at the end of a parallel region, a
\plc{thread-end} event occurs in the context of the thread as the last
event in the thread prior to its destruction.

If a non-master thread is not destroyed at the end of a parallel
region, an \plc{idle-begin} event occurs after the thread's
\plc{implicit-task-end} event for the parallel region.

\tools

A thread invokes a registered \code{ompt\_callback\_parallel\_begin}
...
A thread invokes a registered \code{ompt\_callback\_implicit\_task}
callback for each occurrence of a \plc{implicit-begin} and
\plc{implicit-end} event in that thread. The callback occurs in the
...
On creation of the implicit task, each thread shall invoke the 
\code{ompt\_event\_implicit\_task} callback, if registered, with 
\code{endpoint = ompt\_scope\_begin} after an
implicit \code{task} is fully initialized but before the \code{task} begins 
to execute the structured block of the \code{parallel} construct.

When finishing an implicit task, the executing thread shall invoke the 
\code{ompt\_event\_implicit\_task} callback, if registered, 
with the \code{endpoint = ompt\_scope\_end} after the implicit
\code{task} executes its closing synchronization barrier but before the 
\code{task} is destroyed.
The binding region of the \code{ompt\_event\_implicit\_task} callback is the 
implicit \code{task}. The callback has type signature \code{ompt\_scoped\_implicit\_callback\_t}.

On exit from the \code{parallel} region, the master thread
shall invoke the callback \code{ompt\_event\_parallel\_end} if registered.
The encountering thread invokes this callback 
after a \code{parallel} region executes its closing synchronization barrier 
but before resuming execution of the parent task.  
The callback has type signature \code{ompt\_parallel\_end\_callback\_t}. 
...
\item \code{ompt\_parallel\_begin\_callback\_t} type signature of the parallel 
begin callback, see \specref{sec:ompt_parallel_begin_callback_t}.

\item \code{ompt\_implicit\_callback\_t} type signature of the implicit
task begin and end callbacks, see 
\specref{sec:ompt_implicit_callback_t}.

\item \code{ompt\_parallel\_end\_callback\_t} type signature of the parallel end
callback, see 
\specref{sec:ompt_parallel_end_callback_t}.
...
The \plc{#1{-end}} event occurs after a \code{#1} region finished execution of 
the structured block but before resuming execution of the encountering implicit 
task.

\tools

A thread invokes a registered \code{ompt\_callback\_work}
...
\code{ompt\_scope\_begin} or \code{ompt\_scope\_end} 
as its \plc{endpoint} argument, as appropriate, and the
...
\item \code{ompt\_work\_callback\_t} type signature of the ordered
begin and end callback, see 
\specref{sec:ompt_work_callback_t}.
...
\item \code{ompt\_work\_callback\_t} type signature of the sections
begin and end callback, see 
\specref{sec:ompt_work_callback_t}.
\end{itemize}
...
\tools
On entry to the \code{single} region, the executing thread
may invoke the callback \code{ompt\_event\_work}, if registered, with
\code{endpoint=ompt\_scope\_begin} and \code{wstype=ompt\_work\_single\_executor}.
The threads of the team which do not execute the \code{single} region may invoke 
the callback \code{ompt\_event\_work}, if registered, with
\code{wstype=ompt\_work\_single\_other}.
The encountering thread invokes this callback after an \code{implicit task} encounters a 
\code{single} construct but before the task starts the execution of the structured 
block of the \code{single} region.
This callback has type signature \code{ompt\_work\_callback\_t}.

On exit from the \code{single} region, the executing thread
may invoke the callback \code{ompt\_event\_work}, if registered, with
\code{endpoint=ompt\_scope\_end} and \code{wstype=ompt\_work\_single\_executor}.
The threads of the team which do not execute the \code{single} region may invoke 
the callback \code{ompt\_event\_work}, if registered, with
\code{wstype=ompt\_work\_single\_other}.
The encountering thread invokes this callback 
after a \code{single} region finished execution of the structured block 
but before resuming execution of the encountering implcit task.
...
\item \code{ompt\_work\_callback\_t} type signature of the 
single begin and end callback, see 
\specref{sec:ompt_work_callback_t}.
...
\crossreferences
\begin{itemize}
\item \code{ompt\_work\_callback\_t} type signature of the workshare
begin and end callback, see 
\specref{sec:ompt_work_callback_t}.
\end{itemize}
...
\def\omptTaskCreateEvents#1#2{
  The \plc{#1} event  occurs when the executing thread encounters a \code{#1} construct 
  that causes a \code{task} to be created.
}

\def\omptTaskCreateTools#1#2{
A thread invokes a registered \code{ompt\_callback\_task\_create}
callback for each occurrence of a \plc{#1} event in that
thread.  The callback occurs in the task encountering
the \code{#1} construct. This callback has the type signature
\code{ompt\_callback\_task\_create\_t}.
}

\def\omptTaskCreate#1#2{
  \events
  \omptTaskCreateEvents{#1}{#2}
  \tools
  \omptTaskCreateTools{#1}{#2}  
  }
\omptTaskCreate{task}{ompt\_explicit\_task}

Whenever a \code{task} completes and before scheduling another \code{task}, 
the encountering thread shall invoke the \code{ompt\_event\_task\_schedule} 
callback if registered, with \code{prior\_completed=true}. 
The callback executes in the context of the newly-scheduled \code{task}.
This callback has type signature \code{ompt\_task\_schedule\_callback\_t}.
\notestart
\noteheader – For convenience of implementation, the task schedule might switch
to the task bond to the scheduling point, before the next task is scheduled.
\noteend
...
\item \code{if} Clause, see \specref{sec:if Clause}.
\item \code{ompt\_task\_create\_callback\_t} type signature of the task generation
callback, see 
\specref{sec:ompt_task_create_callback_t}.
\item \code{ompt\_task\_schedule\_callback\_t} type signature of the task scheduling
callback, see 
\specref{sec:ompt_task_schedule_callback_t}.
\end{itemize}
...
\tools
\def\omptWorkTaskloop#1#2
{
On entry to the \code{#1} region, the executing thread
may invoke the callback \code{ompt\_event\_work}, if registered, with
\code{endpoint=ompt\_scope\_begin} and \code{wstype=#2}.
The encountering thread invokes this callback after a \code{task} encounters a 
\code{#1} construct but before the task starts the execution of any taskloop iterations.
This callback has type signature \code{ompt\_scoped\_work\_callback\_t}.

On exit from the \code{#1} region, the executing thread
may invoke the callback \code{ompt\_event\_work}, if registered, with
\code{endpoint=ompt\_scope\_end} and \code{wstype=#2}.
The encountering thread invokes this callback 
after a \code{#1} region finished execution of the all taskloop iterations 
but before resuming execution of the encountering task.
}
\omptWorkTaskloop{taskloop}{ompt\_work\_taskloop}
...
\toolsToDo{No tools paragraph needed for \code{taskyield}?}
...
When scheduling a new \code{task} at a scheduling point, 
the scheduling thread shall invoke the \code{ompt\_event\_task\_schedule} 
...
an OpenMP implementation will invoke the this callback for a target device
after OpenMP is initialized for the device but before beginning
execution of any OpenMP construct on the device.  This callback has
type signature \code{ompt\_device\_initialize\_callback\_t}.

\crossreferences
\begin{itemize}
\item \code{ompt\_device\_initialize\_callback\_t}, see
\specref{sec:ompt_device_initialize_callback_t}.
\end{itemize}
...
\def\omptTargetRegionEvents#1#2
{
The \plc{#1{-begin}} event occurs after a thread enters a
\code{#1} region.

The \plc{#1{-end}} event occurs when a thread exits a
\code{#1} region.
}

\def\omptTargetRegionTools#1#2
{
A thread invokes a registered \code{ompt\_callback\_target}
callback for each occurrence of a \plc{#1{-begin}} and
\plc{#1{-end}} event in that thread. The callback has type signature
...
\def\omptTargetRegion#1#2
{
\events
\omptTargetRegionEvents{#1}{#2}
\tools
\omptTargetRegionTools{#1}{#2}
}
\omptTargetRegion{\code{target}~\code{data}}{ompt\_target}
...
\item \code{ompt\_target\_callback\_t} type signature of the target
begin and end callback, see 
\specref{sec:ompt_target_callback_t}.
...
\omptTaskCreateEvents{#1}{ompt\_task\_target}
\omptTargetRegionEvents{#1}{#2}

\tools

\omptTaskCreateTools{#1}{ompt\_task\_target}
\omptTargetRegionTools{#1}{#2}
}
\omptTarget{\code{target}~\code{enter}~\code{data}}{ompt\_target\_enter\_data}
...
When an \code{if} clause is present and the \code{if} clause expression evaluates to \plc{false}, the device is the host. 
%\tools
\omptTarget{\code{target}~\code{exit}~\code{data}}{ompt\_target\_exit\_data}
...
\omptTarget{\code{target}}{ompt\_target}

Prior to submitting a kernel for execution on a target device, 
an OpenMP implementation invokes callback 
\code{ompt\_event\_target\_submit}. This callback has type
signature \code{ompt\_target\_submit\_callback\_t}.  
...
%\tools
\omptTarget{\code{target}~\code{update}}{ompt\_target\_update}
...
\item \code{ompt\_work\_callback\_t} type signature of the distribute
begin and end callback, see 
\specref{sec:ompt_work_callback_t}.
\end{itemize}
...
\tools
On entry to the \code{master} region, the master thread
may invoke the callback \code{ompt\_event\_master} with
\code{endpoint=ompt\_scope\_begin} if registered.
The master thread invokes this callback after an \code{implicit task} encounters a 
\code{master} construct but before the task starts the execution of the structured 
block of the \code{master} region.
This callback has type signature \code{ompt\_master\_callback\_t}.

On exit from the \code{master} region, the master thread
may invoke the callback \code{ompt\_event\_master} with
\code{endpoint=ompt\_scope\_end} if registered.
The master thread invokes this callback 
after a \code{master} region finished execution of the structured block 
but before resuming execution of the task.
...
\crossreferences
\begin{itemize}
\item \code{ompt\_master\_callback\_t} type signature of the master
begin and end callback, see 
\specref{sec:ompt_master_callback_t}.
\end{itemize}
...
\tools
\def\omptMutex#1{
On entry to the \code{#1} region, the encountering thread
may invoke the callback \code{ompt\_event\_mutex\_acquire}, if registered, with
\code{kind=ompt\_mutex\_#1}.
The encountering thread invokes this callback after an \code{implicit task} encounters a 
\code{#1} construct but before the associated lock is requested.
This callback has type signature \code{ompt\_mutex\_acquire\_callback\_t}.

Before executing the \code{#1} region's structured block, but after acquiring
the lock, the executing threads may invoke the callback 
\code{ompt\_event\_mutex\_acquired}, if registered, with \code{kind=ompt\_mutex\_#1}.
This callback has type signature \code{ompt\_mutex\_callback\_t}.

On exit from the \code{#1} region, the encountering thread may invoke the 
callback \code{ompt\_event\_mutex\_release}, if registered, with
\code{kind=ompt\_mutex\_#1}.
The encountering thread invokes this callback 
after a \code{#1} region finished execution of the structured block and released
the associated lock
but before resuming execution of the task.
This callback has type signature \code{ompt\_mutex\_callback\_t}.
}
...
\item \code{ompt\_mutex\_acquire\_callback\_t} type signature of the critical 
acquire callback, see 
\specref{sec:ompt_mutex_acquire_callback_t}.

\item \code{ompt\_mutex\_callback\_t} type signature of the critical 
acquired and released callback, see 
\specref{sec:ompt_mutex_callback_t}.
\end{itemize}
...
On entry to the \code{#1} region, the encountering thread
may invoke the callback \code{ompt\_event\_sync\_region}, if registered, with
\code{endpoint=ompt\_scope\_begin}.
The encountering thread invokes this callback after a \code{task} encounters a 
\code{#1} construct but before the task starts the execution of the 
\code{#1} region.
This callback has type signature \code{ompt\_sync\_region\_callback\_t}.

On exit from the \code{#1} region, the encountering thread
may invoke the callback \code{ompt\_event\_sync\_region}, if registered, with
\code{endpoint=ompt\_scope\_end}.
The encountering thread invokes this callback 
after a \code{#1} region finished the synchronization
but before resuming execution of the encountering task.}
\omptSyncRegion{barrier}
...
\crossreferences
\begin{itemize}
\item \code{ompt\_sync\_region\_callback\_t} type signature of the barrier
begin and end callback, see 
\specref{sec:ompt_sync_region_callback_t}.
\end{itemize}
...
\item \code{ompt\_sync\_region\_callback\_t} type signature of the taskwait
begin and end callback, see 
\specref{sec:ompt_sync_region_callback_t}.
\end{itemize}
...
\item \code{ompt\_sync\_region\_callback\_t} type signature of the taskgroup
begin and end callback, see 
\specref{sec:ompt_sync_region_callback_t}.
\end{itemize}
...
The invokation of callbacks is only intended for cases where an \code{atomic}
construct actually results in acquiring a lock.
...
\item \code{ompt\_mutex\_acquire\_callback\_t} type signature of the atomic 
acquire callback, see 
\specref{sec:ompt_mutex_acquire_callback_t}.

\item \code{ompt\_mutex\_callback\_t} type signature of the atomic 
acquired and released callback, see 
\specref{sec:ompt_mutex_callback_t}.
\end{itemize}
...
\tools
A thread that encounters a \code{flush} construct
may invoke the callback \code{ompt\_event\_flush} if registered just after
the flush operation is executed.
This callback has type signature \code{ompt\_flush\_callback\_t}.

\crossreferences
\begin{itemize}
\item \code{ompt\_flush\_callback\_t} type signature of the flush 
begin and end callback, see 
\specref{sec:ompt_flush_callback_t}.
\end{itemize}
...
\item \code{ompt\_mutex\_acquire\_callback\_t} type signature of the ordered 
acquire callback, see 
\specref{sec:ompt_mutex_acquire_callback_t}.

\item \code{ompt\_mutex\_callback\_t} type signature of the ordered 
acquired and released callback, see 
\specref{sec:ompt_mutex_callback_t}.
\end{itemize}
...
If a \code{depend} clause enforces additional constraints on the scheduling of a
\code{task}, the encountering thread may invoke the \code{ompt\_event\_task\_dependences} 
...
The OpenMP encountering thread may invoke the \code{ompt\_event\_task\_dependence\_pair} callback 
to report a dependence between a producer (\code{src\_task\_data})
and a consumer (\code{sink\_task\_data}) that blocked execution of the consumer.
If the encountering thread invokes the callback, it will occur before the consumer knows 
that the dependence is satisfied. This may happen early or late.
Note: this callback is used only to report blocking dependences between sibling tasks whose lifetimes overlap.
No callback will occur if a producer \code{task} finishes before a consumer \code{task} is created.
...
\item \code{ompt\_task\_dependences\_callback\_t} type signature of the 
task dependences callback, see 
\specref{sec:ompt_task_dependences_callback_t}.

\item \code{ompt\_task\_dependence\_callback\_t} type signature of the 
blocking task dependence callback, see 
\specref{sec:ompt_task_dependence_callback_t}.
\end{itemize}
...
\tools
The encountering thread may invoke the \code{ompt\_event\_cancel} callback. 
This callback occurs after encountering the cancel construct and before initiating the requested cancellation.
The callback parameter \code{flags} indicates that a cancelation occurred by setting the \code{ompt\_cancel\_activated} bit. 
The type of construct being canceled is also indicated by setting the corresponding bit defined by the \code{ompt\_cancel\_flag\_t} enumeration.
...
\item \code{ompt\_cancel\_callback\_t} type signature of the cancel callback, see \specref{sec:ompt_cancel_callback_t}.
\end{itemize}
...
\tools
When the encountering thread has detected the activation of a cancellation, it may invoke the \code{ompt\_event\_cancel} callback.
This callback occurs after detecting the activation of a cancelation and before initiating the requested cancellation.
The callback parameter \code{flags} indicates that a cancelation is detected by setting the \code{ompt\_cancel\_detected} bit. 
The type of construct being canceled is also indicated by setting the corresponding bit defined by the \code{ompt\_cancel\_flag\_t} enumeration.
