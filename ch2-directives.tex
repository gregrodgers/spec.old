...
% 383: should we state that a ref to a struct member results in including the whole struct, unless one or more struct member is explicitly referenced? But this cause a problem too, because if a struct member could have been included in a target data, and not in the surrounding target... so I think we cannot force implicit ref goes to imply the full struct member.
...
% example for 383
% typedef struct S {int x, y, z[100]} S; 
% S s, t[100];

% 383: #pragma omp target map(alloc: s) map(to: s.x) map(from: s.y) // allowed
List items of \code{map} clauses in the same construct must not share original storage, with the exceptions of structures sharing storage with their structure elements.
...
% 383 allow structures: remove  
%\item A variable that is part of another variable (such as a field of a structure) but is not an array element or an array section cannot appear in a \code{map} clause. 

%383 #pragma omp target data map(s.x)
%       #pragma omp target map(s.y)  // illegal as some elements of s are already mapped prior to encountering this construct
...
% 383: map(t.z[10:20]),  or map(t[0:20].x), or map(t[10:20].z[10:20])
%         but a different structure u.a.b[0:100].c.d is not allowed
\item If a variable is an element of a structure, only the leftmost symbol of the variable reference, in the rightmost symbol of the variable reference, or both can be array sections.

% 383: map(t[0:20].x, t[0:20].y) is allowed, but map(t[0:20].x, t[30:20].y) are not
\item If two items in the list are elements of a common structure variable, any common array sections must have the same offset and sizes.

% 383: map(s.x) firstprivate(s.y) is not allowed
\item A variable, or the structure of which the variable is a member, cannot be in a private or firstprivate clause in the same construct as
If a variable is a structure or a member of a structure, then this structure or any member of this structure cannot also be declared \code{private} or \code{firstprivate}.
