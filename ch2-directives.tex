...
\code{VALUE} attribute have the same data-sharing attributes as the associated
actual arguments if the actual argument is a scalar variable, structure, an
array that is not a pointer or assumed-shape array, or a simply contiguous
array section.  Otherwise, it is implementation-defined whether such dummy
arguments have the same data-sharing attributes as the associated actual
arguments or are private.
...
argument does not have the \code{VALUE} attribute and is not determined to be
shared as per the rules in 
...
the dummy argument by any other task must be appropriately synchronized with
