...
The syntax of the \code{linear} clause is as follows:
\begin{boxedcode}
linear(\plc{[modifier}:\plc{]list[ }:\plc{ linear-step]})
\end{boxedcode}
...
described in 
\specref{subsubsec:private clause} 
except as noted. In addition, if the \code{val} modifier is used, the value of the 
...
times \plc{linear-step} times the type size. If the \code{ref} modifier is used, the value of the new list item on each iteration of the associated loop(s) corresponds to the value of dereferencing the address obtained of adding that of the original list item and the logical number of the iteration times \plc{linear-step}. If no modifier is specified it is assumed to be \code{val}.
...
\item The \code{ref} modifier can only be used if  the \plc{list-item} is of a reference type.
\cppspecificend

\fortranspecificstart
\item A \plc{list-item} that appears in a \code{linear} clause must be of type \code{integer}.
\item The \code{ref}  modifier can only be used if the \plc{list-item} is a dummy argument.
