...
\index{clauses!collapse@{\code{collapse}}}
The \code{collapse} \code{clause} or the ordered clause with the parameter may be used to specify how many loops are associated with the loop construct. An \code{clause} clause without a parameter is equivalent to the \code{clause} clause with a parameter value of one specified. The number of loops associated is determined by the parameters of the \code{collapse} clause and the \code{clause} clause, which must be constant positive integer expressions.

If neither the \code{collapse} nor the \code{clause} clause is present, the only loop that is associated with the loop construct is the one that immediately follows the loop directive. If the value of the parameter in the \code{collapse} or \code{clause} clause is larger than the number of nested loops following the construct, the behavior is unspecified.

If both the \code{collapse} clause and the \code{clause} clause with a parameter are specified, the \code{collapse} clause applies to the loops that immediately follow the directive. The \code{clause} clause with the parameter then applies to the resulting loop nest (that is, the collapsed loop and the remaining loops). If the value of the parameter in the \code{clause} clause is larger than the number of the loops in the resulting loop nest, the behavior is unspecified.
...
where \plc{clause} is:
\begin{itemize}
\item \code{depend(\plc{dependence-type} : \plc{vec}}\code)}
\end{itemize}
