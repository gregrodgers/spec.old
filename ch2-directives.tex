...
shall trigger the callback \code{ompt\_event\_parallel\_begin} if registered.
...
shall trigger the callback \code{ompt\_event\_parallel\_end} if registered.
...
\def\omptTaskEventForTaskOrTarget#1{
  \ifnum\pdfstrcmp{#1}{task}=0
    Upon encountering a \code{task} construct that causes a \code{task} to be created, the runtime shall trigger the callback \code{ompt\_event\_task\_create}  in the context of the \code{task} that encountered the \code{task} construct.
  \else
    \ifnum\pdfstrcmp{#1}{target}=0
      Upon encountering a \code{target} construct that causes a \code{task} to be created, the runtime shall trigger the callback \code{ompt\_event\_task\_create}  in the context of the \code{task} that encountered \code{target} construct.
    \fi
  \fi
  This callback has type signature \code{ompt\_task\_create\_callback\_t}.
}

\omptTaskEventForTaskOrTarget{task}

Whenever a \code{task} completes and before scheduling another \code{task}, the runtime shall invoke the
\code{ompt\_event\_task\_schedule} callback. The callback executes in the context of the newly-scheduled \code{task}.
This callback has type signature \code{ompt\_task\_schedule\_callback\_t}.

The runtime shall invoke the \code{ompt\_event\_implicit\_task} callback with \code{endpoint = ompt\_scope\_begin} after an
implicit \code{task} is fully initialized but before the \code{task} begins to work.
The runtime invokes this callback with the \code{endpoint = ompt\_scope\_end} after the implicit
\code{task} executes its closing synchronization barrier but before the \code{task} is destroyed.
This callback executes in the context of the implicit \code{task}.
This callback has type signature \code{ompt\_scoped\_implicit\_callback\_t}.

If a \code{task} has any dependences with respect to data objects that constrain its ordering with respect to other tasks,
the runtime may invoke the \code{ompt\_event\_task\_dependences} callback immediately after the callback
announcing the task's creation to announce its dependences with respect to data objects.
This callback has type signature \code{ompt\_task\_dependences\_callback\_t}.

The OpenMP runtime may invoke the \code{ompt\_event\_task\_dependence\_pair} callback to report a dependence between a producer (\code{src\_task\_data})
and a consumer (\code{sink\_task\_data}) that blocked execution of the consumer.
If the runtime invokes the callback, it will occur before the consumer knows that the dependence is satisfied. This may happen early or late.
...
\omptTaskEventForTaskOrTarget{target}
...
may trigger the callback \code{ompt\_event\_master} with
