...
\binding
If the size \plc{x} is 8, 16, 32, or 64 bits and byte aligned to a multiple of
its size, then the binding thread set for the atomic region is all threads on
the device. Otherwise, the binding thread set for an atomic region is all
threads in the contention group.  \code{atomic} regions enforce exclusive
access with respect to other \code{atomic} regions that access the same
storage location \plc{x} among all threads in the binding thread set without
