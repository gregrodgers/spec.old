...
\item \code{ompt\_callback\_work\_t} type signature of the ordered
...
\item \code{ompt\_callback\_work\_t} type signature of the sections
begin and end callback, see 
\specref{sec:ompt_callback_work_t}.
\end{itemize}
...
This callback has type signature \code{ompt\_callback\_work\_t}.
...
\item \code{ompt\_callback\_work\_t} type signature of the 
...
\crossreferences
\begin{itemize}
\item \code{ompt\_callback\_work\_t} type signature of the workshare
begin and end callback, see 
\specref{sec:ompt_callback_work_t}.
\end{itemize}
...
\events
The \plc{task-create} event  occurs when a thread encounters a \code{task} construct 
that causes a new explicit task to be created after it is initialized but before it begins execution or is deferred. 

The \plc{task-schedule} event occurs in a thread when it switches tasks at a task scheduling point.
...
A thread invokes a registered \code{ompt\_callback\_task\_schedule}
callback for each occurrence of a \plc{task-schedule} event in 
the context of the task that is resumed. This callback has the type signature
...
\item \code{if} Clause, see \specref{sec:if Clause}.
\item \code{ompt\_callback\_task\_create\_t} type signature of the task generation
callback, see 
\specref{sec:ompt_callback_task_create_t}.
\item \code{ompt\_callback\_task\_schedule\_t} type signature of the task scheduling
callback, see 
...
This callback has type signature \code{ompt\_callback\_scoped\_work\_t}.
...
This callback has type signature \code{ompt\_callback\_task\_schedule\_t}.
...
type signature \code{ompt\_callback\_device\_initialize\_t}.
...
{
The \plc{#3{-begin}} event occurs after a thread enters a
\code{#1} region.
...
\def\omptTargetRegion#1#2#3
{
\events
\omptTargetRegionEvents{#1}{#2}{#3}
\tools
\omptTargetRegionTools{#1}{#2}{#3}
}
\omptTargetRegion{\code{target}~\code{data}}{ompt\_target}{target-data}
...
\item \code{ompt\_callback\_target\_t} type signature of the target
begin and end callback, see 
...
\events
Events associated with a \plc{target task} are the same as for the \code{task} construct defined in \specref{sec:Tasking Constructs}.

\tools
Callbacks associated with a \plc{target task} are the same as for the \code{task} construct defined in \specref{sec:Tasking Constructs}.
...
\omptTarget{\code{target}~\code{enter}~\code{data}}{ompt\_target\_enter\_data}{target-enter-data}
...
When an \code{if} clause is present and the \code{if} clause expression evaluates to \plc{false}, the device is the host. 
%\tools
\omptTarget{\code{target}~\code{exit}~\code{data}}{ompt\_target\_exit\_data}{target-exit}
...
\omptTarget{\code{target}}{ompt\_target}{target}
...
signature \code{ompt\_callback\_target\_submit\_t}.  
...
%\tools
\omptTarget{\code{target}~\code{update}}{ompt\_target\_update}{target-update}
...
\item \code{ompt\_callback\_work\_t} type signature of the distribute
begin and end callback, see 
\specref{sec:ompt_callback_work_t}.
\end{itemize}
...
The \plc{master-begin} event occurs in the master thread of the
current team encountering the \code{master} construct on entry to the
\code{master} region.

The \plc{master-end} event occurs in the master thread of the current
team encountering the \code{master} construct on exit from the
\code{master} region.

\tools

A thread invokes a registered \code{ompt\_callback\_master}
callback for each occurrence of a \plc{master-begin} and a
\plc{master-end} event in that thread.  The callback occurs in the
master thread of the current team encountering
the \code{master} construct.  This callback has the type signature
...
\crossreferences
\begin{itemize}
\item \code{ompt\_callback\_master\_t} type signature of the master
begin and end callback, see 
...
\item \code{ompt\_callback\_mutex\_acquire\_t} type signature of the critical 
acquire callback, see 
\specref{sec:ompt_callback_mutex_acquire_t}.

\item \code{ompt\_callback\_mutex\_t} type signature of the critical 
...
\crossreferences
\begin{itemize}
\item \code{ompt\_callback\_sync\_region\_t} type signature of the barrier
begin and end callback, see 
...
\item \code{ompt\_callback\_sync\_region\_t} type signature of the taskwait
begin and end callback, see 
\specref{sec:ompt_callback_sync_region_t}.
\end{itemize}
...
\item \code{ompt\_callback\_sync\_region\_t} type signature of the taskgroup
begin and end callback, see 
\specref{sec:ompt_callback_sync_region_t}.
\end{itemize}
...
\item \code{ompt\_callback\_mutex\_acquire\_t} type signature of the atomic 
acquire callback, see 
\specref{sec:ompt_callback_mutex_acquire_t}.

\item \code{ompt\_callback\_mutex\_t} type signature of the atomic 
acquired and released callback, see 
\specref{sec:ompt_callback_mutex_t}.
\end{itemize}
...
The \plc{flush} event occurs in the thread encountering the
\code{flush} construct.

\tools

A thread invokes a registered \code{ompt\_callback\_flush} callback
...
\crossreferences
\begin{itemize}
\item \code{ompt\_callback\_flush\_t} type signature of the flush 
begin and end callback, see 
...
\item \code{ompt\_callback\_mutex\_acquire\_t} type signature of the ordered 
acquire callback, see 
\specref{sec:ompt_callback_mutex_acquire_t}.

\item \code{ompt\_callback\_mutex\_t} type signature of the ordered 
acquired and released callback, see 
\specref{sec:ompt_callback_mutex_t}.
\end{itemize}
...
There are two events associated with a depend clause. First, a \plc{task\_dependence} event  
announce the dependences with respect to the list items in the \code{depend} clause.
Second, a \plc{task dependence pair} event indicates the presence of a potentially blocking dependence between two sibling tasks. 
The \plc{task\_dependence} event occurs immediately after a 
...
This callback has type signature \code{ompt\_callback\_task\_dependences\_t}.
...
This callback has type signature \code{ompt\_callback\_task\_dependence\_t}.
...
\item \code{ompt\_callback\_task\_dependences\_t} type signature of the 
task dependences callback, see 
\specref{sec:ompt_callback_task_dependences_t}.

\item \code{ompt\_callback\_task\_dependence\_t} type signature of the 
...
\item \code{ompt\_callback\_cancel\_t} type signature of the cancel callback, see \specref{sec:ompt_callback_cancel_t}.
\end{itemize}
