...
\item At entry to and exit from an \code{ordered} region, if a \code{threads} or a \code{depend} clause is present, or if no clauses are present.
...
the \code{ordered} construct with the \code{depend} clause is ignored.
