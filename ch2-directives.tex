...
For a \plc{vec} element of \code{sink} \plc{dependence-type} of the form
x$_{i}$ $+$ d$_{i}$ or x$_{i}$ $-$ d$_{i}$, the behavior is unspecified if
the expression x$_{i}$ $+$ d$_{i}$ or x$_{i}$ $-$ d$_{i}$ for any
value of the loop iteration integral or pointer type variable x$_{i}$ that
can encounter the \code{ordered} construct cannot be represented in the
loop iteration variable's type without overflow.  For C++ if x$_{i}$ is of a random
access iterator type other than pointer type, the behavior is unspecified if
the expression $($ x$_{i}$ - lb$_{i}$ $)$ $\pm $ d$_{i}$ for any value of
the loop iteration variable x$_{i}$ that can encounter the \code{ordered}
construct cannot be represented in the type that would be used
by \plc{std::distance} applied to variables of the type of x$_{i}$.
