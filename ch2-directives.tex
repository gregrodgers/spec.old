...
\def\omptTaskCreateEvents#1#2{
  Upon encountering a \code{#1} construct that causes a \code{task} to be created, 
  the encountering thread shall invoke the callback \code{ompt\_event\_task\_create}, if registered,
  with \code{type=#2}
  in the context of the \code{task} that encountered the \code{#1} construct.
...
{
On entry to the \code{#1} region, the executing thread
shall invoke the callback \code{ompt\_event\_target}, if registered, with
\code{endpoint=ompt\_scope\_begin} and \code{kind=#2}.
This callback has type signature \code{ompt\_target\_callback\_t}.

On exit from the \code{#1} region, the executing thread
shall invoke the callback \code{ompt\_event\_target}, if registered, with
\code{endpoint=ompt\_scope\_end} and \code{kind=#2}.
}
