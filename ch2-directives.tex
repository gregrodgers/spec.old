...
shall trigger the event \code{ompt\_event\_parallel\_begin} 
(see \specref{subsec:ompt_event_parallel_begin}) if registered.

At the end of the execution of the \code{parallel} region, the master thread
shall trigger the event \code{ompt\_event\_parallel\_end} 
(see \specref{subsec:ompt_event_parallel_end}) if registered.
...
\tools
When a \code{task} is generated, the encountering thread shall trigger the event 
\code{ompt\_event\_task\_create} 
(see \specref{subsec:ompt_event_task_create}) if registered.

When a \code{task} starts or resumes execution, the thread execution the task
shall trigger the event \code{ompt\_event\_task\_schedule} 
(see \specref{subsec:ompt_event_parallel_begin}) if registered. 

On completion of a \code{task} region, the \code{prior\_completed} in
the event \code{ompt\_event\_task\_schedule} is set to \code{true}.
