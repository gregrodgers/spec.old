...
loop construct. The parameter of the \code{collapse} clause must be a constant positive 
integer expression. If no \code{collapse} clause is present, the only loop that is associated 
...
according to the \code{schedule} clause. The sequential execution of the iterations in all 
...
\subsubsection{\code{depend} Clause}
\index{depend@{\code{depend}}}
\index{clauses!depend@{\code{depend}}}
\label{subsubsec:depend Clause}
\summary
The \code{depend} clause enforces additional constraints on the scheduling of tasks. These 
...
\item Task scheduling constraints, 
...
firstprivate(\plc{list})
lastprivate(\plc{list})
default(privat | firstprivat | shared | none)
grainsize(\plc{grain-size})
num_tasks(\plc{num-tasks})
collapse(\plc{n})
if(\plc{scalar-expr})
...
If any of the loop iteration variables would otherwise be shared, they are implicitly made private for the loop-iteration tasks created by the \code{taskloop} construct. Unless the loop iteration variables are specified in a \code{lastprivate} clause on the loop construct, their values after the loop are unspecified.
\fortranspecificend

\binding
The binding thread set of the taskloop construct is the current team. A taskloop region binds to the innermost enclosing parallel region.
...
\item The \code{grainsize} clause and \code{num\_tasks} clause are mutually exclusive and may not appear on the same taskloop directive.
...
\section{Master and Synchronization Constructs}
\label{sec:Master and Synchronization Constructs}
\index{master and synchronization constructs}
...
\descr
If no clause is specified, the ordered construct behaves as if the threads clause had been specified. If the \code{threads} clause is specified, the threads in the team executing the loop region execute \code{ordered} regions sequentially in the order of the loop iterations. When the thread executing the first 
iteration of the loop encounters an \code{ordered} construct, it can enter the \code{ordered} 
region without waiting. When a thread executing any subsequent iteration encounters an 
\code{ordered} region, it waits at the beginning of that \code{ordered} region until execution of 
the \code{ordered} regions belonging to all previous iterations have completed. If the \code{simd} clause is specified, the \code{ordered} regions encountered by any thread will use only a single SIMD lane to execute the \code{ordered} regions in the order of the loop iterations.

\restrictions
Restrictions to the \code{ordered} construct are as follows:

\begin{itemize}
\item The loop region to which an \code{ordered} region with a \code{threads} clause binds must have an \code{ordered} clause specified on the corresponding loop (or parallel loop) construct.
\item During execution of an iteration of a loop or a loop nest within a loop region, a thread must not execute more than one \code{ordered} region with the \code{threads} clause that binds to the same loop region.
\end{itemize}
...
it.
\item An \code{ordered} region with the \code{simd} clause must be closely nested to a \code{simd} or \code{declare}~\code{simd} construct.
...
\fortranspecificstart
\begin{itemize}
\item An \code{ordered} region with the \code{simd} clause must be closely nested to a \code{simd} construct or to a function or subroutine that is the target of a \code{declare}~\code{simd} construct.
\end{itemize}
\fortranspecificend
...
\begin{enumerate}
\item If the \plc{map-type} is \code{to} or \plc{tofrom} the corresponding list item is assigned the value of the original list item. 
\end{enumerate}
...
\begin{enumerate}
\item If a corresponding list item of the original list item is not present in the device data environment the list item is ignored.
...
\begin{enumerate}
\item if the \plc{map-type} is \code{to}, \code{from}, \code{tofrom}, or \code{release}, the corresponding list item's reference count is decremented by one.
\item If the \plc{map-type} is \code{delete} the corresponding list item's reference count is set to zero. 
\end{enumerate}
\item If the corresponding list item's reference count is zero or the \code{always} \plc{map-type-modifier} is present, then:
\begin{enumerate}
\item If the \plc{map-type} is \code{from} or \code{tofrom}, the original list item is assigned the value of the corresponding list item. 
\end{enumerate}
\item If the corresponding list item's reference count is zero, then:
\begin{enumerate}
\item The corresponding list item is removed from the device data environment 
\end{enumerate}
\end{enumerate}
\end{enumerate}
