...
The following events only occurs, if a lock is acquired for the \code{#1} region.

The \plc{#1-acquire} event occurs in the thread encountering the
\code{#1} construct on entry to the #1 region before an associated lock
is requested.

The \plc{#1-acquired} event occurs in the thread encountering the
\code{#1} construct after acquiring an associated lock, but before executing the
structured block of the \code{#1} region.

The \plc{#1-release} event occurs in the thread encountering the
\code{#1} construct after an associated lock was released.

\tools

A thread invokes a registered \code{ompt\_callback\_mutex\_acquire}
callback for each occurrence of a \plc{#1-acquire} event 
...
callback for each occurrence of a \plc{#1-acquired} event 
in that thread.  This callback has the type signature \code{ompt\_mutex\_callback\_t}.
A thread invokes a registered \code{ompt\_callback\_mutex\_release}
callback for each occurrence of a \plc{#1-release} event 
in that thread.  This callback has the type signature \code{ompt\_mutex\_callback\_t}.
The callbacks occur in the task encountering
the #1 construct.  The callbacks receive \code{ompt\_mutex\_#1}
as their \plc{kind} argument.
}
...
The \plc{#1-begin} event occurs in the thread encountering the
\code{#1} construct on entry to the #1 region.

The \plc{#1-end} event occurs in the thread encountering the
\code{#1} construct after the #1 synchronization on exit from the #1 region.


\tools

A thread invokes a registered \code{ompt\_callback\_sync\_region}
