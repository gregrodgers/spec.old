...
\item At most one \code{final} clause can appear on the directive. 
\item At most one \code{priority} clause can appear on the directive.
\item A \code{conditional} modifier must not appear in a \code{lastprivate} clause that is present on the directive.
\end{itemize}
...
if there exists an iteration or \code{section} construct in which a lexically
visible assignment to a list item is encountered, it is the new list item from
the sequentially last such iteration or lexically last such section whose value is assigned to the
original list item.
...
\code{conditional} modifier is modified but not with a lexically visible
assignment to the list item in the construct, the value assigned to the original list item is
unspecified.
...
\item If the original list item has the \code{ALLOCATABLE} attribute, the
    corresponding list item that is assigned to the original list item must have an allocation status of allocated upon exit from
