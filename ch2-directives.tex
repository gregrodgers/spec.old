...
\tablelasttail{\hline}
\bottomcaption{ICV Initial Values\label{tab:ICV Initial Values}}
...
\tablelasttail{\hline}
\bottomcaption{Ways to Modify and to Retrieve ICV Values\label{tab:Ways to Modify and to Retrieve ICV Values}}
...
\tablelasttail{\hline}
\bottomcaption{Scopes of ICVs\label{tab:Scopes of ICVs}}
...
\tablelasttail{\hline}
\bottomcaption{ICV Override Relationships\label{tab:ICV Override Relationships}}
...
exception is for the \code{static} schedule as specified in Table~\ref{tab:Schedule-Values}.
...
The schedule \plc{kind} can be one of those specified in Table~\ref{tab:Schedule-Values}.

The schedule \plc{modifier} can be one of those specified in Table~\ref{tab:Schedule Clause Modifier Values}.
...
\tablelasttail{\hline}
\bottomcaption{\code{schedule} Clause \plc{kind} Values\label{tab:Schedule-Values}}
...
\tablelasttail{\hline}
\bottomcaption{\code{schedule} Clause \plc{modifier} Values\label{tab:Schedule Clause Modifier Values}}
%% \vspace{1ex}
\begin{supertabular}{ p{0.8in} p{4.3in} }
...
the generated tasks will have the \plc{priority-value} as if it was 
specified for each individual task.  
If the \code{priority} clause has not been specified, tasks generated by 
...
\ccppspecificstart
\item If an array section is derived from a variable for which the 
type is pointer, reference to array or reference to pointer then the
behavior is unspecified  if that variable is modified in the region.
\ccppspecificend
...
where \plc{hint-expression} is an integer constant expression which
...
\begin{boxedcode}
!\$omp critical \plc{[}(\plc{name}) \plc{[}hint(\plc{hint-expression})\plc{] ] new-line}
    \plc{structured-block}
!\$omp end critical \plc{[}(\plc{name})\plc{]}  
\end{boxedcode}

where \plc{hint-expression} is a constant expression that evaluates to
a scalar value with kind \code{omp\_lock\_hint\_kind} with a value
...
\descr
When a thread encounters a \code{taskgroup} construct, it starts executing the region. All child tasks generated in the \code{taskgroup} region and all their descendants that bind to the same \code{parallel} region as the \code{taskgroup} region are part of the \plc{taskgroup set} associated with the \code{taskgroup} construct.

There is an implicit task scheduling point at the end of the \code{taskgroup} region. The current 
task is suspended at the task scheduling point until all tasks in the \plc{taskgroup set} complete execution.
...
\binding
The binding thread set of the \code{cancel} region is the current team and therein the \code{cancel} region 
binds to the innermost enclosing region of the type corresponding to the \plc{construct-type-clause} 
specified in the directive (that is, the innermost \code{parallel}, \code{sections}, loop, or 
\code{taskgroup} region).

\descr
The \code{cancel} construct activates cancellation of the binding region only if \plc{cancel-var} 
...
If \plc{cancel-var} is \plc{true} and \plc{construct-type-clause} is \code{taskgroup} the encountering task continues execution at the end of the current task region.
If \plc{cancel-var} is \plc{false}, the \code{cancel} construct is 
ignored.
...
\begin{itemize}
\item If the thread is at a \code{cancel} or \code{cancellation point} region, the encountering task immediately checks for active cancellation of the innermost enclosing region of the type specified.  
If cancellation has been activated, the thread continues execution at the end of the canceled region if \plc{construct-type-clause} is \code{parallel}, \code{for}, \code{do}, or \code{sections}.
If cancellation has been activated and \plc{construct-type-clause} is \code{taskgroup}, the encountering task continues at the end of the current task region.

\item If the encountering task is at a \code{barrier} region, the encountering task immediately checks for active cancellation of the innermost enclosing parallel region. If cancellation has been activated, then the encountering task continues execution at the end of the canceled region.

\item If the encountering task is at an implicit barrier region, the encountering task immediately checks for active cancellation of the innermost parallel region that encloses the parallel region to which the implicit barrier belongs. If cancellation has been activated, then the encountering task continues execution at the end of the canceled region.
\end{itemize}
...
When cancellation of tasks is activated through the \code{cancel}~\code{taskgroup} construct, the tasks that belong to \plc{taskgroup set} of the  
innermost enclosing \code{taskgroup} region will be canceled. The task that encountered the 
...
\code{cancel} region must be strictly closely nested inside a \code{taskgroup} region. If
...
\item If \plc{construct-type-clause} is \code{taskgroup} and the \code{cancel} region is not strictly closely nested 
...
\binding
The binding task set of the \code{cancellation point} region is the current task region and therein the \code{cancellation point} region binds to the innermost enclosing region of the type corresponding to the \plc{construct-type-clause} 
specified in the directive (that is, the innermost \code{parallel}, \code{sections}, loop, or 
\code{taskgroup} region).
...
execution at the end of the binding region if \plc{construct-type-clause} is \code{parallel}, \code{for}, \code{do}, or \code{sections}. 
If \plc{construct-type-clause} is \code{taskgroup} the encountering task continues execution at the end of the current task region.
...
region must be strictly closely nested inside a \code{taskgroup} region. A
...
during the execution of \code{parallel}, \code{task}, \code{taskloop}, \code{simd}, and worksharing regions.

\begin{itemize}
\item \specref{subsec:Data-sharing Attribute Rules} 
describes how the data-sharing attributes of variables 
referenced in \code{parallel}, \code{task}, \code{taskloop}, \code{simd}, and worksharing regions are determined.
...
variables referenced in \code{parallel}, \code{task}, \code{taskloop}, \code{simd}, or worksharing constructs are 
...
\code{parallel}, \code{task}, \code{taskloop}, \code{simd}, and worksharing regions are determined. The following two 
...
\item A loop iteration variable for a sequential loop in a \code{parallel}, \code{task}, or \code{taskloop} construct is 
...
\code{parallel}~\code{for}, or \code{taskloop} construct may be listed in a \code{private} or \code{lastprivate} clause. 
...
\code{parallel}~\code{do}, or \code{taskloop}
...
\code{task}, or \code{taskloop} construct may be listed in data-sharing clauses on the construct itself, and on 
...
\begin{itemize}
\item In a \code{parallel}, \code{task}, or \code{taskloop} construct, the data-sharing attributes of these variables are 
...
\item For constructs other than \code{task} or \code{taskloop}, if no \code{default} clause is present, these variables reference the variables with the same names that exist in the enclosing context.

\item In a \code{task} or \code{taskloop} construct, if no \code{default} clause is present, a variable that in the 
...
\fortranspecificstart
\begin{itemize}
\item In an orphaned \code{task} construct or an orphaned \code{taskloop} 
...
\begin{itemize}
\item In a \code{task} construct or a \code{taskloop} construct, if no 
\code{default} clause is present, a variable whose data-sharing 
...
implicitly determined in a \code{task} or \code{taskloop} construct are described in 
...
\renewcommand{\arraystretch}{1.5}
\begin{center}
\bottomcaption{Implicitly Declared C/C++ \plc{reduction-identifiers}\label{tab:Implicitly Declared C/C++ Reduction Identifiers}}
%% \bottomcaption*{}
\tablefirsthead{%
\hline
...
\tablelasttail{\hline}
\bottomcaption{Implicitly Declared Fortran \plc{reduction-identifiers}\label{tab:Implicitly Declared Fortran Reduction Identifiers}}
...
\item An original list item with the \code{ALLOCATABLE} attribute or
  ...
  referenced in \plc{combiner} must be in the allocated state at entry
...
\descr
The list items that appear in a \code{map} clause may include array sections as well as struct members.
