...
\item The \code{ref} or \code{uval} modifiers can only be used if the \plc{list-item} is of a reference type.
\cppspecificend

\fortranspecificstart
\item A \plc{list-item} that appears in a \code{linear} clause without the \code{ref} modifier must be of type \code{integer}.
\item The \code{ref} or \code{uval} modifiers can only be used if the \plc{list-item} is a dummy argument without the \code{VALUE} attribute.
