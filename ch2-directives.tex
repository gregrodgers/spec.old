...
When a \code{linear} clause is specified on the \code{simd} construct, the \code{linear} clause provides a superset of
...
parameter (or Fortan dummy argument) specified in the \code{linear} clause without a modifier, the value of the list 
item on each lane of the associated function corresponds to the value of the list item before entering the 
function plus the logical number of the lane times \plc{linear-step}. If \plc{linear-step} is not specified it is 
assumed to be 1. 

\begin{itemize}
\item If the list item appears in the \code{linear} clause with \code{ref} modifier, it indicates the value on each 
lane corresponds the location of the list item, and computed as above stated for read/write of data through memory 
dereferencing.  

\item If the list item appears in the \code{linear} clause with \code{uval} modifier, it indicates the location of 
the list item is same for all lanes, and the value of the list item is computed as aboved stated for each lane.
The corresponding memory of the list item is updated with the value of the last lane.

\item If the list item appears in the \code{linear} clause with \code{val} modifier, it indicates the locations 
of the list item may be different for each lane. The value of the list item is computed as aboved stated for each lane. The corresponding memory of the list item for each lane may be updated, and any use of updated value has to be 
appear before the updates to the memory for each lane.
\end{itemize}
