...
where \plc{hint-expression} is a constant expression that evaluates to a value of type \code{omp\_lock\_hint\_t}.

\fortranspecificstart
The syntax of the \code{critical} construct is as follows:

\begin{boxedcode}
!\$omp critical \plc{[}(\plc{name})\plc{]} \plc{[}hint(\plc{hint-expression})\plc{] new-line}
   \plc{structured-block}
!\$omp end critical \plc{[}(\plc{name})\plc{]}
\end{boxedcode}

where \plc{hint-expression} is a constant expression that evaluates to a scalar value with kind \code{omp\_lock\_hint\_kind}. 
\fortranspecificend
...
If no \code{hint} clause is specified, the effect is as if \code{hint(none)} has been specified.
