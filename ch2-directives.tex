...
at the beginning of the closing synchronization barrier for the
parallel region.

The \plc{implicit-barrier-wait-begin} event occurs when a task starts waiting
while executing an implicit barrier at the end of a 
\code{parallel} region.

The \plc{implicit-barrier-wait-end} event occurs when a task
resumes executing an implicit barrier at the end of a 
\code{parallel} region.

The \plc{implicit-barrier-end} event occurs in each implicit task
at the end of the closing synchronization barrier for the parallel
region.

When a thread finishes an implicit task, an \plc{implicit-task-end} event
occurs in the thread after the \plc{implicit-barrier-end} event in the implicit task

The \plc{parallel-end} event occurs in the thread encountering the
\code{parallel} construct after the \code{parallel} region 
executes its \plc{implicit-barrier-end} event 
...
\plc{implicit-barrier-end} event 
in that thread.  The callback occurs in the implicit task performing
...
for each occurrence of a \plc{implicit-barrier-wait-begin} and 
\plc{implicit-barrier-wait-end} event.
This callback has type signature \code{ompt\_callback\_sync\_region\_t}. 
The callback occurs in the implicit task performing the \code{parallel} construct.
The callback receives
\code{ompt\_sync\_region\_barrier} as its \plc{kind} argument and
\code{ompt\_scope\_begin} or \code{ompt\_scope\_end} 
as its \plc{endpoint} argument, as appropriate.
...
This composite construct generates the same events as the \code{for} or \code{do} worksharing construct.

\tools

This composite construct generates the same callbacks as the \code{for} or \code{do} worksharing construct.
...
\item Events and tool callbacks for worksharing construct, see
\specref{subsec:Loop Construct} and \specref{subsec:workshare Construct}.
\end{itemize}
\end{samepage}
...
This composite construct generates the same callbacks as the \code{taskloop} construct.
...
\omptTargetRegion{\code{target}~\code{data}}{ompt\_target}{target-data}{the task encountering the construct}
...
%\tools
\omptWorksharing{distribute}{ompt\_work\_distribute}{an initial thread}
...
This composite construct generates the same callbacks as the \code{distribute} construct.
...
This composite construct generates the same callbacks as the \code{distribute} and parallel loop constructs.
