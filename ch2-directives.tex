...
These dependences are satisfied when the threads executing these iterations encounter
...
\item If a \code{depend(sink:\plc{vec})} clause appears on an \code{ordered}
construct, \code{depend(source)} clause may not appear on the same
construct.
