...
parallel region. Section~\ref{subsec:implict-barrier} describes events
...
the parallel region. This section describes the OMPT events and
...
The \plc{implicit-barrier-wait-begin} event occurs when a task starts
spin waiting or suspends while executing an implicit barrier.

The \plc{implicit-barrier-wait-end} event occurs when a task
resumes execution of an implicit barrier after spin waiting or suspending.
...
callback for each occurrence of a \plc{cancellation} event in that
thread.  The callback occurs in the context of the implicit task.  The
