...
The \code{source} \plc{dependence-type} specifies the completion of cross-iteration dependences arise from the current iteration.

The \code{sink} \plc{dependence-type} specifies sink of cross-iteration dependences. The current iteration is blocked until all the source iterations specified complete execution.

The iteration vector vec must have the form of 
(x$_{1}$ $\pm $ d$_{1}$, x$_{2}$ $\pm $ d$_{2}$ , ... , x$_{n}$ $\pm $ d$_{n}$), 
where \plc{n} is the value specified by the \code{ordered} clause in the loop construct, 
x$_{\plc{i}}$ denotes the loop iteration variable of \plc{i}-th nested loop associated with the loop construct, and 
