...
that causes a new explicit, non-merged task to be created. The event occurs after the task is initialized but before 
...
\events
No events are associated with the \emph{implicit parallel region} in each \emph{initial thread}.

The \plc{initial-thread-begin} event occurs in an \emph{initial thread} after the OpenMP runtime invokes the tool initializer 
but before the \emph{initial thread} begins to execute the first OpenMP \emph{region}. 
...
The \plc{initial-thread-end} event occurs in an \emph{initial thread} at the end of an \emph{initial task} 
