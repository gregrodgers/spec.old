...
A \code{task\_reduction} clause that is present at the \code{taskloop} construct behaves as if it was applied to the implicit \code{taskgroup} construct enclosing the \code{taskloop} construct.  
For each task that is generated by the \code{taskloop} construct an \code{in\_reduction} clause with the same reduction operator and the same list items of the \code{task\_reduction} clause is applied to the task.

For any \code{in\_reduction} clause present at the \code{taskloop} construct, an \code{in\_reduction} clause with the same reduction operator and the same list items of the \code{task\_reduction} clause is applied to the generated tasks.
...
\item If a \code{task\_reduction} clause is present at the \code{taskloop} directive, the \code{nogroup} clause must not be specified.
\item The same list item cannot appear in both a \code{task\_reduction} and an \code{in\_reduction} clause.
