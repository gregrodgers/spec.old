...
\def\omptWorksharing#1#2#3
{
\events

The \plc{#1{-begin}} event occurs after an #3 encounters a 
...
\omptWorksharing{loop}{ompt\_work\_loop}{implicit task}
...
%\tools
\omptWorksharing{sections}{ompt\_work\_sections}{implicit task}
...
\code{ompt\_scope\_begin} or \code{ompt\_scope\_end} 
as its \plc{endpoint} argument, as appropriate, and
\code{ompt\_work\_single\_executor} or or \code{ompt\_work\_single\_other} 
...
%\tools
\omptWorksharing{workshare}{ompt\_work\_workshare}{implicit task}
...
%callback, see \specref{sec:ompt_callback_task_schedule_t}.
\end{itemize}
...
\omptWorksharing{taskloop}{ompt\_work\_taskloop}{task}
...
\omptTargetRegion{\code{target}~\code{data}}{ompt\_target}{target-data}{task encountering the construct}
...
\omptTarget{\code{target}~\code{enter}~\code{data}}{ompt\_target\_enter\_data}{target-enter-data}{target task on the host}
...
When an \code{if} clause is present and the \code{if} clause expression evaluates to \plc{false}, the device is the host. 
%\tools
\omptTarget{\code{target}~\code{exit}~\code{data}}{ompt\_target\_exit\_data}{target-exit}{target task on the host}
...
%\tools
\omptTarget{\code{target}~\code{update}}{ompt\_target\_update}{target-update}{target task on the host}
...
%\tools
\omptWorksharing{distribute}{ompt\_work\_distribute}{initial thread}
...
The callback occurs  in the context of the implicit task executed by the master thread.
