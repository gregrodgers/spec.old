...
\plc{place-partition-var} & (none) & \code{omp\_get\_partition\_num\_places()} \\
...
\item A variable that is a member of a structure and that is referenced in 
a \code{target} construct that is not declared in the construct is implicitly 
treated as if a reference to the whole structure had appeared in a \code{map} 
clause with a \plc{map-type} of \code{tofrom}. 
