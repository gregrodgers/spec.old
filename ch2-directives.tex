...
\item During a \code{cancel} or a \code{cancellation point} region, if \plc{cancel-var} is \plc{true}.
\end{itemize}
...
When a thread reaches one of the above cancellation points and if \plc{cancel-var} is \plc{true},  then:
\begin{itemize}
\item If the thread is at a \code{cancel} or \code{cancellation point} region, the thread immediately checks for active cancellation of the innermost enclosing region of the type specified.  
If cancellation has been activated, the thread continues execution at the end of the canceled region.

\item If the thread is at a \code{barrier} region, the thread immediately checks for active cancellation of the innermost enclosing parallel region. If cancellation has been activated, then the thread continues execution at the end of the canceled region.

\item If the thread is at an implicit barrier region, the thread immediately checks for active cancellation of the innermost parallel region that encloses the parallel region to which the implicit barrier belongs. If cancellation has been activated, then the thread continues execution at the end of the canceled region.
\end{itemize}
