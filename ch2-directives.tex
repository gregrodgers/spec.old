...
\specref{subsubsec:private clause}. 
In addition, when a 
\code{lastprivate} clause without the \code{conditional} modifier present appears on the directive that identifies a worksharing construct 
...
original list item. When the \code{lastprivate} clause with the
\code{conditional} modifier present appears on such a directive, the original list item is assigned the value from the
sequentially last iteration or lexically last \code{section} construct to write
to one of its corresponding new list items.
...
\code{conditional} modifier present must be a scalar variable.
