...
\begin{boxedcode}
\#pragma omp ordered \plc{new-line}
   \plc{structured-block}
\end{boxedcode}
...
\descr
The threads in the team executing the loop region execute \code{ordered} regions sequentially in the order of the loop iterations or the constraints governed by the \code{depend} clauses if specified. When the thread executing the first iteration of the loop encounters an \code{ordered} construct, it can enter the \code{ordered} region without waiting. When a thread executing any subsequent iteration encounters an \code{ordered} region, it waits at the beginning of that ordered region until execution of all \code{ordered} regions belonging to all previous iterations or iterations specified by the \code{depend} clauses have completed.

\restrictions
Restrictions to the \code{ordered} construct are as follows:

\begin{itemize}
\item The loop region to which an \code{ordered} region without any clause binds must have an \code{ordered} clause without the parameter specified on the corresponding loop (or parallel loop) construct. 

\item The loop region to which an \code{ordered} region with the \code{depend} clauses binds must have an \code{ordered} clause with the parameter specified on the corresponding loop (or parallel loop) construct. 
...
\item An \code{ordered} construct with the \code{depend} clause specified must not be closely nested in a simd construct 
