...
\summary
The \code{omp\_get\_num\_places} routine returns the number of places in the place list available to the execution environment.
...
\effect
The \code{omp\_get\_place\_num\_procs} routine returns the number of processors associated with the place numbered \code{place\_num}. The routine returns zero when \code{place\_num} is negative, or is equal or larger than \code{omp\_get\_num\_places()}. 
...
\binding
The binding thread set for an \code{omp\_get\_place\_proc\_ids} region is all threads on a device. The effect of executing this routine is not related to any specific region corresponding to any construct or API routine.

\effect
The \code{omp\_get\_place\_proc\_ids} routine returns the numerical identifiers of each processor associated with the place numbered \code{place\_num}. The numerical identifiers are nonnegative, and their meaning is implementation defined.  The numerical identifiers are returned in the array ids and their order in the array is implementation defined. The array must be sufficiently large to contain \code{omp\_get\_place\_num\_procs(place\_num)} integers; otherwise, the program is non-compliant.  The routine has no effect when \code{place\_num} has a negative value, or a value equal or larger than \code{omp\_get\_num\_places()}.
...
\effect
When the encountering thread is bound to a place, the \code{omp\_get\_place\_num} routine returns the place number associated with the thread. The returned value is between zero and \code{omp\_get\_num\_places()-1}, inclusively. When the encountering thread is not bound to a place, the routine returns the value \code{-1} .
...
\binding
The binding thread set for an  \code{omp\_get\_partition\_num\_places} region is the encountering implicit task.
...
\binding
The binding thread set for an \code{omp\_get\_partition\_place\_nums} region is the encountering implicit task.

\effect
The \code{omp\_get\_partition\_place\_nums} routine returns the list of place numbers corresponding to the places in the \plc{place-partition-var} ICV of the innermost implicit task. The array must be sufficiently large to contain \code{omp\_get\_partition\_num\_places()} integers; otherwise, the program is non-compliant.
...
\summary
The \code{omp\_get\_initial\_device} routine returns a device num representing
the initial device.
...
\fortranspecificstart
\begin{boxedcode}
logical function omp\_get\_initial\_device()
\end{boxedcode}
\fortranspecificend
...
\effect
The effect of this routine is to return the device num of the initial device.
The value of the device num is implementation defined. If it falls between 0 and
\verb#omp_get_num_devices()-1# then it is valid for use with all device
constructs and routines, if it is outside that range, then it is only valid
for use with the target routines and not the \verb#device()# clause.
...
\verb#omp_get_num_devices()# or the result of a call to
\verb#omp_get_initial_device()#. When called from within a \code{target} region 
the effect of this routine is unspecified.

The \verb#omp_target_alloc# routine returns NULL if it cannot dynamically
allocate the memory in the device data environment. 

The device address returned by \verb#omp_target_alloc# can be used in a 
...
\item \verb#omp_get_initial_device# routine, see 
...
\summary
The \verb#omp_target_free# routine frees the device memory referred to by the
...
A program that calls\verb#omp_target_free# with a non-\code{NULL} pointer 
that does not have a value returned from \verb#omp_target_alloc# is
non-conforming. When called from within a \code{target} region 
the effect of this routine is unspecified. The \verb#device_num#
must be greater than or equal to zero and less than the result of
\verb#omp_get_num_devices()# or the result of a call to
\verb#omp_get_initial_device()#.

\effect

The \verb#omp_target_free# routine frees the memory in the device data
environment associated with \verb#device_ptr#.  If \verb#device_ptr# is
\verb#NULL#, the operation is ignored.

Synchronization must be inserted to ensure that all accesses to 
\verb#device_ptr# are completed before the call to \code{omp\_target\_free}.
...
\format
\begin{boxedcode}
int omp_target_is_present(void * ptr, size_t offset,
                          int device_num);
\end{boxedcode}

\constraints

The value of \verb#ptr# must be a valid host pointer or NULL.
The device specified by \verb#device_num# must be valid.
When called from within a \code{target} region 
the effect of this routine is unspecified.
The \verb#device_num#
must be greater than or equal to zero and less than the result of
...
and offset would be found present on device \code{device\_num} by a map
...
The \code{omp\_target\_memcpy} routine copies between any combination
...
\code{length} bytes of memory at offset \verb#src_offset# from  \code{src} are
copied to \code{dst} starting at offset \verb#dst_offset#.  Each device must
be compatible with the device pointer specified on the same side of the copy.
Return values are zero on success and non-zero on failure.  The initial device
and initial device data environment can be referenced with the device id
returned by \verb#omp_get_initial_device#.  This routine contains a task
scheduling point.
...
The \verb#omp_target_memcpy_rect# routine copies a rectangular subvolume from
...
value of \verb#num_dims#
The \verb#dst_device_num# and \verb#src_device_num#
must be greater than or equal to zero and less than the result of
\verb#omp_get_num_devices()# or the result of a call to
\verb#omp_get_initial_device()#.

The value of \verb#num_dims# must be between 1 and the implementation-defined
...
This routine copies a multi-dimensional rectangular subvolume of \verb#src# 
to \verb#dst#. The volume is specified in terms of the size of the element, 
number of dimensions, and constant arrays of length \verb#num_dims#.  The 
...
length, in number of elements, to copy in each dimension from \verb#src# 
to \verb#dst#. The \verb#dst_offsets# (\verb#src_offsets#) parameter specifies
number of elements from the origin of \code{dst} (\code{src}) in elements.  
The \verb#dst_dimensions# (\verb#dst_dimensions#) parameter specifies the 
length of each dimension of \code{dst} (\code{src})

The routine returns zero if successful. If both \verb#dst# and \verb#src# are
\code{NULL} pointers, the routine returns the number of dimensions supported
by the implementation for the specified device numbers. The initial device and
initial device data environment can be referenced with the device id returned
by \verb#omp_get_initial_device#.  Otherwise, it returns a non-zero value. The
...
The value of \verb#device_ptr# value must be a valid pointer to device 
memory for the device denoted by the value of\code{device\_num}.
The \verb#device_num# argument
must be greater than or equal to zero and less than the result of
\verb#omp_get_num_devices()# or the result of a call to
\verb#omp_get_initial_device()#.
...
data motion associated with that host pointer.  The \verb#device_offset#
parameter specifies what offset into \verb#device_ptr# will be used as the
...
directly through the device pointer results in undefined behavior.  The
...
same offset will be treated as a no-op and return zero.  The
...
\code{ptr}. Programs that call this routine on a pointer that is not 
\code{NULL} and does not have associated data on the given device are
non-conforming.  The reference count of the mapping is reduced to zero,
regardless of its current value.


After a call to \verb#omp_target_disassociate_ptr#, the contents of the device
