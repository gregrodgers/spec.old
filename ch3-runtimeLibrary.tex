...
control the operation of a tool. 
...
\ccppspecificstart
\begin{boxedcode}
void omp\_tcontrol(uint64\_t \plc{command}, uint64\_t \plc{modifier});
\end{boxedcode}
\ccppspecificend
}
...
\fortranspecificstart
\begin{boxedcode}
subroutine omp\_tcontrol(\plc{command}, \plc{modifier})
...
a tool, if a tool is present, enabled, and registered a callback to receive
notification of \code{omp\_tcontrol} calls.
Using \code{omp\_tcontrol}, an application can request that
a tool start data collection when a code
...
A tool may allow an application to turn monitoring on and off many times 
during an execution to monitor only code regions of interest.
Additionally, \code{omp\_tcontrol} can be used to pass 
tool-specific commands to a particular tool.

\constraints

The following enumeration type defines four standard command values 
to pass as the first argument.  

\vbox{
\ccppspecificstart
\begin{boxedcode}
typedef omp_tcontrol_t \{
...
\noindent
%
A command of \verb|omp_tcontrol_start| asks a tool to start or 
...
%
A command of \verb|omp_tcontrol_pause| asks a tool to temporarily 
turn monitoring off. If monitoring is already off, it is idempotent.
%
A command of \verb|omp_tcontrol_flush| asks a tool to flush any data that 
it has buffered. This command may be applied whether monitoring is on or off.
%
A command of \verb|omp_tcontrol_end| turns monitoring off permanently;  
the tool may perform finalization at this point and write all of its outputs.
Other values of \code{omp\_tcontrol} command and modifier arguments
appropriate for any tool will be tool specific.  
Tool-specific commands codes must be $\geq$ 64.
Tools must ignore command codes that they are not explicitly 
designed to handle. 

\tools

Tool support for \code{omp\_tcontrol} is optional. 
If a tool wishes to receive notification of 
\code{omp\_tcontrol} calls, 
the tool can register a callback for the \code{ompt\_event\_tcontrol} event.

While tool support for \code{omp\_tcontrol} is optional, an OpenMP implementation
is required to notify a tool \code{omp\_tcontrol} call if the tool registered a 
callback for the \code{ompt\_event\_tcontrol} event.

If the user program calls \code{omp\_tcontrol}, the runtime shall invoke the 
\code{ompt\_event\_tcontrol} callback.
...
\code{omp\_tcontrol}.  If any of the four standard commands is
presented to a tool, the tool will ignore the modifier value.
...
\item \code{ompt\_event\_tcontrol} callback, see
\specref{subsec:ompt_event_tcontrol}
\end{itemize}
