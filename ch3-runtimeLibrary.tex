...
The effect of the hint is implementation-defined and can be safely ignored by the OpenMP implementation.
...
typedef enum omp\_lock\_hint\_t \{
  ...
  omp\_lock\_hint\_implementation_base = 65536
\} omp\_lock\_hint\_t;
\end{codepar}
\ccppspecificend
...
The hint \code{omp\_lock\_hint\_implementation\_base} is used to separate the hints defined above and hints that are additionally defined by the OpenMP implementation.
...
\effect
If the specified lock contains a hint, the effects on the lock by calling these routines are implementation-defined, but retain the mutual exclusion semantics of locks.
