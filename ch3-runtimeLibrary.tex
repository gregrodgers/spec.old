...
\fortranspecificstart
\begin{boxedcode}
function omp\_tool\_control(\plc{command}, \plc{modifier}, \plc{error\_code})
integer \plc{command}
integer \plc{modifier}
integer \plc{error\_code}
\end{boxedcode}
\fortranspecificend
...
\ccppspecificstart
\begin{boxedcode}
  #define OMP_TOOL_RESULT_NOTOOL     (-2)
  #define OMP_TOOL_RESULT_NOCALLBACK (-1)
  #define OMP_TOOL_RESULT_SUCCESS      0
  #define OMP_TOOL_RESULT_IGNORED      1
\end{boxedcode}
\ccppspecificend
}
...
\fortranspecificstart
\begin{boxedcode}
integer(kind=omp\_tool\_control\_kind), &
        parameter :: OMP\_TOOL\_RESULT\_NOTOOL =     -2 
integer(kind=omp\_tool\_control\_kind), &
        parameter :: OMP\_TOOL\_RESULT\_NOCALLBACK = -1
integer(kind=omp\_tool\_control\_kind), &
        parameter :: OMP\_TOOL\_RESULT\_SUCCESS =     0
integer(kind=omp\_tool\_control\_kind), &
        parameter :: OMP\_TOOL\_RESULT\_IGNORED =     1
\end{boxedcode}
\fortranspecificend
}

If no tool is active, the OpenMP implementation will return
\code{OMP\_TOOL\_RESULT\_NOTOOL}. If a tool is active, but it has not
registered a callback for the \plc{tool-control} event, the OpenMP
implementation will return \code{OMP\_TOOL\_RESULT\_NOCALLBACK}.  
...
\code{OMP\_TOOL\_RESULT\_SUCCESS} indicates that the tool has performed the
specified command.  A return value of \code{OMP\_TOOL\_RESULT\_IGNORED}
indicates that the tool has ignored the specified command.
A tool may return other positive values that are tool-defined. 
When \code{omp\_tool\_control} is called from C/C++, the tool return value will be
returned from \code{omp\_tool\_control}. 
When \code{omp\_tool\_control} is called from Fortran, the tool return value will be
...
\begin{boxedcode}
integer(kind=omp\_tool\_control\_kind), &
        parameter :: omp\_tool\_control\_start = 1
integer(kind=omp\_tool\_control\_kind), & 
        parameter :: omp\_tool\_control\_pause = 2
integer(kind=omp\_tool\_control\_kind), & 
        parameter :: omp\_tool\_control\_flush = 3
integer(kind=omp\_tool\_control\_kind), &
        parameter :: omp\_tool\_control\_end = 4
\end{boxedcode}
\fortranspecificend
}
...
\code{omp\_tool\_control}. If the call is made in Fortran, the tool will be
