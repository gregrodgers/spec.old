...
\ccppspecificstart
\vspace{-3\baselineskip}
\section{Tool Event Callback Functions}
\index{tool event callback functions}
% \index{target memory routines}
\label{sec:Tool Event Callback Functions}
This section describes callback functions that an OMPT tool might register
and that are called during runtime of an OpenMP program.



\subsection{\code{ompt\_event\_parallel\_begin}}
\index{ompt\_event\_parallel\_begin@{\code{ompt\_event\_parallel\_begin}}}
\label{subsec:ompt_event_parallel_begin}
\summary
The \code{ompt\_event\_parallel\_begin} callback is a mandatory OMPT event to be
triggered by the master thread at the begin of a \code{parallel} region.

\format
\begin{boxedcode}
void ompt_event_parallel_begin (
...
\binding
The binding region for an \code{ompt\_event\_parallel\_begin} event is the 
task which encountered the \code{parallel} construct.

\descr
The runtime invokes this callback 
after a \code{task} encounters a \code{parallel} construct
...
includes a parameter \code{requested\_team\_size} 
...
\subsection{\code{ompt\_event\_parallel\_end}}
\index{ompt\_event\_parallel\_end{\code{ompt\_event\_parallel\_end}}}
\label{subsec:ompt_event_parallel_end}
\summary
The \code{ompt\_event\_parallel\_end} callback is a mandatory OMPT event to be
triggered by the master thread at the end of a \code{parallel} region.

\format
\begin{boxedcode}
void ompt_event_parallel_end (
               ompt_data_t * \plc{parallel_data},
...
\binding
The binding region for an \code{ompt\_event\_parallel\_end} event is the 
task which encountered the \code{parallel} construct.
...
\item \code{parallel} construct, see 
\specref{sec:parallel Construct}.
\item \code{ompt\_parallel\_end\_callback\_t} type signature or the event, see 
...
\ccppspecificstart
\vspace{-3\baselineskip}
\section{Tool Inquiry Routines}
\index{tool inquiry routines}
% \index{target memory routines}
\label{sec:Tool Inquiry Routines}
This section describes routines that provide runtime information to first party tools. 
