...
\begin{itemize}
  ...
  \item The pointer returned by \verb#omp_target_alloc# is guaranteed to be
...
\begin{itemize}
  \item \verb#omp_target_free# may only be called with a pointer
    returned from \verb#omp_target_alloc#, \specref{subsec:omp_target_alloc},  or \code{NULL}.
...
\begin{itemize}
  \item The value of \verb#ptr# must be a valid host pointer or NULL
  \item The device specified by \verb#device_num# must be valid
\end{itemize}
...
Return values are zero on success and non-zero on failure.  This routine
contains a task scheduling point.

\restrictions

\begin{itemize}
  \item This routine is not declared for use in target regions, attempts to
...
bone multi-dimensional array to another, potentially across devices.
...
Copies a multi-dimensional rectangular subvolume of \verb#src# to \verb#dst#.
The volume is specified in terms of the size of the element, number of
dimensions, and constant arrays of length \verb#num_dims#.  The maximum number
of dimensions supported is at least three, support for higher dimensionality
is implementation defined.  The volume array
specifies the length, in number of elements, to copy in each dimension from
\verb#src# to \verb#dst#.  The \verb#*_offsets# parameters specify the offset
from the origin of each of \code{src} and \code{dst} in elements.  Finally the
\verb#*_dimensions# arguments specify the complete dimensions of each of
...
%NOTE: pagebreak here because samepage failed to prevent a bad split
\pagebreak
\begin{samepage}
\ccppspecificstart
\begin{boxedcode}
...
The \verb#omp_target_memcpy_rect# routine contains a tasks scheduling point.
Return values are zero on success and non-zero on failure.  Calling the function
with both \verb#dst# and \verb#src# as NULL pointers causes it to return the number of dimensions supported
...
\begin{itemize}
  \item This routine is not declared for use in target regions, attempts to
    use it will result in undefined behavior
  \item The offset and dimension arrays must be of a length equal to or
    greater than \verb#num_dims#
  \item \verb#num_dims# must fall between 1 and the implementation-defined
    limit, values between one and three are always available
\end{itemize}
...
be returned from \code{omp\_target\_alloc}, \specref{subsec:omp_target_alloc}
or implementation defined runtime routines, to a host pointer in the manner of
a map clause.
...
\begin{itemize}
  \item The \verb#device_ptr# value must be a valid pointer to device memory for the
    ...
    \item Only one device buffer can be associated with a given host pointer
      ...
      will return failure.  The \code{omp\_target\_is\_present},
      \specref{omp_target_is_present}, function can be used
