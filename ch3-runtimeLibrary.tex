...
\item The \code{omp\_init\_lock\_hinted} routine initializes a simple lock and attaches a hint to it.
...
\item The \code{omp\_init\_nest\_lock\_hinted} routine initializes a nestable lock and attaches a hint to it.
...
\subsection[\code{omp\_init\_lock\_hinted} and \code{omp\_init\_nest\_lock\_hinted}]{\code{omp\_init\_lock\_hinted} and \\\code{omp\_init\_nest\_lock\_hinted}}
\label{subsec:omp_init_lock_hinted and omp_init_nest_lock_hinted}
\index{omp\_init\_lock@{\code{omp\_init\_lock}}}
\index{omp\_init\_nest\_lock@{\code{omp\_init\_nest\_lock}}}
\summary
These routines initialize an OpenMP lock with hint.  The effect of the hint is implementation-defined and can also be safely ignored by the OpenMP implementation.
...
\effect
The effect of these routines is to initialize the lock to the unlocked state and attach the hint to it; 
that is, no task owns the lock. In addition, the nesting count for a nestable lock is set to zero.
