...
\effect
The effect of this routine is to return the device number of the initial device.
The value of the device number is implementation defined. If it falls between 0 and
\code{omp\_get\_num\_devices()-1} then it is valid for use with all device
constructs and routines; if it is outside that range, then it is only valid
for use with the device memory routines (\specref{sec:device_memory}) and not in the \code{device} clause.
...
The return value is zero on success and non-zero on failure.  The initial device
...
\plc{ptr}. Programs that call this routine on a pointer that is not 
