...
\item Runtime states 
(\specref{sec:runtimeStates}).

\item OMPT Query Functions
(\specref{sec:omptFunctions}).

\item Execution routines to control the application monitoring
(\specref{sec:tool control for applications})
\end{itemize}
...
To enable an understanding of what a runtime is currently executing, the runtime 
will maintain state information for each OpenMP thread that can be queried, e.g., by a tool. 
The state maintained for each thread by the OpenMP runtime is an
approximation of the thread's instantaneous state. 
OpenMP uses the enumeration type \code{omp\_state\_t} for states;
...
\subsection{Work States (Option 1)}
...
\subsection{Work States (Option 2)}

\subsubsection{\code{omp\_state\_work\_serial}}

The thread is executing code outside all parallel regions. 

\subsubsection{\code{omp\_state\_work\_parallel|}} 

The thread is executing code within the scope of a parallel region construct.

\subsubsection{\code{omp\_state\_work\_reduction}}
...
\subsection{Barrier Wait States}
...
\subsection{\code{enum omp\_state\_e}}

This section defines an enumeration type that specifies the set of runtime states. 
The purpose of these states is described above.
...
\index{tool inquiry routines}
 ...
 The \code{ompt\_enumerate\_states} function allows tools implemented on top of the OMPT interface to query all runtime states available in the particular runtime implementation.
...
\index{ompt\_get\_thread\_data@{\code{ompt\_get\_thread\_data}}}
\label{subsec:ompt_get_thread_data}
\summary
The \code{ompt\_get\_thread\_data} routine returns a pointer to the tool's 
identifier for the current thread.
\format
\begin{boxedcode}
ompt\_thread\_data\_t * ompt\_get\_thread\_data(void); 
\end{boxedcode}

\binding
The binding thread for an \code{ompt\_get\_thread\_data} routine is the executing 
thread.

\effect
The \code{ompt\_get\_thread\_data} routine returns the pointer to the tools owned
thread specific value. If the current thread is unknown to the OpenMP runtime, 
...
\index{tool control for applications}
\label{sec:tool control for applications}

The OMPT API provides only one application-facing routine: \code{ompt\_control}.
An application may call the function \code{ompt\_control} to control tool operation.
While tool support for \code{ompt\_control} is optional, the runtime is required to pass a control command to a tool if the tool registered a callback with the \code{ompt\_event\_control} event.
As an application-facing routine, this function has type signatures for both C and Fortran:
...
integer*8 \plc{command}
integer*8 \plc{modifier}
\end{boxedcode}
\fortranspecificend

A classic use case for \code{ompt\_control} is for an application to start and stop data collection by a tool.
A tool may allow an application to turn monitoring on and off many times during an execution to monitor only code regions of interest.
To support this common usage, the OMPT standard defines four values for \plc{command}:
...
\noindent
%
A \plc{command} code of 1 asks a tool to start or restart monitoring if it is off. If monitoring is already on, this command is idempotent. If monitoring has already been turned off permanently, this command will have no effect.
%
A \plc{command} code of 2 asks a tool to temporarily turn monitoring off. If monitoring is already off, it is idempotent.
%
A \plc{command} code of 3 asks a tool to flush any performance data that it has buffered. This command may be applied whether monitoring is on or off.
%
A \plc{command} code of 4 turns monitoring off permanently;  the tool may perform finalization at this point and write all of its outputs.

Other values of \plc{command} and \plc{modifier} appropriate for any tool will be tool specific.
Tool-specific commands codes must be $\geq$ 64.
Tools must ignore \plc{command} codes that they are not explicitly designed to handle and implement callbacks for such codes as no-ops.

\tools

If the user program calls \code{ompt\_control}, the runtime shall invoke the \code{ompt\_event\_control} callback.
The callback executes in the context of the call that occurs in the user program.
...
\crossreferences
\begin{itemize}
\item tools support, see
\specchapterref{chap:ToolsSupport}
\item \code{ompt\_event\_control} callback, see
\specref{subsec:ompt_event_control}
\end{itemize}
