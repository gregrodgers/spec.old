...
executing a \code{omp\_init\_lock} or \code{omp\_init\_nest\_lock} function
after initialization of the lock, but before returning from the function call.

\tools

A thread dispatches a registered \code{ompt\_callback\_mutex\_init}
callback for each occurrence of a \plc{lock-init} or \plc{nest-lock-init} event 
in that thread.  This callback has the type signature \code{ompt\_callback\_mutex\_init\_t}.
...
\crossreferences
\begin{itemize}
\item \code{ompt\_callback\_mutex\_init\_t} type signature for the lock
  and nest lock initialization, see \specref{sec:ompt_callback_mutex_init_t}.
\end{itemize}
...
\crossreferences
\begin{itemize}
\item \code{ompt\_callback\_mutex\_init\_t} type signature for lock
  and nest lock initialization, see \specref{sec:ompt_callback_mutex_init_t}.
\end{itemize}
...
executing a \code{omp\_init\_destroy} or \code{omp\_init\_nest\_destroy} function
before returning from the function call.

\tools

A thread dispatches a registered \code{ompt\_callback\_mutex\_destroy}
...
executing a \code{omp\_set\_nest\_lock} function
...
The \plc{nest-lock-kept} event occurs in the thread 
executing a \code{omp\_unset\_nest\_lock} function
when the thread still ownes the lock, 
before returning from the function call.
...
A thread dispatches a registered \code{ompt\_callback\_nested\_lock}
callback for each occurrence of a \plc{nest-lock-kept} event 
in that thread. This callback has the type signature \code{ompt\_callback\_nested\_lock\_t}.
