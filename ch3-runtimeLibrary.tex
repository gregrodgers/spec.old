...
\summary
The \verb#omp_target_free# routine frees the memory referred to by the
...
\begin{itemize}
  \item \verb#omp_target_free# may only be called with a pointer
    returned from \verb#omp_target_alloc# or \code{NULL}.
...
\ccppspecificstart
\begin{boxedcode}
int omp_target_memcpy(void * dst, void * src, size_t size,
...
\code{size} bytes of memory at offset \verb#src_offset# from  \code{src} are
...
with two NULL pointers causes it to return the number of dimensions supported
