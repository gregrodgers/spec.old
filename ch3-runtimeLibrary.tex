...
The \code{omp\_target\_alloc} routine returns a device pointer pointing to the
base of a buffer of size \code{size} on device \code{device\_num} if
successful, on failure NULL is returned.
...
\begin{itemize}
  ...
  \item The pointer returned by \vcode$omp_target_alloc$ is guaranteed to be
    compatible with the \code{device\_ptr} clause, \specref{subsec:target Construct}.
\end{itemize}
...
\begin{itemize}
  \item \vcode$omp_target_free$ may only be called with a pointer
    returned from \vcode$omp_target_alloc$ or \code{NULL}.
...
\code{size} bytes of memory at offset \vcode|src_offset| from  \code{src} are
copied to \code{dst} starting at offset \vcode|dst_offset|.  Each device must
be compatible with the device pointer specified on the same side of the copy.

\subsection{\code{omp\_target\_memcpy\_async}}
\index{omp\_target\_memcpy\_async@{\code{omp\_target\_memcpy\_async}}}
\label{subsec:omp_target_register_ptr}
\summary

The \code{omp\_target\_memcpy\_async} routine performs a copy between any combination
of host and device pointers asynchronously.

\format
\ccppspecificstart
\begin{boxedcode}
int omp\_target\_memcpy\_async(void * dst, void * src, size\_t size,
                        size\_t dst\_offset, size\_t src\_offset,
                        int dst\_device, int src\_device,
                        void* depend\_in, void* depend\_out);
\end{boxedcode}
\ccppspecificend

\effect

This routine performs an asynchronous memcopy.  Logically the
\vcode|omp_target_memcpy_async| routine generates a target task.
Dependencies are specified by passing the address to a variable as used in a
task \code{depends} clause.  Specifying \code{NULL} for any dependency is
handled specially and will cause the copy to behave as though it has an input
dependency on all target tasks on the source device and an output dependency
on all target tasks on the source and destination devices.

\subsection{\code{omp\_target\_memcpy\_rect\_async}}
\index{omp\_target\_memcpy\_rect\_async@{\code{omp\_target\_memcpy\_rect\_async}}}
\label{subsec:omp_target_register_ptr}
\summary

The \code{omp\_target\_memcpy\_rect\_async} routine performs a copy between any combination
of host and device pointers asynchronously.

\format
\ccppspecificstart
\begin{boxedcode}
int omp\_target\_memcpy\_rect\_async(
                        void * dst, void * src,
                        size_t element_size,
                        int num\_dims,
                        const size_t* volume,
                        const size_t* dst\_offsets,
                        const size_t* src\_offsets,
                        const size_t* dst\_dimensions,
                        const size_t* src\_dimensions,
...
Copies a multi-dimensional rectangular subvolume of \vcode|src| to \vcode|dst|
asynchronously.  The volume is specified in terms of the size of the element,
number of dimensions, and constant arrays of length \vcode|num_dims|.  The
volume array specifies the length, in number of elements, to copy in each dimension from \vcode|src|
to \vcode|dst|.  The \vcode|*_offsets| parameters specify the offset from the
origin of each of \code{src} and \code{dst} in elements.  Finally the
\vcode|*_dimensions| arguments specify the complete dimensions of each of
\code{src} and \code{dst}.  As an example, this routine could be used to copy
a $3\times3\times3$ cube from the center of a $9\times9\times9$ cube in
\code{src} to a \code{dst} matrix of size $3\times3\times3$ on the host as
follows:

\ccppspecificstart
\begin{boxedcode}
int src[9][9][9], dst[3][3][3];
omp\_target\_memcpy\_rect\_async(
          dst, src,
...
The \code{omp\_target\_associate\_ptr} routine ties a device pointer, as may
be returned from \code{omp\_target\_alloc} or implementation defined runtime
routines, to a host pointer in the manner of a map clause.
...
data motion associated with that host pointer.  The \vcode|device_offset|
parameter specifies what offset into \vcode|device_ptr| will be used as the
base address for the device side of the map.

\restrictions

\begin{itemize}
  \item The \vcode|device_ptr| value must be a valid pointer to device memory for the
