...
The \plc{lock-tested} or \plc{nest-lock-tested} event occurs in the thread 
executing a \code{omp\_test\_lock} or \code{omp\_test\_nest\_lock} region
after either successfully acquiring or failing to acquire the associated lock, 
if the thread did not already own the lock, 
but before finishing the region.
...
A thread dispatches a registered \code{ompt\_callback\_mutex\_acquired}
callback for each occurrence of a \plc{lock-tested} or \plc{nest-lock-tested} event 
in that thread.  This callback has the type signature \code{ompt\_callback\_mutex\_t} 
and returns the same value as returned by the routine.
