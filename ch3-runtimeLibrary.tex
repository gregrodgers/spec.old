...
current thread, as returned by the \code{omp\_get\_level} routine, the routine returns minus one (-1).
...
routine, the routine returns minus one (-1). Inactive parallel regions are regarded like active parallel 
...
\code{place\_num}. The numerical identifiers are non-negative, and 
...
encountering thread is not bound to a place, the routine returns minus one (-1).
