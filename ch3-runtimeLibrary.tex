...
Each type of lock can also have a \emph{lock hint} that contains information about the indented usage 
of the lock by the application code.  The OpenMP implementation can use this hint to select a proper 
usage-specific lock if supported by the implementation.  A conforming implementation can safely ignore 
the lock hint and map use non-hinted locks instead.
...
\index{omp\_init\_nest\_lock@{\code{omp\_init\_nest\_lock}}}
\summary
These routines initialize an OpenMP lock with hint.  The effect of the hint is implementation-defined and can be safely ignored by the OpenMP implementation.
...
\fortranspecificstart
\begin{boxedcode}
subroutine omp\_init\_lock(\plc{svar}, \plc{hint})
integer (kind=omp\_lock\_kind) \plc{svar}
...
The second argument passed to this routine (\plc{hint}) can be one of the valid OpenMP lock hints below
or any implementation-specific hint. The C/C++ header file 
(\code{omp.h}) and the Fortran include file (\code{omp\_lib.h}) and/or Fortran~90 module file 
(\code{omp\_lib}) define the valid constants for the hints.  The valid constants must include the following, 
which can be extended with implementation specific values:
...
typedef enum omp\_lock\_hint\_t \{
  ...
  omp\_lock\_hint\_uncontended,
  omp\_lock\_hint\_contended,
  omp\_lock\_hint\_nonspeculative,
  omp\_lock\_hint\_speculative,
  omp\_lock\_hint\_adaptive
\} omp\_lock\_hint\_t;
\end{codepar}
\ccppspecificend
...
\effect
The effect of these routines is to initialize the lock to the unlocked state and store the hint with it. 
...
\effect
If the specified lock contains a hint, the effects on the lock by calling these routines are implementation-defined.
