...
\item The type \code{omp\_tool\_control\_t}.

\item The type \code{omp\_tool\_control\_result\_t}.
...
the lock init function.  The callback receives  \code{omp\_lock\_hint\_none} as
...
\crossreferences
\begin{itemize}
\item \code{ompt\_callback\_lock\_init\_t} type signature for the lock
  and nest lock initialization, see \specref{sec:ompt_callback_lock_init_t}.
\end{itemize}
...
in that thread.  This callback has the type signature \code{ompt\_callback\_lock\_init\_t}.
The callbacks occur in the task encountering
the lock init function.  The callback receives the function's \plc{hint} argument as
\plc{hint} argument and
\code{ompt\_mutex\_lock} or 
 \code{ompt\_mutex\_nest\_lock} as \plc{kind} argument as appropriate.
...
\crossreferences
\begin{itemize}
\item \code{ompt\_callback\_lock\_init\_t} type signature for lock
  and nest lock initialization, see \specref{sec:ompt_callback_lock_init_t}.
\end{itemize}
...
The callbacks occur in the task encountering
the lock destroy function.  The callbacks receive \code{ompt\_mutex\_lock} or 
 \code{ompt\_mutex\_nest\_lock} as their \plc{kind} argument as appropriate.


\crossreferences
\begin{itemize}
\item \code{ompt\_callback\_lock\_destroy\_t} type signature for the lock
  and nest lock destruction, see \specref{sec:ompt_callback_lock_destroy_t}.
\end{itemize}
...
\crossreferences
\begin{itemize}
\item \code{ompt\_callback\_mutex\_acquire\_t} type signature for lock
  and nest lock acquire, see \specref{sec:ompt_callback_mutex_acquire_t}.
\item \code{ompt\_callback\_mutex\_t} type signature for
  successful acquisition of a lock or nest lock, see \specref{sec:ompt_callback_mutex_t}.
\item \code{ompt\_callback\_nest\_lock\_t} type signature for setting
  an owned nest lock, see \specref{sec:ompt_callback_nest_lock_t}.
\end{itemize}
...
the lock function.  The callback receives \code{ompt\_mutex\_lock} or 
 \code{ompt\_mutex\_nest\_lock} as \plc{kind} argument as appropriate.
...
\crossreferences
\begin{itemize}
\item \code{ompt\_callback\_mutex\_t} type signature for lock
  or outermost nest lock release, see \specref{sec:ompt_callback_mutex_t}.
\item \code{ompt\_callback\_nest\_lock\_t} type signature for inner nest lock
  release, see \specref{sec:ompt_callback_nest_lock_t}.
\end{itemize}
...
\index{tool control}
\label{sec:tool_control}

\summary
The \code{omp\_tool\_control} routine enables a program to 
...
\ccppspecificstart
\begin{boxedcode}
int omp\_tool\_control(int \plc{command}, int \plc{modifier}, void *\plc{arg});
\end{boxedcode}
\ccppspecificend
}
...
\fortranspecificstart
\begin{boxedcode}
integer function omp\_tool\_control(\plc{command}, \plc{modifier})
integer (kind=omp\_tool\_control\_kind) \plc{command}
integer (kind=omp\_tool\_control\_kind) \plc{modifier}
\end{boxedcode}
\fortranspecificend

\descr

An OpenMP program may use \code{omp\_tool\_control} to pass commands to 
a tool. 
Using \code{omp\_tool\_control}, an application can request that
...
Additionally, \code{omp\_tool\_control} can be used to pass 
...
\ccppspecificstart
\begin{boxedcode}
typedef enum omp_tool_control_result_t \{
  omp_tool_control_notool = -2,
  omp_tool_control_nocallback = -1,
  omp_tool_control_success = 0,
  omp_tool_control_ignored = 1
\} omp_tool_control_result_t;
\end{boxedcode}
\ccppspecificend
}
...
\fortranspecificstart
\begin{boxedcode}
integer (kind=omp\_tool\_control\_result\_kind), &
        parameter :: omp\_tool\_control\_notool = -2 
integer (kind=omp\_tool\_control\_result\_kind), &
        parameter :: omp\_tool\_control\_nocallback = -1
integer (kind=omp\_tool\_control\_result\_kind), &
        parameter :: omp\_tool\_control\_success = 0
integer (kind=omp\_tool\_control\_result\_kind), &
        parameter :: omp\_tool\_control\_ignored = 1
\end{boxedcode}
\fortranspecificend
}

If no tool is active, the OpenMP implementation will return
\code{omp\_tool\_control\_notool}. If a tool is active, but it has not
registered a callback for the \plc{tool-control} event, the OpenMP
implementation will return \code{omp\_tool\_control\_nocallback}.  
...
\code{omp\_tool\_control\_success} indicates that the tool has performed the
specified command.  A return value of \code{omp\_tool\_control\_ignored}
...
\ccppspecificstart
\begin{boxedcode}
typedef enum omp_tool_control_t \{
  omp_tool_control_start = 1,
  omp_tool_control_pause = 2,
  omp_tool_control_flush = 3,
  omp_tool_control_end = 4
\} omp_tool_control_t;
\end{boxedcode}
\ccppspecificend
}
...
\fortranspecificstart
\begin{boxedcode}
integer, parameter :: omp\_tool\_control\_start = 1
integer, parameter :: omp\_tool\_control\_pause = 2
integer, parameter :: omp\_tool\_control\_flush = 3
integer, parameter :: omp\_tool\_control\_end = 4
\end{boxedcode}
\fortranspecificend
}
...
\code{omp\_tool\_control\_start} & Start or 
...
\code{omp\_tool\_control\_pause} & Temporarily
turn monitoring off. If monitoring is already off, it is idempotent.\\\hline
\code{omp\_tool\_control\_flush} & Flush any data buffered by a tool.
This command may be applied whether monitoring is on or off.\\\hline
\code{omp\_tool\_control\_end} & Turn monitoring off permanently; 
...
to \code{omp\_tool\_control} at a point inside its associated OpenMP region.

\tools

An OpenMP implementation dispatches a registered
\code{ompt\_callback\_tool\_control} callback for each occurrence of a
...
signature \code{ompt\_callback\_tool\_control\_t}.The callback may return
any non-negative value, which will be returned to the application by
the OpenMP implementation as the return value of the
\code{omp\_tool\_control} call that triggered the callback.


Arguments passed to the callback are those passed by the user to
\code{omp\_tool\_control}. If the call is made in Fortran, the tool
...
\item \code{ompt\_callback\_tool\_control\_t}, see \specref{sec:ompt_callback_tool_control_t}
\end{itemize}
