...
\begin{itemize}
  \item Pointer arithmetic on the host value of the void* returned by
    \code{omp\_target\_alloc()} results in unspecified behavior.
...
\begin{itemize}
  \item Calling \code{omp\_target\_free} with any value other than one
    returned from \code{omp\_target\_alloc} or \code{NULL} results in
    unspecified behavior.
...
\subsection{\code{omp\_target\_device\_ptr}}
\index{omp\_target\_device\_ptr@{\code{omp\_target\_device\_ptr}}}
\label{subsec:omp_target_register_ptr}
\summary

The \code{omp\_target\_device\_ptr} routine returns the underlying
implementation's representation of the specified device pointer.

\format
\ccppspecificstart
\begin{boxedcode}
void* omp\_target\_device\_ptr(void * device\_ptr);
\end{boxedcode}
\ccppspecificend

\effect

The \code{omp\_target\_device\_ptr} routine converts an OpenMP device pointer
to a pointer representation usable by the underlying model.  This operation is
an unscoped equivalent of the \code{use\_device} clause.

\subsection{\code{omp\_target\_register\_ptr}}
\index{omp\_target\_register\_ptr@{\code{omp\_target\_register\_ptr}}}
\label{subsec:omp_target_register_ptr}
\summary

The \code{omp\_target\_register\_ptr} routine registers an
implementation-specific device pointer with the OpenMP runtime, and returns a
handle equivalent to those returned by \code{omp\_target\_alloc}.

\format
\ccppspecificstart
\begin{boxedcode}
void* omp\_target\_register\_ptr(void * impl\_device\_ptr, size\_t size, int device\_num);
\end{boxedcode}
\ccppspecificend

\effect

The \code{omp\_target\_register\_ptr} routine converts a pointer to memory
allocated by an underlying runtime, and returns an OpenMP compatible device
pointer handle.  The resulting handle can be used with the \code{impl\_device\_ptr}
clause, \code{omp\_target\_associate}, or any of the target memcpy routines.
...
\begin{itemize}
  ...
  \item Pointer arithmetic on the host handle returned by
    \code{omp\_target\_register\_ptr()} results in unspecified behavior.
\end{itemize}

\subsection{\code{omp\_target\_deregister\_ptr}}
\index{omp\_target\_deregister\_ptr@{\code{omp\_target\_deregister\_ptr}}}
\label{subsec:omp_target_deregister_ptr}
\summary

The \code{omp\_target\_deregister\_ptr} routine deregisters a device handle
that was previously registered.

\format
\ccppspecificstart
\begin{boxedcode}
void* omp\_target\_deregister\_ptr(void * device\_ptr);
\end{boxedcode}
\ccppspecificend

\effect \code{omp\_target\_deregister\_ptr} frees any host memory associated
...
\begin{itemize}
  \item The \code{device\_ptr} value must have been returned from a previous
    call to \code{omp\_target\_register\_ptr} or be NULL.
\end{itemize}
...
\code{omp\_target\_register\_ptr} to a host pointer in the manner of a map
clause.
