...
\summary
The \code{omp\_get\_initial\_device} routine returns the device id of the
initial device.
...
\effect
The effect of this routine is to return the device id of the initial device.
...
base of a buffer of size \code{size} on device \code{device\_num} if the 
allocation is successful. Otherwise the routine returns NULL.

The pointer returned by \verb#omp_target_alloc# is guaranteed to be compatible
 with the \code{device\_ptr} clause, \specref{subsec:target Construct}.

The effect of pointer arithmetic on the host value of the void 
pointer returned by \code{omp\_target\_alloc} is unspecified.
...
Programs that call this routine in target regions are non-conforming.
...
base address for the device side of the mapping.

The routine returns zero if successful. Otherwise it returns a non-zero value.

Only one device buffer can be associated with a given host pointer
value and device number pair. Attempting to associate a second buffer
will return non-zero.  The \code{omp\_target\_is\_present} function 
can be used to test whether a given host pointer has been associated 
on a given device.
