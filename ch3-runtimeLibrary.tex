...
\pagebreak
\section{Target Device Memory Routines}
\index{target device memory routines}
\index{target memory}
...
\summary
The \code{omp\_target\_alloc} routine allocates memory in the data environment
of the specified target device and returns an opaque host pointer to that
memory.
...
The \code{omp\_target\_alloc} routine returns a host handle to a
target buffer of size \code{size} on device \code{device\_num} if successful,
on failure NULL is returned.

\restrictions

\begin{itemize}
  \item Pointer arithmetic is not supported on the host value of the void* returned by
    \code{omp\_target\_alloc()}.
  \item The pointer returned by omp\_target\_alloc can be used with the
    \code{device\_ptr} clause, \specref{subsec:target Construct}.
\end{itemize}

\subsection{\vcode$omp_target_free$}
\index{omp\_target\_free@{\vcode{omp_target_free}}}
\label{subsec:omp_target_free}
\summary
The \vcode$omp_target_free$ routine frees the memory referred to by the
specified host handle.

\format
\ccppspecificstart
\begin{boxedcode}
void omp\_target\_free(void * target\_device\_ptr);
\end{boxedcode}
\ccppspecificend

\effect

The \vcode$omp_target_free$ routine frees the memory in the device data
environment associated with \vcode$device_ptr$.  If \vcode$device_ptr$ is
\vcode$NULL$, the operation is ignored.

\restrictions

\begin{itemize}
  \item \code{omp\_target\_free} may only be called with a pointer
    returned from \code{omp\_target\_alloc} or \code{NULL}.
  \item All asynchronous operations involving \code{device\_ptr} must be
    synchronized before a call to \code{omp\_target\_free}, no synchronization
    is implied.
\end{itemize}

\subsection{\code{omp\_get\_native\_ptr}}
\index{omp\_get\_native\_ptr@{\code{omp\_get\_native\_ptr}}}
\label{subsec:omp_target_register_ptr}
\summary

The \code{omp\_get\_native\_ptr} routine returns a native device pointer,
for use with native routines.

\format
\ccppspecificstart
\begin{boxedcode}
void* omp\_get\_native\_ptr(void * target\_ptr);
\end{boxedcode}
\ccppspecificend

\effect

The \code{omp\_get\_native\_ptr} routine converts an OpenMP target pointer
to a native device pointer. The representation of the native device
pointer is implementation defined, but guaranteed to be consistent for a given
device number during a given run.  This operation does for an OpenMP target
pointer what the \code{use\_device} clause does for mapped buffers.

\subsection{\code{omp\_get\_target\_ptr}}
\index{omp\_get\_target\_ptr@{\code{omp\_get\_target\_ptr}}}
\label{subsec:omp_target_register_ptr}
\summary

The \code{omp\_get\_target\_ptr} routine registers a native device pointer
with the OpenMP runtime, and returns an OpenMP target pointer handle
equivalent to those returned by \code{omp\_target\_alloc}.

\format
\ccppspecificstart
\begin{boxedcode}
void* omp\_get\_target\_ptr(void * native\_device\_ptr, size\_t size, int device\_num);
\end{boxedcode}
\ccppspecificend

\effect

The \code{omp\_get\_target\_ptr} routine converts a native pointer to device
memory into an OpenMP target pointer.  The resulting pointer can be used with the
\code{impl\_device\_ptr} clause, \code{omp\_target\_associate}, or any of the
target memcpy routines.

\restrictions

\begin{itemize}
  \item The \code{device\_ptr} value must be a valid native pointer to device memory for the
    device numbered \code{device\_num}.
  \item Pointer arithmetic must not be used on the host handle returned by
    \code{omp\_get\_target\_ptr()}.
\end{itemize}

\subsection{\code{omp\_release\_target\_ptr}}
\index{omp\_release\_target\_ptr@{\code{omp\_release\_target\_ptr}}}
\label{subsec:omp_target_deregister_ptr}
\summary

The \code{omp\_release\_target\_ptr} routine releases an OpenMP target pointer
that was previously created by \code{omp\_get\_target\_ptr}.

\format
\ccppspecificstart
\begin{boxedcode}
void* omp\_release\_target\_ptr(void * device\_ptr);
\end{boxedcode}
\ccppspecificend

\effect \code{omp\_release\_target\_ptr} frees any host memory associated
with \code{device\_ptr}.  The device buffer referenced by the handle is
\emph{not} freed.

\restrictions

\begin{itemize}
  \item The \code{device\_ptr} value must have been returned from a previous
    call to \code{omp\_get\_target\_ptr} or be NULL.
\end{itemize}

% \subsection{\code{omp\_target\_is\_device\_ptr}}
% \index{omp\_target\_is\_device\_ptr@{\code{omp\_target\_is\_device\_ptr}}}
% \label{subsec:omp_target_register_ptr}
% \summary
%
% The \code{omp\_target\_is\_device\_ptr} routine validates whether the pointer
% it is passed is a valid OpenMP device pointer or not.
%
% \format
% \ccppspecificstart
% \begin{boxedcode}
% int omp\_target\_is\_device\_ptr(void * device\_ptr);
% \end{boxedcode}
% \ccppspecificend
%
% \effect
%
% The effect of this routine is to return \plc{true} if the specified pointer is
% a valid OpenMP device pointer; otherwise, it returns \plc{false}.
...
\ccppspecificstart
\begin{boxedcode}
int omp\_target\_is\_present(void * ptr, size\_t offset, int device\_num);
\end{boxedcode}
\ccppspecificend

\effect

The effect of this routine is to return \plc{true} if the specified pointer
and offset would be found present on device \code{device\_num}; otherwise, it
returns \plc{false}.
...
The \code{omp\_target\_memcpy} routine performs a copy between any combination
of host and device pointers.

\format
\ccppspecificstart
\begin{boxedcode}
int omp\_target\_memcpy(void * dst, void * src, size\_t size);
\end{boxedcode}
\ccppspecificend

\effect

\code{size} bytes of memory in \code{src} are copied to the first \code{size}
bytes of \code{dst}.  Each pointer is checked for type by a mechanism
equivalent to \code{omp\_target\_is\_device\_ptr()}, and if the result is
false the pointer is treated as a pointer to host memory.

\subsection{\code{omp\_target\_memcpy\_offset}}
\index{omp\_target\_memcpy\_offset@{\code{omp\_target\_memcpy\_offset}}}
\label{subsec:omp_target_register_ptr}
\summary

The \code{omp\_target\_memcpy\_offset} routine performs a copy between any combination
of host and device pointers, with the option of explicitly specifying offsets
and direction.

\format
\ccppspecificstart
\begin{boxedcode}
  typedef enum omp\_copy\_direction\_t {
    OMP\_COPY\_DIRECTION\_AUTO,
    OMP\_COPY\_DIRECTION\_HOST\_TO\_DEVICE,
    OMP\_COPY\_DIRECTION\_DEVICE\_TO\_HOST,
    OMP\_COPY\_DIRECTION\_DEVICE\_TO\_DEVICE,
    OMP\_COPY\_DIRECTION\_HOST\_TO\_HOST
  } omp\_copy\_direction\_t;

int omp\_target\_memcpy\_offset(void * dst, void * src, size\_t size,
                        size\_t dst\_offset, size\_t src\_offset,
                        omp\_copy\_direction direction);
\end{boxedcode}
\ccppspecificend

\effect

\code{size} bytes of memory in \code{src} are copied \code{dst} starting at
their respective offsets. The direction can be any value of
\code{omp\_copy\_direction\_t}.  If the value of direction is \code{AUTO},
then each pointer is checked for type by a mechanism
equivalent to \code{omp\_target\_is\_device\_ptr()}, and if the result is
false the pointer is treated as a pointer to host memory. For any other value
the direction specified is respected, and passing pointers from another type
of memory results in unspecified behavior.
...
The \code{omp\_target\_associate\_ptr} routine ties an OpenMP device pointer,
as returned from either \code{omp\_target\_alloc} or
\code{omp\_get\_target\_ptr} to a host pointer in the manner of a map
clause.

\format
\ccppspecificstart
\begin{boxedcode}
int omp\_target\_associate\_ptr(void * host\_ptr, void * device\_ptr, size\_t size,
                                size\_t device\_offset, int device\_num);
\end{boxedcode}
\ccppspecificend
...
data motion associated with that host pointer.

\restrictions

\begin{itemize}
  \item The \code{device\_ptr} value must be a valid pointer to device memory for the
    device numbered \code{device\_num}.
    \item Only one device buffer can be associated with a given host pointer
      value and device number pair.  Attempting to associate a second buffer
      will return failure.  The \code{omp\_target\_is\_present} function can be used
      to test whether a buffer has been associated or mapped on a given
      device.
\end{itemize}
...
on device \code{device\_num} from the presence table for host pointer
\code{ptr}.   Calling this function on a pointer that is not NULL and does not
have associated data on the given device results in unspecified behavior.
