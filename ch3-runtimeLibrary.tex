...
\effect
The \code{omp\_get\_place\_num} routine returns the place number of the place to which the encountering thread is bound. The returned value is between zero and \code{omp\_get\_num\_places()-1}, inclusively.
...
\binding
The binding thread set for an  \code{omp\_get\_partition\_num\_places} region is the encountering task.
...
\binding
The binding thread set for an \code{omp\_get\_partition\_place\_nums} region is the encountering task.
