...
\summary
The \code{omp\_target\_free} routine frees the memory referred to by the
specified host handle.

\format
\ccppspecificstart
\begin{boxedcode}
void omp\_target\_free(void * device\_ptr);
\end{boxedcode}
\ccppspecificend
...
environment associated with \code{device\_ptr}.  If \code{device\_ptr} is
