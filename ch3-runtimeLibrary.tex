...
A conforming implementation can safely ignore the lock hint and use non-hinted locks instead.
...
\summary
These routines initialize an OpenMP lock with no hint.
...
The effect of the hint is implementation-defined; The effect of the hint is implementation defined; the hint can safely be ignored by the OpenMP implementation without changing the program semantics.
...
The second argument passed to this routine (\plc{hint}) can be one of the valid OpenMP lock hints below or any implementation-specific hint. 
The C/C++ header file (\code{omp.h}) and the Fortran include file (\code{omp\_lib.h}) and/or Fortran~90 module file (\code{omp\_lib}) define the valid constants for the hints.  
The valid constants must include the following, which can be extended with implementation specific values:
...
Combing \code{omp\_lock\_hint\_none} with any other hint has no effect.
...
\notestart
\noteheader – Future OpenMP specifications may add additional hints to the \code{omp\_lock\_hint\_t} type.  
Implementers are advised to add implementation-defined hints starting from the most significant bit of the \code{omp\_lock\_hint\_t} type and to include the name of the implementation in the name of the added hint to avoid name conflicts with other OpenMP implementations.
\noteend


\effect
The effect of these routines is to initialize the lock to the unlocked state and, optionally, use the hint to choose a specific lock implementation. 
...
\effect
If the specified lock contains a hint, the effects of calling these routines on the lock are implementation-defined, but retain the mutual exclusion semantics of locks.

Each of these routines may cause suspension of the task executing the routine until the 
specified lock is available and then sets the lock, or continues to speculatively execute the task region. 

A simple lock with no hint is available if it is unlocked. Ownership of the lock is granted to the task 
executing the routine.

A nestable lock without hint is available if it is unlocked or if it is already owned by the task 
...
\effect
If the specified lock contains a hint, the effects by calling these routines on the lock are implementation-defined, but retain the mutual exclusion semantics of locks.

For a simple lock without hint, the \code{omp\_unset\_lock} routine causes the lock to become unlocked.

For a nestable lock without hint, the \code{omp\_unset\_nest\_lock} routine decrements the nesting 
count, and causes the lock to become unlocked if the resulting nesting count is zero.

For either routine, if the lock without hint becomes unlocked, and if one or more task regions were 
