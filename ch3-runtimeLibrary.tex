...
pass commands to an attached tool.
...
\ccppspecificstart
\begin{boxedcode}
void omp\_tcontrol(long long int \plc{command}, long long int \plc{modifier});
\end{boxedcode}
\ccppspecificend
}
...
% A tool may allow an application using this interface to turn monitoring on and off many times during an execution to monitor only code regions of interest.
...
The \code{omp\_tcontrol} routine also allows tool-specific 
values for \plc{command} and \plc{modifier}. 
...
\verb|omp_tcontrol_start| & Start or 
restart monitoring if it is off. If monitoring is already on, this 
command is idempotent. If monitoring has already been turned off 
permanently, this command will have no effect.\\\hline
\verb|omp_tcontrol_pause| & Temporarily
turn monitoring off. If monitoring is already off, it is idempotent.\\\hline
\verb|omp_tcontrol_flush| & Flush any data buffered by a tool.
This command may be applied whether monitoring is on or off.\\\hline
\verb|omp_tcontrol_end| & Turn monitoring off permanently; 
the tool finalize itself and flush all output.\\\hline
\end{tabular}
\vskip 1ex
...
If a tool wants notification of 
\code{omp\_tcontrol} calls, 
the tool must register a callback for the \code{ompt\_event\_tcontrol} event.

If the user program calls \code{omp\_tcontrol}, an OpenMP implementation 
shall invoke a registered \code{ompt\_event\_tcontrol} callback.
...
\item \code{ompt\_tcontrol\_callback\_t} callback signature, see
\specref{subsec:ompt_tcontrol_callback_t}
\end{itemize}
