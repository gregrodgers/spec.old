% This is ch4-environmentVariables.tex of the OpenMP specification.
...
value that can be specified in the \code{priority} clause of the \code{task} construct. A value of
zero (default value) specifies that the runtime will ignore the \code{priority} clause.
...
The following grammar describes the values of \plc{list} accepted for the \code{OMP\_PLACES} environment
variable.

\begin{bnf*}
   \bnfprod{list}{
      \bnfpn{p-list} \bnfor
      \bnfpn{aname-list} \bnfor
      \bnfpn{p-list} \bnfts{,} \bnfpn{list} \bnfor
      \bnfpn{aname-list} \bnfts{,} \bnfpn{list}
      } \\
   \bnfprod{p-list}{
   ...
   \bnfprod{res-list}{
      ...
      \bnfpn{res-interval} \bnfts{,} \bnfpn{res-interval}
      } \\
   ...
   \bnfprod{aname-list}{
      \bnfpn{aname} \bnfts{,} \bnfpn{aname} \bnfor
      \bnfpn{aname}
   } \\
   \bnfprod{aname}{
   ...
   \bnfprod{num-places}{
      \bnftd{positive integer (not null)}
   } \\
   \bnfprod{stride}{
      \bnftd{integer}
   } \\
...
\medskip
\textsf{\textbf{\footnotesize{TABLE 4-1}}}\hspace{1em} List of defined abstract names for \code{OMP\_PLACES}
...
display any information when the \code{OMP\_DISPLAY\_ENV} environment variable is 
\code{FALSE}, undefined, or any other value than \code{TRUE} or \code{VERBOSE}.
...
The \code{OMP\_TASK\_MAX\_PRIORITY} environment variable controls the use of task
...
variable is set to zero, the OpenMP implementation will ignore the \code{priority}
clause in any \code{task} directive. Setting the variable to a non-zero integer value
will inform the implementation that values specified in \code{priority} clauses can be
...
% This is the end of ch4-environmentVariables.tex
