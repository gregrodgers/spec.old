...
The signature for \code{ompt\_tool} is as follows:
\vbox{
\ccppspecificstart
\begin{boxedcode}
extern "C" \{
...
The type signature \code{ompt\_initialize\_fn\_t} 
for a tool initializer callback is as follows:
...
The first argument to a tool initializer is \plc{lookup}---a callback 
to that a tool must use to 
% interrogate the runtime system to 
obtain a pointer to each OMPT interface function.
...
\noteheader -- OMPT interface functions should not be global symbols in 
an OpenMP implementation. A tool developer might be tempted to call a global 
OMPT interface function directly. Since there exist cases where a global 
symbol exported by an OpenMP implementation will not be visible to a tool,
an OpenMP implementation should avoid providing global symbols for OMPT
interface functions that {\em should not} be used by any tool.
\noteend
...
The third argument to a tool initializer is a version number that
unambiguously identifies the version of the OMPT interface supported
by an OpenMP implementation.  
...
an OpenMP program executes by using the OMPT interface function 
...
implement mutual exclusion for various purposes.
 ...
 \subsubsection{\code{ompt\_enumerate\_states}}
...
to enumerate all thread states supported by a particular OpenMP 
implementation.
...
OMPT_API int ompt_enumerate_states(
  uint32_t \plc{current_state}, 
  uint32_t *\plc{next_state}, 
...
be the numerical value for a valid thread state supported by an OpenMP
implementation.  
...
Subsequent invocations of \code{ompt\_enumerate\_states} by a tool
...
\ccppspecificstart
\begin{boxedcode}
uint32_t state = omp_state_undefined;
...
\subsubsection{\code{ompt\_enumerate\_mutex\_kinds}}
\label{ompt_enumerate_mutex_kinds}
 
\format
An OpenMP implementation may implement
locks, nest locks, critical sections, and atomic regions in several
different ways. 
\begin{comment}
...
\end{comment}
The \verb|ompt_enumerate_mutex_kinds| routine enables a tool to enumerate
the different kinds mutual exclusion implementations that 
...
as the first argument of
a call to \verb|ompt_enumerate_mutex_kinds|, the call will
assign the next mutex kind in an enumeration of all mutex kinds
supported by the implementation to the pointer passed as the second
argument of the call.
The third argument of a call to \verb|ompt_enumerate_mutex_kinds| 
is the address of a character pointer
that will be assigned a pointer to the name associated with
the mutex kind value assigned to the pointer passed as the second argument of
the call.
Whenever one or more mutex kinds are left in the enumeration, 
\verb|ompt_enumerate_mutex_kinds| will return $1$.
When the last mutex kind in the enumeration is passed 
to \verb|ompt_enumerate_mutex_kinds| the first argument, 
the function will return $0$ 
indicating that the enumeration is complete.

\constraints

The first argument passed to a call to \verb|ompt_enumerate_mutex_kinds| must
be the numerical value for a valid mutex implementation kind 
supported by an OpenMP implementation.
To begin enumerating the mutex implementation kinds 
that an OpenMP implementation supports,
a tool should pass the value \verb|ompt_mutex_kind_unknown| as the
first argument of a call to \verb|ompt_enumerate_mutex_kinds|.
Subsequent invocations of \verb|ompt_enumerate_mutex_kinds| by a tool
should pass the value
assigned to the pointer passed as the second argument to the prior call.
...
Section~\ref{sec:task-region}) in a signal handler.  A task's frame
...
\section{Runtime Inquiry Routines}
...
\subsection{\code{ompt\_get\_thread\_data}}
\label{sec:thread-inquiry}

\summary
The \verb|ompt_get_thread_data| routine returns the address of the thread 
...
may modify the identifier.  {\em This function is async signal
safe.}

\subsection{\code{ompt\_get\_state}}

\summary
The \verb|ompt_get_state| routine returns the state and the wait identifier of the 
current thread. 
...
\subsection{\code{ompt\_get\_parallel\_info}} \label{sec:parallel-inquiry}

\summary
The \verb|ompt_get_parallel_info| routine returns information about
...
\subsection{\code{ompt\_get\_task\_info}} \label{sec:task-region}

\summary
The \verb|ompt_get_task_info| routine provides information about the
...
higher ancestor levels.  Function \verb|ompt_get_task_info| returns
a Boolean, which indicates whether there is a task at the specified
ancestor level.  {\em This function is async signal safe.}
...
\subsection{\code{ompt\_get\_target\_info}}
\label{sec:target-region}

\summary
The \verb|ompt_get_target_info| routine returns identifiers that specify a
...
\subsection{\code{ompt\_get\_num\_devices}}
\label{sec:target-device}

\summary
The \verb|ompt_get_num_devices| routine returns the number of visible devices.
...
% {\it This function is async signal safe.}


\subsection{\code{ompt\_target\_get\_device\_info}}

\summary
The \verb|ompt_get_device_info| routine 
...
\subsection{\code{ompt\_target\_get\_device\_id}}

\summary
The \verb|ompt_target_get_device_id| routine returns the device
...
then it will return a value of -1. {\it This function is async
signal safe.}

\subsection{Target-specific Inquiry Routines}
\subsubsection{\code{ompt\_target\_get\_device\_time}}

\summary
The \verb|ompt_target_get_time| routine
...
\subsubsection{\code{ompt\_target\_translate\_time}}
...
the same meaning as the {\tt double} returned from \verb|omp_get_wtime|.
...
\label{subsec:ompt_event_tcontrol}
...
\subsection{\code{ompt\_tcontrol\_callback\_t}}
\index{ompt\_tcontrol\_callback\_t@{\code{ompt\_tcontrol\_callback\_t}}}
\label{subsec:ompt_tcontrol_callback_t}
\format
\vbox{
\ccppspecificstart
\begin{boxedcode}
...
\section{Runtime Callbacks}

This section contains type signatures for callback interfaces to routines
in the OpenMP runtime that are present exclusively for use by tools.

\subsection{Target Device Trace Control Callbacks}
...
return value to function \verb|ompt_target_get_device_info|) and
...
string that is returned by \verb|ompt_target_get_device_info|.
...
OMPT_TARG_API _Bool ompt_target_start_trace(
  ...
  ompt_get_target_info_inquiry_t get_info
);
\end{boxedcode}
\ccppspecificend}
...
for an event depends upon its type. For this purpose,
...
To inspect a native trace record for a device, a tool invokes the following function to obtain a pointer to a trace record in the native format for the device associated with a buffer. \vbox{\ccppspecificstart
