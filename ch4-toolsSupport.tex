...
\item \code{ompt\_callback\_tool\_control}, see \specref
...
\code{ompt\_finalize\_fn\_t}, specified by the
\plc{finalize} field in this structure will be called as the OpenMP
implementation shuts down.

\crossreferences
\begin{itemize}
\item \code{ompt\_finalize\_fn\_t}, \specref{sec:ompt_finalize_fn_t}
\end{itemize}
...
\ompttoolsignature{\code{ompt\_tool}}
\label{sec:ompt_tool}
...
A tool may use its \callbackarg{} \plc{ompt\_version} to determine
...
\argdesc
The \callbackarg{} \plc{ompt\_version} is a version number that 
unambiguously identifies the version of the 
...
\restrictions
Tool callbacks may not use OpenMP directives.
...
\ccppspecificstart
\begin{boxedcode}
typedef int (*ompt_initialize_fn_t) (
...
Of the OpenMP runtime library routines described in described in
Section~\ref{chap:Runtime Library Routines},
a tool initializer may only call OpenMP execution environment routines,
described in Section~\ref{sec:Execution Environment Routines}. 
...
\restrictions
A tool finalizer may not call any runtime library routines
described in Section~\ref{chap:Runtime Library Routines}.

\argdesc
The \callbackarg{} \plc{fns} is the value returned by \code{ompt\_tool}.
...
\restrictions
Of the OpenMP runtime library routines described in 
Section~\ref{chap:Runtime Library Routines}, event callbacks 
may only call execution environment \plc{getter}
routines described in Section~\ref{sec:Execution Environment Routines}.
...
\omptcallbacksignature{\code{ompt\_callback\_tool\_control\_t}}
\index{ompt\_callback\_tool\_control\_t@{\code{ompt\_callback\_tool\_control\_t}}}
\label{sec:ompt_callback_tool_control_t}
\format
\vbox{
\ccppspecificstart
\begin{boxedcode}
typedef int (*ompt_callback_tool\_control_t) (
...
\descr 
The callback with type signature \code{ompt\_callback\_tool\_control\_t},
includes \callbackarg{}s \plc{command} and \plc{modifier}. Standard values for
\plc{command} are defined by \code{omp\_tool\_control\_t}. 
defined in \specref{sec:tool_control}.
...
\crossreferences
\begin{itemize}
\item \code{omp\_tool\_control\_t} enumeration type, see \specref{sec:tool_control}.
\end{itemize}
...
\summary
A tool entry point known as \code{ompt\_enumerate\_states} 
...
A tool entry point known as \code{ompt\_enumerate\_mutex\_impls} 
...
A tool entry point known as \code{ompt\_callback\_set} 
...
A tool entry point known as \code{ompt\_callback\_get} 
...
\summary
A tool entry point known as \code{ompt\_get\_thread\_data}
...
to obtain the places in the current thread's affinity set.
...
A tool may invoke a runtime entry point known as
\code{ompt\_get\_thread\_affinity} with type signature
\code{ompt\_get\_thread\_affinity\_t} to obtain the places in the current thread's
...
places in the current thread's affinity set and the number of elements in the
...
Otherwise, the places in the current thread's
...
\summary
A tool entry point known as \code{ompt\_get\_state}
...
A tool entry point known as \code{ompt\_get\_parallel\_info} 
...
\summary
A tool entry point known as \code{ompt\_get\_task\_info} 
...
\summary
A tool entry point known as \code{ompt\_get\_target\_info} 
...
\summary
A tool entry point known as \code{ompt\_get\_num\_devices} 
...
\summary
A tool entry point for a device known 
...
\descr
A tool entry point for a device known as \code{ompt\_translate\_time} 
with type signature \code{ompt\_translate\_time\_t} translates
a time value obtained from the specified device to a corresponding time
...
\summary
A tool entry point for a device known as \code{ompt\_set\_trace\_ompt} 
...
\summary
A tool entry point for a device known as \code{ompt\_set\_trace\_native} 
...
\summary
A tool entry point for a device known as \code{ompt\_start\_trace} 
...
\summary
A tool entry point for a device known as \code{ompt\_pause\_trace} 
...
\summary
A tool entry point for a device known as \code{ompt\_stop\_trace} 
...
\summary
A tool entry point for a device known as \code{ompt\_advance\_buffer\_cursor} 
...
\summary
A tool entry point for a device known as
...
\summary
A tool entry point for a device known as \code{ompt\_buffer\_get\_record\_ompt} 
