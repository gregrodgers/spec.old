...
enumeration type \verb|omp_state_t| or
...
with OpenMP, the implementation reports the state as \verb|omp_state_undefined|.
...
Some values of the enumeration type \verb|omp_state_t| are used by all 
...
\sloppy
  ...
  A  compliant implementation may report \verb|omp_state_wait_barrier| for implicit barriers.
  ...
  early, when the thread encounters a barrier, or late, when the
  thread begins to wait at the barrier.
  A  compliant implementation may report \verb|omp_state_wait_barrier| for explicit barriers.
...
\item \verb|omp_state_wait_target_data| 
  ...
  A  compliant implementation may report \verb|omp_state_wait_target| for target data constructs.

\item \verb|omp_state_wait_target_update| 

  The thread is waiting for a target  update operation to complete. 
  A  compliant implementation may report \verb|omp_state_wait_target| for target update constructs.
...
\label{sec:omptFunctions}
 ...
 the tools. 
 All functions in the inquiry API are marked with \verb|OMPT_API|. These functions should not be global symbols in an OpenMP implementation to avoid tempting tool developers to call them directly. Section~\ref{sec:init} describes how a tool should obtain pointers to these inquiry functions.
