...
performance analysis tools running in the same
address space as the OpenMP program through the OpenMP Tools Interface or OMPT.
...
Activating a tool, i.e., the procedure necessary to use a tool during the
execution of an OpenMP application, is a five step hand-shake process: first, before
the initialization of the OpenMP implementation, the tools makes itself available by
providing an initialization routine using a known-symbol; second, the OpenMP
implementation during its initialization, detects the tool and activates by calling
the function with the known symbol; third, the tool registers itself by returning 
an initialization function; 
fourth, immediately after its complete initialization the OpenMP implementation
executes the tool
initialization function; and fifth, the tool uses this invocation to initialize
itself and setup its state as well as register the needed event callbacks. After these steps
...
function \code{ompt\_tool} exists in a program's address space and, if so, calls it.
...
For any separate OpenMP implementation initialized later, e.g., after a process fork or for accelerators that maintain
...
return a pointer to a function with type signature \code{ompt\_initialize\_fn\_t} 
(described in  Section~\ref{sec:ompt_initialize_fn_t}). This returned function pointer
can either be \code{NULL}, if the tool (e.g., after checking the OpenMP environment
or after verifying versioning information) 
decides to not register itself, or be a pointer to the tool's initialization function.

\subsection{Step 4: Tool Invocation}
\index{tool invocation}
\label{sec:tool-invocation}

If the value returned by the tool is a non-\code{NULL} pointer to a tool initializer function, 
...
environment routine invocation. 
...
\section{Tasks During Tool Initialization}
\index{tool initialization tasks}
\label{sec:tool-initialize-tasks}
