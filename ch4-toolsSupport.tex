...
The parameter \plc{codeptr\_ra} is used to relate the implementation
of an OpenMP region back to its source code.  In cases where a runtime
routine implements the construct associated with this callback,
...
\code{ompt\_tool}. There are three ways
...
space. If multiple implementations of \code{ompt\_tool} are available,
the OpenMP implementation will use the first tool-provided
implementation of \code{ompt\_tool} found.
...
has several duties.  First, the tool initializer should use its third argument
...
\plc{lookup} function at any time and the function will return the
...
If \code{ompt\_tool} returned a non-\code{NULL} pointer when an OpenMP
implementation was initialized, the tool finalizer specified by the
...
\summary
A tool's implementation of \code{ompt\_tool} returns a pointer to an
...
typedef struct ompt_fns_t \{
  ompt_initialize_fn_t \plc{initialize};
  ompt_finalize_fn_t \plc{finalize};
\} ompt_fns_t;
\end{boxedcode}
\ccppspecificend
}
...
\code{ompt\_fns\_t} structure returned by \code{ompt\_tool} must be
non-\code{NULL}.
...
\code{ompt\_tool} return a non-\code{NULL} pointer to an
\code{ompt\_control\_t} structure, which contains pointers to
...
The version of OMPT described by this document is 1, and available
through the constant \code{OMPT\_VERSION} defined in the
\code{ompt.h} header file.
...
\omptcallbacksignature{\code{ompt\_finalize\_fn\_t}}
\label{sec:ompt_finalize_fn_t}

\summary
A tool implements an finalizer with the type signature
\code{ompt\_finalize\_fn\_t} to finalize the tool's use of
an OpenMP implementation's OMPT interface.
...
\ccppspecificstart
\begin{boxedcode}
typedef void (*ompt_finalize_fn_t) (
  ompt_fns_t *\plc{fns}
);
\end{boxedcode}
\ccppspecificend
...
\descr
The finalizer for an OpenMP implememtation is invoked by an OpenMP
implementation as it shuts down.

\argdesc
The argument passed to a tool finalizer 
is the value returned by \code{ompt\_tool}.
...
\argdesc
The parameter \plc{thread\_type}
indicates the type of the new thread: initial, worker, or other.
The binding of parameter \plc{thread\_data} is the new thread.
...
\argdesc
The binding of parameter \plc{thread\_data} is the thread that is terminating.
...
\argdesc
The parameter \plc{endpoint}
indicates whether the callback signals the begin or end of the scope.
...
\argdesc
The binding of parameter \plc{parent\_task\_data} is the enclosing task. 
The \plc{parent\_frame} points to the frame object associated with the
parent task.
The binding of parameter \plc{parallel\_data} is the parallel region that is beginning.
The parameter \plc{requested\_team\_size}
...
The \plc{invoker} argument explains whether the code for the parallel
...
\argdesc
The binding of parameter \plc{parallel\_data} is the parallel region that is ending.
The binding of parameter \plc{task\_data} is the encountering task.
The parameter \plc{invoker} explains whether the execution of the parallel
...
\argdesc
The binding of parameter \plc{parallel\_data} is the current parallel region.
The binding of parameter \plc{task\_data} is the encountering task.
\codeptrdesc
...
\argdesc
The binding of parameter \plc{parent\_task\_data} is the encountering task,
The parameter \plc{parent\_frame}
indicates the frame information of the encountering task. 
The binding of parameter \plc{new\_task\_data} is the created task.
The parameter \plc{type} that indicates the kind of the task: initial, explicit or target.
\codeptrdesc
...
\argdesc
The binding of parameter \plc{task\_data} is the task being created.
The parameter \plc{deps} that lists all dependences of a new task and
...
\argdesc
The binding of parameter \plc{src\_task\_data} is a running task with an outgoing dependence.
The binding of parameter \plc{sink\_task\_data} is a task with an unsatisfied incoming dependence.
...
\argdesc
The parameter \plc{prior\_task\_status} indicates the status of the task that arrived at a task scheduling point. 
The binding of parameter \plc{prior\_task\_data} is the task that arrived at the scheduling point. 
The binding of parameter \plc{next\_task\_data} is the task that will resume at the scheduling point.
...
\descr
\argdesc
The parameter \plc{endpoint} that indicates whether the callback signals
...
The binding of parameter \plc{parallel\_data} is the current parallel region.
The binding of parameter \plc{task\_data} is the implicit task executing the parallel
...
\argdesc
The parameter \plc{kind} indicates indicates the kind of region causing the wait.  
The parameter \plc{endpoint} indicates whether the callback indicates whether the
...
The binding of parameter \plc{parallel\_data} is the current parallel region.
The binding of parameter \plc{task\_data} is the current task.
\codeptrdesc
...
\argdesc
The parameter \plc{kind} indicates the kind of the lock.
The parameter \plc{wait\_id} indicates the object being awaited. 
\codeptrdesc
...
\argdesc
The parameter \plc{kind} indicates the kind of the lock.
The parameter \plc{wait\_id} identifies the lock.
\codeptrdesc
...
\argdesc
The parameter \plc{kind} indicates the kind of the lock.
The parameter \plc{hint} indicates the hint provided when initializing
an implementation of mutual exclusion.
The parameter \plc{impl} indicates the mechanism chosen by the
runtime to implement the mutual esclusion.
The parameter \plc{wait\_id} indicates the object being awaited. 
%
\codeptrdesc
...
\argdesc
The \plc{target\_id} argument is a unique identifier for the
...
Note: {\em the buffer request callback is not 
required to be async-signal safe.}

\argdesc
The parameter \plc{device\_id} specifies the device. 
...
Note: {\em the buffer completion callback is not 
required to be async-signal safe.}
...
The argument \plc{buffer\_owned} is 1 if the data
...
callback interface, returning \code{ompt\_set\_error}. 
...
A return value of \code{ompt\_set\_none} indicates that the callback
...
\descr
This entry point enables tracing on a device. It provides tool
...
\argdesc
%%add args 
The parameter \emph{buffer request} specifies a tool 
callback that will supply a device with a buffer to deposit events.
The parameter \emph{buffer complete} specifies a tool callback that 
