...
typedef struct ompt_record_ompt_s \{
  ...
  union \{
    ...
    ompt_record_thread_end_t \plc{thread_end};
...
\ccppspecificstart
\begin{boxedcode}
typedef struct ompt_record_thread_end_s \{
  ompt_id_t \plc{thread_id};
\} ompt_record_thread_end_t;
\end{boxedcode}
\ccppspecificend
}
\end{targetRecord}
...
\argdesc 
The \callbackarg{} \plc{command} passes a command from an application to a tool. 
Standard values for
\plc{command} are defined by \code{omp\_control\_tool\_t}. 
defined in \specref{sec:control_tool}.
The \callbackarg{} \plc{modifier} passes a command modifier from an application to a tool. 
The callback allows tool-specific values for \plc{command} and \plc{modifier}.
Tools must ignore \plc{command} values that they are not explicitly designed to handle.
...
\argdesc 
The \callbackarg{} \plc{flags}, defined by the enumeration \code{ompt\_cancel\_flag\_t}, indicates
...
bit in the flags will be set. 
The \callbackarg{} \plc{task\_data} corresponds to the task encountering a \code{cancel} construct, 
a \code{cancellation point} construct, or a construct defined as having an implicit cancellation point.
\codeptrdesc
...
associated with the place numbered \plc{place\_num}. 
The numerical identifiers are non-negative, and
their meaning is implementation defined. 
When the array \plc{ids} of size \plc{ids\_size} is large enough 
to contain all identifiers, they are returned in the \plc{ids} array  and
their order in the array is implementation defined. Otherwise, when the 
\plc{ids} array is too small, the values in the \plc{ids} array are unchanged.
In both case, the routine returns the number of numerical identifiers 
available to the execution environment in the specified place.
...
The runtime entry point known as
\code{ompt\_get\_partition\_place\_nums} with type signature
\code{ompt\_get\_partition\_place\_nums\_t} returns
the list of place numbers corresponding to the places in the \plc{place-partition-var}
ICV of the innermost implicit task.
When the array \plc{place\_nums} of size \plc{place\_nums\_size} is large enough 
to contain all identifiers, they are returned in the \plc{place\_nums} array. Otherwise, 
when the \plc{place\_nums} array is too small, the values in the \plc{place\_nums} array are 
unchanged.
In both case, the routine returns the number of places in the \plc{place-partition-var}
ICV of the innermost implicit task.
