...
codes for \code{ompt\_set\_callback} and their meanings are shown in
...
performance monitoring interface for tracing activity on target
devices. Trace activity on a target device involves the following
steps:

\begin{itemize}
\item When an OpenMP implementation initializes itself, it will invoke
  a tool initializer.
\item The tool initializer should register a device initialization
  callback for the \code{ompt\_event\_device\_initialize} event.
\item When an OpenMP implementation initializes a target device, the
  ...
  callback.
\item when the target device needs a trace buffer, it invokes a
  tool-supplied callback function to request a new buffer,
\item a target device records events that occur during its execution
  in a trace buffer,
\item when a trace buffer fills on a device (or it is otherwise useful
  to flush the buffer), the device provides it to a tool on the host,
  by invoking a tool-supplied callback function to process and empty
  the buffer.
\end{itemize}


When a tool receives this callback for a device, it will have an
opportunity to look up entry points for the tracing interface for the
by invoking the \code{lookup} function provided as a parameter to the
