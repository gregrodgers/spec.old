...
``\code{ompt\_get\_thread\_place}'' & \code{ompt\_get\_thread\_place\_t}\\
...
\item \code{ompt\_get\_thread\_place\_t}, see \specref{sec:ompt_get_thread_place_t}.
...
The \callbackarg{} \plc{type} that indicates the kind of the task: initial, explicit or target.
...
The \callbackarg{} \plc{deps} that lists all dependences of a new task and
a \callbackarg{} \plc{ndeps} that specifies the length of the list.
...
\argdesc
The \callbackarg{} \plc{kind} indicates indicates the kind of region causing the wait.  
\epdesc
The binding of \callbackarg{} \plc{parallel\_data} is the current parallel region.
...
The \callbackarg{} \plc{endpoint} indicates whether the callback indicates the
begin or end of the region.
%%add args 
\codeptrdesc
...
\omptruntimeentrypoint{\code{ompt\_get\_thread\_place\_t}}
\label{sec:ompt_get_thread_place_t}
\label{sec:ompt_get_thread_place}

\summary 

An OpenMP implementation's tool entry point known as
\code{ompt\_get\_thread\_place} with type signature
\code{ompt\_get\_thread\_place\_t} returns the current place of the
calling thread.
...
\ccppspecificstart
\begin{boxedcode}
typedef int (*ompt_get_thread_place_t)(void);
\end{boxedcode}
\ccppspecificend
}

\descr
A tool may invoke an OpenMP implementation's entry point known as  
\code{ompt\_get\_thread\_place}
with type signature \code{ompt\_get\_thread\_place\_t} 
to obtain the current place of the calling thread.
