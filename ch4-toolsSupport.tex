...
a registered \code{ompt\_thread\_begin\_callback} in the initial
thread. The callback occurs in the context of the thread.  The
callback has type signature \code{ompt\_thread\_begin\_callback\_t}.
...
``\code{ompt\_device\_initialize\_callback}'' & \code{ompt\_device\_initialize\_callback\_t}\\\hline
\end{tabular}
\vskip 1ex
...
callbacks may fail if the \code{ompt\_set\_callback} entry point is
...
\code{ompt\_t\_callback}.  While this is a compromise, it is better
...
\begin{tabular}{lp{3em}p{3em}p{3em}p{3em}}
                                ...
                                \midrule
\code{ompt\_thread\_begin\_callback}          &   &   & * \\
\code{ompt\_thread\_end\_callback}            &   &   & * \\
\code{ompt\_parallel\_begin\_callback}        &   &   & * \\
\code{ompt\_parallel\_end\_callback}          &   &   & * \\
\code{ompt\_task\_create\_callback}           &   &   & * \\
\code{ompt\_task\_schedule\_callback}         &   &   & * \\
\code{ompt\_implicit\_callback\_task}         &   &   & * \\
\code{ompt\_target\_callback}                 &   &   & * \\
\code{ompt\_target\_data\_callback}           &   &   & * \\
\code{ompt\_target\_submit\_callback}         &   &   & * \\
\code{ompt\_tcontrol\_callback}               &   &   & * \\
\code{ompt\_runtime\_shutdown\_callback}      &   &   & * \\
\code{ompt\_idle\_callback}                   & * & * & * \\
\code{ompt\_sync\_region\_wait\_callback}     & * & * & * \\
\code{ompt\_mutex\_release\_callback}         & * & * & * \\
\code{ompt\_task\_dependences\_callback}      & * & * & * \\
\code{ompt\_task\_dependence\_pair\_callback} & * & * & * \\
\code{ompt\_work\_callback}                   & * & * & * \\
\code{ompt\_master\_callback}                 & * & * & * \\
\code{ompt\_target\_data\_map\_callback}      & * & * & * \\
\code{ompt\_sync\_region\_callback}           & * & * & * \\
\code{ompt\_init\_lock\_callback}             & * & * & * \\
\code{ompt\_destroy\_lock\_callback}          & * & * & * \\
\code{ompt\_mutex\_acquire\_callback}         & * & * & * \\
\code{ompt\_mutex\_acquired\_callback}        & * & * & * \\
\code{ompt\_nested\_lock\_callback}           & * & * & * \\
\code{ompt\_flush\_callback}                  & * & * & * \\
\code{ompt\_device\_initialize\_callback}     & * & * & * \\
\code{ompt\_cancel\_callback}                 & * & * & * \\
\bottomrule
\end{tabular}
\vskip 1ex
\caption{Valid return codes of \code{ompt\_set\_callback} for each callback.}
...
\code{ompt\_event\_sync\_region\_wait} event is used for multiple
...
\code{ompt\_event\_sync\_region\_wait} represents
...
\specref{sec:ompt_t_callback}.
\item \code{ompt\_set\_callback\_t} type signature, see \specref{sec:ompt_set_callback_t}
\item \code{ompt\_t\_callback} type signature, see \specref{sec:ompt_t_callback}.

\item \code{ompt\_thread\_begin\_callback\_t}, see \specref{sec:ompt_thread_begin_callback}. 
\item \code{ompt\_event\_thread\_end}, see \specref{sec:ompt_thread_end_callback}. 
\item {\em incomplete cross references stubbed out} 
\begin{comment}
\item \code{ompt\_event\_parallel\_begin}, see \specref
\item \code{ompt\_event\_parallel\_end}, see \specref
\item \code{ompt\_event\_task\_create}, see \specref
\item \code{ompt\_event\_task\_schedule}, see \specref
\item \code{ompt\_event\_implicit\_task}, see \specref
\item \code{ompt\_event\_target}, see \specref
\item \code{ompt\_event\_target\_data}, see \specref
\item \code{ompt\_event\_target\_submit}, see \specref
\item \code{ompt\_event\_tcontrol}, see \specref
\end{comment}
\item \code{ompt\_event\_runtime\_shutdown}, see \specref{sec:ompt_runtime_shutdown_callback}.
\item \code{ompt\_event\_idle}, see \specref{sec:ompt_idle_callback}. 
\item \code{ompt\_event\_sync\_region\_wait}, see \specref{sec:ompt_runtime_shutdown_callback}.
\item {\em incomplete cross references stubbed out} 
\begin{comment}
\item \code{ompt\_event\_mutex\_release}, see \specref
\item \code{ompt\_event\_task\_dependences}, see \specref
\item \code{ompt\_event\_task\_dependence\_pair}, see \specref
\item \code{ompt\_event\_worksharing}, see \specref
...
\end{comment}
\item \code{ompt\_event\_flush}, see \specref{subsec:flush Construct}.
\item \code{ompt\_event\_device\_initialize}, see \specref{sec:Device Constructs}.
\item \code{ompt\_event\_cancel}, see \specref{sec:Cancellation Constructs}.
\end{itemize}
...
\item To prepare to trace activity on a target device, when a tool
  ...
  \code{ompt\_event\_device\_initialize} event.
...
\item The target device will monitor execution on the target device as
  ...
  callback for the \code{ompt\_event\_target\_submit} event, which
...
\ccppspecificstart
\begin{boxedcode}
typedef enum ompt_t_callback \{
  ompt_thread_begin_callback             = 1,
  ompt_thread_end_callback               = 2,
  ompt_parallel_begin_callback           = 3,
  ompt_parallel_end_callback             = 4,
  ompt_task_create_callback              = 5,
  ompt_task_schedule_callback            = 6,
  ompt_implicit_callback_task            = 7,
  ompt_target_callback                   = 8,
  ompt_target_data_callback              = 9,
  ompt_target_submit_callback            = 10,
  ompt_tcontrol_callback                 = 11,
  ompt_runtime_shutdown_callback         = 12,
  ompt_idle_callback                     = 13,
  ompt_sync_region_wait_callback         = 14,
  ompt_mutex_release_callback            = 15,
  ompt_task_dependences_callback         = 16,
  ompt_task_dependence_pair_callback     = 17,
  ompt_work_callback                     = 18,
  ompt_master_callback                   = 19,
  ompt_target_data_map_callback          = 20,
  ompt_sync_region_callback              = 21,
  ompt_init_lock_callback                = 22,
  ompt_destroy_lock_callback             = 23,
  ompt_mutex_acquire_callback            = 24,
  ompt_mutex_acquired_callback           = 25,
  ompt_nested_lock_callback              = 26,
  ompt_flush_callback                    = 27,
  ompt_device_initialize_callback        = 28
  ompt_cancel_callback                   = 29
\} ompt_t_callback;
\end{boxedcode}
\ccppspecificend
}
...
\ompteventswithoutdirectives{\code{ompt\_thread\_begin\_callback}}
\label{sec:ompt_thread_begin_callback}
...
\code{ompt\_thread\_begin\_callback\_t}.  The callback argument
...
\label{sec:ompt_thread_end_callback}
...
callback has type signature \code{ompt\_idle\_callback\_t}.

\ompteventswithoutdirectives{\code{ompt\_event\_sync\_region\_wait}}
\label{sec:ompt_sync_region_wait_callback}
...
\code{ompt\_sync\_region\_callback\_t}.  One region may generate
...
\label{sec:ompt_runtime_shutdown_callback}
...
callback has type signature \code{ompt\_t\_callback}.
...
typedef struct ompt_record_ompt_s \{
  ...
  union \{
    ...
    ompt_target_data_record_t \plc{data};
    ompt_target_data_map_record_t \plc{data_map};
    ompt_target_submit_record_t \plc{target_submit};
...
\ompttype{\code{ompt\_t\_callback}}
\label{sec:ompt_t_callback}
...
a dummy type \code{ompt\_t\_callback}.

\vbox{
\ccppspecificstart
\begin{boxedcode}
typedef void (*ompt_t_callback)(void);
\end{boxedcode}
\ccppspecificend
}
...
\item \code{ompt\_thread\_begin\_callback\_t} callback type, see \specref{sec:ompt_thread_begin_callback_t}.
\item \code{ompt\_parallel\_end\_callback\_t} callback type, see
...
\item \code{ompt\_task\_schedule\_callback\_t} callback type, see
  \specref{sec:ompt_task_schedule_callback_t}.
\item \code{ompt\_implicit\_task\_callback\_t} callback type, see
  \specref{sec:ompt_task_implicit_task_callback_t}.
\item \code{ompt\_task\_callback\_sync\_region} callback type, see
  \specref{sec:ompt_task_sync_region_callback_t}.
\item \code{ompt\_task\_master\_callback\_callback\_t} callback type,
  see \specref{sec:ompt_task_master_d_callback}.
\item \code{ompt\_task\_work\_callback\_t} callback type, see \specref{sec:ompt_task_work_callback_t}.
\item \code{ompt\_task\_target\_callback\_t} callback type, see \specref{sec:ompt_task_target_callback_t}.\item \code{ompt\_flush\_callback\_t} callback type, see \specref{sec:ompt_flush_callback_t}.
\end{itemize}
\end{comment}
...
\begin{itemize}
\item \code{ompt\_get\_task\_info} inquiry function, see \specref{sec:ompt_get_task_info}.
\item \code{ompt\_parallel\_begin\_callback\_t} callback type, see
...
\item \code{ompt\_work\_callback\_t} callback type, see \specref{sec:ompt_work_callback_t}.
...
\crossreferences
\begin{itemize}
\item \code{ompt\_callback_parallel\_begin\_t}, see \specref{sec:ompt_parallel_begin__callback_t}.
\item \code{ompt\_record\_parallel\_begin\_t}, see \specref{sec:ompt_record_parallel_begin_t}.
\item \code{ompt\_parallel\_end\_callback\_t}, see \specref{sec:ompt_parallel_end_callback_t}.
...
\index{ompt\_thread\_end\_callback\_callback\_t@{\code{ompt\_thread\_end\_callback\_t}}}
\label{sec:ompt_thread_end_callback_t}
\format
\vbox{
\ccppspecificstart
\begin{boxedcode}
typedef void (*ompt_thread_end_s_callback) (
  ompt_data_t *\plc{thread_data}
);
\end{boxedcode}
\ccppspecificend
...
\ccppspecificstart
\begin{boxedcode}
typedef void (*ompt_parallel_end_callback_callback_t) (
               ompt_data_t *\plc{parallel_data},
...
\omptcallbacksignature{\code{ompt\_implicit\_task\_callback\_t}}
\index{ompt\_implicit\_callback\_t@{\code{ompt\_implicit\_task\_callback\_t}}}
\label{sec:ompt_implicit_task_callback_t}
\format
\vbox{
\ccppspecificstart
\begin{boxedcode}
typedef void (*ompt_implicit_task_callback_t) (
  ompt_scope_endpoint_t \plc{endpoint},
  ompt_data_t *\plc{parallel_data},
  ompt_data_t *\plc{task_data},
...
\descr
The callback with type signature \code{ompt\_implicit\_task\_callback\_t},
...
\omptcallbacksignature{\code{ompt\_work\_callback\_t}}
\index{ompt\_work\_callback\_t@{\code{ompt\_work\_callback\_t}}}
\label{sec:ompt_work_callback_t}
\format
\vbox{
\ccppspecificstart
\begin{boxedcode}
typedef void (*ompt_work_callback_t) (
...
\descr
The callbacks with type signature \code{ompt\_work\_callback\_t},
include a parameter \plc{requested\_team\_size}
that indicates the number of threads requested by the user, and a parameter
\plc{actual\_team\_size} that indicates the number of threads in the team.
The \plc{invoker} argument explains whether the execution of the parallel
...
\ccppspecificstart
\begin{boxedcode}
typedef void ompt_target_data_map_record_s \{
  ompt_id_t \plc{target_id};
  unsigned int \plc{nitems};
  void **\plc{host_addr};
  void **\plc{device_addr};
...
construct associated with this operation.  The callback argument
\plc{host\_op\_id} provides a unique host-side identifier that
represents the activity on the device.  The callback arguments
\plc{requested\_num\_teams} and \plc{granted\_num\_teams} indicate,
respectively, the number of teams requested by the user and granted by
the runtime.
...
\ccppspecificstart
\begin{boxedcode}
typedef struct ompt_target_submit_record_s \{
  ompt_id_t \plc{target_id};
  ompt_id_t \plc{host_op_id};
  unsigned int \plc{requested_num_teams};
  unsigned int \plc{granted_num_teams};
\} ompt_target_submit_record_t;
\end{boxedcode}
\ccppspecificend
}
\vbox{
...
\ccppspecificstart
\begin{boxedcode}
typedef int (*ompt_tcontrol_callback_t) (
...
\omptcallbacksignature{\code{ompt\_cancel\_callback\_t}}
...
typedef int (*ompt_set_callback_t)(
  ompt_t_callback \plc{event},
  ompt_t_callback \plc{callback}
);
\end{boxedcode}
\ccppspecificend
}
...
\crossreferences
\begin{itemize}
\item \code{ompt\_event\_t} enumeration type, see \specref{sec:ompt_t_callback}.
\item \code{ompt\_t\_callback} type, see \specref{sec:ompt_t_callback}.
...
see \specref{sec:ompt_t_callback}.
\end{itemize}
