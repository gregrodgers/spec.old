...
The binding of \callbackarg{} \plc{parallel\_data} is the enclosing
parallel region.
...
value. Task types that a tool may observe on a thread's stack include
initial, implicit, explicit, target, and degenerate tasks.  In this
...
pointer return value.  If the task at the specified ancestor level is
degenerate, the pointer returned in *\plc{task\_data} will be NULL. A
degenerate task will be associated with the enclosing parallel region.

The \callbackarg{} \plc{task\_frame} is a pointer to a task frame
pointer return value. The value returned *\plc{task\_frame} represents an
...
create a new task. At ancestor level 0, the \plc{exit\_frame} may or
...
the pointer returned in *\plc{parallel\_data} will be \code{NULL}.  

The \callbackarg{} \plc{thread\_num} is a pointer to a return value
for a thread number. The value returned in *\plc{thread\_num}
...
A positive value for \callbackarg{} \code{enable} indicates that
recording of one or more events specified by \callbackarg{}
\code{etype} should be enabled; a value of 0 for \code{enable}
indicates that recording of events should be disabled by this
invocation.

An \callbackarg{} \code{etype} value 0 indicates that traces for all
event types will be enabled or disabled.  Passing a positive value for
\code{etype} inidicates that recording should be enabled or disabled
for the event in \code{ompt\_callbacks\_t} that matches \code{etype}.
...
The \callbackarg{} \plc{device} is a handle that identifies a target
device. 
