...
\summary
A tool implements \code{ompt\_tool} to announce its interest in using 
...
an OpenMP implementation's OMPT interface, \code{ompt\_tool} 
...
The runtime will call the tool initializer
after the runtime fully initializes itself and before 
beginning execution of any OpenMP construct
or completing any execution environment routine invocation.  

\argdesc
The first argument to a tool initializer is \plc{lookup}---a callback 
to an OpenMP runtime that a tool must use to 
