...
\code{ompt\_fns\_t} structure must be non-\code{NULL}.
...
typedef struct ompt_record_ompt_s \{
  ...
  union \{
    ...
    ompt_target_data_op_t \plc{target_data_op};
    ...
    ompt_nest_lock_record_t \plc{nest_lock};
...
\argdesc
The binding for \plc{thread\_data} is the thread that is terminating.
...
\argdesc
The binding of \plc{parent\_task\_data} is the enclosing task. 
The \plc{parent\_frame} points to the frame object associated with the
parent task.
The binding of \plc{parallel\_data} is the parallel region's team.
...
The last argument, \plc{codeptr\_ra}, is the return address following the
task's call to a routine to implement the waiting or invoke this
callback; \plc{codeptr\_ra} may be NULL.  
...
\argdesc
The binding of \plc{parallel\_data} is the parallel region's team.
The binding of \plc{task\_data} is the task encountering the parallel
region. 
...
\argdesc
The parameter \plc{codeptr\_ra} which contains the return address of the
...
that indicates the frame information of the encountering task. The parameter
...
\argdesc
The parameter \plc{prior\_task\_status}
indicates the current status of the task that reached the task
scheduling point. 
...
\argdesc
The parameter \plc{requested\_team\_size}
indicates the number of threads requested by the user, and a parameter
\plc{actual\_team\_size} that indicates the number of threads in the team.
%%fixme johnmc
The last argument, \plc{codeptr\_ra}, is the return address following the
task's call to a routine to implement the waiting or invoke this
...
\ccppspecificstart
\begin{boxedcode}
typedef struct ompt_nest_lock_record_s \{
  ompt_scope_endpoint_t \plc{endpoint};
  ompt_wait_id_t \plc{wait_id};
  const void *\plc{codeptr_ra};
\} ompt_nest_lock_record_t;
\end{boxedcode}
\ccppspecificend
}
\end{targetRecord}
...
\argdesc
The argument, \plc{codeptr\_ra}, is the return address following the
invocation of the runtime library to implement the flush;
\plc{codeptr\_ra} may be NULL.  
...
%%add args 
The last argument, \plc{codeptr\_ra}, is the return address following the
call to the runtime library; \plc{codeptr\_ra} may be NULL.  
...
\ccppspecificstart
\begin{boxedcode}
typedef struct ompt_target_data_op_record_s \{
  ompt_id_t \plc{host_op_id};
  ompt_target_data_op_t \plc{optype};
  void *\plc{host_addr};
  void *\plc{device_addr};
...
Appendix~\ref{app:frames} discusses an example that
