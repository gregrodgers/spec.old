...
\end{tabular}
\vskip 1ex
\caption{Host OMPT interface function names and their type signatures.}
...
\label{sec:ompt_worksharing\_type_t} 
...
\subsection{General Callback Signatures}
...
\summary
The runtime callback with type signature \code{ompt\_function\_lookup\_t}
is used by a tool to obtain pointers to runtime callback routines that are
...
implementation as an argument to a tool initializer. 
This lookup function enables a tool to obtain pointers to 
OMPT interface functions supported by the OpenMP implementation on the host. 
A separate target device lookup callback can be obtained from each target device to 
...
\subsection{Host Callback Signatures}
...
ways.  This callback enables a tool to enumerate all of the different
kinds mutual exclusion implementations that an OpenMP implementation
employs.
...
\summary
The host runtime callback with type signature \code{ompt\_set\_callback\_t} is
used by a tool to register a tool callback to invoke when a host OpenMP
event occurs.
