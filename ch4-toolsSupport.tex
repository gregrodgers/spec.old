...
\section{Tool Interface Routines}
...
\subsection{Initializer Callback Signatures}
...
\subsubsection{\code{ompt\_get\_target\_info\_inquiry\_t}}
...
\section{Runtime Callback Signatures}

\subsection{Runtime Interface Callback Routines}
...
a call to \code{ompt\_enumerate\_states\_t}, the call will 
...
The third argument of a call to \code{ompt\_enumerate\_states\_t}
...
\code{ompt\_enumerate\_states\_t} will return $1$.
When the last state in the enumeration is passed 
to \code{ompt\_enumerate\_states\_t} the first argument, 
...
The first argument passed to a call to \code{ompt\_enumerate\_states\_t} must
...
first argument of a call to \code{ompt\_enumerate\_states\_t}.
Subsequent invocations of \code{ompt\_enumerate\_states\_t} by a tool
...
\label{ompt_enumerate_mutex_kinds_t}
...
\end{comment}
The \code{ompt\_enumerate\_mutex\_kinds\_t} routine enables a tool to enumerate
...
as the first argument of
a call to \code{ompt\_enumerate\_mutex\_kinds\_t}, the call will
assign the next mutex kind in an enumeration of all mutex kinds
supported by the implementation to the pointer passed as the second
argument of the call.
The third argument of a call to \code{ompt\_enumerate\_mutex\_kinds\_t} 
is the address of a character pointer
that will be assigned a pointer to the name associated with
...
\code{ompt\_enumerate\_mutex\_kinds\_t} will return $1$.
When the last mutex kind in the enumeration is passed 
to 
\code{ompt\_enumerate\_mutex\_kinds\_t} the first argument, 
the function will return $0$ 
indicating that the enumeration is complete.

\constraints

The first argument passed to a call to \code{ompt\_enumerate\_mutex\_kinds\_t} must
...
a tool should pass the value \code{ompt\_mutex\_kind\_unknown} as the
first argument of a call to \code{ompt\_enumerate\_mutex\_kinds\_t}. 
Subsequent invocations of \code{ompt\_enumerate\_mutex\_kinds\_t} by a tool
should pass the value
assigned to the pointer passed as the second argument to the prior call.
...
while (ompt_enumerate_mutex_kinds_fn(kind, &kind, &kind_name)) \{
...
\summary
The callback with type signature \code{ompt\_set\_callback\_t} is
used by a tool to register a pointer to a tool routine that will
be invoked when a particular type of OpenMP event occurs during execution
...
event. For an event with enumeration value name \code{e}, 
the type signature for the event is \code{e\_t}. 
...
\summary
The callback with type signature \code{ompt\_get\_callback\_t} is
used by a tool to retrieve a pointer to the tool routine that will
be invoked when a particular type of OpenMP event occurs during execution
of an OpenMP program. 
...
\descr
The callback with type signature \code{ompt\_get\_callback\_t} 
may be invoked at any time to determine whether or not 
a tool has registered a callback for an OpenMP event.

\subsubsection{\code{ompt\_get\_thread\_data\_t}}
\label{sec:thread-inquiry}

\summary
The runtime callback with type signature \code{ompt\_get\_thread\_data\_t} 
...
\subsubsection{\code{ompt\_target\_get\_device\_info\_t}}
\label{sec:ompt_target_get_device_info_t}
...
\vbox{\ccppspecificstart
\begin{boxedcode}
typedef int (*ompt_target_get_device_info_t)(
  int32_t device_id, 
  const char **type, 
...
\label{sec:ompt_target_get_device_id_t}
...
\vbox{\ccppspecificstart
\begin{boxedcode}
typedef int (*ompt_target_get_device_id_t)(void);
\end{boxedcode}
\ccppspecificend}
...
\subsection{Target-specific Callback Routines}
