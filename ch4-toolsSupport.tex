...
events and then to inspect the state of an OpenMP implementation while
executing any tool callback. The second set of entry points enables a
tool to arrange to trace activities on a device. The OpenMP
implementation will then trace activities on the device, collect
buffers full of trace records, and invoke a callback on the host to
process these records. A third 

to arrange for the
host to receive notification whenever a buffer filled with activity
records is availabl3 to process tracing activities tracing
the context of 
first set of 
As an OpenMP implementation is initialized for a device, it will
invoke a callback on the host that enables a tool to set up tracing
for that device. This initialization callback  
 
tracing the host device to
initialize tracing   
The first set of entry points enables a tool to register callbacks
that will be invoked 
capabilities for tools. A tool can
register to trace activities on a device or a tool can register to
receive callbacks
...
callbacks for OpenMP events and to retrieve data from the 
...
a tool should obtain pointers to these entry points functions.
...
\subsection{Entry Points in the OMPT Trace Interface}
