...
\item \code{OMP\_TOOL} sets the \plc{ompt-tool-var} ICV that controls whether or not an OpenMP will try to register a performance tool.
...
The \code{OMP\_TOOL} environment variable sets the \plc{omp-tool-var} variable which controls whether an OpenMP runtime will try to register a performance tool.
...
\crossreferences
\begin{itemize}
\item \plc{omp-tool-var} ICV, see \specref{sec:Internal Control Variables}.
\item Tools Support, see \specref{chap:ToolsSupport}.
\item \code{omp\_tool\_init} routine, see \specref{sec:ToolsSupport_initialize}.
\end{itemize}
