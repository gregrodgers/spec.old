...
\item \code{OMP\_TOOL\_LIBRARIES} sets the \plc{tool-libraries-var} ICV that contains a list of tool libraries that the runtime searches to find a valid tool to be included in the execution
...
The \code{OMP\_TOOL\_LIBRARIES} environment variable sets the \plc{tool-libraries-var} ICV that contains a list of tool libraries that the runtime searches to find 
a tool library appropriate for an OpenMP implementation being initialized.
The value of this environment variable must be a comma separated list of dynamically-linked libraries, each specified by an absolute path.
