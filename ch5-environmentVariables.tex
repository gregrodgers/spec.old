...
\item \code{OMPT\_TOOL\_LIBRARIES\_LIST} sets the \plc{tool-libraries-list} ICV that contains a list of tool libraries that the runtime searches to find a valid tool to be included in the execution
...
\item \code{ompt\_tool} routine, see \specref{sec:ToolsSupport_Registration}.
\end{itemize}




section{\code{OMPT\_TOOL\_LIBRARIES\_LIST}}
\index{OMPT\_TOOL\_LIBRARIES\_LIST@{\code{OMPT\_TOOL\_LIBRARIES\_LIST}}}
\index{environment variables!OMPT\_TOOL\_LIBRARIES\_LIST@{\code{OMPT\_TOOL\_LIBRARIES\_LIST}}}
\label{sec:OMP_TOOL}

The \code{OMPT\_TOOL\_LIBRARIES\_LIST} environment variable sets the \plc{tool-libraries-list} ICV that contains a list of tool libraries that the runtime searches to find a valid tool to be included in the execution.
The value of this environment variable must be a comma separated list of dynamically linked libraries specified by their full absolute path.

\crossreferences
\begin{itemize}
\item \plc{tool-libraries-list} ICV, see \specref{sec:Internal Control Variables}.
\item Tool Interface, see \specref{chap:ToolsSupport}.
\end{itemize}
