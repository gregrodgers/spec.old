...
continues execution at the end of the current \code{task} region. If the 
...
\item If the thread is at a \code{cancel} or \code{cancellation point} region
      ...
      current \code{task} region if cancellation has been activated for any of 
...
tasks that have been created by a \code{task} construct and their descendent 
...
or \code{lastprivate} clause, the final value of the \plc{list-items} that 
...
\code{ompt_cancel_activated} as its \plc{flags} argument for each occurrence 
of a \plc{cancel} event in that thread. The callback occurs in the context of 
the encountering task and has type signature \code{ompt_callback_cancel_t}.
...
context of the encountering task and has type signature \code{ompt_callback_cancel_t}.
...
\item A \code{cancellation}~\code{point} construct for which 
      ...
      inside a \code{task} construct, and the \code{cancellation}~\code{point} region 
      must be closely nested inside a \code{taskgroup} region. A 
      \code{cancellation}~\code{point} construct for which \plc{construct-type-clause}
      is \code{sections} must be closely nested inside a \code{sections} or 
      \code{section} construct. Otherwise, a \code{cancellation}~\code{point} 
      construct must be closely nested inside an OpenMP construct that matches 
      the type specified in \plc{construct-type-clause}.
\end{itemize}
