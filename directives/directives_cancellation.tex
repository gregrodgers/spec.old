...
\code{cancel} construct and before activating a cancellation, 
when the cancel construct activates a cancellation.
...
The callback occurs in the context of the task that is dropping a discarded task. % task can be discarded lazyly, or not at all.
The callback has type signature \code{ompt\_callback\_cancel\_t}. 
The callback receives the \plc{ompt\_data\_t} associated with the discarded task as its \plc{task\_data} argument.
The callback receives \code{ompt\_cancel\_discarded\_task} as its \plc{flags} argument. 
