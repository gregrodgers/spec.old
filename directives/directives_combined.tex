...
\index{master taskloop construct}
\index{constructs!master taskloop construct}
\index{combined constructs!master taskloop construct}

\summary
The \code{master}~\code{taskloop} construct is a shortcut for specifying a \code{master}
construct containing a \code{taskloop} construct and no other statements.
...
\descr
The semantics are identical to explicitly specifying a \code{master} directive immediately
followed by a \code{taskloop} directive. 
...
\index{parallel master construct}
\index{constructs!parallel master construct}
\index{combined constructs!parallel master construct}
\summary
The \code{parallel}~\code{master} construct is a shortcut for specifying a 
\code{parallel} construct containing one \code{master} construct and no other statements.
...
\index{combined constructs!parallel master taskloop construct}
\label{subsec:parallel master taskloop Construct}

\summary
The \code{parallel}~\code{master}~\code{taskloop} construct is a shortcut for specifying a 
\code{parallel}~\code{master} construct containing one \code{taskloop} construct and no other statements.

\syntax
\begin{ccppspecific}
The syntax of the \code{parallel}~\code{master}~\code{taskloop} construct is as follows:
...
where \plc{clause} can be any of the clauses accepted by the \code{parallel}~\code{master} 
or \code{taskloop} directives, with identical meanings and restrictions.
\end{ccppspecific}
...
where \plc{clause} can be any of the clauses accepted by the \code{parallel}~\code{master} or
\code{taskloop} directives, with identical meanings and restrictions.
\end{fortranspecific}

\description
The semantics are identical to explicitly specifying a \code{parallel}~\code{master} directive
immediately followed by a \code{taskloop} directive.

\restrictions
\item The restrictions for the \code{parallel}~\code{master} construct and the
\code{taskloop} construct apply.

\crossreferences
\item \code{parallel master} construct, see
\specref{subsec:parallel master Construct}.
\item \code{taskloop} construct, see
\specref{subsec:taskloop Construct}.
\item Data attribute clauses, see
\specref{subsec:Data-Sharing Attribute Clauses}.
...
\index{master taskloop simd construct}
\index{constructs!master taskloop simd construct}
\index{combined constructs!master taskloop simd construct}

\summary
The \code{master}~\code{taskloop}~\code{simd} construct is a shortcut for specifying a \code{master}
construct containing a \code{taskloop}~\code{simd} construct and no other statements.
...
If an \code{end}~\code{master}~\code{taskloop}~\code{simd} directive is not specified, an
\code{end}~\code{master}~\code{taskloop}~\code{simd} directive is assumed at the end of
the \plc{do-loops}.
\end{fortranspecific}

\descr
The semantics are identical to explicitly specifying a \code{master} directive immediately
followed by a \code{taskloop}~\code{simd} directive. 

\restrictions
The restrictions for the \code{master} and \code{taskloop}~\code{simd} constructs apply.
...
\index{combined constructs!parallel master taskloop simd construct}
\label{subsec:parallel master taskloop simd Construct}

\summary
The \code{parallel}~\code{master}~\code{taskloop}~\code{simd} construct is a shortcut for specifying a 
\code{parallel}~\code{master} construct containing one \code{taskloop}~\code{simd} construct and no other statements.

\syntax
\begin{ccppspecific}
The syntax of the \code{parallel}~\code{master}~\code{taskloop}~\code{simd} construct is as follows:
...
where \plc{clause} can be any of the clauses accepted by the \code{parallel}~\code{master} 
or \code{taskloop}~\code{simd} directives, except the \code{in_reduction} clause, with 
identical meanings and restrictions.
\end{ccppspecific}
...
where \plc{clause} can be any of the clauses accepted by the \code{parallel}~\code{master} or
\code{taskloop}~\code{simd} directives, , except the \code{in_reduction} clause, with 
identical meanings and restrictions.
\end{fortranspecific}
...
\restrictions
\item The restrictions for the \code{parallel}~\code{master} construct and the
\code{taskloop}~\code{simd} construct apply.

\crossreferences
\item \code{parallel}~\code{master} construct, see
\specref{subsec:parallel master Construct}.
\item \code{taskloop}~\code{simd} construct, see
\specref{subsec:taskloop simd Construct}.
\item Data attribute clauses, see
...
\item To the \code{parallel} construct if it is among the constituent
      constructs and the worksharing-loop or \code{taskloop} constructs are not;
...
\item The \code{parallel} construct, when combined with the
      worksharing-loop, \code{taskloop}, \code{loop}, or \code{sections} construct; and
