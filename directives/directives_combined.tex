...
\specref{subsec:Worksharing-Loop SIMD Construct}.
...
If an \code{order(concurrent)} clause is present the effect will be as if it is
applied to both the worksharing-loop and \code{simd} constructs.
...
If an \code{order(concurrent)} clause is present the effect will be as if it is
applied to both the \code{distribute} and simd constructs.
...
If an \code{order(concurrent)} clause is present the effect will be as if it is
applied to both the \code{distribute} and \code{simd} constructs.
...
If an \code{order(concurrent)} clause is present the effect will be as if it is
applied to both the \code{distribute} and worksharing-loop constructs.
...
If an \code{order(concurrent)} clause is present the effect will be as if it is
applied to both the \code{distribute}, worksharing-loop, and \code{simd} constructs.
...
If an \code{order(concurrent)} clause is present the effect will be as if it is
applied to both the \code{distribute}, worksharing-loop, and simd constructs.
...
\item to the loop construct if it is among the constituent constructs,

\item to the \code{parallel} construct if it is among the constituent constructs and the loop construct is not,
...
loop or \code{distribute} construct.
\end{itemize}
...
\item If a \code{reduction} clause with the \code{task} modifier appears on a
combined construct, at least one of the constructs that constitute the combined
construct must allow the modifier to be specified on the clause.  The effect
is as if the \code{task} modifier only modifies the behavior of the
\code{reduction} clause for the innermost construct that constitutes the
combined construct and accepts the \code{task} modifier on the
\code{reduction} clause. For the other construct that constitutes the combined
construct, the behavior is as if the \code{task} modifier were not specified.
\end{itemize}
