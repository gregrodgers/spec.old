...
\item to the \code{target} construct if it is among the constituent
      constructs and the same list item does not appear in \code{lastprivate} 
      or \code{map} clause.
\end{itemize}
...
If the \code{parallel} construct is among the constituent constructs and the
list item is not also mentioned in the \code{firstprivate} clause, then the 
...
If the \code{target} construct is among the constituent constructs and the 
list item doesn't appear in a \code{map} clause the effect is as if the same 
...
clauses the effect is as if it is applied to all the constituent constructs 
that permit those clauses.
