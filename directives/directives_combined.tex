...
if it is combined with the worksharing-loop, \code{sections}, or \code{loop} construct;
instead, the list items are treated as if they appeared in a \code{shared}
clause for the \code{parallel} construct.  If \plc{reduction-modifier} is
specified, the effect is as if it only modifies the
behavior of the \code{reduction} clause for the innermost construct that
