...
\begin{itemize}
\item the innermost constituent construct that permits it,

\item to the loop construct if it is among the constituent constructs and is
not the innermost constituent construct that permits it,

\item to the \code{distribute} construct if it is among the constituent
constructs and is not the innermost constituent construct that permits it.
\end{itemize}
...
clauses on the outer constituent constructs per the rules specified above.  If the list item is the
iteration variable of the \code{simd} or loop SIMD construct and it is not declared in the construct,
the effect is as if the list item is present in the \code{lastprivate} clauses on the outer
constituent constructs per the rules specified above.
