...
\code{parallel} construct containing one \code{master} construct and no other 
statements.
...
specifying a \code{parallel} construct containing one \code{master}~\code{taskloop}
construct and no other statements.
...
shortcut for specifying a \code{parallel} construct containing one 
...
\begin{itemize}
\item The \code{parallel} construct, when combined with the worksharing-loop, 
      \code{taskloop}, \code{loop}, or \code{sections} construct; and
