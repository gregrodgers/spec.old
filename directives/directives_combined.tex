...
\item For the \code{reduction} clause the effect is as if it is applied to all the constructs that permit
the clause, except for the following constructs where instead the behavior is as if the
list items appeared in a \code{shared} clause for the construct:
\begin{itemize}
\item the \code{parallel} construct, when combined with the
        worksharing-loop or \code{sections} construct;
\item the \code{teams} or \code{parallel} construct, when combined with
        the \code{loop} construct.
\end{itemize}
