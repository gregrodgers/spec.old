...
\subsection{\hcode{teams}~\hcode{parallel} Construct}
\index{teams parallel@{\code{teams}~\code{parallel}}}
\index{constructs!teams parallel@{\code{teams}~\code{parallel}}}
\index{combined constructs!teams parallel@{\code{teams}~\code{parallel}}}
\label{subsec:teams parallel Construct}
\summary
The \code{teams}~\code{parallel} construct is a shortcut for specifying a \code{teams} construct containing a \code{parallel} construct and no other statements.

\syntax
\begin{ccppspecific}
The syntax of the \code{teams}~\code{parallel} construct is as follows:

\begin{ompcPragma}
#pragma omp teams parallel \plc{[clause[ [},\plc{] clause] ... ] new-line}
    \plc{structured-block}
\end{ompcPragma}

where \plc{clause} can be any of the clauses accepted by the \code{teams} or \code{parallel}
directives with identical meanings and restrictions.
\end{ccppspecific}
...
\begin{ompfPragma}
!$omp teams parallel \plc{[clause[ [},\plc{] clause] ... ]}
    \plc{structured-block}
\plc{[}!$omp end teams parallel\plc{]}
\end{ompfPragma}
...
followed by a \code{parallel} directive.


\restrictions
The restrictions for the \code{teams} and \code{parallel} constructs apply.
...
\subsection{Teams Parallel Worksharing-Loop Construct}
\index{teams parallel worksharing-loop construct}
\index{constructs!teams parallel worksharing-loop construct}
\index{constructs!teams parallel for@{\code{teams}~\code{parallel}~\code{for} \emph{C/C++}}}
\index{constructs!teams parallel do@{\code{teams}~\code{parallel}~\code{do} \emph{Fortran}}}
\index{combined constructs!teams parallel worksharing-loop construct}
\label{subsec:Teams Parallel Worksharing-Loop Construct}
\summary
The teams parallel worksharing-loop construct is a shortcut for specifying a \code{teams} construct containing a parallel worksharing-loop construct and no other statements.

\syntax
\begin{ccppspecific}
The syntax of the teams parallel worksharing-loop construct is as follows:

\begin{ompcPragma}
#pragma omp teams parallel for \plc{[clause[ [},\plc{] clause] ... ] new-line}
    \plc{for-loops}
\end{ompcPragma}

where \plc{clause} can be any of the clauses accepted by the \code{teams} or \code{parallel}~\code{for}
directives with identical meanings and restrictions.
\end{ccppspecific}
...
\begin{ompfPragma}
!$omp teams parallel do \plc{[clause[ [},\plc{] clause] ... ]}
    \plc{do-loops}
\plc{[}!$omp end teams parallel do\plc{]}
\end{ompfPragma}

where \plc{clause} can be any of the clauses accepted by the \code{teams} or \code{parallel}~\code{do}
directives with identical meanings and restrictions.

If an \code{end}~\code{teams}~\code{parallel}~\code{do} directive is not specified, an
\code{end}~\code{teams}~\code{parallel}~\code{do} directive is assumed at the end of the
\plc{do-loops}.
\code{nowait} may not be specified on an \code{end}~\code{teams}~\code{parallel}~\code{do} directive.
\end{fortranspecific}

\descr
The semantics are identical to explicitly specifying a \code{teams} directive immediately
followed by a teams parallel worksharing-loop directive.
...
\subsection{Teams Parallel Worksharing-Loop SIMD Construct}
\label{subsec:Teams Parallel Worksharing-Loop SIMD Construct}
\index{teams parallel worksharing-loop SIMD construct}
\index{constructs!teams parallel worksharing-loop SIMD construct}
\index{combined constructs!teams parallel worksharing-loop SIMD construct}
\summary
The teams parallel worksharing-loop SIMD construct is a shortcut for specifying a \code{teams} construct
containing one parallel worksharing-loop SIMD construct and no other statement.

\begin{samepage}
\syntax
\begin{ccppspecific}
The syntax of the teams parallel worksharing-loop SIMD construct is as follows:

\begin{ompcPragma}
#pragma omp teams parallel for simd \plc{[clause[ [},\plc{] clause] ... ] new-line}
    \plc{for-loops}
\end{ompcPragma}

where \plc{clause} can be any of the clauses accepted by the \code{teams}
or \code{parallel}~\code{for}~\code{simd} directives, except the \code{nowait} clause, with
identical meanings and restrictions.
\end{ccppspecific}
\end{samepage}
...
\begin{ompfPragma}
!$omp teams parallel do simd \plc{[clause[ [},\plc{] clause] ... ]}
    \plc{do-loops}
\plc{[}!$omp end teams parallel do simd\plc{]}
\end{ompfPragma}
\end{samepage}

where \plc{clause} can be any of the clauses accepted by the \code{teams}
or \code{parallel}~\code{do}~\code{simd} directives, with identical meanings and restrictions.

\begin{samepage}
If an \code{end}~\code{teams}~\code{parallel}~\code{do}~\code{simd} directive is not specified, an
\code{end}~\code{teams}~\code{parallel}~\code{do}~\code{simd} directive is assumed at the end of the
\plc{do-loops}. \code{nowait} may not be specified on
an \code{end}~\code{teams}~\code{parallel}~\code{do}~\code{simd} directive.
\end{samepage}
\end{fortranspecific}

\descr
The semantics of the teams parallel worksharing-loop SIMD construct are identical to explicitly specifying
a \code{teams} directive immediately followed by a parallel worksharing-loop SIMD directive.

\restrictions
The restrictions for the \code{teams} construct and the parallel worksharing-loop SIMD construct apply.
