...
If the \code{task} \plc{reduction-modifier} is specified, the effect is as if it only modifies
...
Section~\ref{subsubsec:reduction clause}). If the \code{inscan}
\plc{reduction-modifier} is specified, the effect is as if it modifies the
behavior of the \code{reduction} clause on all constructs that constitute
