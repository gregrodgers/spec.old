...
\item For the \code{reduction} clause the effect is as if it is applied to all the constructs that permit
the clause, except that it is not applied to the \code{parallel} construct
if it is combined with the worksharing-loop or \code{sections} construct;
instead, the list items are treated as if they appeared in a \code{shared}
clause for the \code{parallel} construct. If the construct is combined with
...
\item If a \code{reduction} clause with the \code{inscan} or \code{task} modifier appears on a
...
is as if the modifier only modifies the behavior of the \code{reduction}
clause for the innermost construct that constitutes the combined construct and
accepts the modifier on the \code{reduction} clause. For the other construct
that constitutes the combined construct, the behavior is as if the
\code{default} modifier was specified.
