...
\index{constructs!target parallel loop construct}
\index{constructs!target parallel loop@{\code{target}~\code{parallel}~\code{loop}}}
\index{combined constructs!target parallel loop construct}
\label{subsec:Target Parallel Loop Construct}
\summary
The target parallel loop construct is a shortcut for specifying a \code{target} construct containing a parallel loop construct and no other statements.
...
where \plc{clause} can be any of the clauses accepted by the \code{target} or \code{parallel}~\code{loop}
directives with identical meanings and restrictions.
\end{ccppspecific}
...
where \plc{clause} can be any of the clauses accepted by the \code{teams} or \code{parallel}~\code{loop}
directives with identical meanings and restrictions.

If an \code{end}~\code{target}~\code{parallel}~\code{loop} directive is not specified, an
\code{end}~\code{target}~\code{parallel}~\code{loop} directive is assumed at the end of the
\plc{do-loops}.
\code{nowait} may not be specified on an \code{end}~\code{target}~\code{parallel}~\code{loop} directive.
\end{fortranspecific}
...
\subsection{Target Teams Loop Construct}
\label{subsec:Target Teams Loop construct}
\index{target teams distribute loop construct}
\index{constructs!target teams loop construct}
\index{combined constructs!target teams loop construct}
\summary
The target teams loop construct is a shortcut for specifying a \code{target}
...
where \plc{clause} can be any of the clauses accepted by the \code{target} or
\code{teams}~\code{loop} directives with identical
...
If an \code{end}~\code{target}~\code{teams}~\code{loop} directive is not specified, an
\code{end}~\code{target}~\code{teams}~\code{loop}
directive is assumed at the end of the \plc{do-loops}.
\end{fortranspecific}
...
\restrictions
The restrictions for the \code{target} and teams loop constructs.
...
base array or base pointer appears in a \code{shared} clause for the construct.
