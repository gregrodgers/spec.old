...
message(const char *\plc{msg-string})
\end{indentedcodelist}
\end{samepage}
\end{ccppspecific}
...
The \code{error} directive to perform an error action. The error action
...
action. When the \code{at} clause is specified as \code{compile}, the
error action is performed during compilation. When the \code{at} clause is 
specified as \code{runtime}, the error action is performed during program
execution. If the \code{at} clause is not specified then the \code{error}
directive behaves as if the \code{at} clause is specified as \code{compile}.

The \code{severity} clause determines the action that the implementation
performs. When the \code{severity} is specified as \code{warning}, the
implementation takes no action besides displying the message. When the 
\code{severity} is specified as \code{fatal} and the \code{at} clause is
specified as \code{compile} then the message is displayed and compilation 
of the current compilation is aborted. When the \code{severity} is specified 
as \code{fatal} and the \code{at} clause is specified as \code{runtime}
then the message is displayed and program execution is aborted. If no
\code{severity} clause is specified then the \code{error} directive
behaves as if the \code{severity} clause is specified as \code{fatal}.

If the \code{message} clause is specified then the \plc{msg-string} is
included in the implementation defined message.
