...
\begin{itemize}
    \item as if the \code{release} clause is present if the construct performs
        an atomic write or atomic update operation;
    \item as if the \code{acquire} clause is present if the construct performs
        an atomic read operation;
    \item as if the \code{acq_rel} clause is present if the construct
        performs an atomic captured update operation.
\end{itemize}
...
{\scode{omp_default_mem_alloc}} & Use default storage for allocations.\\
{\scode{omp_large_cap_mem_alloc}} & Use storage with large capacity.\\
{\scode{omp_const_mem_alloc}} & Use storage optimized for read-only variables. The result of writing to memory returned by this memory allocator is unspecified.\\
{\scode{omp_high_bw_mem_alloc}} & Use storage with high bandwidth.\\
{\scode{omp_low_lat_mem_alloc}} & Use storage with low latency.\\
{\scode{omp_cgroup_mem_alloc}} & Use storage that is close to all threads in the same contention group of the thread requesting the allocation. Attempts to access the memory returned by this allocator from a thread that is not part of the same contention group as the thread that allocated the memory result in unspecified behavior.\\
{\scode{omp_pteam_mem_alloc}} & Use storage that is close to all threads that bind to the same {\scode{parallel}} region of the thread requesting the allocation. Attempts to access the memory returned by this allocator from a thread that does not bind to the same {\scode{parallel}} region as the thread that allocated the memory results in unspecified behavior.\\
{\scode{omp_thread_mem_alloc}} & Use storage that is close to the {\splc{thread}} requesting the allocation. Attempts to access the memory returned by this allocator from a thread other than the one that allocated the memory results in unspecified behavior.\\
\end{supertabular}
