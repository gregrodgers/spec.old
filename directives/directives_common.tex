...
\item A \code{requires} directive with the \code{unified_shared_memory}
 clause must appear in all \emph{compilation units} of a program that contain device
...
\begin{ccppspecific}
\item Each of the \plc{begin}, \plc{end} and \plc{step} expressions if specified must be a valid \plc{assignment-expression} of the base language.
...
The behavior is unspecified if in any of the $i_{j} + step$ computations for $j >= 0$ and $j < N$ $i_{j} + step$ can't be represented in \plc{iterator-type}.
