...
clause of the form {\code|ext_\plc{implementation-defined-requirement}|} for an
...
The \code{requires} directive specifies features an implementation must support
for correct execution of the current translation unit, or in Fortran the main
program, subprogram or module.  The behavior specified by a requirement clause
...
\begin{itemize}
  ...
  \item In an application composed of multiple translation units or in
  Fortran more than one main program, subprogram or module all must have the
  same requirements.
...
\begin{itemize}
\item
  \plc{ompd-tool-var} - determines whether an OpenMP implementation will collect
...
{\plc|dyn-var|} & {\code|OMP_DYNAMIC|} & See description below\\
{\plc|nest-var|} & {\code|OMP_NESTED|} & Implementation defined\\
{\plc|nthreads-var|} & {\code|OMP_NUM_THREADS|} & Implementation defined\\
{\plc|run-sched-var|} & {\code|OMP_SCHEDULE|} & Implementation defined\\
{\plc|def-sched-var|} & (none) & Implementation defined\\
{\plc|bind-var|} & {\code|OMP_PROC_BIND|} & Implementation defined\\
{\plc|stacksize-var|} & {\code|OMP_STACKSIZE|} & Implementation defined\\
{\plc|wait-policy-var|} & {\code|OMP_WAIT_POLICY|} & Implementation defined\\
{\plc|thread-limit-var|} & {\code|OMP_THREAD_LIMIT|} & Implementation defined\\
{\plc|max-active-levels-var|} & {\code|OMP_MAX_ACTIVE_LEVELS|} & See description below\\
{\plc|active-levels-var|} & (none) & {\plc|zero|}\\
{\plc|levels-var|} & (none) & {\plc|zero|}\\
{\plc|place-partition-var|} & {\code|OMP_PLACES|} & Implementation defined\\
{\plc|cancel-var|} & {\code|OMP_CANCELLATION|} & {\plc|false|}\\
{\plc|display-affinity-var|} & {\code|OMP_DISPLAY_AFFINITY|} & {\plc|false|}\\
{\plc|affinity-format-var|} & {\code|OMP_AFFINITY_FORMAT|} & Implementation defined \\
{\plc|default-device-var|} & {\code|OMP_DEFAULT_DEVICE|} & Implementation defined\\
{\plc|target-offload-var|} & {\code|OMP_TARGET_OFFLOAD|} & {\code|DEFAULT|} \\
{\plc|max-task-priority-var|} & {\code|OMP_MAX_TASK_PRIORITY|} & {\plc|zero|}\\
{\plc|tool-var|} & {\code|OMP_TOOL|} & {\plc|enabled|}\\
{\plc|tool-libraries-var|} & {\code|OMP_TOOL_LIBRARIES|} & {\plc|empty string|}\\
{\plc|ompd-tool-var|} & {\code|OMPD_ENABLED|} & {\plc|not set|} \\
{\plc|def-allocator-var|} & {\code|OMP_ALLOCATOR|} & Implementation defined\\
\end{supertabular}
...
\item \code{OMPD_ENABLED} environment variable, see \specref{sec:OMPD_ENABLED}.
...
{\plc|dyn-var|} & {\code|omp_set_dynamic()|} & {\code|omp_get_dynamic()|}\\

{\plc|nest-var|} & {\code|omp_set_nested()|} & {\code|omp_get_nested()|}\\

{\plc|nthreads-var|} & {\code|omp_set_num_threads()|} & {\code|omp_get_max_threads()|}\\

{\plc|run-sched-var|} & {\code|omp_set_schedule()|} & {\code|omp_get_schedule()|}\\

{\plc|def-sched-var|} & (none) & (none)\\

{\plc|bind-var|} & (none) & {\code|omp_get_proc_bind()|}\\

{\plc|stacksize-var|} & (none) & (none)\\

{\plc|wait-policy-var|} & (none) & (none)\\

{\plc|thread-limit-var|} & {\code|thread_limit|} clause & {\code|omp_get_thread_limit()|}\\

{\plc|max-active-levels-var|} & {\code|omp_set_max_active_levels()|} & {\code|omp_get_max_active_levels()|}\\

{\plc|active-levels-var|} & (none) & {\code|omp_get_active_level()|}\\

{\plc|levels-var|} & (none) & {\code|omp_get_level()|}\\

{\plc|place-partition-var|} & (none) & See description below \\

{\plc|cancel-var|} & (none) & {\code|omp_get_cancellation()|}\\

{\plc|display-affinity-var|} & (none) & (none)\\

{\plc|affinity-format-var|} & {\code|omp_set_affinity_format()|} & {\code|omp_get_affinity_format()|}\\

{\plc|default-device-var|} & {\code|omp_set_default_device()|} & {\code|omp_get_default_device()|}\\

{\plc|target-offload-var|} & (none) & (none)\\

{\plc|max-task-priority-var|} & (none) & {\code|omp_get_max_task_priority()|}\\
{\plc|tool-var|} & (none) & (none)\\
{\plc|tool-libraries-var|} & (none) & (none)\\
{\plc|ompd-tool-var|} & (none) & (none) \\

{\plc|def-allocator-var|} & {\code|omp_set_default_allocator()|} & {\code|omp_get_default_allocator()|}\\
...
{\plc|display-affinity-var|} & global \\
{\plc|affinity-format-var|} & device \\
{\plc|default-device-var|} & data environment\\
{\plc|target-offload-var|} & global\\
{\plc|max-task-priority-var|} & global\\
{\plc|tool-var|} & global\\
{\plc|tool-libraries-var|} & global\\
{\plc|ompd-tool-var|} & global \\
{\plc|third-party-tool-var|} & global \\
{\plc|def-allocator-var|} & implicit task\\
\end{supertabular}
...
{\plc|cancel-var|} & (none)\\
{\plc|display-affinity-var|} & (none) \\
{\plc|affinity-format-var|} & (none) \\
{\plc|default-device-var|} & (none)\\
{\plc|target-offload-var|} & (none)\\
{\plc|max-task-priority-var|} & (none)\\
{\plc|tool-var|} & (none)\\
{\plc|tool-libraries-var|} & (none)\\
{\plc|ompd-tool-var|} & (none) \\
{\plc|def-allocator-var|} & {\code|allocator|}\\
\end{supertabular}
...
OpenMP memory allocators can be used by a program to make allocation requests. When a memory allocator receives a request to allocate storage of a certain size, it will try to return an allocation of logically consecutive memory in its associated storage resources of at least the size being requested. This allocation will not alias with any other existing allocation from an OpenMP memory allocator. Table~\ref{tab:Predefined Allocators} shows the list of predefined memory allocators. The use of a given memory allocator expresses an intent to use storage with certain traits for the allocations. The actual storage resources that each memory allocator will use are implementation defined.

\nolinenumbers
\renewcommand{\arraystretch}{1.5}
\tablefirsthead{%
\hline
\textsf{\textbf{Allocator name}} & \textsf{\textbf{Storage selection intent}}\\
\hline\\[-3ex]
}
\tablehead{%
\multicolumn{2}{l}{\small\slshape table continued from previous page}\\
...
{\code|omp_default_mem_alloc|} & Use default storage for allocations.\\
{\code|omp_large_cap_mem_alloc|} & Use storage with large capacity.\\
{\code|omp_const_mem_alloc|} & Use storage optimized for read-only variables. The result of writing to memory returned by this memory allocator is unspecified.\\
{\code|omp_high_bw_mem_alloc|} & Use storage with high bandwidth.\\
{\code|omp_low_lat_mem_alloc|} & Use storage with low latency.\\
{\code|omp_cgroup_mem_alloc|} & Use storage that is close to all threads in the same contention group of the thread requesting the allocation. Attempts to access the memory returned by this allocator from a thread that is not part of the same contention group as the thread that allocated the memory result in unspecified behavior.\\
{\code|omp_pteam_mem_alloc|} & Use storage that is close to all threads that bind to the same {\code|parallel|} region of the thread requesting the allocation. Attempts to access the memory returned by this allocator from a thread that does not bind to the same {\code|parallel|} region as the thread that allocated the memory results in unspecified behavior.\\
{\code|omp_thread_mem_alloc|} & Use storage that is close to the {\plc|thread|} requesting the allocation. Attempts to access the memory returned by this allocator from a thread other than the one that allocated the memory results in unspecified behavior.\\
\end{supertabular}

\linenumbers

For memory allocators other than \code{omp_cgroup_mem_alloc}, \code{omp_pteam_mem_alloc} and \code{omp_thread_mem_alloc} the returned memory must be accessible by all threads in the device where the allocation was requested. %The allocator will return the value zero it fails to allocate the requested memory.
