...
The \code{dynamic_allocators} clause
\begin{itemize}
 ...
 \item allows the \code{omp_init_allocator} and \code{omp_destroy_allocator} API calls in \code{target} regions.
 \item allows default allocators in an \code{allocate} directive, an \code{allocate} clause or an \code{omp_alloc} API routines in \code{target} regions.
\end{itemize}
