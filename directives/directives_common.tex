...
(\plc{s}_\plc{0},\plc{s}_\pcl{1},...,\plc{s}_{\plc{n-1}}), where \plc{n} 
...
\begin{indentedcodelist}
[\plc{ shaped-pointer-variable } := ([\plc{s}_\plc{0}][\plc{s}_\plc{1}]...[\plc{s}_\plc{n-1}}])\plc{pointer-variable}
\end{indentedcodelist}
...
\item If the type of the base expression of the array section is a pointer 
      type, the base expression must be an lvalue expression.

\begin{cppspecific}
\item If the type of the base expression of a shape-operator is a reference 
      to a type \plc{T} then the type will be considered to be \plc{T} for 
      all purposes of the designated array.
