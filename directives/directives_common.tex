...
An array section is formed by a \emph{base expression} of array or pointer type
that is followed by one or more \emph{array section subscripts}. At least one
of the array section subscripts contains an array subscript expression that is
extended with the following syntax, and otherwise they must contain array
subscript expressions permitted by the base language:
...
A \emph{subscript tuple} is a tuple of array subscripts obtainable from the
array section as follows. Where the array section subscript contains a base
language array subscript expression, the corresponding subscript in the
subscript tuple contains the same expression. Otherwise, an integer value
taken from the set of integer values associated with an array section
subscript is used as the array subscript expression of the corresponding
subscript in the subscript tuple.  An array section contains the set of
array elements specified by applying the subscripts from each obtainable
subscript tuple to the array section's base expression.
...
section. The sixth example is not contiguous.  The remaining examples show
array sections formed from more general lvalue expressions.
...
\item The array section's base expression must be an lvalue expression.

\item The array element expressions formed from the array section's base expression
and the subscript tuples obtainable from the array section
must be valid expressions in the base language.
\end{ccppspecific}

\begin{cspecific}
%\item The type of the variable appearing in an array section must be array or pointer.
%\item The type of an array section's lvalue expression must be array or pointer.
\end{cspecific}

\begin{cppspecific}
%\item If the type of an array section's lvalue expression is a reference to type \plc{T} then the type will be considered to be \plc{T} for all purposes of the array section.

\item If the type of an array section base expression is a reference to a type \plc{T} then the type will be considered to be \plc{T} for all purposes of the array section.
