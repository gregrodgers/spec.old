...
\code{omp\_team\_local\_mem\_alloc} & Use storage that is close to the \plc{team} requesting the allocation. Attempts to access the memory returned by this allocator from a thread that is not part of the team that allocated the memory results in unspecified behavior.\\
\code{omp\_thread\_local\_mem\_alloc} & Use storage that is close to the \plc{thread} requesting the allocation. Attempts to access the memory returned by this allocator from a thread other than the one that allocated the memory results in unspecified behavior.\\
\end{supertabular}
