...
{\splc{max-active-levels-var}} & {\scode{OMP_MAX_ACTIVE_LEVELS}}, {\scode{OMP_NESTED}}, {\scode{OMP_NUM_THREADS}}, {\scode{OMP_PROC_BIND}} & See description below\\
...
\item The initial value of \plc{max-active-levels-var} is implementation 
      defined if none of the environment variables associated with it in
      \tabref{tab:ICV Initial Values} are set. The
      \plc{max-active-levels-var} ICV will be initialized to the number of 
      active levels of parallelism that the implementation supports if
      \code{OMP_NUM_THREADS} or \code{OMP_PROC_BIND} is set to a
      comma-separated list of more than one value. This value may be
      overridden by setting \code{OMP_MAX_ACTIVE_LEVELS} or
      \code{OMP_NESTED}. See \specref{sec:OMP_MAX_ACTIVE_LEVELS} and
      \specref{sec:OMP_NESTED} for details.
\end{itemize}
