...
\item Each trait that appears in the \plc{construct} trait set in the OpenMP context 
      is given the value $2^{p-1}$ where $p$ is the position of trait in the set;
...
At any point after a variant for a given base function is declared at which 
a direct call to that base function occurs, the compiler determines if any
declared variant is compatible with the OpenMP context. If so, the variant 
with the highest score according to the algorithm described in 
Section \ref{subsec:Matching and Scoring Context Selectors} is selected from 
all compatible variants. If multiple variants have the highest score, the 
selected variant is unspecified. If a compatible variant exists, the
call to the base function is replaced with a call to the selected variant.
