...
\begin{itemize}
\item \plc{num-procs-var} - the number of processors that are available to the device.
      There is one copy of this ICV per device.
...
\item \plc{final-task-var} - whether the current task is a final task. There is one copy
      of this ICV per data environment.
\item \plc{implicit-task-var} - whether the current task is an implicit task. There is
      one copy of this ICV per data environment.
...
When a \plc{target task} executes an active \code{target} region, the generated initial
...
When a \plc{target task} executes an inactive \code{target} region, the generated initial
task uses the values of the data environment scoped ICVs from the data
environment of the task that encountered the \code{target} construct.
