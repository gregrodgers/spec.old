...
{\splc{ompd-tool-var}} & {\scode{OMPD_ENABLED}} & {\splc{not set}} \\
...
{\scode{omp_const_mem_space}} & Represents storage optimized for read-only variables. The result of writing to this storage is unspecified.\\
...
%{\scode{omp_cgroup_mem_space}} & Represents storage that is close to all threads in the same contention group of the thread requesting the allocation. Attempts to access the memory returned by this allocator from a thread that is not part of the same contention group as the thread that allocated the memory result in unspecified behavior.\\
%{\scode{omp_pteam_mem_space}} & Represents storage that is close to all threads that bind to the same {\scode{parallel}} region of the thread requesting the allocation. Attempts to access the memory returned by this allocator from a thread that does not bind to the same {\scode{parallel}} region as the thread that allocated the memory results in unspecified behavior.\\
%{\scode{omp_thread_mem_space}} & Represents storage that is close to the {\splc{thread}} requesting the allocation. Attempts to access the memory returned by this allocator from a thread other than the one that allocated the memory results in unspecified behavior.\\
\end{supertabular}

\linenumbers

%Memory spaces other than \code{omp_cgroup_mem_space}, \code{omp_pteam_mem_space} and \code{omp_thread_mem_space} must represent memory accessible by all threads in the device where the allocation was %requested. 
...
OpenMP memory allocators can be used by a program to make allocation requests. When a memory allocator receives a request to allocate storage of a certain size, it will try to return an allocation of logically consecutive memory in its associated memory space resources of at least the size being requested. This allocation will not alias with any other existing allocation from an OpenMP memory allocator. 
...
\tablelasttail{\hline}
\tablecaption{Predefined Allocators\label{tab:Allocator traits}}
\begin{supertabular}{p{2.1in}p{3in}p{2.1in}}
{\scode{threadmodel}} & {\scode{shared},\scode{exclusive}} & {\scode{shared}}\\
{\scode{alignment}} & 0, power of two integer & 0\\
{\scode{access}} & {\scode{all}, \scode{cgroup}, \scode{pteam}, \scode{thread}} & {\scode{all}}\\
\end{supertabular}
\linenumbers

When an allocator {\scode{threadmodel}} trait is defined to be {\scode{exclusive}} the implementation can assume that no operation will be performed on the allocator by more than one thread at a time.
If the allocator {\scode{alignment}} trait is greater than zero the allocated memory will be byte aligned to at least the value specified for the trait.

Allocators with the \scode{access} trait defined to be \scode{all} must return memory accessible by all threads in the device where the allocation was requested. Allocators with the \scode{access} trait defined to be \scode{cgroup} will return memory close to all threads in the same contention group of the thread requesting the allocation. Attempts to access the memory returned by an allocator with the \scode{access} trait defined to be \scode{cgroup} from a thread that is not part of the same contention group as the thread that allocated the memory result in unspecified behavior. Allocators with the \scode{access} trait defined to be \scode{pteam} will return memory close to all threads that bind to the same {\scode{parallel}} region of the thread requesting the allocation. Attempts to access the memory returned by  an allocators with the \scode{access} trait defined to be \scode{pteam} from a thread that does not bind to the same {\scode{parallel}} region as the thread that allocated the memory result in unspecified behavior. Allocators with the \scode{access} trait defined to be \scode{thread} return memory close to the {\splc{thread}} requesting the allocation. Attempts to access the memory returned by an allocators with the \scode{access} trait defined to be \scode{thread} from a thread other than the one that allocated the memory result in unspecified behavior.
...
\tablefirsthead{%
\hline
\textsf{\textbf{Allocator name}} & \textsf{\textbf{Associated memory space}} & \textsf{\textbf{Non-default traits}}\\
\hline\\[-3ex]
}
\tablehead{%
\multicolumn{3}{l}{\small\slshape table continued from previous page}\\
...
\begin{supertabular}{p{2.1in}p{3in}p{3in}}
...
{\scode{omp_cgroup_mem_alloc}} & Implementation defined & {\scode{access}} is {\scode{cgroup}\\
{\scode{omp_pteam_mem_alloc}} & Implementation defined & {\scode{access}} is {\scode{pteam}\\
{\scode{omp_thread_mem_alloc}} & Implementation defined & {\scode{access}} is {\scode{thread}\\
\end{supertabular}
