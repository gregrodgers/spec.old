...
\emph{Use of this directive makes your code less portable, be aware that not all
...
The \code{unified\_address} extension is activate if the \code{unified\_address}
...
not present are not set to null.  Target devices may still have discrete
memories and dereferencing a device pointer on the host device remains
unspecified behavior.

The \code{unified\_memory} extension is activate if the \code{unified\_memory}
clause appears on an \code{extension} directive in the current translation unit.
...
valid host pointers.  Memory local to a specific execution context may not be
accessible from devices other than the device executing that context while still
conforming to this extension.
...
The \code{full\_system\_atomics} extension is activate if the
...
The \code{full\_system\_coherence} extension is activate if the
...
implementation-defined extensions should begin with that prefix.
