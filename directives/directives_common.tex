...
must provide in order for the code to compile and execute correctly.
...
clause of the form \code{ext\_\plc{implementation-defined-requirement}} for an
...
When the \code{unified\_address} clause appears on a \code{requires}
...
\code{omp\_target\_alloc} and accessed through \code{use\_device\_ptr} are
guaranteed to be pointer values that can support pointer arithmetic while
still being native device pointers. The \code{is\_device\_ptr} clause is not
...
The \code{unified\_shared\_memory} clause implies the \code{unified\_address}
...
local to an execution context as defined in \code{unified\_address} above may remain
...
The \code{unified\_shared\_memory} clause makes the \code{map} clause optional
...
requirement clauses.  Requirement names that do not start with \code{ext\_} are
...
\plc{nest-var} & \code{OMP\_NESTED} & Implementation defined\\
...
\plc{display-affinity-var} & \code{OMP\_DISPLAY\_AFFINITY} & \plc{false}\\
\plc{affinity-format-var} & \code{OMP\_AFFINITY\_FORMAT} & Implementation defined \\
\plc{default-device-var} & \code{OMP\_DEFAULT\_DEVICE} & Implementation defined\\
\plc{target-offload-var} & \code{OMP\_TARGET\_OFFLOAD} & \code{DEFAULT} \\
...
\plc{ompd-tool-var} & \code{OMPD\_ENABLED} & \plc{not set} \\
\plc{def-allocator-var} & \code{OMP\_ALLOCATOR} & Implementation defined\\
\end{supertabular}
\linenumbers
...
\item \code{OMP\_DISPLAY\_AFFINITY} environment variable, see \specref{sec:OMP_DISPLAY_AFFINITY}.

\item \code{OMP\_AFFINITY\_FORMAT} environment variable, see \specref{sec:OMP_AFFINITY_FORMAT}.

\item \code{OMP\_DEFAULT\_DEVICE} environment variable, see \specref{sec:OMP_DEFAULT_DEVICE}.

\item \code{OMP\_TARGET\_OFFLOAD} environment variable, see \specref{sec:OMP_TARGET_OFFLOAD}.
...
\item \code{OMPD\_ENABLED} environment variable, see \specref{sec:OMPD_ENABLED}.

\item \code{OMP\_ALLOCATOR} environment variable, see \specref{sec:OMP_ALLOCATOR}.

\item \code{OMP\_TARGET\_ALLOCATOR} environment variable, see \specref{sec:OMP_TARGET_ALLOCATOR}.
\end{itemize}
...
\plc{affinity-format-var} & \code{omp\_set\_affinity\_format()} & \code{omp\_get\_affinity\_format()}\\
...
\plc{def-allocator-var} & \code{omp\_set\_default\_allocator()} & \code{omp\_get\_default\_allocator()}\\
...
\item \code{omp\_set\_target\_default\_allocator} routine, see \specref{subsec:omp_set_target_default_allocator}.

\item \code{omp\_get\_target\_default\_allocator} routine, see \specref{subsec:omp_get_target_default_allocator}.
\end{itemize}
...
\plc{def-target-allocator-var} & implicit task\\
\end{supertabular}
...
\code{omp\_default\_mem\_alloc} & Use default storage for allocations.\\
\code{omp\_large\_cap\_mem\_alloc} & Use storage with large capacity.\\
\code{omp\_const\_mem\_alloc} & Use storage optimized for read-only variables. The result of writing to memory returned by this memory allocator is unspecified.\\
\code{omp\_high\_bw\_mem\_alloc} & Use storage with high bandwidth.\\
\code{omp\_low\_lat\_mem\_alloc} & Use storage with low latency.\\
\code{omp\_cgroup\_mem\_alloc} & Use storage that is close to all threads in the same contention group of the thread requesting the allocation. Attempts to access the memory returned by this allocator from a thread that is not part of the same contention group as the thread that allocated the memory result in unspecified behavior.\\
\code{omp\_pteam\_mem\_alloc} & Use storage that is close to all threads that bind to the same \code{parallel} region of the thread requesting the allocation. Attempts to access the memory returned by this allocator from a thread that does not bind to the same \code{parallel} region as the thread that allocated the memory results in unspecified behavior.\\
\code{omp\_thread\_mem\_alloc} & Use storage that is close to the \plc{thread} requesting the allocation. Attempts to access the memory returned by this allocator from a thread other than the one that allocated the memory results in unspecified behavior.\\
\end{supertabular}

For memory allocators other than \code{omp\_cgroup\_mem\_alloc}, \code{omp\_pteam\_mem\_alloc} and \code{omp\_thread\_mem\_alloc} the returned memory must be accessible by all threads in the device where the allocation was requested. %The allocator will return the value zero it fails to allocate the requested memory.
