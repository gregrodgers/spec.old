...
\item \plc{def-allocator-var} - determines the memory allocator to be used by memory allocation routines, directives and clauses when a memory allocator is not specified by the user except those cases determined by the \plc{target-def-allocator-var} ICV. There is one copy of this ICV per implicit task.

\item \plc{target-def-allocator-var} - determines the memory allocator to be used by the target directive and its clauses when a memory allocator is not specified by the user. There is one copy of this ICV per implicit task.
...
\plc{target-def-allocator-var} & \code{OMP\_TARGET\_ALLOCATOR} & Implementation defined\\
\end{supertabular}
\linenumbers
...
\plc{target-def-allocator-var} & {\scriptsize\code{omp\_set\_target\_default\_allocator()}} & {\scriptsize\code{omp\_get\_target\_default\_allocator()}}\\
...
\plc{target-def-allocator-var} & \code{allocator}\\
\end{supertabular}
\linenumbers
