...
\subsection{\hcode{warn} Directive}
\index{warn@{\code{warn}}}
\index{directives!warn@{\code{warn}}}
\label{subsec:warn Directive}
\summary
The \code{warn} directive 

PASTED BUT NOT YET UPDATED

declares a specialized variant
of a base function and specifies the context in which that specialized variant
is used. The \code{declare}~\code{variant} directive is a declarative
directive.
...
\begin{ccppspecific}
\begin{samepage}
The syntax of the \code{warn} directive is as follows:

\begin{ompcPragma}
#pragma omp warn \plc{[clause[ [},\plc{] clause] ... ] new-line}
\end{ompcPragma}
\end{samepage}
...
\begin{indentedcodelist}
message(\plc{compile-time-string})
