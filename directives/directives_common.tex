...
The \code{dynamic_allocators} clause makes optional the use of the \code{uses_allocator} clause on \code{target} constructs for the purpose of using allocators in the corresponding \code{target} regions. The \code{dynamic_allocators} clause also allows the usage of the \code{omp_init_allocator} and \code{omp_destroy_allocator} API calls in \code{target} regions. The \code{dynamic_allocators} clause also allows default allocators to be implicitly specified by an \code{allocate} directive or an \code{allocate} clause that appears in a \code{target} region.
