...
\code{omp\_high\_cap\_mem\_alloc} & Use storage with a high capacity.\\
...
\code{omp\_teams\_mem\_alloc} & Use storage that is close to all threads in the same contention gorup of the thread requesting the allocation. Attempts to access the memory returned by this allocator from a thread that is not part of the same contention group as the thread that allocated the memory results in unspecified behavior.\\
\code{omp\_thread\_team\_mem\_alloc} & Use storage that is close to all threads in the same \code{parallel} region of the thread requesting the allocation. Attempts to access the memory returned by this allocator from a thread that is not part of the same \code{parallel} region as the thread that allocated the memory results in unspecified behavior.
...
For memory allocators other than \code{omp\_team\_local\_mem\_alloc} and \code{omp\_thread\_local\_mem\_alloc} the returned memory must be accessible by all threads in the device where the allocation was requested. %The allocator will return the value zero it fails to allocate the requested memory.
