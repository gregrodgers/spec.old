...
\begin{supertabular}{p{1.7in}p{3in}}
\code{omp\_default\_mem} & Use \plc{default} storage for allocations.\\
\code{omp\_large\_mem} & Use storage with large capacity.\\
\code{omp\_const\_mem} & Use storage optimized for read-only variables. The result of writing to memory returned by this memory allocator is unspecified.\\
...
For memory allocators other than \code{omp\_team\_local\_mem} and \code{omp\_thread\_local\_mem} the returned memory must be accessible by all threads in the device where the allocation was requested. The allocator will return the value zero if fails to allocate the requested memory.
