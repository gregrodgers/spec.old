...
\begin{ccppspecific}
OpenMP directives for C/C++ are specified with \pcode{\#pragma} preprocessing directives.
...
separate the words in a directive. Preprocessing tokens following \pcode{\#pragma} \code{omp}
...
sentinel differs between fixed form and free form source files, as described in
...
comma-separated collection of one or more \plc{list items}. An
...
\begin{ccppspecific}
A \plc{list item} is a variable or array section. An \plc{extended list item} is a \plc{list item} or a function name.  A \plc{locator list item} is any lvalue
...
the \plc{list} or the \plc{extended-list} appears.
...
apply to the directive line. Initial directive lines must have a space or a zero in column 6,
...
(space and tab characters). It must appear as a single word with no intervening
white space. Fortran free form line length, white space, and continuation rules apply to the
...
\begin{fortranspecific}
\begin{itemize}
    \item A stand-alone directive may not be used as the action statement in an \code{if}
...
may override the normal behavior specified elsewhere in this document. It is
implementation defined whether the feature specified by a given requirement
clause is supported.
...
The \code{dynamic_allocators} clause removes certains restrictions on the use
...
\item A \code{requires} directive with any of the following clauses
 must appear in all \emph{compilation units} of a program that contain device
...
\item \plc{active-levels-var} - the number of nested active \code{parallel} regions enclosing the current
...
\item \plc{def-sched-var} - controls the implementation defined default
...
\item Each data environment has its own copies of ICVs with data environment
    scope.
...
When a \code{parallel} construct is encountered, the value of each ICV with implicit task scope is inherited, unless otherwise specified, from the implicit binding task of the generating task unless otherwise specified.
...
schedule kind is specified, all
...
a type \plc{T}, then the type will be considered to be \plc{T} for all purposes of the array section.
...
\item \plc{range-specification} is of the form \plc{begin}\code{:}\plc{end[}\code{:}\plc{step]} where \plc{begin} and \plc{end} expressions for which their types can be converted to the \plc{iterator-type} type and \plc{step} is an integral expression.
...
$i_{0}=begin$,  $i_{j}=i_{j-1} + step$ and:
\begin{itemize}
\item $i_{0} < (\plc{iterator-type})end$ and $i_{N-1} < (\plc{iterator-type})end$ and $(\plc{iterator-type})(i_{N-1} + step) >= (\plc{iterator-type})end$ if $step > 0$.
\item $i_{0} > (\plc{iterator-type})end$ and $i_{N-1} > (\plc{iterator-type})end$ and $(\plc{iterator-type})(i_{N-1} + step) <= (\plc{iterator-type})end$ if $step < 0$.
\end{itemize}
\end{ccppspecific}
\begin{fortranspecific}
The values of the iterator are the set of values $i_{1}$...$i_{N}$ where
$i_{1}=begin$,  $i_{j}=i_{j-1} + step$ and:
\begin{itemize}
...
The behavior is unspecified if $i_{j} + step$ cannot be represented in \plc{iterator-type} in any of the $i_{j} + step$ computations for any $0 <= j < N$.
...
a \code{target} construct, the following rules will be applied in order:

\begin{enumerate} 
 \item For functions with a \code{declare simd} directive, the \plc{simd}
     trait will be added to the beginning of the set for the generated SIMD versions.  
 \item For functions with a \code{declare variant} directive, the selectors of
     the \code{construct} selector set will be added in the same order to the beginning of the set.

 \item For functions within a \code{declare target} block, the \plc{target}
     trait will be added to the beginning of the set for the versions of the function being generated for \code{target} regions.
\end{enumerate}
...
name and semantics. The \plc{simd} trait will define at least the
...
The \plc{implementation} set includes traits that describe the functionality supported by the OpenMP implementation at that point in the program. At least the following traits can be defined:
\begin{itemize}
...
The \code{construct} selector set defines which \plc{construct} traits should
...
The \code{construct} selector is an ordered list.

The \code{device} and \code{implementation} selector sets define which traits
...
selector set can also be set to the value \code{any}, which is as if no \code{kind} selector was specified. 
...
\end{cspecific}
\begin{cppspecific}
The \code{condition(}\plc{boolean-expr}\code{)} selector defines a \code{constexpr} expression that must evaluate to true for the selector to be true.  
\end{cppspecific}
\begin{fortranspecific}
...
\begin{itemize}
 \item The \code{simdlen(}\plc{N}\code{)} property of the selector matches the
     \plc{simdlen(M)} trait of the OpenMP context if $M \% N$ equals zero.
...
\begin{enumerate}
 \item Each trait appearing in the \plc{construct} trait set in the OpenMP context is given the value $2^{p-1}$ where $p$ is the position of trait in the set.
 \item The \code{kind}, \code{arch} and \code{isa} selectors are given the
     values $2^{l}$, $2^{l+1}$ and $2^{l+2}$, respectively, where $l$ is the number of traits in the \plc{construct} set.
 ...
 \item Context selectors that are a strict subset of another context selector
     ...
     corresponding to the \code{construct} selectors appear multiple times in the OpenMP context, the highest valued subset of traits that contains all the selectors in the same order will be used.
\end{enumerate}
...
function. If no \code{match} clause is specified then the context selector for
the variant is empty. If a \code{match} clause is specified then the context
...
\begin{itemize}
\item At most one \code{match} clause can appear on a \code{declare variant} directive.
\item If the function definition or a declaration of the function in the same
    compilation unit has a \code{declare}~\code{variant} directive, then calling the variant function directly in an OpenMP context that is different than the one specified by the \code{construct} set of the context selector is non-conforming.

\begin{ccppspecific}
\item If the function has any declarations, then the
    \code{declare}~\code{variant} directives for any
declarations that have one must be equivalent. If the function definition has a \code{declare}~\code{variant} it must also be equivalent.
...
\item The \plc{base-func-name} of a \code{declare}~\code{variant} directive cannot be a template function.
\item The \plc{base-func-name} of a \code{declare}~\code{variant} directive cannot be a virtual function.
\end{cppspecific}

\begin{fortranspecific}
\item \plc{proc-name} must not be a generic name, procedure pointer or entry
    name.
\item If \plc{proc-name} is omitted, the \code{declare}~\code{variant} directive must appear in the specification part of a subroutine subprogram or a function subprogram.
\item Any \code{declare}~\code{variant} directive must appear in the specification part of a subroutine, subprogram, function subprogram or interface body to which it applies.
\item If a \code{declare}~\code{variant} directive is specified in an
    interface block for a procedure, it must match a \code{declare}~\code{variant} directive in the definition of the procedure.
...
\item If a \code{declare}~\code{variant} directive is specified for a
    procedure name with an explicit interface and a \code{declare}~\code{variant}
    directive is also specified for the definition of the procedure, the two
    \code{declare}~\code{variant} directives must match. Otherwise the result is unspecified.
\end{fortranspecific}
\end{itemize}
...
\subsection{Metadirective Directive}
\index{meta-directive}
\index{directives!metadirective@{\code{metadirective}}}
\label{subsec:Metadirective Meta-Directive}
\summary
The metadirective directive is a meta-directive that can specify multiple directive variants
of which one may be conditionally selected to replace the meta-directive based
on the enclosing OpenMP context.

\syntax
\begin{ccppspecific}
The syntax of the metadirective directive takes one of the
following forms:
\begin{ompcPragma}
...
\begin{fortranspecific}
The syntax of the metadirective directive takes one of the following
forms:
...
to the above rules, the meta-directive has no effect on the execution of the
program. 
...
replace the \code{begin}~\code{metadirective} directive, the
...
\restrictions
Restrictions to the metadirective directive are as follows:
...
\begin{itemize}
    ...
    \item Any replacement that occurs for a metadirective directive must
    ...
    \item Any directive variant that is specified by a \code{when} or \code{default}
        ...
        directive must be an OpenMP directive that has a paired \code{end}~\plc{directive}, and
