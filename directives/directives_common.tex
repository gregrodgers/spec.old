...
\item For functions mentioned in a \code{declare}~\code{variant} directive
      as function variants, the selectors 
...
\summary
The \code{declare}~\code{variant} directive declares specialized variant
...
\begin{ompcPragma}
#pragma omp declare variant(\plc{variant-func-name}) \plc{clause new-line}
\plc{[}#pragma omp declare variant(\plc{variant-func-name}) \plc{clause new-line]}
\plc{[ ... ]}
...
and where \plc{variant-func-name} is either a base language identifier, or
for C++ a \plc{template-id}.
\end{ccppspecific}
...
\begin{ompfPragma}
!$omp declare variant(\plc{[proc-name}:\plc{]variant-proc-name}) \plc{clause}
\end{ompfPragma} %$ close off misinterpreted dollar sign/math symbol
...
The \code{declare}~\code{variant} directive declares the base function to
have \plc{variant-func-name} or \plc{variant-proc-name} as a function
variant. The context selector in the \code{match} clause is associated
with the variant.
...
There may be multiple \code{declare}~\code{variant} directives for the same
base function.
...
\begin{cppspecific}
The function variant is determined by what function would be called if there
was a function call to \plc{variant-func-name} with argument types of the
base function possibly modified in implementation defined manner depending
on what the specific OpenMP context requires at the beginning of base
function.
\end{cppspecific}
...
\begin{itemize}
\item Calling functions mentioned in \code{declare}~\code{variant} directives as
      function variants directly in an OpenMP context that is different than
      the one specified by the \code{construct} set of the 
      context selector is non-conforming.

\item If a function is mentioned as function variant in more than one
      \code{declare}~\code{variant} directives, then the \code{construct}
      set of their context selectors must be the same.
