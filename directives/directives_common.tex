...
rules ([basic.lookup]) of \plc{variant-func-id} with arguments corresponding
to the argument types in the base function declaration.
If the type of the resulting function variant is incompatible with
the type of the base function transformed in an implementation
defined way according to OpenMP context, the behavior is unspecified.
...
\item Calling functions that a \code{declare}~\code{variant} directive determined
      to be a function variant directly in an OpenMP context that
