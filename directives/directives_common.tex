...
\item \plc{levels-var} - the number of nested active parallel regions enclosing the current task such that
...
{\splc{nest-var}} & {\scode{OMP_NESTED}} & See description below\\
...
\item The initial value of the \plc{nest-var} ICV is determined to be \plc{true} if \plc{max-active-levels-var} is set to a value greater than 1 (one); otherwise, it is set to \plc{false}.

\item The value of the \plc{nthreads-var} ICV is a list.

\item The value of the \plc{bind-var} ICV is a list.

\item  The default initial value for \plc{max-active-levels-var} is implementation defined, unless \plc{nthreads-var} or \plc{bind-var} is initialized to a list consisting of more than one element via the respective environment variables, in which case the default initial value for \plc{max-active-levels-var} is the number of active levels of parallelism that the implementation supports.
\end{itemize}
...
\item \code{omp_get_max_active_levels_supported}, see \specref{subsec:omp_get_max_active_levels_supported}.
...
2-dimensional array to which the pointer \code{b[10]} points. The seventh
...
\begin{ccppspecific}
The values of the iterator are the set of values $i_{0}$...$i_{N-1}$ where $i_{0}=(\plc{iterator-type})begin$, $i_{j}=(\plc{iterator-type})(i_{j-1} + step)$ and
\begin{itemize}
