...
\begin{ccppspecific}
OpenMP directives for C/C++ may be specified with \pcode{\#pragma} directives as follows:
\begin{ompcPragma}
#pragma omp\plc{ directive-name [clause[ [},\plc{] clause] ... ] new-line}
\end{ompcPragma}

Each \pcode{\#pragma} directive  starts with \pcode{\#pragma}~\code{omp}. The remainder of
...
\begin{fortranspecific}
A \plc{list item} is a variable that is not coindexed, an array section that
is not coindexed, or a common block name (enclosed in slashes).  An
\plc{extended list item} is a \plc{list item} or a procedure name. A
\plc{locator list item} is a \plc{list item}, or a reserved locator.
...
The reserved locator \code{omp_all_memory} is a reserved identifier that
...
\begin{fortranspecific}
\item may contain \code{STOP} or \code{ERROR}~\code{STOP} statements.
\end{fortranspecific}
\end{itemize}
...
statement, \code{ERROR}~\code{STOP} statement, or access to a coindexed
...
\item A stand-alone directive may not be used in place of the statement following
      an \code{if}, \code{else}, \code{while}, \code{do}, \code{for}, \code{switch},
      or a label.
\end{itemize}
\end{ccppspecific}
...
\begin{enumerate}
\item For functions with a declare SIMD directive, the \plc{simd} trait
...
OpenMP context and a \code{default} clause that specifies a directive
...
\restrictions
Restrictions to the \code{declare}~\code{variant} directive are as follows:
...
\begin{itemize}
\item \plc{num-procs-var} - provides the number of processors that are available to the
      device at the time a routine is called and looking at this ICV.
      This value may change between the time that it is determined by a routine and the
      time that it is read in the calling context due to system actions outside the
      control of the OpenMP implementation.
\item \plc{thread-num-var} - provides the thread number of an implicit task within its
      binding team. There is one copy of this ICV for each task.
\item \plc{final-task-var} - stores whether the task is a final task. There is one copy
      of this ICV for each task.
\item \plc{implicit-task-var} - stores whether the task is an implicit task. There is
      one copy of this ICV for each task.
\item \plc{team-size-var} - stores the size of a team. There is one copy of this ICV
      for each parallel team.
\end{itemize}
...
{\splc{max-active-levels-var}} & {\scode{OMP_MAX_ACTIVE_LEVELS}}, {\scode{OMP_NESTED}}, {\scode{OMP_NUM_THREADS}}, {\scode{OMP_PROC_BIND}} & Implementation defined\\
...
{\splc{num-procs-var}}         & (none)                          & Implementation defined \\
{\splc{thread-num-var}}        & (none)                          & {\splc{zero}}          \\
{\splc{final-task-var}}        & (none)                          & {\splc{false}}         \\
{\splc{implicit-task-var}}     & (none)                          & {\splc{true}}          \\
{\splc{team-size-var}}         & (none)                          & {\splc{one}}           \\
...
If an ICV is not listed in this table, there is no OpenMP API routine to modify or retrieve this ICV.
...
{\splc{num-procs-var}}         & (none)                                & {\scode{omp_get_num_procs()}}         \\

{\splc{thread-num-var}}        & (none)                                & {\scode{omp_get_thread_num()}}        \\

{\splc{final-task-var}}        & (none)                                & {\scode{omp_in_final()}}              \\

{\splc{team-size-var}}         & (none)                                & {\scode{omp_get_num_threads()}}       \\
...
\item \code{omp_get_thread_num} routine, see \specref{subsec:omp_get_thread_num}.

\item \code{omp_get_num_procs} routine, see \specref{subsec:omp_get_num_procs}.
...
\item \code{omp_in_final} routine, see \specref{subsec:omp_in_final}.
...
{\splc{num-procs-var}}         & device \\
{\splc{thread-num-var}}        & implicit task \\
{\splc{final-task-var}}        & data environment \\
{\splc{implicit-task-var}}     & data environment \\
{\splc{team-size-var}}         & team \\
...
\subsection{\hcode{nothing} Directive}
\index{nothing@{\code{nothing}}}
\index{directives!nothing@{\code{nothing}}}
\label{subsec:nothing Directive}
\summary
The \code{nothing} directive has no effect. It can be used in a 
...
\begin{ccppspecific}
\begin{samepage}
The syntax of the \code{nothing} directive is as follows:

\begin{ompcPragma}
#pragma omp nothing \plc{new-line}
\end{ompcPragma}
\end{samepage}
\end{ccppspecific}
...
The \code{nothing} directive has no effect on the execution of 
the OpenMP program. 

\begin{crossrefs}
\item Metadirectives, see \specref{subsec:Metadirective Meta-Directive}.
\end{crossrefs}



\subsection{\hcode{error} Directive}
\index{warn@{\code{erorr}}}
\index{directives!error@{\code{error}}}
\label{subsec:error Directive}
\summary
The \code{error} directive instructs the compiler or runtime to display
a message and to perform an error action. If the \code{at} clause is
specified with the \code{runtime} value then the \code{error} directive 
is a stand-alone directive.

\syntax
\begin{ccppspecific}
\begin{samepage}
The syntax of the \code{error} directive is as follows:
...
message(\plc{msg-string})
\end{indentedcodelist}

where \plc{msg-string} is a string of const char * type.
\end{samepage}
\end{ccppspecific}
...
\begin{ompfPragma}
!$omp error \plc{[clause[ [},\plc{] clause] ... ]}
\end{ompfPragma} %$ close off misinterpreted dollar sign/math symbol
\end{samepage}
...
where \plc{msg-string} is a character string of character(len=*) type
\end{samepage}
\end{fortranspecific}

\descr

The \code{error} directive performs an error action. The error action
includes the display of an implementation defined message. The \code{severity}
clause determines if the error action includes anything other than the
message display.

The \code{at} clause determines when the implementation performs the error 
action. When the \code{at} clause specifies \code{compile}, the error action 
is performed during compilation if the \code{error} directive appears in a 
declarative context or in an executable context that is reachable at
runtime. When the \code{at} clause specifies \code{compile} and the 
\code{error} directive appears in an executable context that is not 
reachable at runtime, the error action may or may not be performed. When the 
\code{at} clause specifies \code{runtime}, the error action is performed 
during program execution when a thread encounters the directive. If the 
\code{at} clause is not specified then the \code{error} directive behaves 
as if the \code{at} clause specifies \code{compile}.

The \code{severity} clause determines the action that the implementation
performs. When the \code{severity} clause specifies \code{warning}, the 
implementation takes no action besides displaying the message. When the 
\code{severity} clause specifies \code{fatal} and the \code{at} clause 
specifies  \code{compile} then the message is displayed and compilation 
of the current compilation unit is aborted. When the \code{severity} 
clause specifies \code{fatal} and  the \code{at} clause specifies 
\code{runtime} then the message is displayed and program execution is 
aborted. If no \code{severity} clause is specified then the \code{error} 
directive behaves as if the \code{severity} clause specifies \code{fatal}.

If the \code{message} clause is specified then \plc{msg-string} is included 
in the implementation defined message.

\restrictions
Restrictions to the \code{error} directive are as follows:

\begin{itemize}
\item At most one \code{at} clause can appear on the directive.
\item At most one \code{severity} clause can appear on the directive.
\item At most one \code{message} clause can appear on the directive.
\end{itemize}

\begin{crossrefs}
\item Stand-Alone Directives, see \specref{subsec:Stand-Alone Directives}.
\end{crossrefs}
