...
\item For functions with a \code{declare}~\code{simd} directive, the \plc{simd} trait
      ...
      versions if any SIMD versions are created so the total size of the set is
      increased by 1.
\item For functions with a \code{declare}~\code{variant} directive, the selectors 
...
\item For functions within a \code{declare}~\code{target} block, the \plc{target}
      ...
      the function that are generated for \code{target} regions if any so the total 
...
and where \plc{variant-func-name} is the name of a function variant that is
...
The \code{declare}~\code{variant} directive declares a function, the
...
\begin{cppspecific}
The specified function variant is the function that would be called if there was a
function call to \plc{variant-func-name} with arguments corresponding to
the argument types in the base function prototype
at the beginning of the base function.
\end{cppspecific}
...
\begin{itemize}
\item Calling functions determined to be function variants by
      \code{declare}~\code{variant} directives directly in an OpenMP context that
      is different than the one specified by the \code{construct} selector set of the 
