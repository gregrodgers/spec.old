...
An array section is formed by an \emph{array section base expression}, of
array or pointer type, that is followed by one or more \emph{array section subscripts}. Each array section
subscript is an array subscript permitted by the base language, where at least one is extended with the following syntax:
...
A subscript sequence is a sequence of array subscripts obtainable
from the array section, where the value contained in each subscript is an integer value taken
from the corresponding array section subscript. An array section contains the set of array
elements specified by applying each obtainable subscript sequence to the array
section base expression.
...
+\item The array section base expression must be an lvalue expression.

\item The array element expressions formed from the array section base expression
and the subscript sequences obtainable from the array section
