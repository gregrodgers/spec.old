...
\item \plc{def-allocator-var} - determines the allocator to be used by allocation routines, directives 
and clauses on the host device when an allocator is not specified by the user.

\item \plc{target-def-allocator-var} - determines the allocator to be used by allocation routines, directives 
and clauses on a target device when an allocator is not specified by the user.
...
\plc{def-allocator-var} & thread\\
\plc{def-target-allocator-var} & thread\\
\end{supertabular}
...
\item Each thread has its own copy of ICVs with thread scope 
\end{itemize}
...
\plc{def-allocator-var} & (none)\\
\plc{def-target-allocator-var} & (none)\\
\end{supertabular}
\linenumbers
...
OpenMP allocators can be used by a program to make allocation requests. When an allocator receives a request to allocate storage of a certain size, it will try to return an allocation of logically consecutive virtual memory in its associated storage resources of at least the size being requested. Table~\ref{tab:Predefined Allocators} shows the list of OpenMP predefined allocators. The use of a given allocator expresses an intent to use storage with certain traits for the allocations. The actual storage resources that each allocator will use its implementation defined.
...
\begin{supertabular}{p{1.5in}p{3in}}
\code{omp\_default\_mem} & Use \plc{default} storage for allocations.\\
\code{omp\_large\_mem} & Use storage with large capacity.\\
\code{omp\_const\_mem} & Use storage optimized for read-only variables. The result of writing to memory returned by this allocator is unspecified.\\
\code{omp\_high\_bw\_mem} & Use storage with high bandwidth.\\
\code{omp\_low\_lat\_mem} & Use storage with low latency.\\
\code{omp\_team\_local\_mem} & Use storage that is close to the \plc{team} requesting the allocation. Trying to access the memory returned by this allocator from a thread that is not part of the team that allocated the memory results in unspecified behavior.\\
\code{omp\_thread\_local\_mem} & Use storage that is close to the \plc{thread} requesting the allocation. Trying to access the memory returned by this allocator from a thread other than the one that allocated the memory results in unspecified behavior.\\
\code{omp\_low\_prio\_mem} & Use storage considered as non-critical.\\ 
\end{supertabular}

For allocators other than \code{omp\_team\_local\_mem} and \code{omp\_thread\_local\_mem} the returned memory must be accessible by all threads in the device where the allocation was requested. The allocator will return the value zero if fails to allocate the requested memory.

\begin{fortranspecific}
If any operation of the base language causes a reallocation of an array that is allocated with an OpenMP allocator then that allocator will be used to release the current memory and to allocate the new memory.
\end{fortranspecific}
