...
compilation unit is aborted. When the \code{severity} specifies \code{fatal} 
and  the \code{at} clause specifies \code{runtime} then the message is 
displayed and program execution is aborted. If no \code{severity} clause 
is specified then the \code{error} directive behaves as if the \code{severity}
clause specifies \code{fatal}.
