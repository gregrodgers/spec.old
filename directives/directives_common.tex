...
\begin{itemize}
 \item The \plc{kind(kind-name-list)} trait specifies the general kind of the device. OpenMP defined values for \plc{kind-name} are defined in the ``OpenMP Context Definitions'' document which is available on http://www.openmp.org/. 
...
The \plc{offload} set includes traits that define the characteristics of the compiler offload target device at that point in the program if one exists. Otherwise it will be an empty set. The traits that can be defined in the \plc{offload} set are the same as in the \plc{device} set.
...
\begin{itemize}
 \item The \plc{vendor(vendor-name)} trait specifies the name of the vendor of the implementation. OpenMP defined values for \plc{vendor-name} are defined in the ``OpenMP Context Definitions'' document which is available on http://www.openmp.org/. 
 ...
 \item A trait with the same name corresponding to each clause in the \code{requires} directive.
\end{description}
...
\begin{enumerate}
 ...
 \item Other context selectors score result of the addition of the values of all the selectors specified. If a \code{construct} selector appears multiple times in the context only the highest value is used. If any of the \code{kind},\code{uarch} or \code{isa} selectors are specified both in the \code{device} set and the \code{offload} set it will be only be considered once.
\end{enumerate}
