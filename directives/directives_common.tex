...
%
%\begin{ccppspecific}
%An array section is formed by a \emph{base expression} of array or pointer
%type that is followed by one or more \emph{array section subscripts}.  At
%least one of the array section subscripts contains an array subscript
%expression that is extended with the following syntax, and otherwise they
%contain array subscript expressions permitted by the base language:
%
%
%\begin{quote}
...
%\end{quote}
%
%The base expression of an array section is its named array or named pointer.
%
...
%array sections formed from more general base expressions.
%
%\end{note}
%
%\medskip
...
%
%\item An array section must have a named array or named pointer.
%
%\begin{ccppspecific}
%%\item The base expression of the array section must be an lvalue expression.
%
%\item An array element expression derived from a given array section, formed
%by the base expression of the array section followed by array subscripts that each
%contain a value taken from the corresponding array section subscripts,
%must be a valid expression in the base language.
%\end{ccppspecific}
%
%\begin{cppspecific}
...
%The \emph{base expression} of an array section is its named array or named pointer, if
%either exists. Otherwise, it is an lvalue expression that specifies the
%initial element of the original array.

The \emph{base expression} of an array section is an expression that specifies
the initial element of the original array.
...
remaining examples show array sections formed from more general base
expressions.
\end{note}
\medskip
\end{ccppspecific}
...
\item If the type of the base expression of the array section is pointer type, the
    base expression must be an lvalue expression.
% TODO: consider adding this in future ticket
%\item An array element expression derived from a given array section, formed
%by the base expression of the array section followed by array subscripts
%that each contain a value taken from the corresponding array section
%subscripts, must be a valid expression in the base language.
\end{ccppspecific}
