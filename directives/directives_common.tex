...
\begin{quote}
\code{\plc{lvalue-expression}[\plc{ lower-bound }:\plc{ length }]} or

\code{\plc{lvalue-expression}[\plc{ lower-bound }:\plc{ }]} or

\code{\plc{lvalue-expression}[ :\plc{ length }]} or

\code{\plc{lvalue-expression}[\plc{ }:\plc{ }]}
\end{quote}
...
section. The sixth example is not contiguous.  The seventh example is an array section where \code{S.c} is a structure member variable.  The remaining examples are more complex lvalue-expressions.
\end{note}
\medskip
\end{ccppspecific}
...
\item An array section can be specified for an lvalue expression.
\end{ccppspecific}
...
\begin{cppspecific}
\item If the type of an array section's lvalue expression is reference to type \plc{T} then the type will be considered to be \plc{T} for all purposes of the array section.
