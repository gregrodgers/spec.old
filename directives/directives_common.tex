...
\summary
This construct declares the extensions an implementation must provide in order
...
\subsection{\code{unified\_address} Clause}
\label{subsec:unified_address Clause}
...
OpenMP will use the same address to refer to different physical memory
locations.  The pointers returned by \code{omp\_target\_alloc} and accessed
through \code{use\_device\_ptr} are guaranteed to be pointer values that can
support pointer arithmetic while still being native pointers.  The
\code{is\_device\_ptr} clause is not necessary for device pointers to be
translated in \code{target} regions, and pointers found not present are not set
to null.  Target devices may still have discrete memories and dereferencing a
device pointer on the host device remains unspecified behavior.

\subsection{\code{unified\_memory} Clause}
\label{subsec:unified_memory Clause}
...
below.  Every device address allocated through OpenMP target API routines are
...
This extension makes the \code{map} clause optional on target constructs.  Note
that scalar variables are still required to be \code{firstprivate} by default
for target regions.  Cache coherence across devices \emph{is not} guaranteed
except across synchronization points between the host and a given device where
memory is flushed.  Mutable access to nearby addresses from two separate devices
without synchronization remains a potential race condition.
...
This extension builds on \code{unified\_memory} and additionally allows all
devices accessible through to OpenMP interact cache-coherently across all memory
in the system.

\subsection{Implementation-Defined Clauses}
\label{subsec:Implementation-Defined Clauses}

An implementor is allowed to include additional implementation defined
extension clauses.  Extension names that do not start with \code{ext\_} are reserved,
all implementation-defined extensions should begin with that prefix.
