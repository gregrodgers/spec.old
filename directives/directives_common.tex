...
The \code{dynamic_allocators} clause makes optional the use of the \code{uses_allocator} clause on \code{target} constructs for the purpose of using allocators in the corresponding \code{target} regions, including the implicit use of default allocators by an \code{allocate} directive or an \code{allocate} clause. The \code{dynamic_allocators} clause also allows the use of the \code{omp_init_allocator} and \code{omp_destroy_allocator} API calls in \code{target} regions.
