...
memory is flushed.  Accesses to the same memory unit from different devices
without synchronization remains a potential race condition.

When the \code{full\_system\_atomics} extension is active, the binding thread
set for atomic constructs includes all threads across all devices
accessible by OpenMP.  Non-atomic accesses across devices remain
non-coherent.

The \code{full\_system\_coherence} extension builds on \code{unified\_memory}
and additionally allows all devices accessible through to OpenMP interact
cache-coherently across all memory in the system.
