...
The \plc{construct} set is composed of the directive names, each being a trait, of all enclosing executable directives at that point in the program up to a \code{target} directive. The set is ordered by their nesting level. In addition, if the point in the program is not enclosed by a \code{target} directive, the following rules will be applied in order:
\begin{enumerate} 
 ...
 \item for functions within a \code{declare target} block the \plc{target} trait will be added at the beginning of the set.
\end{enumerate}
...
\begin{itemize}
 \item The \plc{kind(kind-name-list)} trait specifies the general kind of the device. The following \plc{kind-name} are defined:
 \begin{itemize}
  \item The \plc{host} trait specifies that the device is the host device.
  \item The \plc{nohost} trait specifies that the devices is not the host device.
 \end{itemize}
 Additional OpenMP defined values for \plc{kind-name} are defined in the ``OpenMP Context Definitions'' document which is available on \url{http://www.openmp.org/}. 
...
Implementations can define further traits in the \plc{device} and \plc{implementation} sets. All implementation defined traits and properties must follow the syntax of base language identifiers.
...
The \code{user} selector set defines selectors that provide additional user-defined. The \code{condition}(\plc{boolean-expr}) selector defines a compile-time condition that must  evaluate to true for the selector to be true. 
...
\begin{enumerate}
 ...
 \item Other context selectors score result of the addition of the values of all the specified selectors plus $1$. If a \code{construct} selector appears multiple times in the context only the highest value is used.
\end{enumerate}
