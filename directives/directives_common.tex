...
\item A \code{requires} directive shall appear lexically before any affected
  construct in the compilation unit.

\item A \code{requires} directive with the \code{unified_shared_memory} clause
  must appear in all \emph{compilation units} of a program that contain device
  constructs or device memory routines or in none of them.
