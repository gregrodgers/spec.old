...
\code{omp\_cgroups\_mem\_alloc} & Use storage that is close to all threads in the same contention group of the thread requesting the allocation. Attempts to access the memory returned by this allocator from a thread that is not part of the same contention group as the thread that allocated the memory result in unspecified behavior.\\
\code{omp\_team\_mem\_alloc} & Use storage that is close to all threads that bind to the same \code{parallel} region of the thread requesting the allocation. Attempts to access the memory returned by this allocator from a thread that does not bind to the same \code{parallel} region as the thread that allocated the memory results in unspecified behavior.\\
