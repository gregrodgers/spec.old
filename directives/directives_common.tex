...
\section{\code{extension} Directive}
\label{sec:extension Directive}
\index{extension@{\code{extension}}}
\index{directives!extension@{\code{extension}}}

\summary
This directive declares the extensions an implementation must provide in order
for the code in the current translation unit to compile and execute correctly.
...
\syntax
\begin{ccppspecific}
  The syntax of the \code{extension} directive is as follows:
\begin{boxedcode}
\#pragma omp extension \plc{[clause[ [},\plc{] clause] ... ] new-line}
\end{boxedcode}
...
\begin{boxedcode}
!\$omp extension \plc{[clause[ [},\plc{] clause] ... ]}
\end{boxedcode}

\end{fortranspecific}

Where \plc{clause} is either one of the extension clauses listed below or a
clause of the form \code{ext\_\plc{implementation-defined-extension}} for an
implementation defined extension clause.
...
The extension directive asserts to an implementation what the code of the
current translation unit requires to function correctly.  Behaviors specified by
an extension \emph{override} restrictions and behaviors specified elsewhere
in the OpenMP specification wherever they conflict.

\begin{note}
Use of this directive makes your code less portable, be aware that not all
devices or implementations support all extensions.
\end{note}

The \code{unified\_address} extension is active if the \code{unified\_address}
clause appears on an \code{extension} directive in the current translation unit.
When the \code{unified\_address} extension is active, the implementation
guarantees that all devices accessible through OpenMP API routines and
directives use a unified address space such that no two pointers used on any
pair of devices accessible through OpenMP will use the same address to refer to
different physical memory locations.  The pointers returned by
\code{omp\_target\_alloc} and accessed through \code{use\_device\_ptr} are
guaranteed to be pointer values that can support pointer arithmetic while still
being native pointers.  The \code{is\_device\_ptr} clause is not necessary for
device pointers to be translated in \code{target} regions, and pointers found
not present are not set to null. Memory local to a specific execution context may be
exempt from this, following the restrictions of locality to a given team or
thread.  Target devices may still have discrete
memories and dereferencing a device pointer on the host device remains
unspecified behavior.

The \code{unified\_memory} extension is active if the \code{unified\_memory}
clause appears on an \code{extension} directive in the current translation unit.
The \code{unified\_memory} extension implies the \code{unified\_address}
extension, inheriting all of its behaviors.  Additionally host addresses are
valid device addresses for all devices accessible through OpenMP except as noted
below.  Every device address allocated through OpenMP device memory routines are
valid host pointers.  Memory local to an execution context as defined in unified
address above may also remain inaccessible between devices.

Further the \code{unified\_memory} extension makes the \code{map} clause
optional on target constructs.  Note that scalar variables are still required to
be \code{firstprivate} by default for target regions.  Cache coherence across
devices \emph{is not} guaranteed except across synchronization points between
the host and a given device where memory is flushed.  Accesses to the same
memory unit from different devices without synchronization remains a potential
race condition.

The \code{full\_system\_atomics} extension is active if the
\code{full\_system\_atomics} clause appears on an \code{extension} directive in
the current translation unit.When the \code{full\_system\_atomics} extension is
active, the binding thread set for atomic constructs includes all threads across
all devices accessible by OpenMP.  Non-atomic accesses across devices remain
non-coherent.

The \code{full\_system\_coherence} extension is active if the
\code{full\_system\_coherence} clause appears on an \code{extension} directive
in the current translation unit.The \code{full\_system\_coherence} extension
builds on \code{unified\_memory} and additionally allows all devices accessible
through to OpenMP interact cache-coherently across all memory in the system.

Implementors are allowed to include additional implementation defined extension
clauses.  Extension names that do not start with \code{ext\_} are reserved. All
implementation-defined extensions should begin with \code{ext\_}.
