...
current translation unit, or program unit in Fortran, requires to function
correctly.  Behaviors specified by a requirement \emph{override} restrictions
and behaviors specified elsewhere in the OpenMP specification wherever they
conflict.
...
clause appears on a \code{requirements} directive in the current translation
unit, or program unit in Fortran.
...
clause appears on a \code{requirements} directive in the current translation
unit, or program unit in Fortran.  The \code{unified\_memory} requirement
implies the \code{unified\_address} requirement, inheriting all of its
behaviors.  Additionally host addresses are valid device addresses for all
devices accessible through OpenMP except as noted below.  Every device address
allocated through OpenMP device memory routines are valid host pointers.  Memory
local to an execution context as defined in unified address above may
remain inaccessible between devices.

Further the \code{unified\_memory} requirement makes the \code{map} clause
optional on target constructs as well as the \code{declare}~\code{target}
directive on global variables accessed in \code{declare}~\code{target}
functions.  Note that scalar variables are still made \code{firstprivate} by
default for target regions.  Cache coherence across devices \emph{is not}
guaranteed except across synchronization points between the host and a given
device where memory is flushed.  Accesses to the same memory unit from different
devices without synchronization remains a potential race condition.
...
\begin{itemize}
  \item In an application composed of multiple translation units, all
    translation units must activate the same requirements.
\end{itemize}
