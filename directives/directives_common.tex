...
should be used. The \code{declare}~\code{variant} directive is a declarative
directive.
...
function call to \plc{variant-func-id} with arguments corresponding to
...
\begin{fortranspecific}
\item \plc{base-proc-name} must not be a generic name, procedure pointer or entry name.
\item If \plc{base-proc-name} is omitted, the \code{declare}~\code{variant} directive 
