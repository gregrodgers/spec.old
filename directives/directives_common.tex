...
\item \plc{def-allocator-var} - determines the memory allocator to be used by memory allocation routines, directives and clauses when a memory allocator is not specified by the user except for those determined by the \plc{target-def-allocator-var} ICV.

\item \plc{target-def-allocator-var} - determines the memory allocator to be used by the target construct and its clauses when a memory allocator is not specified by the user.
...
OpenMP memory allocators can be used by a program to make allocation requests. When a memory allocator receives a request to allocate storage of a certain size, it will try to return an allocation of logically consecutive virtual memory in its associated storage resources of at least the size being requested. This allocation will not alias with any other previous allocation from an OpenMP allocator. Table~\ref{tab:Predefined Allocators} shows the list of predefined memory allocators. The use of a given memory allocator expresses an intent to use storage with certain traits for the allocations. The actual storage resources that each memory allocator will use its implementation defined.
...
\code{omp\_default\_mem\_alloc} & Use \plc{default} storage for allocations.\\
\code{omp\_large\_mem\_alloc} & Use storage with large capacity.\\
...
\code{omp\_team\_local\_mem\_alloc} & Use storage that is close to the \plc{team} requesting the allocation. Trying to access the memory returned by this allocator from a thread that is not part of the team that allocated the memory results in unspecified behavior.\\
\code{omp\_thread\_local\_mem\_alloc} & Use storage that is close to the \plc{thread} requesting the allocation. Trying to access the memory returned by this allocator from a thread other than the one that allocated the memory results in unspecified behavior.\\
\code{omp\_low\_prio\_mem\_alloc} & Use storage considered as non-critical.\\ 
\end{supertabular}

For memory allocators other than \code{omp\_team\_local\_mem\_alloc} and \code{omp\_thread\_local\_mem\_alloc} the returned memory must be accessible by all threads in the device where the allocation was requested. The allocator will return the value zero if fails to allocate the requested memory.
