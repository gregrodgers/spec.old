...
program, subprogram or module.  The behavior specified by a \code{requires}
directive clause may override the normal behavior specified elsewhere in this
document.
...
When the \code{unified\_address} requirement appears on a \code{requirements}
...
The \code{unified\_shared\_memory} requirement implies the
\code{unified\_address} requirement, inheriting all of its behaviors.
Additionally host addresses are valid device addresses for all devices
accessible through OpenMP except as noted below.  Every device address allocated
through OpenMP device memory routines are valid host pointers.  Memory local to
an execution context as defined in unified address above may remain inaccessible
between devices.

Further the \code{unified\_shared\_memory} requirement makes the \code{map} clause
...
default for target regions.  Values stored into shared memory by one device may
...
\begin{itemize}
  \item In an application composed of multiple translation units, all
    translation units must have the same requirements.
\end{itemize}
