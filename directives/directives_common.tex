...
\summary
This directive declares the requirements an implementation must provide in order
...
The \code{requires} directive asserts to an implementation what the code of the
current translation unit requires to function correctly.  Behaviors specified by
a requirement \emph{override} restrictions and behaviors specified elsewhere
...
The \code{unified\_address} requirement is active if the \code{unified\_address}
clause appears on an \code{requirements} directive in the current translation unit.
When the \code{unified\_address} requirement is active, the implementation
...
The \code{unified\_memory} requirement is active if the \code{unified\_memory}
clause appears on an \code{requirements} directive in the current translation unit.
The \code{unified\_memory} requirement implies the \code{unified\_address}
requirement, inheriting all of its behaviors.  Additionally host addresses are
...
Further the \code{unified\_memory} requirement makes the \code{map} clause
...
The \code{full\_system\_atomics} requirement is active if the
\code{full\_system\_atomics} clause appears on an \code{requirements} directive in
the current translation unit.When the \code{full\_system\_atomics} requirement is
...
The \code{full\_system\_coherence} requirement is active if the
\code{full\_system\_coherence} clause appears on an \code{requirements} directive
in the current translation unit.The \code{full\_system\_coherence} requirement
...
Implementors are allowed to include additional implementation defined
