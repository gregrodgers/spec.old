...
performs. When the \code{severity} specifies \code{warning}, the 
implementation takes no action besides displying the message. When the 
\code{severity} specifies \code{fatal} and the \code{at} clause specifies 
\code{compile} then the message is displayed and compilation of the current 
compilation is aborted. When the \code{severity} specifies \code{fatal} and 
the \code{at} clause specifies \code{runtime} then the message is displayed 
and program execution is aborted. If no \code{severity} clause is specified 
then the \code{error} directive behaves as if the \code{severity} clause 
specifies \code{fatal}.
