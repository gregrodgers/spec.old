...
At any point a program an OpenMP context exists that defines traits about the active OpenMP constructs, the execution devices, and the implementation support. The traits are grouped in sets. The following sets exist: \plc{construct}, \plc{device}, \plc{offload} and \plc{implementation}.

The \plc{construct} set is composed of the directive names, each being a trait, of all enclosing executable directives at that point in the program up to a \code{target} directive. The set is ordered by their nesting level. In addition,
\begin{itemize}
 \item for functions with a \code{declare simd} directive the \plc{simd} trait will be the first one of the set for the generated SIMD versions.  
 \item for functions within a \code{declare target} block the \plc{target} trait will be the first one of the set.
 \item for functions with a \code{declare variant} directive the selectors of the \code{construct} set will be added in the same order at the beginning of the set.
\end{itemize}

The following properties can be defined in an OpenMP context for the \plc{simd} trait:
\begin{itemize}
 \item The \plc{linear(var-list:step)} property lists all variables that are known to be linear with a given step in that particular \plc{simd} construct.
 \item The \plc{uniform(var-list)} property lists all variables that are known to be uniform in that particular \plc{simd} construct.
 \item The \plc{reduction(operator:var-list)} property lists all variables that are known to be reduced using \plc{operator}.
 \item The \plc{simdlen(len)} property describes the SIMD length that will be used for that particular \plc{simd} construct.
 \item The \plc{safelen(len)} property describes the SIMD safe length that was specified, if any, for that particular \plc{simd} construct.
 \item The \plc{notinbranch} property can be set if the \plc{simd} construct is known not to have branches. Otherwise, the \plc{inbranch} property will be set.
 \item The \plc{aligned(var-list:alignment)} property lists all variables that are known to be aligned to \plc{alignment}.
\end{itemize}

The \plc{device} set includes traits that define the characteristics of the compiler target device at that point in the program. At least the following traits must be defined:
\begin{itemize}
 \item The \plc{kind(kind-name-list)} trait specifies the general kind of the device. OpenMP defined values for \plc{kind-name} are defined in the ``OpenMP Context Definitions'' which is available on http://www.openmp.org/. 
 ...
 \item The \plc{uarch(uarch-name-list)} trait specifies the microarchitectures supported by the device. The accepted \plc{uarch-name} values are implementation defined.
\end{itemize}

The \plc{offload} set includes traits that define the characteristics of the compiler offload target device at that point in the program. The traits that can be defined in the \plc{offload} set are the same as in the \plc{device} set.

The \plc{implementation} set includes traits that define the implementation support at that point in the program. At least the following traits can be defined:
\begin{description}
 \item[vendor(\plc{name})]
 \item[unified\_address]
 \item[unified\_shared\_memory]
 \item[dynamic\_allocators]
 \item[extension(\plc{extension-list})]
\end{description}

Implementations can define further traits in the \plc{device},\plc{offload} and \plc{implementation} sets.
...
The syntax to define context selectors is the following:

\begin{ompSyntax}
\plc{context-selector-set}[,\plc{context-selector-set}[,...]]
\end{ompSyntax}

where \plc{context-selector-set} is:

\begin{ompSyntax}
\plc{context-selector-set-name}={\plc{context-selector-name}[(\plc{selector-property}[,[\plc{selector-property}[...]])][,[\plc{context-selector-name}[,...]]}
\end{ompSyntax}

The \code{construct} selector set defines which \plc{construct} traits should be active in the OpenMP context. The following selectors can be defined in the \code{construct} set: \code{target}, \code{teams}, \code{parallel}, \code{simd}. The properties of each selector are the same defined for the corresponding trait. The \code{construct} selector is an ordered list.

TODO: SIMD property equivalences and restrictions

The \code{device},\code{offload} and \code{implementation} selector sets define which traits should be active in the corresponding trait set of the OpenMP context. The same traits defined in the corresponding traits sets can be used as selectors with the same properties.

The \code{user} selector set defines selectors that provide additional user-defined. The \code{condition(\plc{boolean-expr}} selector defines a compile-time condition that must be evaluate to true for the selector to be true. 

Implementations can allow further selectors to be specified. Implementations can ignore user specified selectors that are not those described in this section.

\restrictions
\begin{itemize}
 \item Each \plc{context-selector-set-name} can only be specified once.
 \item Each \plc{context-selector-name} can only be specified once.
\end{itemize}
...
\begin{itemize}
 \item All selectors in the \code{user} set are true.
 \item All selectors in the \code{implementation} and \code{construct} sets appear in the corresponding trait set of the context with the same properties.
 \item All selectors in the \code{construct} set appear in the same relative order as the one in the \plc{construct} trait set.
 \item The properties of the \code{isa} selector of the \code{device} set are a subset of the properties of the \plc{isa} trait of the \plc{device} trait set.
 \item The properties of the \code{uarch} selector of the \code{device} set are a subset of the properties of the \plc{uarch} trait of the \plc{device} trait set.
 \item The properties of the \code{kind} selector of the \code{device} set are the same as the properties of the \plc{kind} trait on the \plc{device} trait set.
 \item The properties of the \code{isa} selector of the \code{offload} set are a subset of the properties of the \plc{isa} trait of the \plc{offload} trait set.
 \item The properties of the \code{uarch} selector of the \code{offload} set are a subset of the properties of the \plc{uarch} trait of the \plc{offload} trait set.
 \item The properties of the \code{kind} selector of the \code{offload} set are the same as the properties of the \plc{kind} trait on the \plc{offload} trait set.
\end{itemize}
...
\begin{enumerate}
 ...
 \item The \code{kind}, \code{uarch} and \code{isa} selectors will have the value $2^{l+1}$, $2^{l+2}$ and $2^{l+3}$ respectively where $l$ is the number of traits in the \plc{construct} set.
 ...
 \item Other context selectors score result of the addition of the values of all the selectors specified. If any of the \code{kind},\code{uarch} or \code{isa} selectors are specified both in the \code{device} set and the \code{offload} set it will be only be considered once.
\end{enumerate}
...
\begin{indentedcodelist}
match(\plc{context-specification})
\end{indentedcodelist}
\end{samepage}
\end{ccppspecific}
...
The use of a \code{declare}~\code{variant} directives immediately prior to a function declaration or definition declares that function to be a viable function variant of the \plc{base-func-name} function. 
...
The prototype of the variant function shall, in general, match that of the base function. It is implementation defined if for some specific Specialization Contexts the prototype of the variant should differ, and how, from that of the base function.
...
\item \plc{base-func-name} should uniquely identify, according to a base language lookup, a function or procedure in the program.
\item Calling the variant function directly in an OpenMP context that is different than the one specified by the \code{construct} set, if specified, in the \code{match} clause is non-conforming.

\begin{ccppspecific}
\item If the function has any declarations, then the \code{declare}~\code{variant} construct for any
declaration that has one must be equivalent to the one specified for the definition.
Otherwise, the result is unspecified.
\item The \plc{base-name-func} of a \code{declare variant} directive cannot be a template function.
\item The \plc{base-name-func} of a \code{declare variant} directive cannot be a virtual function.
\end{ccppspecific}
...
\crossreferences
\begin{itemize}
\item Context specification, see \specref{subsec:Context Selectors}.
\end{itemize}
