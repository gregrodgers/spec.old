...
An array section is formed by an lvalue expression having an array or pointer type
that is followed by one or more \emph{array section subscripts}, of which at least one must have
...
An array section contains the set of array elements specified by applying each
sequence of subscripts obtainable from the array section subscripts.
Subscript sequences are derived from the Cartesian product over the sets of
integer values corresponding to each array section subscript.  The array
...
\item The array element expressions formed from the resulting sequences of subscripts obtained from the array section subscripts must be valid expressions in the base language.
\end{ccppspecific}
