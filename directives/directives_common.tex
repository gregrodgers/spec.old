...
where \plc{clause} is:
\begin{indentedcodelist}
    when(\plc{context-selector}: \plc{directive-variant})
...
\begin{ompfPragma}
!$omp directive variants \plc{[clause[ [},\plc{] clause] ... ]}
\end{ompfPragma}
...
\code{when} clause has the highest computed score, the directive variant
specified in the same clause will replace the meta-directive. If more than one
compatible context selector specified in a \code{when} has the highest
computed score, the directive variant specified by the first of those \code{when}
clauses will replace the meta-directive.

If no context selector from any \code{when} clause is compatible with the
OpenMP context and a \code{default} clause is not present, the meta-directive
...
The \code{begin}~\code{directive}~\code{variants} directive behaves
identically to the \code{directive}~\code{variants} directive, except that the
...
replace the \code{begin}~\code{directive}~\code{variants} meta-directive, the
\code{end}~\code{directive}~\code{variants} directive will be implicitly
...
\code{end}~\code{directive}~\code{variants} directive is ignored.
...
\begin{itemize}
    \item The directive variant appearing in a \code{when} or \code{default}
        clause must not specify a \code{directive}~\code{variants},
        \code{begin}~\code{directive}~\code{variants}, or
        \code{end}~\code{directive}~\code{variants} directive.
    ...
    \item Any directive variant that appears in a \code{when} or \code{default}
        clause on a \code{begin}~\code{directive}~\code{variants}
        meta-directive must specify an OpenMP directive that has a paired \code{end}~\plc{directive}, and
        the \code{begin}~\code{directive}~\code{variants} directive must have a paired
        \code{end}~\code{directive}~\code{variants} directive.
