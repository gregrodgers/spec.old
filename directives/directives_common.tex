...
A \plc{score-expression} must be an integer expression that the compiler 
can evaluate.
...
\item The \code{user} selectors are given the value specified by the
      \plc{trait-score} \plc{score-expression}; if no \plc{trait-score} is 
      specified then they are given a value of zero;
