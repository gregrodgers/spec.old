...
When the \code{reverse_offload} clause appears on a \code{requires}
directive the \code{target} construct can be used inside of a target
region with the \code{device} clause in which the \code{ancestor} \plc{device-modifier}
is present to send work
back to the parent device for execution.
...
\item A \code{requires} directive with the \code{unified_shared_memory} or \code{reverse_offload} 
