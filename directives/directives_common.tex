...
The OpenMP context for a given metadirective is defined according to Section
\ref{subsec:OpenMP Context}.  For each \code{when} clause that appears on a
metadirective, the specified directive variant, if present, is a candidate to
replace the metadirective if the corresponding context selector is compatible
...
lexical order of those \code{when} clauses will replace the metadirective.
