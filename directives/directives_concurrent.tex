...
\begin{note}
If the \code{nodep} clause appears on the \code{loop} construct,  the
directive permits an implementation to use additional threads or SIMD
instructions to facilitate concurrent execution of the associated loop(s) when
this would not be observable given the restrictions defined for this
construct. Iteration reordering transformations may also be performed by the
implementation to take advantage of the assertion of independence provided by
the directive.
\end{note}
...
\item If a threadprivate variable is referenced inside a \code{loop} region
    corresponding to a \code{loop} construct with the \code{nodep} clause
