...
If the \code{binds} clause is present on the construct, the binding region is
...
binding region is \code{loop} region corresponding to that construct.
Otherwise, the binding region is undefined.
...
binding thread set is the binding thread set for that region. If the binding
region is undefined, then the binding thread set is the encountering thread.
...
If the \code{binds} clause is present then \plc{binding-region} may be one of
...
thread. The binding thread set defines the set of threads that may
execute iterations of the \code{loop} region.
...
\code{nodep} clause appears on the \code{loop} construct, this instructs the
implementation that the logical iterations may execute in any order, including
concurrently. If the \code{seq} clause is present, the iterations must execute
as if in their logical iteration order.

\begin{note}
If the \code{nodep} clause appears on the \code{loop} construct, the threads
that actually execute the iterations of the loop are not observable to the
program since any OpenMP constructs or calls to OpenMP API routines that may
expose this are not permitted in the region. An implementation is therefore
free to use threads outside of the binding thread set or use SIMD instructions
to facilitate concurrent execution of the associated loops. Additional
iteration reordering optimizations may also be employed by the implementation
to take advantage of the assertion of independence provided by the directive.
\end{note}
...
\begin{itemize}
  ...
  \item If a \code{loop} construct is not nested inside another OpenMP
      ...
      \code{binds} clause must be present.
...
%    % it can parallelize the loop.
  ...
  \item A \code{loop} region may not contain calls to the OpenMP Runtime
    API.
