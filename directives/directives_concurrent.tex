...
\summary
  ...
  the associated loops can be safely run concurrently in any order.
...
threads, or SIMD lanes) used to execute the loop iterations. The result of the
reduction operation must be usable immediately following the end of the
associated loops.


\binding
The binding for a given \code{concurrent} region is determined by the
\code{teams} or \code{parallel} region in which it is most closely nested. An
implementation may choose to combine distributing, worksharing, and/or SIMD
vectorizing, in which case the binding will be as described for the most
specific type of parallelism applied to the associated loops.
...
\code{distribute},
Loop, and/or SIMD construct if it would be conforming for a programmer to have
explicitly specified the selected construct. If the implementation chooses to
treat an \code{concurrent} construct as one of these constructs then the default data
