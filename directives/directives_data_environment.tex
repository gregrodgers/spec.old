...
treated as if it is a list item in single \code{map} clause with a
...
\begin{itemize}
    ...
    \item If a structure sibling list items does not appear in a \code{map}
    ...
    plc{map-type} of \code{delete} and the strucuture sibling list item does
    not appear in a \code{map} clause on the construct with a \plc{map-type} of
    \code{delete} then the structure sibling list item is treated as if it is a
