...
structure elements from the containing structure variable form a \plc{structure
sibling list}.  The \code{map} clause and the strucuture sibling list are
...
construct is encountered then: \begin{itemize}
    ...
    list item has a \plc{map-type} of \code{delete} and the strucuture sibling
...
in multiple \code{map} clauses on the same construct unless one of the
\code{map} clauses has a \plc{map-type} of \code{delete}.

\item At most one list item can be an array item derived from a given variable in a \code{map}
clause.

\item List items of \code{map} clauses on the same construct must not share original storage unless one of \code{map} clauses has a \plc{map-type} of \code{delete}.
