...
\plc{map-type-modifier} is present in a \code{map} \plc{map-type} clause, that
the effect of the clause is ordered before all other \code{map} \plc{map-types}.
If a \code{mapper} \plc{map-type-modifier} is present along with a \code{require_present}
\plc{map-type-modifier} the all elements of the object are treated as if they had a \code{require_present} \plc{modifier}.
...
\begin{ompSyntax}
  defaultmap(\plc{[require_present,]}\plc{implicit-behavior[:variable-category]})
\end{ompSyntax}
