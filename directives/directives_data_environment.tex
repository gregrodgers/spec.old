...
\item \plc{iterator-type} is an integral type.
\item \plc{range-specification} is of the form \plc{lower,upper[,step]} where \plc{lower}, \plc{upper} and \plc{step} are expressions of the \plc{iterator-type} type.
\end{itemize}
...
In an \plc{iterator-specifier}, if the \plc{iterator-type} is not specified then the type of that iterator is an integer type.

In a \plc{range-specification}, if the \plc{step} is not specified its value is implicitly defined to 1.

The values of the iterator are the set of values $i_{0}$...$i_{N}$ where $i_{0}=lower$,  $i_{j}=i_{j-1} + step$ and
\begin{ccppspecific}
\begin{itemize}
\item $i_{N} < upper$ and $i_{N} + step >= upper$ if $step > 0$
\item $i_{N} > upper$ and $i_{N} + step <= upper$ if $step < 0$
\end{itemize}
\end{ccppspecific}
\begin{fortranspecific}
...
\begin{itemize}
\item If the step expression of a \plc{range-specification} equals zero the behavior is unspecified.
\end{itemize}
