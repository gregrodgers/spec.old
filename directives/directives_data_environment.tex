...
\item If a list item in a \code{map} clause on the \code{target} construct has a named pointer, and the named pointer is a
...
If a list item in a \code{map} clause has a named pointer, and a pointer
variable is present in the device data environment that corresponds to the named pointer
when the effect of the \code{map} clause occurs, then if the corresponding
list item is created in the device data environment: 

\begin{enumerate}

\item The corresponding pointer variable is assigned address of the storage location of the corresponding
list item minus \plc{offset}, where \plc{offset} is the address offset
given by the address of the original list item minus the value of its named
pointer.
...
\item If more than one list item of the \code{map} clauses on the same
construct have the same named array or named pointer, and if any of the list
...
must refer to the same array elements of either the named array or the
implicit array of the named pointer.
...
\item If a list item is an element of a structure, only the rightmost symbol
