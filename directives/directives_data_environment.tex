...
I.E. struct {int a, b, c} S; map(S) -> map(S.a, S.b, S.c)

If a structure member variable appears in multiple map clauses on the same
construct, the map-type for the variable is the union of the map-types that
appear in each clause.

I.E. map(alloc:S) map(to:S.a) map(from:S.b) // to:S.a, from:S.b, alloc:S.c
...
clause on the same construct, then the variable is mapped before the array
section is mapped.

I.E. map(S.a) map(S.a[:100])  // S.a is mapped when mapping the array section 
...
the pointer variable is mapped then:
...
I.E. target data exit map(from:S.a) // map(release:S.b, S.c)
...
I.E. target data exit map(delete:S.a) // map(delete:S.b, S.c)
