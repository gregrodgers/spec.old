...
Specifying a variable on a \code{firstprivate}, \code{lastprivate},
\code{linear}, \code{reduction} or \code{copyprivate} clause of an enclosed
construct causes an implicit reference to the variable in the enclosing
construct. Specifying a variable on a \code{map} clause of an enclosed
construct may cause an implicit reference to the variable in the enclosing
construct.  Such implicit references are also subject to the data-sharing
...
The \code{iterator} clause defines one or more identifiers that will expand to
multiples values in the directive on which it appears.

\syntax

The syntax of the \code{iterator} clause is as follows:
\begin{boxedcode}
iterator(\plc{iterator-specifier-or-iterator-list})
\end{boxedcode}
where \plc{iterator-specifier-or-iterator-list} is either an \plc{iterator-specifier} or an \plc{iterator-list}.
...
\item \plc{range-specification} is of the form \plc{begin, end[, step]} where \plc{begin}, \plc{end} and \plc{step} are expressions whose types can be converted to the \plc{iterator-type} type.
\end{itemize}
...
If the \plc{identifier} in the \plc{iterator-specifier} is a variable, the behavior will be as if the variable had appeared in a \code{private} clause.

\begin{ccppspecific}
\item The \plc{iterator-type} must be an integral or pointer type.
\item In an \plc{iterator-specifier}, if the \plc{iterator-type} is not specified then the type of that iterator is an \code{int} type.
\end{ccppspecific}
\begin{fortranspecific}
\item The \plc{iterator-type} must be an integer type.
\item In an \plc{iterator-specifier}, if the \plc{iterator-type} is not specified then the type of that iterator is a default integer.
\end{fortranspecific}
...
The values of the iterator are the set of values $i_{0}$...$i_{N}$ where $i_{0}=begin$,  $i_{j}=i_{j-1} + step$ and
\begin{ccppspecific}
\begin{itemize}
