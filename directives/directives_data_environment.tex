...
NOTE: Perhaps this new stuff should be placed before the above mapping algorithm.

NOTE: The following text enables structure variables to make sense in the
mapping algorithm.  According to the algorithm, each structure element would
get a ref-count increment, but an implementation can optimize that to one
reference count for the whole structure.  We also need it to union map-types.

If a list item in a \code{map} clause is a variable of type structure then it is
treated as if each structure element contained in the variable
is a list item in the clause with the variable's \plc{map-type}.
...
clause where the \plc{map-type} is the union of the \plc{map-types} that appear
in each clause.

NOTE: map(alloc:S) map(to:S.a) map(from:S.b) // to:S.a, from:S.b, alloc:S.c

NOTE: maybe this is not c/c++ specific comment.

If a structure element is a list item in a \code{map} clause on a
\code{target}~\code{data}~\code{exit} construct then each structure element in
the structure variable that contained the list item is treated as if it is a
list item in a \code{map} clause on the construct with a \plc{map-type} of
\code{release}.

NOTE: target data exit map(from:S.a) // map(release:S.a S.b, S.c)

NOTE: map(from:S.a) UNION map(release:S.a) = map(from:S.a)

If a structure element is a list item in a \code{map} clause on a
\code{target}~\code{data}~\code{exit} construct and it has a \plc{map-type} of
\code{delete} then each structure element in the structure vairable that
contained the list item is treated as if it is a list item in a \code{map}
clause on the construct with a \plc{map-type} of \code{delete}.

NOTE: target data exit map(delete:S.a) // map(delete:S.b, S.c)

NOTE: There is a problem here with delete union from (for example).  Delete
union anything is delete?

NOTE: BEGIN ARRAY SECTION POINTER ATTACH HERE:

If a list item in a \code{map} clause is a variable of type pointer, the
variable is the lvalue expression in an array section, and the array section is
a list item in a \code{map} clause on the same construct then the effects of a
\code{map} clause on the pointer variable and a \code{map} clause on the array
...
NOTE: map(S.a) map(S.a[:100])  // on entry 1) map(S.a) then 2) map(S.a[:100])
                               // on exit  1) unmap(S.a[:100]) then 2) unmap(S.a)
