...
\begin{itemize}
\item \specref{subsubsec:Variables Referenced in a Construct}
describes the data-sharing attribute rules for variables
referenced in a construct.

\item \specref{subsubsec:Variables Referenced in a Region but not in a Construct} describes the data-sharing attribute rules for variables
...
Specifying a variable on a data-sharing attribute clause, except for the \code{private} clause,
...
\item The loop iteration variable in the associated \plc{for-loop} of a
  \code{simd} or \code{loop}  construct with just
...
\code{simd} or \code{loop} construct with multiple associated \plc{for-loops} are lastprivate.

\item Variables with static storage duration that are declared in a scope inside the construct
are shared.

\item If a list item in a \code{map} clause on the \code{target} construct has
...
\code{simd} or \code{loop} construct with multiple associated \plc{do-loops} are lastprivate.
...
\code{ASSOCIATE} or \code{SELECT}~\code{TYPE} statement.
\end{itemize}
\end{fortranspecific}
%
...
\item The loop iteration variable(s) in the associated \plc{for-loop(s)} of a \code{loop}
construct may be listed in a \code{private} clause.
...
\item The loop iteration variable(s) in the associated \plc{do-loop(s)} of a \code{loop}
construct may be listed in a \code{private} clause.
...
\item In a \code{target} construct, variables that are not mapped after applying data-mapping attribute rules (see \specref{subsec:Data-Mapping Attribute Rules, Clauses, and Directives}) are firstprivate.
\end{itemize}
...
\syntax
\begin{ompSyntax}
reduction(\plc{[reduction-modifier},\plc{] }\plc{reduction-identifier }:\plc{ list})
\end{ompSyntax}
...
combined construct for which any of the aforementioned constructs is a
...
construct, a \code{simd} construct or a combined construct for which any of
the aforementioned constructs is a constitutent construct.
