...
and \plc{list-item-category} is one of:
\begin{indentedcodelist}
  scalar
  aggregate
  pointer
...
\code{defaultmap(tofrom:aggregate)}, \code{defaultmap(default:pointer)} and
\code{defaultmap(firstprivate:scalar)}.  Note that the default pointer behavior
can only be specified with the \code{default} implicit behavior argument, any
other choice will apply to the scalar storing the pointer but will skip the
zero-length array section lookup based on the value of the pointer.  All
variables of a given list-item category referenced in the construct that have
implicitly determined data-mapping attributes use the specified data-mapping
attribute.  If no list-item category is specified in the clause, the
data-mapping attribute is used as the default for all categories.
