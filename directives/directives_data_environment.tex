...
\item If a list item in a \code{reduction} or \code{in_reduction} clause
      on a construct is an array section or an array element whose base has pointer
      or reference to pointer type, the base variable is private.
\end{itemize}
\end{ccppspecific}
...
it is associated with a target or a  subobject of a target that that appears
...
\item If a list item is an array section or an array element, accesses to the elements of the array
outside the specified array section or array element result in unspecified behavior.

\item If a list item is an array section or an array element, all threads participating in the reduction
must use the same array section.
...
\item If a list item in a \code{reduction} clause on a worksharing construct is an
array section or an array element then the base pointer must point to the same variable
