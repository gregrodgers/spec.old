...
construct is encountered then: \begin{itemize}

    \item If the structure sibling list item does not appear in a \code{map}
    clause on the construct then: \begin{itemize} 
...
\begin{ccppspecific}
If a list item in a \code{map} clause is a variable of type pointer, and the
...
base expression is a variable of type pointer, and the pointer variable has a
...
\item If a list item is an array section, it must specify simply contiguous storage.

\item At most a list item can be an array item derived from a given variable in
two \code{map} clauses on the same construct as long as they are equivalent
list items and one of the \code{map} clauses has a \plc{map-type} of
\code{delete}.

\item At most one list item can be an array item derived from a given variable in a \code{map}
clause.

\item At most two list items from multiple \code{map} clauses on the same
construct can share original storage as long as they are equivalent list items and
one of the \code{map} clauses has a \plc{map-type} of \code{delete}.

\item List items in a \code{map} clause must not share original storage.
