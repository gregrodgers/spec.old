...
% \item If an array section is a list item in a \code{map} clause on the
% \code{target} construct, the base expression of the array section is a scalar
% variable for which the type is pointer, and that variable does not appear in a
% \code{map} clause on the construct then the variable is firstprivate.

\item If an array section having a named pointer is a list item in a
...
\code{map} clause on the same construct, then the effects of a \code{map} clause
on the pointer variable and a \code{map} clause on the array section are
ordered as follows:
...
If an array section having a named pointer is a list item in a \code{map}
clause and there exists a corresponding pointer variable to the named pointer
in the device data environment when the effects of the \code{map} clause
occur, then:
...
%when the effects of the \code{map} clause occur, then:
...
corresponding array section. The corresponding pointer variable is now defined
as an \emph{attached corresponding pointer} variable for the duration of its
lifetime in the device data environment.
...
incremented because of the effects of a \code{map} clause on the
construct then:
\begin{enumerate}
...
\item If the \plc{map-type} is \code{to} or \code{tofrom}, then the
    corresponding list item is assigned the value of the original list item.
\end{enumerate}
...
\begin{note}
If the effects of \code{map} clauses on a construct would assign the
corresponding list item the value of the original list item more than once,
then an implementation is allowed to ignore redundant assignments, but must
perform at least one assigment.  
\end{note}
...
\item If the corresponding list item's reference count was not already
decremented because of the effects of a \code{map} clause on the
construct then:
\begin{enumerate}
...
\begin{enumerate}
    \item If the corresponding list item is not an attached corresponding pointer, then the original list item is assigned the value of the corresponding list item.
\end{enumerate}
\end{enumerate}
...
\begin{note}
If the effects of \code{map} clauses on a construct would assign the
original list item the value of the corresponding list item more than once,
then an implementation is allowed to ignore redundant assignments, but must
perform at least one assigment.  
\end{note}
...
%\item At most one list item can be an array item derived from a given variable
%in map clauses of the same construct.

%\item If more than one list item of \code{map} clauses on the same construct
%are array items that refer to the same array, they must indicate identical
%original storage.
% Examples:
% map(a, a)          ok
% map(a[10],a[10])   ok
...
\item If more than one list item of \code{map} clauses on the same construct
are array items having the same named array, they must indicate
identical original storage.

%\item List items of \code{map} clauses on the same construct must not share
%original storage, unless they indicate identical storage and refer to the
%same variable.
% Examples:
% map(x, x)              ok
% map(x, y)              not ok if x and y alias eachother
% map(S, S.a)            ok, since S is treated as if its replaced with each of its
%                        structure elements.
% map(a[0:10], b[10:10]) ok if storage doesn't overlap, even if a = b.
...
% Examples? Not sure that we still need this.
