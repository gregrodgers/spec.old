...
\item Any procedure used in the \code{initializer} clause or \plc{combiner}
      expression must either be elemental or have scalar dummy arguments
      if the list item in the corresponding \code{reduction} clause is an array
      or array section.
\item If a generic name is used in the \plc{combiner} expression and
      the list item in the corresponding \code{reduction} clause is an array or
      array section, its interface must not have any specific procedures
      to which it can be resolved and that have array dummy arguments.
