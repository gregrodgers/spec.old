...
construct, the variable is treated as it if appeared in a single \code{map}
clause where the \plc{map-type} is a combination of the \plc{map-types} that appear
in each clause.

The rules for combining \plc{map-types} are as follows:

\begin{itemize}
\item map(alloc, x) and map(to, x) is map(to,x)
\item map(alloc, x) and map(from, x) is map(from, x)
\item map(to, x) and map(from, x) is map(tofrom, x)
\item map(release, x) and map(from, x) is map(from,x)
\item map(delete, x) and map(from,x) is map(delete,x)
\item map(delete, x) and map(release,x) is map(delete,x)
\end{itemize}

NOTE: map(alloc:S.a,S.b.S.c) map(to:S.a) map(from:S.b) // to:S.a, from:S.b, alloc:S.c

NOTE: is the combining rule c/c++ specific?
...
NOTE: struct \{int a, b, c\} S; map(S) -> map(S.a, S.b, S.c)

If a structure element is a list item in a \code{map} clause then all the other
structure elements from the containing structure variable form a \pcl{structure
...
\begin{itemize}
  ...
  \code{target}~\code{data}~\code{enter} construct is encountered then, the
...
NOTE: target data enter map(S) // map(S.a, S.b, S.c)

NOTE: target data enter map(to:S.b) // map(to:S.b) map(alloc:S.a, S.c)

NOTE: target data exit map(from:S.a) // map(from:S.a) map(release:S.b, S.c)

If a structure element is a list item in a \code{map} clause on a
\code{target}~\code{data}~\code{exit} construct and it has a \plc{map-type} of
...
NOTE: target data exit map(delete:S.a) // map(delete:S.a, S.b, S.c)

NOTE: BEGIN ARRAY SECTION POINTER ATTACH HERE:

If a list item in a \code{map} clause is a variable of type pointer, and the
variable is the lvalue expression in an array section that is a list item in a
...
NOTE: map(S.a) map(S.a[:100])  // on entry 1) map(S.a) then 2) map(S.a[:100])
NOTE:                          // on exit  1) unmap(S.a[:100]) then 2) unmap(S.a)

NOTE: a variable that appears in the to clause of a declare target construct
will be treated as if it had appeared in a map clause...

If an array section is a list item in a \code{map} clause, the array
section's lvalue expression is a variable of type pointer, and the variable has
a reference count >= 1  then:

NOTE: Attach top-down rule. The pointer is (must be) mapped 'first', then
map the array section and then attach. Then Detach bottom-up.

NOTE: Unless the lvalue-expression is a variable of type array, array sections
only map the array section (the pointed-to memory)!  Not the lvalue-expression.
Even if the lvalue is a simple pointer variable.  The pointer variable must
appear in map clause or declare target - mapped explicitly.  Of course, on
the target construct, if the pointer variable is a scalar (not a struct member)
it is private in the target region.  But this text here is about attachment
to mapped pointers.
