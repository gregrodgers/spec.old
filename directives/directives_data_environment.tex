...
The \code{iterator} clause specifies an identifier that will expand to multiples values in the directive on which it appears.
...
\item \plc{iterator-type} is the type of the iterator.
...
\item In an \plc{iterator-specifier}, if the \plc{iterator-type} is not specified then the type of that iterator is an integer type of default kind.
\end{fortranspecific}
