...
For constructs on which the \code{nodep} clause is not present, the behavior
of the \code{private} clause is as follows. Each task that references a list
...
For constructs on which the \code{nodep} clause is present, the behavior of
...
unspecified. If a SIMD construct or a task does not reference a list item that
...
\code{private} clause had been used. For the \code{loop} construct, one or
more private copies of each list item are created and initialized, as if the \code{private}
clause had been used. At the end of a region corresponding to an above construct for
