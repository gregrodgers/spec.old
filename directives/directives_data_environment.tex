...
If a list item in a \code{map} clause is a structure variable then it is
treated as if each structure element variable appears as a list item in the
clause with the structure variable's map-type.

NOTE: struct \{int a, b, c\} S; map(S) -> map(S.a, S.b, S.c)

NOTE: This enables structure variables to make sense in the mapping algorithm.
According to the algorithm, each structure element would get a ref-count increment,
but an implementation can optimize that to one reference count for the whole
structure.  We also need it to union map-types.

If a variable appears as a list item in multiple \code{map} clauses on the same
construct, the variable is treated as it if appeared in a single \code{map} clause
where the map-type is the union of the map-types that appear in each clause.

NOTE: map(alloc:S) map(to:S.a) map(from:S.b) // to:S.a, from:S.b, alloc:S.c

If a variable with type pointer appears as a list item in a \code{map} clause and the
variable is the lvalue expression in an array section that appears in a \code{map}
clause on the same construct, then:

\begin{itemize}

\item If the \code{map} clauses appear on a \code{target},
\code{target}~\code{data}, or \code{target}~\code{enter}~\code{data} construct
then on entry to the region the effects of the map clause on the pointer
variable are ordered to occur before the effects of the map clause on the array
section.

\item If the \code{map} clauses appears on a \code{target},
\code{target}~\code{data}, or \code{target}~\code{exit}~\code{data} construct
then on exit from the region the effects of the map clause on the array
section are ordered to occur before the effects of the map clause on the
pointer variable.
...
If an array section is a list item in a \code{map} clause and the array section's
lvalue expression is a pointer variable (a variable with type pointer) and
the pointer variable has a reference count >= 1  then:
...
enovironment, the corresponding pointer variable is initialized with the
address of the corresponding array section.
...
original array section. (NOTE: detach bottom-up, door open for shadow pointers).
...
If a structure element from a strucure variable appears as a list item in a
\code{map} clause on a \code{target}~\code{data}~\code{exit} construct then
each structure element contained in the structure variable is treated as if it
appears as a list item in a \code{map} clause on the construct with a map-type
of release.

NOTE: target data exit map(from:S.a) // map(release:S.a S.b, S.c)
NOTE: map(from:S.a) UNION map(release:S.a) = map(from:S.a)

If a structure element from a structure variable appears as a list item in a 
\code{map} clause on a \code{target}~\code{data}~\code{exit} construct and it 
has a map-type of delete then each structure element contained in the structure
vairable is treated as if it appears as a list item in \code{map} clause on
the construct with a map-type of delete.
