...
\item The storage location of a list item that appears in an
\code{in_reduction} clause of a task-generating construct must correspond to
the storage location of a list item that appears in a \code{task_reduction}
clause of a construct giving rise to a taskgroup region that includes the
participating task in its \plc{taskgroup set}.  Alternatively, the storage
location of the list item must correspond to the storage of a private copy,
resulting from a \code{task_reduction} or \code{in_reduction} clause, used to
perform the task reduction. The construct corresponding to the innermost region
that meets this condition must specify the same \plc{reduction-identifier} as
