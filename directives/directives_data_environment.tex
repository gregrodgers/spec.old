...
\plc{[initializer-clause]} %$ Close off misinterpreted dollar sign/math symbol
\end{ompfPragma}
...
target, which has the same rank and bounds as the pointer target of
...
\item For each part of the list item that is an attached pointer, that
  ...
  associated with the same pointer target that it was associated with
  immediately prior to the effect of the \code{map} clause; and
\end{fortranspecific}
\item[a)] For each part of the list item that is not an attached pointer, the value
...
\item The pointer target with which a pointer that has a corresponding
    attached pointer is associated may not change for the duration of the
    lifetime of the list item to which the corresponding pointer is attached
