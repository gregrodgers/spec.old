...
explicitly specified with the \verb`mapper` map-type-modifier, then the map-type
of an element given a map-type in the mapper is limited to the lower of the two
map-types.

If a mapper is specified for the type being mapped, or explicitly specified with
the \verb`mapper` map-type-modifier, then all map clauses that appear on the
...
expanded to map an appropriate array section, then map each element according to
the mapper.
...
\begin{enumerate}
  ...
  \item If an \verb`alloc` mapper stage is specified it is invoked
\end{enumerate}
...
\item If the \plc{map-type} is \code{to} or \code{tofrom}, then:
    \begin{itemize}
      ...
      \item If \verb`pack_to` and \verb`unpack_to` mapper stages are specified
        for this list-item, they are invoked to update the variable in the
        device data environment, otherwise:
...
\begin{enumerate}
  ...
  \item If a corresponding list item of the original list item is present in the device data environment, then:
    \begin{enumerate}
        ...
        \begin{enumerate}
          \item If the \plc{map-type} is \code{from} or \code{tofrom} then:
            \begin{itemize}
              ...
              \item If \verb`pack_from` and \verb`unpack_from` mapper stages are specified
                for this list-item, they are invoked to update the variable in the
                host data environment, otherwise:
      ...
      \item If the corresponding list item's reference count is finite, then:
        \begin{enumerate}
          \item If the corresponding list item's reference count was decremented due to
            this map then:
            \begin{enumerate}
              \item If the \plc{map-type} is not \code{delete} and a \verb`release` mapper
                stage is specified, it is invoked
            \end{enumerate}
          \item If the \plc{map-type} is \code{delete} and a \verb`delete` mapper stage
