...
\item For each part of the list item that is an attached pointer, that part 
      ...
      prior to the effect of the \code{map} clause; and
...
The \code{defaultmap} clause explicitly determines the data-mapping attributes of 
referenced in a \code{target} construct and would otherwise be implicitly determined 
(see \specref{subsec:Data-Mapping Attribute Rules, Clauses, and Directives}).
...
\restrictions
The restrictions to the \code{declare}~\code{mapper} clause are as follows:
...
\item Each \plc{map-type-modifier} can appear at most once on the
      \code{map} clause.
 \item A \plc{mapper-identifier} may not be re-declared in the current scope for the 
