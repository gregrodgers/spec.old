...
\item A variable that is of type pointer is treated as if it is the named
...
\begin{ccppspecific}
%If \plc{item1} is a list item in a \code{map} clause that is a variable \plc{x} of pointer type
%or is an array item that has a base expression \plc{x}, \plc{item2} is
%another list item in a \code{map} clause that is an array section that has
%a named pointer, and \plc{x} lexically appears in the lvalue expression for the
%named pointer of \plc{item2}, then:
...
same construct that has a named pointer that is, or is part of,
