...
\begin{itemize}
\item In a \code{target} region, but outside the construct, variables are shared if they are \code{declare}~\code{target} variables or if the \code{unified\_shared\_memory} requirement is specified.  In all other cases acces to them is unspecified behavior. 
\end{itemize}
