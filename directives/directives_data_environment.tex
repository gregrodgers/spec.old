...
If a list item in a map clause is a structure variable then it is treated as if
each structure element variable appears as a list item in the clause with the
structure variable's map-type.

NOTE: struct \{int a, b, c\} S; map(S) -> map(S.a, S.b, S.c)

If a variable appears as a list item in multiple map clauses on the same
construct, the variable is treated as it if appeared in a single map clause
where the map-type for the variable is the union of the map-types that
appear in each clause.

NOTE: map(alloc:S) map(to:S.a) map(from:S.b) // to:S.a, from:S.b, alloc:S.c

If a variable with type pointer appears as a list item in a map clause and the
variable is the lvalue expression in an array section that appears in a map
...
If an array section is a list item in a map clause and the array section's
...
NOTE: Attach top-down rule. The pointer is (must be) mapped 'first' and then attach when
the array section is mapped.

\begin{enumerate}

\item Prior to the execution of the construct, if the array section has a
reference count of 0, the corresponding pointer variable is initialized with the
address of the corresponding array section in the device data environment. 

\item OR: When a corresponding array section is created in the device data
enovironment, the corresponding pointer variable is initialized with the
address of the corresponding arrary section.

NOTE: detach bottom-up rule.

\item Regardless of the map-type, the value of the corresponding pointer
variable is never assigned to the original pointer variable. 

\item Regardless of the map-type, and after initialization, the value of the
original pointer variable is never assigned to the corresponding pointer
variable. 

\item OR: When a corresponding array section is removed from a device data
environment, the corresponding pointer variable is assigned the address
of the original array section. (The start of shadow pointers).

\item The pointer variable cannot be modified while the array section is
mapped. (until we support shadow pointers) 

\end{enumerate}

If structure elements from a variable with structure type are explicit list
items in a map clause on a target data exit construct then a structure element
not listed in a map clause is implicitly treated as if it had appeared in a
map clause on the construct with a map-type of release.

NOTE: target data exit map(from:S.a) // map(release:S.b, S.c)

If structure elements from a variable with structure type are explicit list
items in a map clause on a target data exit construct and at least one of the
structure element variables has a map-type of delete then all the structure
elements are treated as if they had appeared in a map clause with a  map-type of delete.
