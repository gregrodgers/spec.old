...
pointer target is a list item inthe clause.  The corresponding pointer
is associated with the corresponding pointer target in the device data
environment.  The corresponding pointer becomes an attached pointer
unless it is already associated with the corresponding pointer target.

If a list item is a structure component that is not a pointer and if
the rightmost \plc{part-ref} except the last is a pointer or a pointer
component, then the corresponding pointer is associated with the
pointer target.  For any other \plc{part-ref} that is pointer or
pointer component must be associated with the pointer target in the
device data environment.  Any component of the pointer target that is
not explicitly specified is not accessible in the device data
environment.
\end{fortranspecific}
