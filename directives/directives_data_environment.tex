...
This section describes how the data-sharing attributes of variables in data environments are determined. 
%\code{target}, \code{parallel}, \code{task}, \code{taskloop}, \code{simd}, and worksharing regions are determined. 
...
Specifying a variable on a \code{firstprivate}, \code{lastprivate}, \code{linear}, \code{reduction}, \code{copyprivate},
or \code{iterator} clause of an enclosed construct causes an implicit reference to the 
...
\subsubsection{\code{iterator} Clause}
\index{iterator@{\code{iterator}}}
\index{clauses!iterator@{\code{iterator}}}
\label{subsubsec:iterator clause}

\summary

The \code{iterator} clause specifies an identifier that will take a range of values in the directive on which it appears.

\syntax

The syntax of the \code{iterator} clause is as follows:

\begin{boxedcode}
iterator(iterator-specifier-or-list)
\end{boxedcode}

where iterator-specifier-or-list is either an iterator-specifier or an iterator-list.

The syntax of an iterator-specifier is one of the following:
...
iterator-type is an arithmetic type TODO:(and iterators)
range-specification is of the form \plc{lower,upper[,step]}
lower is an expression of the iterator-type type.
upper is an expression of the iterator-type type.
step is an expression of the iterator-type type.

The syntax of an iterator-list is as follows:

( iterator-specifier )[,( iterator-specifier )[,...]]

\descr

%If the iterator clause contains a list of iterator specifications, it will behave as if an iterator clause would have been specified for each %iterator specification.

Each iterator-specifier specifies an iterator. If no range-specification is specified then the iterator is an implicit iterator. If a range-specification is specified then the iterator is an explicit iterator. If the identifier is of the form \code{fwd(identifier)} then the iterator is a forward iterator.

If the identifier in the iterator-specifier is a variable, then it will be as if the variable had appeared in a \code{private} clause.

An implicit iterator is bound to a loop appearing in the construct that uses the identifier as the induction variable of a loop that has canonical form.

The iterator-type of an implicit iterator is that of the induction variable of the bound loop.

The range-specification for an implicit iterator is obtained from the bound loop according to the clauses where it appears.

The values of the iterator are the set of values i0...iN where i0=lb, i(j)=i(j-1)+step and 
\begin{ccppspecific}
\begin{itemize}
\item iN < ub and iN+1 >= ub if step > 0 
\item iN > ub and iN+1 <= ub if step < 0 
\end{itemize}
\end{ccppspecific}
\begin{fortranspecific}
...
An expression containing an iterator identifier can only appear in clauses that explicitly allow expressions containing iterators. For those clauses, the effect of having an iterator identifier would be as if the clause would appear multiple times substituting the iterator identifier for each of the possible values that iterator can take.

\restrictions

\begin{itemize}
\item An implicit iterator must not specify an iterator-type.
\item If the iterator is a forward iterator then a variable with that identifier must be declared in the construct on which the \code{iterator} clause appears.
\item If the step expression equals zero the behavior is unspecified.
