...
\begin{fortranspecific}
\begin{itemize}
\item If a list item in a \code{map} clause has the POINTER attribute
  and if the association status of the list item is associated, then
  it is treated as if the pointer target is a list item in the clause.

\item If a list item in a \code{map} clause is a pointer component of
  an array of derived type, the containing structure must be present
...
and \plc{list-item-type} is one of:
\begin{indentedcodelist}
  scalar
  aggregate
  pointer
...
\descr The \code{defaultmap} clause sets the implicit mapping behavior for a
target region.  The default set is equivalent to specifying
\code{defaultmap(tofrom:aggregate)}, \code{defaultmap(firstprivate:pointer)} and
\code{defaultmap(firstprivate:scalar)}.  All variables of a given list-item type
referenced in the construct that have implicitly determined data-mapping
attributes use the specified data-mapping attribute.  If no list-item type is
specified in the clause, the data-mapping attribute is used as the default for
all types.  The pointer type's implicit behavior determines the attribute used
to handle the storage of the pointer on the device, not the pointee, while the
default behavior for the pointee remains mapping a zero-length array section.
The only exception to this rule for pointers is the none attribute, which clears
both implicit behaviors and requires an explicit specification.

If a variable referenced in the construct is found to have the data-mapping
attribute none, the program is malformed.  
