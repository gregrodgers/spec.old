...
a scalar variable (or a nonallocatable nonpointer scalar variable, for
Fortran) is not mapped, but instead has an implicit data-sharing
...
\descr
% in multiple map clauses v1

%If a variable is a list item in multiple \code{map} clauses on the same
%construct, the variable is treated as if it appeared in a single \code{map}
%clause where the \plc{map-type} is a combination of the \plc{map-types} that
%appear in each clause.  

%The rules for combining \plc{map-types} are as follows:

%\begin{itemize}
%\item \code{map(alloc:x)} and \code{map(to:x)} is \code{map(to:x)}
%\item \code{map(alloc:x)} and \code{map(from:x)} is \code{map(from:x)}
%\item \code{map(to:x)} and \code{map(from:x)} is \code{map(tofrom:x)}
%\item \code{map(release:x)} and \code{map(from:x)} is \code{map(from:x)}
%\item \code{map(delete:x)} and \code{map(from:x)} is \code{map(delete:x)}
%\item \code{map(delete:x)} and \code{map(release:x)} is \code{map(delete:x)}
%\end{itemize}

% in multiple map clauses v2

%If on the same construct a variable is a list item in a \code{map} clause with
%a \plc{map-type} of \code{alloc} and a list item in a \code{map} clause with a
%\plc{map-type} of \code{to}, \code{from} or \code{tofrom} then the variable is
%treated as if it is a list item in a single \code{map} clause with a
%\plc{map-type} of \code{to}, \code{from} or \code{tofrom}, respectively.

%If on the same construct a variable is a list item in a \code{map} clause with a
%\plc{map-type} of \code{release} and a list item in a \code{map} clause with a
%\plc{map-type} of \code{from} or \code{delete} then the variable is
%treated as if it is a list item in a single \code{map} clause with a
%\plc{map-type} of \code{from} or \code{delete}, respectively.

%If on the same construct a variable is a list item in \code{map} clause with a
%\plc{map-type} of \code{from} and a list item in a \code{map} clause with a
%\plc{map-type} of \code{delete} then the effects of the \code{map} clause
%with the \plc{map-type} of \code{from} are ordered to occur before the
%effects of the \code{map} clause with the \plc{map-type} of \code{delete}.

% in multiple map clauses v3

%If a list item appears in two \code{map} clauses on the same construct
%then:
%\begin{itemize}
%
%    \item If one \code{map} clause has a \plc{map-type} of \code{alloc} and the
%    other \code{map} clause has a \plc{map-type} of \code{to}, \code{from} or
%    \code{tofrom} then the list item is treated as if it only appears in the
%    \code{map} clause with a \plc{map-type} of \code{to}, \code{from} or
%    \code{tofrom}.
%
%    \item If one \code{map} clause has a \plc{map-type} of \code{release} and
%    the other \code{map} clause has a \plc{map-type} of \code{from} or
%    \code{delete} then the list item is treated as if it only appears in the
%    \code{map} clause with a \plc{map-type} of \code{from} or \code{delete}.
%
%    \item If one \code{map} clause has a \plc{map-type} of \code{delete} and
%    the other \code{map} clause has a \plc{map-type} of \code{from} then the
%    effects of the \code{map} clause with the \plc{map-type} of \code{from} are
%    ordered to occur before the effects of the \code{map} clause with the
%    \plc{map-type} of \code{delete}.
%
%\end{itemize}

The \code{map} clause on a given construct controls the mapping between a list
item and its corresponding list item in the device data environment of the
device specified by the construct.
...
\code{map} clause, as detailed later in this section.  The \code{target} and
\code{target}~\code{data} constructs permit \code{alloc}, \code{to},
\code{from}, and \code{tofrom} to be the specified \plc{map-type} for a
\code{map} clause; if a \plc{map-type} is not specified, the \plc{map-type}
defaults to \code{tofrom}.  The \code{target}~\code{enter}~\code{data}
construct permits \code{alloc} and \code{to} to be the specified
\plc{map-type} for a \code{map} clause.  The
\code{target}~\code{exit}~\code{data} construct permits \code{release},
\code{from}, and \code{delete} to be the specified \plc{map-type} for a
\code{map} clause.  An \code{always} \plc{map-type-modifier} may appear with
the \code{to}, \code{from}, or \code{tofrom} \plc{map-type} in a
\code{map} clause.


%The \plc{map-type} and \plc{map-type-modifier} specify the effect of the
%\code{map} clause.  For each target device, a \emph{reference count}
%associated with the list item indicates the number of times the list item has
%been mapped to a device data environment minus the number of times the list
%item has been unmapped from the device data environment. The list item will
%have a corresponding list item in the device data environment as long as its
%reference count is greater than zero.  The \code{alloc} \plc{map-type} maps
%the list item to the device on entry to the \code{target},
%\code{target}~\code{data}, or \code{target}~\code{enter}~\code{data} region,
%and it causes the list item to be unmapped on exit from the \code{target} or
%\code{target}~\code{data} region. The \code{release} \plc{map-type} unmaps the
%list item from the device on exit from the
%\code{target}~\code{exit}~\code{data} region. The \code{delete} \plc{map-type}
%ensures that the corresponding list item, if it exists, is removed from the
%device data environment on exit from the \code{target}~\code{exit}~\code{data}
%region, and its reference count is set to zero. If the \code{to}, \code{from},
%and \code{tofrom} \plc{map-type} is specified, in addition to mapping or
%unmapping the list item from the device data environment it can cause an
%assignment to occur between the original list item and the corresponding list
%item as detailed below.

%Each \plc{map-type}, with an optional \plc{map-type-modifier}, has the
%following precedence ordering as indicated by the $\prec$ operator:
%
%\begin{itemize}
%    \item \code{alloc} $\prec$ \code{to}
%    \item \code{release} $\prec$ \code{from}, \code{delete}
%    \item \code{to} $\prec$ \code{tofrom}, \code{always}~\code{to}, \code{always}~\code{tofrom}
%    \item \code{from} $\prec$ \code{tofrom},  \code{always}~\code{from}, \code{always}~\code{tofrom}
%    \item \code{tofrom} $\prec$ \code{always}~\code{tofrom}
%    \item \code{always}~\code{to} $\prec$ \code{always}~\code{tofrom}
%    \item \code{always}~\code{from} $\prec$ \code{always}~\code{tofrom}
%\end{itemize}
%
%A list item is treated as if it did not appear in the \code{map} clause if the
%specified \plc{map-type} and optional \plc{map-type-modifier} has less
%precedence than the \plc{map-type} and optional \plc{map-type-modifier}
%specified in another \code{map} clause on the same construct in which the list
%item also appears.

%If a list item appears in a \code{map} clause with the \code{tofrom}
%\plc{map-type}, then:
%\begin{itemize}
%    \item If the \code{always} \plc{map-type-modifier} appears in the clause, the
%        behavior is as if the list item appears in a \code{map} clause with the
%        \code{to} \plc{map-type} and \code{always} \plc{map-type-modifier} and
%        in a \code{map} clause with the \code{from} \plc{map-type} and
%        \code{always} \plc{map-type-modifier}.
%
%    \item If the \code{always} \plc{map-type-modifier} does not appear in the
%        clause, the behavior is as if the list item appears in a \code{map}
%        clause with the \code{to} \plc{map-type} and in a \code{map} clause
%        with the \code{from} \plc{map-type}.
%\end{itemize}

%If a list item appears more than once in \code{map} clauses on the same
%construct having the same \plc{map-type} and optional \plc{map-type-modifier},
%all but one of the appearances are ignored.

%If the same list item appears in a \code{map} clause with the \code{to}
%\plc{map-type} and in a \code{map} clause with the \code{from} \plc{map-type}
%and both appearances are in clauses having the same specified
%\plc{map-type-modifier}, the behavior is as if the list item did not appear in
%either clause and instead the list item appeared in a \code{map} clause with
%the \code{tofrom} \plc{map-type} and the specified \plc{map-type-modifier}.

If the same list item appears in a \code{map} clause with the \code{from}
\plc{map-type} and in a \code{map} clause with the \code{delete}
\plc{map-type}, the behavior is as if the effect of the \code{map} clause with
the \code{from} \plc{map-type} is ordered before the effect of the \code{map}
clause with the \code{delete} \plc{map-type}.

If the same list item appears in a \code{map} clause with the \code{release}
\plc{map-type} and in a \code{map} clause with the \code{from}
\plc{map-type}, the behavior is as if the list item is ignored in the
\code{map} clause with the \code{release} \plc{map-type}.

If a list item in a \code{map} clause is a variable of structure type then it
is treated as if the list item did not appear and instead each structure
element that is part of the structure variable is a list item in the clause.

%If a structure element is a list item in a \code{map} clause then all the other
%structure elements from the containing structure variable form a \plc{structure
%sibling list}.  

%If a list item in a \code{map} clause is a structure element that is part of a
%given structure variable, the list item has an associated \plc{structure
%sibling list} consisting of all other structure elements that are part of the
%same structure variable, referred to as sibling structure elements. For each
%sibling structure element in the structure sibling list that
%has a reference count greater than zero when the construct on which the
%\code{map} clause appears is encountered:
%\begin{itemize}
%    \item If the construct is \code{target}, \code{target}~\code{data}, or
%        \code{target}~\code{enter}~\code{data} and the sibling structure element does
%        not appear as a list item in a \code{map} clause on the construct, it
%        is treated as if it is a list item in a \code{map} clause on the
%        construct with the \code{alloc} \plc{map-type}.
%    \item If the construct is \code{target}~\code{exit}~\code{data} and the
%        sibling structure element does not appear as a list item in a \code{map}
%        clause on the construct:
%        \begin{itemize}
%            \item If the list item appears in a \code{map} clause with the
%                \code{delete} \plc{map-type}, the sibling structure element is
%                treated as if it appears in the same \code{map} clause.
%            \item If the list item does not appear in a \code{map} clause with
%                the \code{delete} \plc{map-type}, the sibling structure
%                element is treated as if it appears in a \code{map} clause
%                with the \code{release} \plc{map-type}.
%        \end{itemize}
%\end{itemize}

% If a list item in a \code{map} clause is a structure element then all the other
% structure elements from the containing structure variable form a \plc{structure sibling list}.
% The \code{map} clause and the structure sibling list are
...
%     \end{itemize}
% 
%     %\item If the structure element is a list item in a \code{map} clause with a
%     %\plc{map-type} of \code{delete} 
% 
...
% \end{itemize}
  ...
  %\item If a corresponding list item of the structure sibling list item is
  %present in the device data environment when a \code{target},
  %\code{target}~\code{data} or \code{target}~\code{data}~\code{enter} construct
  %is encountered, then the structure sibling list item is treated as if it is a
  %list item in a \code{map} clause on the construct with a \plc{map-type} of
  %\code{alloc}.

  %\item If a structure sibling list item has a reference count >= 1 before a
  %\code{target}, \code{target}~\code{data} or
  %\code{target}~\code{enter}~\code{data} construct is encountered, then the
  %structure sibling list item is treated as if it is a list item in a
  %\code{map} clause on the construct with a \plc{map-type} of \code{alloc}.

  %\item If a corresponding list item of the structure sibling list item is
  %present in the device data environment when a
  %\code{target}~\code{exit}~\code{data} construct is encountered, then the
  %structure sibling list item is treated as if it is a list item in a
  %\code{map} clause on the construct with a \plc{map-type} of \code{release}.

  %\item If a structure sibling list item has a reference count >= 1 before a
  %\code{target}~\code{data}~\code{exit} construct is encountered then the
  %structure sibling list item is treated as if it is a list item in a
  %\code{map} clause on the construct with a \plc{map-type} of \code{release}.

%If a structure element is a list item in a \code{map} clause on a
%\code{target}~\code{exit}~\code{data} construct and it has a \plc{map-type} of
%\code{delete} then each list item in its associated structure sibling list is
%treated as if it is a list item in a \code{map} clause on the construct with a
%\plc{map-type} of \code{delete}.

\begin{ccppspecific}
If a list item in a \code{map} clause is a variable of pointer type, and the
variable is the base expression in an array section that is a list item in a
\code{map} clause on the same construct, then the effects of the \code{map} clause
on the pointer variable and the \code{map} clause on the array section are
ordered as follows:
...
then on entry to the region the effects of the \code{map} clause on the pointer
variable are ordered to occur before the effects of the \code{map} clause on
the array section.

\item If the \code{map} clauses appears on a \code{target},
\code{target}~\code{data}, or \code{target}~\code{exit}~\code{data} construct
then on exit from the region the effects of the \code{map} clause on the array
section are ordered to occur before the effects of the \code{map} clause on the
pointer variable.

\end{itemize}

If an array section is a list item in a \code{map} clause, the array section's
base expression is a variable of pointer type, and the pointer variable has a
corresponding pointer variable that is present in the device data environment
when the effects of the \code{map} clause occur, then when a corresponding
array section is created in the device data environment the corresponding
pointer variable is assigned the address of the corresponding array section.
The corresponding pointer variable is subsequently defined to be an
\emph{attached corresponding pointer} variable for the duration of its
lifetime in the device data environment. If an attached corresponding pointer
variable is modified on the device, the behavior is unspecified.

%If an array section is a list item in a \code{map} clause, the array
%section's base expression is a variable of type pointer, and the variable has
%a reference count >= 1  then:

%\begin{itemize}

%\item When a corresponding array section is created in the device data
%enovironment, the corresponding pointer variable is assigned the address of the
%corresponding array section.

%\item When a corresponding array section is created in the device data
%environment, the corresponding pointer variable is assigned the address of the
%corresponding array section. The corresponding pointer variable is
%subsequently defined to be an \emph{attached corresponding pointer} variable
%for the duration of its lifetime in the device data environment. If an
%attached corresponding pointer variable is modified on the device, the
%behavior is unspecified.

%[NOTE: after copying the address, the pointer on the device get a new property
%"attached", and that attached property remains on the pointer as long as the
%pointer is mapped. This is longer than between attach/detach, because we
%permanently erased the original value on the device]

%\item When a corresponding array section is removed from a device data
%environment, the corresponding pointer variable is assigned the address of the
%original array section. (NOTE: detach bottom-up, door open for shadow
%pointers).

%\item The value of an attached corresponding pointer variable is never assigned
%to the original pointer variable.

%[Note attached value are never brought back]

%\item The corresponding pointer variable cannot be modified while it is assigned
%the address of the corresponding array section.  (NOTE: until we support shadow
%pointers) 

%\item An attached corresponding pointer variable cannot be modified.

%[NOTE: address cannot be changed. It could be weakened, but its probably better
%to be stronger, at least initially ]

%\end{itemize}

\end{ccppspecific}

The \emph{corresponding enclosing list item} of a given list item is the
corresponding list item to a mapped list item, where the given list item is
part of the mapped list item but the mapped list item is not part of another
mapped list item.
...
\begin{enumerate}
\item If a corresponding enclosing list item of the original list item is present in the device data environment, then:
\begin{enumerate}
    \item The part of the corresponding enclosing list item that has corresponding storage to the original
        list item's storage becomes the corresponding list item to the original list item.
\end{enumerate}
\item If a corresponding enclosing list item of the original list item is not present in the device data environment, then:
\begin{enumerate}
...
\item The new list item becomes the corresponding list item to the original list item in the device data environment and is treated as the corresponding
    enclosing list item of the original list item.
\item The corresponding enclosing list item has a reference count that is initialized to zero. 
\end{enumerate}
\item The corresponding enclosing list item's reference count is incremented by one if
    it has not yet been incremented on entry to the region.
\item If the corresponding enclosing list item's reference count is one or the \code{always} \plc{map-type-modifier} is present, then:
\begin{enumerate}
\item If the \plc{map-type} is \code{to} or \code{tofrom}, then the
    corresponding list item is assigned the value of the original list item if
    it has not yet been assigned on entry to the region.
\end{enumerate}
\item If the corresponding enclosing list item's reference count is one, then:
\begin{enumerate}
...
\begin{enumerate}
\item If a corresponding enclosing list item of the original list item is not present in the device data environment, then the list item is ignored.
\item If a corresponding enclosing list item of the original list item is present in the device data environment, then:
\begin{enumerate}
    \item The part of the corresponding enclosing list item that has corresponding storage to the original
        list item's storage becomes the corresponding list item to the original list item.
\item If the corresponding enclosing list item's reference count is finite, then:
\begin{enumerate}
%\item the corresponding list item's reference count is decremented by one.
\item If the \plc{map-type} is not \code{delete}, then the corresponding
    enclosing list item's reference count is decremented by one if it has not yet been
    decremented on exit from the region.
\item If the \plc{map-type} is \code{delete}, then the corresponding enclosing list item's reference count is set to zero. 
\end{enumerate}
\item If the corresponding enclosing list item's reference count is zero or the \code{always} \plc{map-type-modifier} is present, then:
\begin{enumerate}
\item If the \plc{map-type} is \code{from} or \code{tofrom} and the
    corresponding list item is not an attached corresponding pointer, then the
    original list item is assigned the value of the corresponding list item if
    it has not yet been assigned on exit from the region.
\end{enumerate}
\item If the corresponding enclosing list item's reference count is zero, then the corresponding enclosing list item is removed from the device data environment 
\end{enumerate}
\end{enumerate}
...
%\item Two list items appearing in \code{map} clauses on the same construct
%    must not be array sections having base expressions that are lvalues with
%    the same address, except that list items indicating identical original
%    storage are permitted to appear once in a \code{map} clause with the
%    \code{from} \plc{map-type} and once in a \code{map} clause with the
%    \code{delete} \plc{map-type}.

\item If two different variables are list items appearing in \code{map} clauses on
    the same construct, they must not share storage.

\item If two different array items are list items appearing in \code{map} clauses on
    the same construct and they are derived from the same variable of array
    type, they must indicate identical storage.

\item If two different array items are list items appearing in \code{map} clauses on
    the same construct and they share storage, they must indicate identical
    storage.

%\item Two list items appearing in \code{map} clauses on the same construct
%    must not be array items derived from the same variable of array type,
%    except that list items indicating identical original storage are permitted
%    to appear once in a \code{map} clause with the \code{from} \plc{map-type}
%    and once in a \code{map} clause with the \code{delete} \plc{map-type}.

%\item If two list item of \code{map} clauses on the same construct share
%    original storage, they must be the same list item.
