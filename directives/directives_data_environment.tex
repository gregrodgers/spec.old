...
initialization.  If the list item is a derived type that has type
parameters with default values, the new list item has the same default
values.  Otherwise, the initial value of the new list item is
...
\specref{subsec:scan Directive}).  An unspecified number of private copies of
...
\plc{item}\textsubscript{2} is another list item in a \code{map} clause that has a named
pointer that is, or is part of, \plc{item}\textsubscript{1}, then:
