...
rules of map-type decay.  If a \code{map} clause with a \code{is_present} \plc{map-type} is present
its effect is ordered before all other \code{map} \plc{map-types}.  If a \code{mapper} \plc{map-type-modifier} is
present on a \code{is_present} \plc{map-type} the all elements of the object are treated as if they had a \code{is_present} \plc{map-type}.
...
If the \code{map} clause appears on a \code{target}, \code{target}~\code{data}, or \code{target}~\code{enter}~\code{data} construct with an \code{is_present} \plc{map-type} then on entry to the region if the corresponding list item does not appear in the device data envirionment an exception occurs and the program terminates.
