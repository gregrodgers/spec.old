...
\item The corresponding list item's reference count is incremented by one, if
    it has not yet been updated on entry to the region.
...
\item If the \plc{map-type} is \code{to} or \code{tofrom}, then the
    ...
    if it has not yet been assigned to on entry to the region.
\end{enumerate}
...
\item If the \plc{map-type} is \code{from} or \code{tofrom} and the
    ...
    if it has not yet been assigned to on exit from the region. 
\end{enumerate}
