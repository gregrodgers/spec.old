...
the variable in the program provided no \code{teams} construct that is not nested
inside of a \code{target} construct is encountered between the references and
the initial thread is not nested inside of a \code{teams} region.
For initial threads nested inside of a \code{teams} region, the values of data
in those initial threads' copies of a threadprivate variable are guaranteed
