...
The \code{map} clause determines whether each of its list items will have a
corresponding list item in the device data environment of the device specified
by the construct on which it is present.

The list items that appear in a \code{map} clause may include array sections and structure elements.

The \code{target} and \code{target}~\code{data} constructs permit
\code{alloc}, \code{to}, \code{from}, and \code{tofrom} to be the specified
\plc{map-type} for a \code{map} clause; if a \plc{map-type} is not specified,
the \plc{map-type} defaults to \code{tofrom}.  The
\code{target}~\code{enter}~\code{data} construct permits \code{alloc} and
\code{to} to be the specified \plc{map-type} for a \code{map} clause.  The
...
the the \code{to}, \code{from}, or \code{tofrom} \plc{map-type} in a
\code{map} clause.


%If a list item appears in a \code{map} clause with the \code{tofrom}
%\plc{map-type}, the behavior is as if, instead, the list item appeared in a
%\code{map} clause with the \code{to} \plc{map-type} and once in a \code{map}
%clause with the \code{from} \plc{map-type}. If the \code{always}
%\plc{map-type-modifier} was specified, the behavior is as if it is applied to
%both \code{map} clauses.
...
If a list item has original storage that is fully contained by the original
storage of a previous list item that has a corresponding list item present in
the device data environment, the behavior is as if the list item was replaced
in the \code{map} clause with the previous list item.

Each \plc{map-type}, with an optional \plc{map-type-modifier}, has
the following precedence ordering as indicated by the $\prec$ operator:
...
\end{enumerate}
\item The corresponding list item's reference count is incremented by one if
...
\item If the \plc{map-type} is not \code{delete}, then the corresponding list
    item's reference count is decremented by one if it has not yet been
