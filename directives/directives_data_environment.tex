...
\begin{boxedcode}
iterator(\plc{iterator-specifier-or-list})
\end{boxedcode}
where \plc{iterator-specifier-or-list} is either an \plc{iterator-specifier} or an \plc{iterator-list}.
...
\item \plc{iterator-type} is an arithmetic type TODO:(and iterators).
\item \plc{range-specification} is of the form \plc{lower,upper[,step]} where lower, upper and step are expressions of the iterator-type type.
\end{itemize}

The syntax of an \plc{iterator-list} is as follows:
\begin{indentedcodelist}
( \plc{iterator-specifier} )[, \plc{iterator-list}]
\end{indentedcodelist}
...
Each \plc{iterator-specifier} specifies an iterator. If no \plc{range-specification} is specified then the iterator is an implicit iterator. Otherwise, if a \plc{range-specification} is specified, the iterator is an explicit iterator. If the \plc{iterator-identifier} is of the form \code{fwd(\plc{identifier})} then the iterator is a forward iterator.

If the \plc{identifier} in the \plc{iterator-specifier} is a variable, then it will be as if the variable had appeared in a \code{private} clause.

An implicit iterator is bound to a loop appearing in the construct that uses the \plc{identifier} as the induction variable of a loop that has canonical form.

The \plc{iterator-type} of an implicit iterator is that of the induction variable of the bound loop.

The \plc{range-specification} for an implicit iterator is obtained from the bound loop according to the clauses where it appears.
...
\begin{itemize}
\item An implicit iterator must not specify an \plc{iterator-type}.
