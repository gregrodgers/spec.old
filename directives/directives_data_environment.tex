...
\begin{ccppspecific}
\begin{itemize}
\item Variables that appear in \code{threadprivate} directives or
      variables with the \code{_Thread_local} (in C) or
      \code{thread_local} (in C++) storage-class specifier are threadprivate. 
...
\item The \code{__func__} variable and similar function-local
...
\item The \code{__func__} variable and similar function-local
      predefined variables may be listed in a \code{shared} or
...
\item If it has the \code{POINTER} attribute, each copy will have the
...
A privatized list item may be a selector of an \code{ASSOCIATE} or
...
\end{ompSyntax}
where \plc{data-sharing-attribute} is one of the following:
...
\descr
If \plc{data-sharing-attribute} is \code{shared} or, for Fortran,
\code{firstprivate} or \code{private}, the data-sharing attribute of all
variables referenced in the construct that have implicitly determined
data-sharing attributes will be \plc{data-sharing-attribute}.

\begin{ccppspecific}
If \plc{data-sharing-attribute} is \code{firstprivate} or \code{private}, each
...
list item is an array or array section, the identifiers to which \code{omp_in} and
\code{omp_out} correspond each refer to an array element.
\code{omp_out} holds the final value of the combiner operation.
...
each original list item is assigned the value described in \specref{subsec:scan Directive}.
...
\item A \code{reduction} clause with the \code{inscan} \plc{reduction-modifier}
      may only appear on a worksharing-loop construct, a \code{simd} construct
      or a combined or composite construct for which any of the
      aforementioned constructs is a constituent construct and
      \code{distribute} is not a constituent construct.
...
\syntax
The syntax of the \code{task_reduction} clause is as follows:
...
\syntax
The syntax of the \code{in_reduction} clause is as follows:
...
\syntax
\begin{cspecific}
The syntax of the \code{declare}~\code{reduction} directive is as follows:
...
If a generic name is used in the \plc{combiner} expression and the list item
in the corresponding \code{reduction} clause is an array or array section, it
...
\begin{fortranspecific}
\item Any selectors in the designator of \code{omp_in} and
      \code{omp_out} can only be \plc{component}~\plc{selectors}.
...
\item Any procedure used in the \code{initializer} clause or \plc{combiner}
      expression must either be elemental or have dummy arguments that
      are scalar if the list item in the corresponding
      \code{reduction} clause is an array or array section.
\end{fortranspecific}
\end{itemize}
...
\item If a variable appears in a \code{to} or \code{link} clause on a
      \code{declare}~\code{target} directive that does not have a
      \code{device_type(nohost)} clause then it is treated as if it had
...
The \plc{map-type} and \plc{map-type-modifier} specify the effect of
the \code{map} clause, as described below.
...
rules of map-type decay.  If a \code{map} clause with a \code{present}
...
the \code{present} modifier.  If a \code{mapper} \plc{map-type-modifier}
is present along with a \code{present} \plc{map-type-modifier} then all
elements of the object are treated as if they had a \code{present} modifier.
...
The list items that appear in the \code{map} clause may reference iterators
...
are guaranteed to share storage if the \code{map} clause appears on a
\code{target} construct that corresponds to an inactive \code{target} region,
or if it appears on a \code{target}~\code{data},
\code{target}~\code{enter}~\code{data}, or
\code{target}~\code{exit}~\code{data} construct that applies to the device data
...
construct with a \code{present} \plc{map-type-modifier} then on entry to
...
\item If a list item is an array section, it must specify contiguous storage.
\item If an expression that is used to form a list item in a \code{map} clause
...
\item If an array appears as a list item in a \code{map} clause, multiple parts
      ...
      \code{map} clause, and the corresponding storage for those
      parts were created by maps from more than one earlier
      construct, the behavior is unspecified.
...
\begin{crossrefs}
\item Array sections, see \specref{subsec:Array Sections}.
...
If \plc{implicit-behavior} is \code{none}, each variable referenced in the
...
an \code{is_device_ptr} clause.
If \plc{implicit-behavior} is \code{default},
then the clause has no effect for the variables in the category specified by
\plc{variable-category}.
If \plc{implicit-behavior} is \code{present}, each variable  referenced in the
construct in the category specified by \plc{variable-category} is treated as if it
had been listed in a \code{map} clause with the \plc{map-type} of \code{alloc}
and \plc{map-type-modifier} of \code{present}.
