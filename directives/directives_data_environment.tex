...
When the \code{notinscan} modifier appears on the clause, the reduction clause
behaves as if there were no \plc{reduction-modifer} specified. When the \code{inscan} 
modifier appears on the clause, each list item in the clause will participate in the 
\plc{scan} (see Section \ref{sec:scan Directive}) computation of each logical iteration.
...
Each task, thread or SIMD lane that performs an \plc{inscan} reduction for a
given list item receives two new list items: a private copy (as if the private
clause was used), and additionally a \plc{scan} copy. The scan copy for the
task, thread or SIMD lane that executes logically first iteration is
initialized with the value of the original list item. If the task or SIMD lane
doesn’t execute logically first iteration, the \plc{scan} copy is initialized
with initializer value of the reduction operator. The private copy of the list
item is assigned the initial value according to the reduction indentifier
after entering the loop region with scan computation. The list item in the
loop region is not replaced with the private copy but instead is with the scan
copy. The scan copy is then combined with the scan value over all other
private copies communicated from the scan computation of each preceding
iteration, using  the combiner from reduction-identifier.  At the end of the
loop region, the original value is assigned the scan copy from the last
logical iteration. 
...
%If the \code{task} modifier is present for a \code{simd} or \code{taskloop}
%construct, it has no effect.  
%
%If the \code{task} modifier is present for a \code{parallel} or worksharing
%construct, the code that would otherwise have been performed in each implicit
%task for the construct, not including invocation of the combiner, behaves as
%if it is part of an included task that participates in a task reduction for
%each list item. For each implicit task, the task reductions are performed as
%if they are defined by a \code{taskgroup} construct on which the
%\code{task_reduction} clause appears with the same \plc{reduction-identifier},
%where the corresponding \code{taskgroup} region binds to the implicit task and
%encloses only the included task. If the \code{task} modifier is not present
%for a \code{parallel} or worksharing construct, these task reductions are not
%performed.
%
%If the \code{task} modifier is present for a \code{teams} construct, the code
%that would otherwise have been performed by each team in the league, not
%including invocation of the combiner, behaves as if it is part of an included
%task that participates in a task reduction for each list item.  For each team,
%the task reductions are performed as if they are defined by a \code{taskgroup}
%construct on which the \code{task_reduction} clause appears with the same
%\plc{reduction-identifier}, where the corresponding \code{taskgroup} region
%binds to the initial task associated with the team and encloses the included
%task. If the \code{task} modifier is not present for a \code{teams} construct,
%these task reductions are not performed.
%
%If the \code{task} modifier is present for a \code{target} construct, the code
%that would otherwise have been performed in the \code{target} region, not
%including invocation of the combiner, behaves as if it is part of an included
%task that participates in a task reduction for each list item. The task
%reductions are performed as if they are defined by a \code{taskgroup}
%construct on which the \code{task_reduction} clause appears with the same
%\plc{reduction-identifier}, where the corresponding \code{taskgroup} region is
%nested inside the \code{target} region and encloses the included task. If the
%\code{task} modifier is not present for a \code{target} construct, these task
%reductions are not performed.
