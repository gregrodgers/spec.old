...
\begin{ccppspecific}
\subsubsection{Pointer Translation for Device Data Environments}
\label{subsec:pointer translation}

This section describes how the value of a pointer may be translated to point
...
%If a matching mapped list item is found, the translated value of the pointer
%is such that an indirect reference to the matching mapped list item that uses
%the original value of the pointer becomes an indirective reference to its
%corresponding list item in the device data environment when the translated
%value is instead used.

If a matching mapped list item is found, the pointer is assigned a translated
value such that the corresponding list item in the device data environment can
be accessed through the pointer in a \code{target} region.

If a matching mapped list item is not found, the translated value of the
pointer is NULL.
\end{ccppspecific}
