...
link(\plc{list})
implements(\plc{list})
device_type(\plc{host \textnormal{|} nohost \textnormal{|} any})
\end{indentedcodelist}
\end{ccppspecific}
...
The \code{device\_type} specifies if a version of the procedure should be made 
available on host, device or both. If \plc{host} is specified only host version
of the procedure is made available.  If \plc{nohost} is specified then only device
version of the procedure is made available.  If \plc{any} is specified then both 
...
All calls in \code{target} construct to the function in the \code{implements} clause 
...
If a \code{declare}~\code{target} directive has \code{implements} or \code{device\_type} clause
