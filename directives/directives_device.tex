...
private version is assigned to a device pointer to the corresponding list items
in the device data environment.  List items that appear in a \code{use_device}
clause have the address of the corresponding object in the device data
environment inside the construct.  For objects, any reference to the value of
the object will be to the corresponding object on the device, while references
to the address will result in a valid device address pointing to that object.
...
environment.  If one or more of the \code{use_device_ptr} or \code{use_device}
clauses and one or more \code{map} clauses are present on the same construct,
this conversion will occur as if performed after all variables are mapped
according to those \code{map} clauses.
...
\begin{itemize}
  ...
  \item A list item in a \code{use_device} clause must have a
    corresponding list item in the device data environment.
...
the construct.  If the iteration count of any loop associated with the
\code{distribute} construct that does not enclose the intervening code is zero
