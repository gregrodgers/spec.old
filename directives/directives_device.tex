...
mapper-identifier \code{default}, is as follows unless a custom mapper is
...
No other instances of the current type can be mapped as part of the mapper,
whether directly or indirectly through another type.
...
declared at the same point in the program such that the following is valid.
...
\begin{boxedcode}
  ...
  void foo() {
    C<int> c;
    \#pragma omp target map(mapper(C<int>::Cmap): c) // Cmap is used
    c.p[0] = 1;
    assert(c.p[0] == 1);
    \#pragma omp target map(c) // Cmap is used
    c.p[0] = 5;
    assert(c.p[0] == 5);
  }
\end{boxedcode}
