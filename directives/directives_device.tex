...
environment.  If a \code{mapper()} modifier appears in a to clause, each list
item is replaced with the list items mapped with a \code{to} or \code{tofrom}
map-type according to the rules of map-type decay. If a \code{mapper()} modifier
appears in a from clause, each list item is replaced with the list items mapped
with a \code{from} or \code{tofrom} map-type according to the rules of map-type
decay.
