...
\item A \plc{map-type} in a \code{map} clause must be \code{to}, \code{from}, 
      \code{tofrom} \code{alloc} or \code{is_present}.
...
\item A \plc{map-type} must be specified in all \code{map} clauses and must be 
      either \code{to} \code{alloc} or \code{is_present}.
...
\begin{indentedcodelist}
to([required][[,]mapper(\plc{mapper-identifier}):]\plc{locator-list})
from([required][[,]mapper(\plc{mapper-identifier}):]\plc{locator-list})
...
in the device data environment and there is no \code{required} modifier in the clause,
then no assignment occurs to or from the original list item.  If a \code{required} modifier
is present in the clause and the corresponding list item is not present in the device data
