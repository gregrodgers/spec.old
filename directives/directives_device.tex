...
\item Memory allocators that do not appear as list items in an \code{uses_allocator} clause cannot appear as an allocator in an \code{allocate} clause or be used in the \code{target} region unless a \code{requires} directive with the \code{dynamic_allocators} clause is present in the same unit scope.
