...
If a \code{device} clause with the \code{device_num} modifier is present on the construct, the
\code{device} expression specifies the device number of the target device. If
...
If a \code{device} clause with the \code{ancestor} modifier is present on the
\code{target} construct and the \code{device} expression evaluates to 1,
execution of the \code{target} region occurs on the parent device.  If the
\code{target} construct is not encountered in a \code{target} region or is
encountered in an inactive target region, the current device is treated as the
...
\item If a \code{device} clause with the \code{ancestor} modifier is present
    on the construct, then the following restrictions apply:
\begin{itemize}
   \item A \code{requires} directive with the \code{reverse_offload}
       clause must be present in the same compilation unit.
   \item The \code{device} expression must evaluate to 1.
