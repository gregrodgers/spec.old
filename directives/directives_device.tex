...
determined by the \plc{default-device-var} ICV. Variables are mapped for the
extent of the region, according to any data-mapping attribute clauses, from the
data environment of the encountering task to the device data environment. When
an \code{if} clause is present and the \code{if} clause expression evaluates to
\plc{false}, the device is the host.
...
the \code{use_device_ptr} and \code{use_device_addr} clauses.
...
% TODO: consider changing this to keep host value without unified_address
...
\restrictions
\begin{itemize}
% NOTE: removed target, because ancestor offload requires it
\item If a \code{target}~\code{update},
...
\def\marked{marked-to}

For the purpose of this section, the term ``\marked'' applied to a function or variable refers to that function or variable being treated as if it appeared as a list item in a \code{to} clause on a \code{declare}~\code{target} directive.

\begin{ccppspecific}
If a function is \marked in the same translation unit in which the definition of
the function occurs then a device-specific version of the function is created.

If a variable is \marked in the same translation unit in which the definition
...
\begin{fortranspecific}
If an internal procedure is \marked
then a device-specific version of the procedure is created.

If a variable that is host associated is \marked
...
If a variable is \marked then the corresponding list
...
\begin{ccppspecific}
If a function is referenced in a function that is \marked
then the referenced function is also \marked.
...
static storage duration that is \marked then the referenced variable or
function is \marked.
...
\begin{fortranspecific}
If a procedure is referenced in a procedure that is \marked then the procedure is \marked.
...
\plc{lambda} expression must also be \marked.
\end{itemize}
\end{cppspecific}
