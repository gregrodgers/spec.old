...
environment.  For \code{to} and \code{from} clauses with a \code{mapper()}
modifier, the expansion of list-items occurs according to the rules for
expanding mapper elements in map clauses, with the same rules for transfer type
decay.
...
The default mapper for all types \plc{T} is as follows unless a custom mapper is
specified for that type.

\begin{boxedcode}
  \#pragma omp declare mapper(\plc{T} v) map(tofrom: v) use_by_default
\end{boxedcode}
...
by being explicitly named in a \code{map} clause. A map clause with list-item
\plc{var} invokes the default mapper behavior on \plc{var} and does not recurse.
If another instance of \plc{type} is mapped in the same mapper, it must be
an array section, and those maps will recurse unless the pointer is null, in
which case the recursion will terminate.
...
in the program such that the following is valid.
...
\begin{boxedcode}
  ...
  class C {
    int len;
  public:
    \#pragma omp declare mapper(Cmap: C v) map(v) map(p[:len]) use_by_default
    T* p;
    C() : p(new T[1]), len(1) {}
  };
  void foo() {
    C<int> c;
    \#pragma omp target map(mapper(Cmap): c)
    c.p[0] = 1;
    assert(c.p[0] == 1);
  }
\end{boxedcode}
