...
If a list item in a \code{map} clause has a named pointer that is a
...
in a Construct}), and \plc{offset} is the address offset given by the address of the list item minus the
value of the named pointer, then on entry to the \code{target} region:

\begin{itemize}
\item If the list item is not a zero-length array section, the corresponding
private variable is initialized to the address of the storage
location of the corresponding list item  that is created by the \code{map}
clause minus \plc{offset}.

\item If the list item is a zero-length array section, the corresponding
private variable is initialized to the address of the
corresponding storage location minus \plc{offset}. If the
...
%\item A list item in a \code{map} clause that appears on a \code{target}
%construct must either be a variable, an array section, or have a named pointer.
