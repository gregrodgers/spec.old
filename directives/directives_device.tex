...
The type and the \plc{mapper-identifier} uniquely identify the
...
variables of \plc{type}. All map clauses with this construct in scope
...
\item No instance of the mapper type can be mapped as part of the mapper,
  ...
  as the list item. No cycles are permitted in the mapping of a type.

\item The \plc{type} must be of struct, union or class type in C and C++ or a
  non-intrisic type in Fortran.

\item At least one \code{map} clause that maps at least one element of \plc{var}
  is required.
...
\item Each of the \plc{map-type-modifier}s can appear at most once on the
   \code{map} clause.
