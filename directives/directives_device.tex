...
device(\plc{[device-modifier:] integer-expression})
private(\plc{list})
...
where \plc{device-modifier} is one of:
\begin{indentedcodelist}
ancestor
device_num
\end{indentedcodelist}
...
device(\plc{[device-modifier:] scalar-integer-expression})
private(\plc{list})
firstprivate(\plc{list})
in_reduction(\plc{reduction-identifier }:\plc{ list})
map(\plc{[[map-type-modifier[},\plc{] [map-type-modifier[},\plc{] ...] map-type}:\plc{ ] list})
...
If a \code{device} clause is present with \code{ancestor} as \plc{device-modifier}, execution of the \code{target} region
occurs on the parent device synchronously. The encountering device thread waits for completion of the
\code{target} region on the parent before resuming. If the \code{target} construct is in an inactive target
region the encountering thread continues execution.

For any list item in a map clause of a \code{target} construct with \code{device} clause with \code{ancestor}
as \plc{device-modifier},
if the corresponding item exists in the device data environment of the parent device it is treated 
as if it has a reference count of positive infinity.
...
\item At most one \code{device} clause can appear on the
directive.  If the \code{device} clause contains \code{ancestor} as \plc{device-modifier} then the 
value should be constant 1. If the \plc{device-modifier} is not specified or specified as \code{device-num} then expression 
must evaluate to a non-negative integer value less than the value of
  \code{omp_get_num_devices()}.

If the \code{device} clause contains \code{ancestor} as \plc{device-modifier} then the following 
restrictions apply :
\begin{itemize}
\item The \code{target} construct must not be contained within a \code{target} region for which a \code{device}
clause with \code{ancestor} as \plc{device-modifier} was specified.

\item The \code{omp requires reverse_offload} has to be been specified.

\item The only valid clauses are \code{device}, \code{firstprivate}, \code{private}, \code{defaultmap} and \code{map} clause.

\item No OpenMP constructs or OpenMP API runtime routines are allowed inside the \code{target} region.
\end{itemize}
