...
A thread dispatches a registered \code{ompt_callback_target} callback for each 
occurrence of a \plc{target-data-begin} and \plc{target-data-end} event in that 
thread in the context of the task that encounters the construct. The callback has 
type signature \code{ompt_callback_target_t}. The callback receives 
\code{ompt_scope_begin} or \code{ompt_scope_end}
as its \plc{endpoint} argument, as appropriate, and
\code{ompt_target_enter_data} as its \plc{kind} argument.
