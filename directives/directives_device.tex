...
value assigned into the private versions are determined according to
\specref{subsec:pointer translation}.
\end{ccppspecific}

\begin{fortranspecific}
Pointers that appear in a \code{use_device_ptr} clause are privatized and the device
pointers to the corresponding list items in the device data environment are
assigned into the private versions.
\end{fortranspecific}
...
\begin{ccppspecific}
If a list item that appears in a \code{use_device_addr} clause is an array
section that has a base pointer, the base pointer is privatized and the value assigned
into the private copy is determined according to \specref{subsec:pointer
translation}.
\end{ccppspecific}
...
\begin{fortranspecific}
\item A list item that appears in a \code{use_device_ptr} clause must be a dummy 
...
private variable is initialized to its translated value in the device data
environment, as determined by \specref{subsec:pointer translation}.
\end{itemize}
