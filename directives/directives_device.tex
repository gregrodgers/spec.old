...
The \code{use\_by\_default} clause makes this mapper the default for all variables
of \plc{type}. The \code{skip\_safe} clause informs the compiler and runtime that
any associated custom stages are safe to skip if the variable is already visible
on the target device, note that if \code{skip\_safe} is not specified any
...
\label{subsec:declare mapper_stage Directive}

\summary

The \code{declare mapper\_stage} directive defines a specific stage of the
mapping process for a given user-defined mapper. The \code{declare mapper\_stage}
directive is a declarative directive.

\syntax
\begin{ccppspecific}
The syntax of the \code{declare mapper\_stage} directive is as follows:
