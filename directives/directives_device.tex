...
\code{target}, \code{target data}, or \code{target enter data} construct 
...
device data environment. Pointers that appear in a \code{use_device_ptr}
...
\code{(}\plc{flags}~\code{&}~\code{ompt_task_target)} 
...
The \plc{target-exit-data-begin} event occurs when a thread enters a
\code{target}~\code{exit}~\code{data} region.

The \plc{target-exit-data-end} event occurs when a thread exits a
\code{target}~\code{exit}~\code{data} region.
...
and where \plc{device-modifier} is one of the following:
\begin{indentedcodelist}
ancestor
device_num
\end{indentedcodelist}

and where \plc{allocator} is an identifier of \code{omp_allocator_handle_t} type
...
and where \plc{allocator} is an integer expression of \code{omp_allocator_handle_kind} 
...
\code{target} construct that does not specify a \code{device} clause in which the
\code{ancestor} \plc{device-modifier} appears then that function or subroutine
is treated as if its name had appeared in a \code{to} clause on a
...
(see \specref{subsubsec:Variables Referenced in a Construct}), then on entry 
to the \code{target} region:
...
private variable is initialized according to  \specref{subsec:pointer
initialization}.
\end{itemize}
...
\begin{fortranspecific}
When an internal procedure is called in a \code{target} region, any
...
Events associated with the \plc{initial task} that executes the \code{target} 
region are defined in \specref{subsec:Initial Task}.
...
\item A list item that appears in an \code{is_device_ptr} clause must
      not be specified on any data-sharing attribute clause on the same target construct.
...
\item Non-predefined allocators appearing in a \code{uses_allocators} clause must 
      have \plc{traits} specified.
\item Arrays that contain allocator traits that appear in a \code{uses_allocators} 
...
\begin{ccppspecific}
\item An attached pointer must not be modified in a \code{target} region.
\end{ccppspecific}
...
\item The effect of invoking a virtual member function of an object on a device 
      ...
      defined.
\item A \code{throw} executed inside a \code{target} region must cause execution 
      to resume within the same \code{target} region, and the same thread that 
      threw the exception must catch it.
\end{cppspecific}

\begin{fortranspecific}
\item An attached pointer that is associated with a given pointer target must
    not become associated with a different pointer target in a \code{target}
    region.
\item A list item that appears in an \code{is_device_ptr} clause must
      be a pointer or a dummy argument that does not have the
      \code{ALLOCATABLE} or \code{VALUE} attribute.
...
\item A reference to a coarray that is encountered on a target device
    must not be coindexed or appear as an actual argument to a procedure where the
    corresponding dummy argument is a coarray.
\end{fortranspecific}
\end{itemize}
...
\begin{indentedcodelist}
to(\plc{[ [motion-modifier[},\plc{] [motion-modifier[},\plc{] ...]}: \plc{] locator-list})
from(\plc{[ [motion-modifier[},\plc{] [motion-modifier[},\plc{] ...]}: \plc{] locator-list})
\end{indentedcodelist}

where \plc{motion-modifier} is one of the following:
...
in the device data environment and there is no \code{present} modifier in the clause,
...
\code{tofrom} map-type. If a \code{present} modifier
...
The list items that appear in a \code{to} or \code{from} may reference
iterators defined by an \plc{iterators-definition} appearing on an
\code{iterator} modifier.
...
\end{ccppspecific}
\begin{fortranspecific}
\item On exit from the region that part of the original list item, if
      associated, will be associated with the same pointer target with 
      which it was associated on entry to the region;
\item On exit from the region that part of the corresponding list item, 
      if associated, will be associated with the same pointer target
...
\code{shared} in the data environment of the \plc{target task}.
...
\item A program must not depend on any ordering of the evaluations of the 
      ...
      effects of the evaluations of the clauses.
\item Each of the \plc{motion-modifier} modifiers can appear at most once on a
      motion clause.
...
\begin{crossrefs}
\item Array shaping,
\specref{subsec:Array Shaping}

\item Array sections,
\specref{subsec:Array Sections}

\item Iterators, see \specref{subsec:iterators}.
...
\begin{ompcPragma}
#pragma omp begin declare target \plc{[clause[ [ [},\plc{] clause] ...] ] new-line}
    \plc{declaration-definition-seq}
#pragma omp end declare target \plc{new-line}
\end{ompcPragma}
...
The \code{device_type} clause specifies if a version of the procedure or variable should be made
available on host, device or both.
If \code{host} is specified only a host version of the procedure or variable are made available.
If \code{any} is specified then both device and host versions of the procedure or variable are made available.
If \code{nohost} is specified for a procedure then only a device version of the procedure are made available.
If \code{nohost} is specified for a variable then that variable is not available on the host.

\begin{ccppspecific}
If a function appears in a \code{to} clause in the same compilation unit in which
the definition of the function occurs then a device-specific version of the function
is created.

If a variable appears in a \code{to} clause in the same compilation unit in which
...
\begin{fortranspecific}
If a procedure appears in a \code{to} clause in the same compilation
unit in which the definition of the procedure occurs then a
device-specific version of the procedure is created.
...
clause on a \code{declare}~\code{target} directive that does not specify a \code{device_type}
clause with \code{host} and the function reference is not
enclosed in a \code{target} construct that specifies a \code{device} clause in which the
\code{ancestor} \plc{device-modifier} appears then the name of the referenced function is
...
The form, preceded by either the \code{declare}~\code{target} directive
that has no clauses and no \plc{extended-list} or the
 \code{begin}~\code{declare}~\code{target} directive
and followed by a matching \code{end}~\code{declare}~\code{target} directive,
defines an implicit \plc{extended-list}. The implicit \plc{extended-list}
consists of the variable names of any variable declarations at file or
namespace scope that appear between the two directives and of the function
names of any function declarations at file, namespace or class scope that
appear between the two directives. The implicit \plc{extended-list} is
converted to an implicit \code{to} clause.

The \plc{declaration-definition-seq} preceded by either
\code{begin}~\code{declare}~\code{target} directive or a
\code{declare}~\code{target} directive without any clauses or an \plc{extended-list}
and followed by an \code{end}~\code{declare}~\code{target} directive may contain
...
clause on a \code{declare}~\code{target} directive that does not specify a \code{device_type}
clause with \code{host} and the procedure reference is not
enclosed in a \code{target} construct that specifies a \code{device} clause in which the
\code{ancestor} \plc{device-modifier} appears then the name of the referenced procedure is
treated as if it had appeared in a \code{to} clause on a \code{declare}~\code{target} directive.

If a \code{declare}~\code{target} does not have any clauses and does not have an
\plc{extended-list} then an implicit \code{to} clause with one item is formed from
the name of the enclosing subroutine subprogram, function subprogram or
interface body to which it applies.

If a \code{declare}~\code{target} directive has a
...
\item If a \code{declare}~\code{target} directive has a clause, it must
      contain at least one \code{to} clause or at least one \code{link} clause.
\item A variable for which \code{nohost} is specified may not appear in a \code{link} clause.
\end{itemize}

\begin{cppspecific}
\begin{itemize}
\item A \code{to} clause or a \code{link} clause cannot appear in a
      \code{begin}~\code{declare}~\code{target} directive.
...
\item If a \plc{lambda declaration and definition} appears between a
      ...
      directive, all variables that are captured by the \plc{lambda} expression
...
\item If a \code{declare}~\code{target} directive does not have any clause, has a
      \code{device_type} clause, or has clauses that contain a function name, it
      must appear in the specification part of a subroutine subprogram,
      function subprogram or interface body.
\item If a \code{declare}~\code{target} directive has a variable name
      in \plc{extended-list}, it must appear in the specification part of a
...
\item If a \code{declare}~\code{target} directive is specified in an
      ...
      procedure, including the \code{device_type} clause if present.
...
\item If a \code{declare}~\code{target} directive that specifies a common
      ...
      statement that specifies the same name, after the last such \code{COMMON} 
