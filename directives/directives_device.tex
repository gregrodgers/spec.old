...
mapper for use in a \code{map} clause later in the program.

The \code{use_by_default} clause makes this mapper the default for all variables
of \plc{type}. The \code{skip_safe} clause informs the compiler and runtime that
any associated custom stages are safe to skip if the variable is already visible
on the target device, note that if \code{skip_safe} is not specified any
...
All \code{map} clauses on the construct are expanded into corresponding
\code{map} clauses wherever this mapper is invoked, either by matching type or
by being explicitly named in a \code{map} clause.
...
\begin{cppspecific}
The \code{declare mapper} directive can also appear at points in the program at which
...
\restrictions
\begin{itemize}
\item If the \code{use_by_default} clause is not specified, then a
...
The \code{declare mapper_stage} directive defines a specific stage of the
mapping process for a given user-defined mapper. The \code{declare mapper_stage}
directive is a declarative directive.

\syntax
\begin{ccppspecific}
The syntax of the \code{declare mapper_stage} directive is as follows:
...
\begin{itemize}
  ...
  \item and \plc{clause} is one of the following:
    \begin{itemize}
      \item \code{in\_place}
      \item \code{expr(\plc{stage-expression})} where \plc{stage-expression} is
        a base language expression that performs the stage necessary on
        \plc{mapper-var} and \code{omp\_buf}
      \item \code{size(\plc{size-expression})} where \plc{size-expression} is a
        base language expression that evaluates to the number of bytes the stage
        requires for its output
      \item \code{buffer\_type(\plc{type})}
    \end{itemize}
\end{itemize}

User-defined mappers, defined using the \code{declare mapper} directive, can be
customized beyond expanding \code{map} clauses by specifying custom mapping
stages with the \code{declare mapper\_stage} directive.  Every mapper is
logically composed of six stages: \code{alloc}, \code{pack\_to},
\code{unpack\_to}, \code{pack\_from}, \code{unpack\_from} and \code{release} all
executed one after the other in that order with a logical bitwise-copy between
the \code{pack\_to} and \code{unpack\_to} as well as the \code{pack\_from} and
\code{unpack\_from} stages.
...
mapper, and for all stages but \code{alloc} and \code{release} also a buffer in
the variable \code{omp\_buf}.

\begin{ccppspecific}
  The type of \code{omp\_buf} will be \code{void *} unless an alternate type,
  which must be a pointer type, is specified with a \code{buffer\_type} clause.
\end{ccppspecific}

\begin{fortranspecific}

  The type of \code{omp\_buf} will be
  \code{character(len=1),dimension(:),pointer} unless an alternate type, is
  specified with a \code{buffer\_type} clause. If a \code{buffer\_type} clause is
  specified, the resulting declaration of \code{omp\_buf} must be valid in an
  ...
  where \code{omp\_buf\_ptr} is a pointer to \code{omp\_buf\_size} characters.
...
additionally receive \code{omp\_buf} and \code{omp\_buf\_size}:
...
\begin{description}
  ...
  \item[pack\_to] Packs all data in its input, \plc{mapper-var}, into a bitwise-copyable
    form in \code{omp\_buf}, that will be allocated based on the result of its
    \code{size} clause or the size of a single instance of type if no
    \code{size} clause is specified. Additionally \code{update} directives may
    be used to populate data mapped by the \code{alloc} stage.

  \item[unpack\_to] Unpacks data in its input, \code{omp\_buf},  into an instance
    of \plc{mapper-var} in the device data environment, this expression must be
    valid in a \code{declare target} function.  The \code{size} clause is not
    allowed on this stage.

  \item[pack\_from] Packs data in its input, \plc{mapper-var} in the device data
    environment,   into \code{omp\_buf}, which will be allocated based on the
    result of its \code{size} clause or the size of a single instance of type if
    no \code{size} clause is specified.  This expression must be valid in a
    \code{declare target} function.

  \item[unpack\_from] Unpacks its input, \code{omp\_buf}, into a \plc{mapper-var}
    on the host device.  The \code{size} clause is not allowed on this stage.
    Additionally \code{update} directives may be used to populate data mapped by
    the \code{alloc} stage.
...
\begin{description}
  \item[each time the mapped variable's reference count is incremented] \code{alloc}
  \item[update to] \code{pack\_to} followed by \code{unpack\_to}
  \item[update from] \code{pack\_from} followed by \code{unpack\_from}
  \item[each time the mapped variable's reference count is decrimented] \code{release}
\end{description}

The \code{in\_place} clause is only valid on packer stages and causes
\code{omp\_buf} and \code{omp\_buf\_size} not to be defined, instead using
\plc{mapper-var} as the only predefined variable in its expressions, it is
mutually exclusive with the \code{size} clause.

The \code{declare mapper\_stage} directive can appear at any point where a
\code{declare mapper} clause would be valid.
...
\item If either of \code{alloc} or \code{release} are specified, the other must
  be as well.
