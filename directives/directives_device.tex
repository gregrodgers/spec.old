...
\item Memory allocators that do not appear in a \code{uses_allocator} clause cannot appear as an allocator in an \code{allocate} clause or be used in the \code{target} region unless a \code{requires} directive with the \code{dynamic_allocators} clause is present in the same compilation unit.

\item \plc{allocators} that appear in a \code{uses_allocator} clause cannot appear in other data-sharing attribute clauses or data-mapping attribute clauses in the same construct.

\item predefined allocators appearing in a \code{uses_allocator} clause cannot have \plc{traits} specified.

\item non-predefined allocators appearing in a \code{uses_allocator} clause must have \plc{traits} specified.

\item Arrays containing allocators traits that appear in a \code{use_allocators} clause must be constant arrays, have constant values and be defined in the same scope as the construct in which the clause appears. 
