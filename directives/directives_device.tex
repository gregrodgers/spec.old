...
\item If any IEEE exception flag is set by the \code{ieee_set_flag}
  subroutine, any IEEE status by the \code{ieee_set_status}
  subroutine, any IEEE rounding mode by the
  \code{ieee_set_rounding_mode} subroutine or any IEEE halting mode by
  \code{ieee_set_underflow_mode} subroutine upon entering the target
  region, the corresponding setting in the region is unspecified.
\item If any IEEE exception flag is set by the \code{ieee_set_flag}
  subroutine, any IEEE status by the \code{ieee_set_status}
  subroutine, any IEEE rounding mode by the
  \code{ieee_set_rounding_mode} subroutine or any IEEE halting mode by
  \code{ieee_set_underflow_mode} subroutine upon exiting the target
  region, the corresponding setting on the host device is unspecified.
