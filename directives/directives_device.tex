...
Memory allocators specified in the \code{uses_allocator} clause will be made available in the target region. For non-predefined allocators, an allocator will be created with the specified \plc{traits} at the beginning of the target region and assigned to the \plc{allocator} handle. All allocators will be destroyed at the end of the target region.
...
\item Arrays containing allocators traits that appear in an \code{use_allocators} clause must be constant, have constant values and be defined in the same scope as the construct in which the clause appears. 
\end{cppspecific}
