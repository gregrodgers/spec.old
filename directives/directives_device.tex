...
\begin{fortranspecific}
\item The association status of the attached pointer may not be
modified in a \code{target} region.
...
\item On exit from the region that part of the original list item, if
  associated, will be associated with the same pointer target that it
  was associated with on entry to the region;
\item On exit from the region that part of the corresponding list
  item, if associated, will be associated with the same pointer target
  that it was associated with on entry to the region.
\end{fortranspecific}
\end{itemize}
