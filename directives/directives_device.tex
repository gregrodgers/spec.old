...
\item Memory allocators that do not appear as list items in an \code{uses_allocator} clause cannot appear in a target region unless a \code{requires} directive with the \code{dynamic_allocators} clause is present in the same translation unit.

\item \plc{allocators} that appear in an \code{uses_allocator} clause cannot appear in other data-sharing clauses or data-mapping clauses in the same construct.
