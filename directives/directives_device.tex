...
List items that appear in a \code{use_device_ptr} clause are converted into
device pointers to the corresponding list items in the device data environment.
If a \code{use_device_ptr} clause and one or more \code{map} clauses are
...
\begin{itemize}
  ...
  \item At least one \code{map} or \code{use_device_ptr} clause must appear on the directive.


  \item A list item in a \code{use_device_ptr} clause must have a
  ...
  \item References in the construct to a list item that appears in a
    \code{use_device_ptr} clause must be to the address of the list item.
...
uses_allocator(\plc{allocator}[(\plc{allocator-traits-array})][,\plc{allocator}[(\plc{allocator-traits-array})] ...])
\end{indentedcodelist}
...
where \plc{allocator-traits-array} is an array of \code{type(omp_allocatortrait)}.
...
not appear as a list item in an \code{in_reduction} clause on the construct,
...
If an \code{in_reduction} clause is present on the \code{target} construct,
...
For each memory allocators specified in the \code{uses_allocator} clause will be made available in the target region. For non-predefined allocators, allocators will be created with the specified \plc{traits} at the beginning of the region associated to the construct and assigned to the \plc{allocator} handle. If the \code{access} trait is \code{thread} then one allocator per thread created in the construct will be created. If the \code{access} trait is \code{pteam} then one allocator per parallel team created in the construct will be created. If the \code{access} trait is \code{cgroup} then one allocator per contention group will be created. All allocators will be destroyed at the end of the target region.
...
\item Memory allocators that do not appear as list items in an \code{uses_allocator} clause cannot appear in a target region unless a \code{requires} construct with the \code{dynamic_allocators} clause is present in the same translation unit.

\item \plc{allocators} that appear in an \code{uses_allocator} clause cannot appear in another clause in the same construct.

\item predefined allocators appearing in an \code{uses_allocator} clause cannot have \plc{traits} specified.

\item non-predefined allocators appearing in an \code{uses_allocator} clause must have \plc{traits} specified.

\item Arrays containing allocators traits that appear in an \code{use_allocators} clause must be constant arrays, have constant values and be defined in the same scope as the construct in which the clause appears. 
...
\item Memory allocators, see \specref{subsubsec:Memory Allocators}
...
\code{declare}~\code{target} directives without the \code{device_type} clause.
...
\item The \code{implements} clause and \code{device_type} clause are the only valid combination
...
\subsection{\hcode{declare}~\hcode{mapper} Directive}
...
\begin{ompfPragma}
!$omp declare mapper(\plc{[mapper-identifier:] type :: var})
\plc{[clause[ [,] clause] ... ]}
\end{ompfPragma}
...
\begin{itemize}
  ...
  \item \plc{clause} is one of the following:
    \begin{itemize}
      \item \pcode{map(\plc{[[map-type-modifier[,]] map-type}:\plc{ ] list})}
      \item \code{use_by_default}
    \end{itemize}
...
The \code{use_by_default} clause makes this mapper the default for all
...
declare mapper(\plc{T} v) map(tofrom: v) use_by_default
...
\subsection{\hcode{teams} Construct}
...
\subsection{\hcode{distribute} Construct}
...
\item \code{ompt_work_distribute}, see \specref{sec:ompt_work_type_t}.
...
\subsection{\hcode{distribute}~\hcode{simd} Construct}
