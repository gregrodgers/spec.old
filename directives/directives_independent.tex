...
An implementation may choose to treat an \code{independent} construct as a
\code{distribute},
...
\code{levels} clause appears on the construct, the \code{independent} directive
...
\begin{itemize}
  ...
  \item Only one \code{levels} clause can appear on an independent directive.
  ...
  \item Only the following directives may be nested within an independent
    ...
    \code{single}, and \code{atomic}[?]. %requesting feedback on atomic

  \item An independent region may not contain calls to functions containing
    OpenMP directives. [Is there a lighter hammer we can use?] % Would like to relax this later.
    
  \item An \code{independent} construct must be lexically contained within a teams or
