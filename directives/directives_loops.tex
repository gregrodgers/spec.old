...
A range-based for loop that is valid in the base language and results in a begin
expression that provides a value that satisfies the random access iterator
requirement has \emph{canonical loop form}.  Range-based for loops are of the
following form:

\begin{ompcPragma}
for (\plc{range-decl} : \plc{range-expr}) {
  \plc{structured-block} 
}
\end{ompcPragma}

These range-for loops are treated as though they expand
to the following, or equivalent base-language expansion:

\begin{ompcPragma}
auto && __range = \plc{range-expr};
auto __begin = \plc{begin-expr};
auto __end = \plc{end-expr};
// OpenMP construct
for ( ; __begin != __end; ++__begin) {
  \plc{range-decl} = *__begin;
  \plc{structured-block}
}
\end{ompcPragma}

The \plc{begin-expr} and \plc{end-expr} expressions are determined by the base
language.  Both \code{__begin} and \code{__end} are privatized.  For the purpose
of the rest of the standard \code{__begin} is the iteration variable of the
range-for loop.
...
\begin{cppspecific}
  \begin{itemize}
    ...
    \item Range-for loops must not appear in the loops associated with an
      \code{ordered} clause with a parameter.
  \end{itemize}
\end{cppspecific}
...
%        \plc{incr} must be 0.
    \item The \plc{b}, \plc{lb}, \plc{incr}, and \plc{range-expr} expressions
      may not reference any \plc{var} or member of the \plc{range-decl} of any
