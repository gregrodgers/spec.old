...
\item The \code{nonmonotonic} modifier cannot be specified if an \code{ordered} 
      clause is specified.
\item Each \plc{modifier} may be specified at most once on the same
      \code{schedule} clause.
...
\item At most one \code{order(concurrent)} clause can appear on a worksharing-loop
      directive.
...
\item At most one \code{order(concurrent)} clause can appear on a \code{simd}
      directive.
\item If an \code{order(concurrent)} clause is present, all restrictions from
      the \code{loop} construct with an \code{order(concurrent)} clause also apply.
...
\item If a \code{loop} region binds to a \code{teams} or parallel
      ...
      or by none of them.
\item At most one \code{bind} clause can appear on a \code{loop} directive.
...
\item If a threadprivate variable is referenced inside a \code{loop} region, 
      the behavior is unspecified.
\item At most one \code{order(concurrent)} clause can appear on a \code{loop}
      directive.
\end{itemize}
...
structured block sequence that lexically precede the directive constitute the
\textit{input phase} and all statements in the structured block sequence that lexically follow the
directive constitute the \textit{scan phase}. If the list item appears in an
\code{exclusive} clause, all statements in the structured block sequence
that lexically precede the directive constitute the \textit{scan phase}
and all statements in the structured block sequence that lexically follow the
directive constitute the \textit{input phase}.
...
For list items appearing in an \code{inclusive} clause, at the end of the enclosing
worksharing-loop, worksharing-loop SIMD, or \code{simd} construct, the original list
item is assigned the private copy from the last logical iteration of the loops
associated with the enclosing construct.
For list items appearing in an \code{exclusive} clause, let \plc{L} be the
last logical iteration of the loops associated with the enclosing construct.
At the end of the enclosing construct, the original list item is assigned
the result of the operation \texttt{PRESUM\textsubscript{last}}( \plc{combiner}, 
\plc{orig-val}, \plc{u}\textsubscript{0}, \ldots, \plc{u}\textsubscript{L}).
...
\item Intra-iteration dependences from a statement in the structured block sequence
      preceding a \code{scan} directive to a statement in the structured block sequence
      ...
      clause.
\item The private copy of list items that appear in the \code{inclusive} or
      \code{exclusive} clause may not be modified in the \textit{scan phase}.
\end{itemize}
