...
%\code{loop} construct and the iterations of the associated loops are
...
% If the \code{loop} construct is closely nested inside a \code{teams} construct,
...
% \code{loop} construct is closely nested inside a \code{parallel} construct, the
...
% \code{loop} region is the encountering thread and the binding region is the
% current task region.
...
%with the \code{loop} construct. The parameter of the \code{levels}
...
%clause appears on the construct, the \code{loop} directive applies to
...
% An implementation may choose to treat an \code{loop} construct as a
% \code{distribute}, worksharing-loop, and/or SIMD construct if it would be conforming for a
% programmer to have explicitly specified the selected construct. If the
% implementation chooses to treat a \code{loop} construct as one of these
...
% A \code{loop} construct may be nested within other OpenMP regions, but does
...
%      construct.
  ...
  \item A list item may not appear in a \code{lastprivate} clause, unless it is
