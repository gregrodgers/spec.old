...
{\scode{distribute}} & {\scode{nearest}, \scode{blocked}, \scode{interleaved}} & Implementation defined\\
\end{supertabular}
\linenumbers
...
The {\scode{distribute}} trait describes the intended distribution of allocated
memory over memory domains within the memory space associated with the
allocator and available to the program. The values and their description are:
\begin{itemize}
 \item \scode{nearest}: allocated memory will be placed in the memory domain that is closest to the thread performing the allocation.
 \item \scode{blocked}: allocated memory will be distributed in contiguous parts of the same size over the memory domains with one part per domain.
 \item \scode{interleaved}: allocated memory will be distributed in a round-robin fashion over the memory domains and the size of each part is implementation defined.
\end{itemize}
