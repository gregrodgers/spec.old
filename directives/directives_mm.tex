...
\begin{ccppspecific}
If the directive is associated with an \plc{allocation-stmt}, then the call to \plc{function} in the statement will be replaced by a request to a memory allocator using the arguments the function according to the specification of the \code{declare alloc} directive for \plc{function}. The result of the allocation requests will be stored as if the \plc{function} had been called according to the specification of the \code{declare alloc} directive for \plc{function}.
\end{ccppspecific}
...
\begin{ccppspecific}
\item If the directive is associated with an \plc{allocation-stmt}, the \plc{function} must appear in a previous \code{declare alloc} directive and the \plc{kind} must be either \code{alloc} or \code{realloc}.
...
\item \plc{def-allocator-var} ICV, see \specref{\label{ICV Descriptions}.
\end{itemize}
...
The \code{deallocate} directive replaces a user function with a deallocation request to a memory allocator.
...
where \plc{allocation-stmt} is an expression statement of the following form:
\begin{codepar}
    function(arguments);
\end{codepar}
...
The \plc{deallocation-stmt} associated with the construct will be replaced by a deallocation request to a memory allocator will be replaced by a request to a memory allocator using the arguments the function according to the specification of the \code{declare alloc} directive for \plc{function}. 

If no \code{allocator} clause is specified the memory allocator used to allocate the memory being deallocated will be used. If the \code{allocator} clause is specified, the memory allocator specified in the clause will be used. Specifying a memory allocator in the \code{allocator} clause different than the one used to allocate the memory being deallocated results in undefined behavior.

\restrictions
\begin{itemize}
\item At most one \code{allocator} clause can appear on the \code{deallocate} directive.
\item If the directive is associated with an \plc{allocation-stmt}, the \plc{function} must appear in a previous \code{declare alloc} directive and \plc{kind} must be \code{deallocate}.
\end{itemize}
...
The storage for new list items that arise from \plc{list item} that appear in the directive will be provided by an allocation through a memory allocator. If an \plc{allocator} is specified in the clause this will be the memory allocator used for allocations. For all directives except for the \code{target} construct, if no \plc{allocator} is specified in the clause then the memory allocator specified by the \plc{def-allocator-var} ICV will be used. For the \code{target} construct, if no \plc{allocator} is specified in the clause then the memory allocator specified by the \plc{target-def-allocator-var} ICV will be used.  The inability of a memory allocator to fulfil the allocation request for any list item results in undefined behavior.
...
\item Memory allocators, see \specref{sec:Memory Allocators}.
\item \code{omp\_allocator\_t} and \code{omp\_allocator\_kind}, see \specref{subsec:Memory Management Types}.
\item \plc{def-allocator-var} and \plc{target-def-allocator-var} ICVs, see \specref{\label{ICV Descriptions}.
\end{itemize}
