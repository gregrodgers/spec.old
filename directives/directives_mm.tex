...
item, the behavior is implementation defined. The allocation of each \plc{list item} 
...
\item The directive must appear in the same scope of the \plc{list item} 
      declaration after its declaration.
...
\item If after a \plc{list item} has been allocated the scope that 
      contains the \code{allocate} directive ends abnormally 
      other than through C++ exceptions, such as through a 
      call to the \code{longjmp} function, then the behavior 
      is unspecified.
\end{ccppspecific}
