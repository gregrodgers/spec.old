...
\item \code{allocate} directives appearing in a \code{target} region must specify an \code{allocator} clause and the \plc{allocator} expression must be evaluable by the compiler.
\end{fortranspecific}
\end{itemize}
...
\item \code{allocate} clauses appearing in a \code{target} construct or in a \code{target} region must specify an \plc{allocator} expression must be evaluable by the compiler.
\end{itemize}
