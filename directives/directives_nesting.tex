...
\item The only OpenMP constructs that can be encountered during execution of a \code{simd} (or loop SIMD) region are the \code{atomic} and \code{critical} constructs and an \code{ordered} construct with the \code{simd} clause.
