...
\code{task}, \code{taskloop}, \code{critical}, \code{ordered},
...
\item A worksharing, \code{barrier}, or \code{master} region may not be
    closely nested inside a \code{loop} region that does not bind to a
    \code{teams} region.
...
\item An \code{ordered} region corresponding to an \code{ordered} construct without
the \code{simd} clause specified must be closely nested inside a worksharing-loop region.

\item An \code{ordered} region corresponding to an \code{ordered} construct with the
\code{simd} clause specified must be closely nested inside a \code{simd} or
...
\item An \code{ordered} region corresponding to an \code{ordered} construct with
  ...
  a worksharing-loop SIMD region or closely nested inside a worksharing-loop and \code{simd} region.
...
\code{target} region and a \code{device} clause in which the \code{ancestor}
\plc{device-modifier} appears is not present on the construct, the behavior is unspecified.
...
\item \code{distribute}, \code{distribute simd}, distribute parallel worksharing-loop,
  distribute parallel worksharing-loop SIMD, \code{loop}, \code{parallel} regions, including any
\code{parallel} regions arising from combined constructs, \code{omp_get_num_teams()} regions, and \code{omp_get_team_num()} regions are the only OpenMP regions
...
\item The only constructs that may be nested inside a \code{loop} construct
    are \code{loop}, \code{parallel} and its derivatives, and \code{simd}.

\item A \code{loop} region corresponding to a \code{loop} construct may not contain calls to procedures that contain
OpenMP directives. 
