...
\item A \code{concurrent} region may only be nested within a \code{teams},
  \code{distribute}, \code{parallel}, Loop, SIMD, \code{concurrent}, or regions
  resulting from the combined or composite constructs composed of these
  constructs.

\item Only the following directives may be nested within a \code{concurrent}
  construct: \code{parallel}, Loop, SIMD, \code{concurrent}, \code{single}, and any combined
  or compound construct composed of these constructs.
