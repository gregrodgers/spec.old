...
\textbf{else if} (\plc{ActiveParRegions} $\geq$ 1) \textbf{and} (\plc{nest-var} = \plc{false})
...
OpenMP threads to places within the current place partition, that is, the places 
listed in the \plc{place-partition-var} ICV for the implicit task of the encountering 
thread. If the \code{parallel} directive has a \code{proc_bind} clause then the 
...
\item $T>P$: Each place $P$ will contain $S_{p}$ threads with consecutive thread 
             numbers, where $\blfloor T/P \brfloor \leq Sp \leq \blceil T/P \brceil$. 
