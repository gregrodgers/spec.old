...
A thread dispatches a registered \code{ompt_callback_idle}
...
\code{ompt_callback_idle_t}. The callback receives
\code{ompt_scope_begin} or \code{ompt_scope_end} 
as its \plc{endpoint} argument, as appropriate.
...
\item \code{ompt_callback_idle_t}, see 
...
\textbf{if} a \code{thread_limit} clause exists on a \code{teams} construct associated with an enclosing \code{teams} region
