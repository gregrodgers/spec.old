...
\code{(flags}~\code{&}~\code{ompt_parallel_team)} evaluates to \plc{true}.
...
\code{ompt_callback_implicit_task_t}. In the dispatched
callback, \code{(flags}~\code{&}~\code{ompt_task_implicit)} always evaluates to
\plc{true}.
