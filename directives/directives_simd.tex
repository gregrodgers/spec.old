...
\plc{[ ... ]}
   \plc{function definition or declaration, or lambda declaration and definition}
\end{boxedcode}
...
\descr
\begin{ccppspecific}
The use of one or more \code{declare}~\code{simd} directives
immediately prior to a function declaration or definition, or \plc{lambda
declaration and definition} enables the creation of corresponding SIMD
versions of the associated function that can be used to process
multiple arguments from a single invocation in a SIMD loop
concurrently.
...
\item The \plc{lambda} cannot be passed as a parameter to any functions.
\end{cppspecific}
