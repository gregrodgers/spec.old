...
The \code{nontemporal} clause modifier specifies that accesses to the storage locations to which the list items refer have low temporal locality across the iterations in which those storage locations are accessed.

% The \code{nontemporal} clause modifier is a hint to request that write accesses to the memory locations referred to by the list items within the loops associated with the \code{simd} construct may be written directly to memory bypassing caches.
