...
loop(s) were executed sequentially. When an \code{if} clause is present on the directive and the \code{if} clause
expression evaluates to \plc{false}, no two logical iterations executed concurrently with SIMD instructions. Otherwise, the number of iterations that are
...
\item An \code{atomic} construct, a \code{simd} construct or an \code{ordered} construct with the \code{simd} clause is the only OpenMP
