...
\begin{itemize}
	...
	release flush on exit from the \code{ordered} region,
        and the behavior is as if the release flush executed by one thread
        synchronizes with the acquire flush executed by the next thread to
        execute the \code{ordered} region.
...
\item During \code{omp\_set\_lock}, \code{omp\_test\_lock},
    \code{omp\_set\_nest\_lock}, and \code{omp\_test\_nest\_lock} regions, if
...
\item At entry to a \code{target enter data} and \code{target data} region.
...
\item During \code{omp\_unset\_lock} and \code{omp\_unset\_nest\_lock}
    regions.
...
\item At exit from a \code{target exit data} and \code{target data} region.
