...
different threads in a team. This ordering is imposed by
synchronizing flush operations that are executed as part of the region
associated with the construct.

Synchronization using atomic constructs or explicit flush operations in
combination with non-sequentially consistent atomic constructs is described in
Section \ref{subsec:The Flush Operation} and Section \ref{subsec:OpenMP Memory
Consistency}. Synchronizing flush operations that are implied on other OpenMP
regions are defined in Section \ref{subsec:flush Construct}, and they have the
following synchronizing behavior:

\begin{itemize}
    \item When two different threads execute a \code{critical} region that
        corresponds to a \code{critical} construct having the same name, the
        behavior is as if the release flush performed on exit from one of the
        regions synchronizes with the acquire flush performed on entry to the
        other region.

    \item When a team of threads execute a \code{barrier} region, the
	...
	thread executing a remaining child task performs a release flush upon
        completion of the child task that synchronizes with an acquire flush
        performed during the \code{taskwait} region.

    \item When executing a \code{taskgroup} region, the behavior is as if each
	thread executing a remaining descendant task performs a release flush
        upon completion of the descendant task that synchronizes with an
        acquire flush performed on exit from the \code{taskgroup} region.
    ...
    \item When executing an \code{ordered} region that does not arise from
        ...
        release flush performed by a thread on exit from the region
        synchronizes with the acquire flush performed on entry to an
        \code{ordered} region in the next logical iteration to be executed by
        a different thread, if it exists.

    \item When executing an \code{ordered} region that arises from a
        stand-alone \code{ordered} directive, the behavior is as if the
        release flush performed by the \code{ordered} region from a given
        source iteration synchronizes with the acquire flush performed by all
        \code{ordered} regions executed by a different thread that are waiting
        for dependences on that iteration to be satisfied.

    \item When executing a \code{parallel} region, the release flush performed
        by the master thread on entry to the \code{parallel} region
        synchronizes with the acquire flush performed on entry to each
        implicit task that is assigned to a different thread.

    \item When generating an explicit task, the thread executing the
        generating task performs a release flush that synchronizes with the
        acquire flush performed when scheduling the explicit task on a
        different thread.

    \item When scheduling a dependent task with one or more predecessor tasks
        on a given thread, the behavior is as if each release flush performed
        by a different thread on completion of a predecessor task synchronizes
        with the acquire flush performed when scheduling the dependent task.

    \item When scheduling a task that is mutually exclusive with respect to
        another sibling task that is executed by a different thread, the behavior is
        as if each release flush performed on completion of the sibling task
        synchronizes with the acquire flush performed when scheduling the
        task.

    \item When executing an \code{omp_unset_lock} region that causes the
	specified lock to be unset, a release flush is
        performed that synchronizes with an acquire flush that is performed by
        the next \code{omp_set_lock} or \code{omp_test_lock} region to be
        executed by a different thread that causes the specified lock to be
        set.

    \item When executing an \code{omp_unset_nest_lock} region that causes
	the specified nested lock to be unset, a release flush
	is performed that synchronizes with an acquire flush
	that is performed by the next \code{omp_set_nest_lock} or
	\code{omp_test_nest_lock} region to be executed by a different
        thread that causes the specified nested lock to be set.
\end{itemize}
...
Consistency}.  If the \code{seq_cst} clause is specified, a strong flush on
all shared memory locations is performed on entry to and exit from the atomic
operation, ensuring that the set of all sequentially consistent atomic
operations in the program has a total completion order.  If the \code{seq_cst}
clause is not specified, a strong flush on the location designated by \plc{x}
...
total completion order.  If the atomic operation is a write, update, or
captured update and the \code{release}, \code{acq_rel}, or \code{seq_cst}
clause is specified, the flush on entry to the atomic operation is a release
flush. If the atomic operation is a read or captured update and the
\code{acquire}, \code{acq_rel}, or \code{seq_cst} clause is specified, the
flush on exit from the atomic operation is an acquire flush.  Therefore, if
\plc{memory-order-clause} is specified, release and/or acquire flush
operations are implied and permit synchronization between the threads without
the use of explicit \code{flush} directives.
...
\descr
If \plc{memory-order-clause} is not specified then the \code{flush} constructs
...
\item At entry to and exit from \code{parallel} and \code{target} regions.
\item At exit from worksharing regions unless a \code{nowait} is present.
\item During a \code{cancel} or \code{cancellation}~\code{point} region, if the \plc{cancel-var} ICV is \plc{true} and cancellation has been activated.
\end{itemize}

A \code{flush} region arising from a \code{flush} directive without a list and
with the \code{release} clause present is implied at the following
locations:
\begin{itemize}
...
\item At entry to a \code{target}~\code{update} region whose corresponding construct has a \code{to} clause.
\item At entry to a \code{target}~\code{enter}~\code{data} and \code{target}~\code{data} region.
\item At exit from an \code{ordered} region, if a \code{threads} clause is
    present, if a \code{depend} clause with a \code{source} dependence type is
    present, or if no clauses are present.
\end{itemize}

A \code{flush} region arising from a \code{flush} directive without a list and
with the \code{acquire} clause present is implied at the following
locations:
\begin{itemize}
...
\item At exit from a \code{target}~\code{update} region whose corresponding construct has a \code{from} clause.
\item At exit from a \code{target}~\code{exit}~\code{data} and \code{target}~\code{data} region.
\item At entry to an \code{ordered} region, if a \code{threads} clause is
    present, if a \code{depend} clause with a \code{sink} dependence type is
    present, or if no clauses are present.
\end{itemize}
