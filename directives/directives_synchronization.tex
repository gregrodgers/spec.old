...
\#pragma omp atomic \plc{[memory-order-clause[},\plc{]]} \plc{atomic-clause} \plc{[[},\plc{]memory-order-clause]} \plc{new-line}
   \plc{expression-stmt}
\end{boxedcode}
...
\begin{boxedcode}
\#pragma omp atomic \plc{[memory-order-clause]} \plc{new-line}
   \plc{expression-stmt}
\end{boxedcode}
...
\begin{boxedcode}[fontsize=\small]
\#pragma omp atomic \plc{[memory-order-clause[},\plc{]]} capture \plc{[[},\plc{]memory-order-clause]} \plc{new-line}
    \plc{structured-block}
\end{boxedcode}
...
\begin{boxedcode}
!\$omp atomic \plc{[memory-order-clause]}
    \plc{update-statement }
\plc{[}!\$omp end atomic\plc{]}
\end{boxedcode}
...
\descr
All \code{atomic} constructs force an atomic memory operation to be performed
by the encountering thread, preceded and/or followed by implicit
flush operations as described in this section. Each non-synchronizing implicit flush is
a global flush. If \plc{memory-order-clause} is present and is
\code{seq\_cst}, each synchronizing implicit flush is also a global flush.
...
read is immediately preceded by a non-synchronizing acquire flush of \plc{x}.
\end{samepage}

The \code{atomic} construct with the \code{write} clause forces an atomic write of the location 
designated by \plc{x} regardless of the native machine word size. The atomic
write is immediately followed by a non-synchronizing release flush of \plc{x}.
...
a non-synchronizing acquire flush of \plc{x} and immediately followed by a
non-synchronizing release flush of \plc{x}.

The \code{atomic} construct with the \code{capture} clause forces an atomic
\emph{captured update} --- an atomic update of the 
...
\plc{x}. The atomic captured update is immediately preceded by a non-synchronizing acquire
flush of \plc{x} and immediately followed by a non-synchronizing release flush of
\plc{x}.

A synchronizing release flush operation applied to all variables, with
a sync-set containing the variable that is atomically operated on, is implied
at entry to the \code{atomic} region when the \code{read} clause is not
present and the \code{release}, \code{acq\_rel}, or \code{seq\_cst} clause is
present.  A synchronizing acquire flush operation applied to all
variables, with a sync-set containing the variable that is atomically operated
on, is implied at exit from an \code{atomic} region when the \code{read} or
\code{capture} clause is present and the \code{acquire}, \code{acq\_rel}, or
\code{seq\_cst} clause is present. 
...
\item If the \code{read} clause appears on the construct then
\plc{memory-order-clause} must not be \code{acq\_rel} or \code{release}.

\item If the \code{write} clause appears on the construct then
\plc{memory-order-clause} must not be \code{acq\_rel} or \code{acquire}.

\item If the \code{read}, \code{write}, and \code{capture} clauses do not
appear on the construct then \plc{memory-order-clause} must not be \code{acq\_rel}
or \code{acquire}.
\end{itemize}
...
If \plc{memory-order-clause} is not specified, the flush operation has the
global, release, and acquire flush properties. If \plc{memory-order-clause} is
\code{acq\_rel}, the flush operation has the release and acquire properties.
If \plc{memory-order-clause} is \code{release}, the flush operation has the
release flush property. If \plc{memory-order-clause} is \code{acquire}, the
flush operation has the acquire flush property. The sync-set for a flush
operation that arises from a \code{flush} construct is the same as its
specified flush-set.
...
\subsection{Synchronization in OpenMP Regions}
\label{subsec:Synchronization in OpenMP Regions}
\index{synchronization in openmp regions}


Synchronization between two threads is achieved by performing synchronizing
flush operations.  Synchronization using atomic constructs or explicit flush
operations in combination with non-sequentially consistent atomic constructs
is described in Section \ref{subsec:The Flush Operation} and Section
\ref{subsec:OpenMP Memory Consistency}. Synchronizing flush operations that
are implied on other OpenMP regions are described in Section \ref{subsec:flush Construct}.
These OpenMP regions provide the following synchronization
behavior:

\begin{itemize}
    \item When executing a \code{barrier} region, each thread in the team
	performs a release flush and an acquire flush, and the
	behavior is as if each thread's release flush synchronizes with
	every other thread's acquire flush.

    \item When executing a \code{critical} region, each thread in a contention
	group performs an acquire flush on entry to the \code{critical}
	region and a release flush on exit from the \code{critical} region,
	and the behavior is as if the release flush executed by one thread synchronizes
	with the acquire flush executed by the next thread to execute the
	\code{critical} region.

    \item When executing a \code{taskwait} region, the behavior is as if each
	remaining child task performs a release flush upon its completion
	that synchronizes with an acquire flush performed during the
	\code{taskwait} region.

    \item When executing a \code{taskgroup} region, the behavior is as if each
	remaining descendant task performs a release flush upon its
	completion that synchronizes with an acquire flush performed on
	exit from the \code{taskgroup} region.

    \item When executing an \code{ordered} region that does not arise from
	stand-alone \code{ordered} directive, a thread performs an acquire
        flush on entry to the \code{ordered} region and a
	release flush on exit from the \code{ordered} region,
	and the behavior is as if the release flush executed by one thread synchronizes
	with the acquire flush executed by the next thread to execute
	the \code{ordered} region. 

    \item When executing an \code{ordered} region that arises from a
	stand-alone \code{ordered} directive, a release flush is performed
	when the directive has a \code{depend} clause with a \code{source}
	dependence-type, and an acquire flush is performed when the
	directive has a \code{depend} clause with a \code{sink}
	dependence-type. The behavior is as if the release flush
	performed by the \code{ordered} region from a given source iteration
	synchronizes with the acquire flush performed by all
	\code{ordered} regions waiting for dependences on that iteration to be
	satisfied.

    \item When executing dependent sibling tasks, the behavior is as if each
	release flush performed on completion of a task with
	one or more dependent tasks synchronizes with the acquire flush
	performed when scheduling its dependent tasks.

    \item When executing an \code{omp\_unset\_lock} region that causes the
	specified lock to be unset, a release flush is
	performed that synchronizes with an acquire flush that
	is performed by the next \code{omp\_set\_lock} or
	\code{omp\_test\_lock} region that causes the specified lock to be set.

    \item When executing an \code{omp\_unset\_nest\_lock} region that causes
	the specified nested lock to be unset, a release flush 
	is performed that synchronizes with an acquire flush
	that is performed by the next \code{omp\_set\_nest\_lock} or
	\code{omp\_test\_nest\_lock} region that causes the specified nested
	lock to be set.
\end{itemize}
