...
\restrictions
\begin{itemize}
\item Unless the effect is as \code{hint}(\code{omp_sync_hint_none}) was specified, the \code{critical} construct must specify a name.
...
data object associated to the parallel region rather than a pointer to 
...
clause.  The effect of a hint is implementation defined; if ignoring the 
