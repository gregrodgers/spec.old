...
flush operations as described in this section. Each non-synchronizable implicit flush is
a global flush. If \plc{memory-order-clause} is present and is
\code{seq\_cst}, each synchronizable implicit flush is also a global flush.
...
read is immediately preceded by a non-synchronizable acquire flush of \plc{x}.
\end{samepage}

The \code{atomic} construct with the \code{write} clause forces an atomic write of the location 
designated by \plc{x} regardless of the native machine word size. The atomic
write is immediately followed by a non-synchronizable release flush of \plc{x}.
...
a non-synchronizable acquire flush of \plc{x} and immediately followed by a
non-synchronizable release flush of \plc{x}.
...
non-synchronizable acquire flush of \plc{x} and immediately followed by a
non-synchronizable release flush of \plc{x}.

A synchronizable release flush operation applied to all variables, with
...
present.  A synchronizable acquire flush operation applied to all
...
non-sequentially consistent \code{atomic} construct for which
