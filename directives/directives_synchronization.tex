...
\code{depend} clause on a \code{taskwait} construct.
 \item If the \plc{dependence-type} of a \code{depend} clause is \code{depobj}, then dependence
 objects of the \code{depend} clause cannot represent dependences with the 
...
When the \code{depend} clause \plc{dependence-type} is \code{depobj}, the task dependences are 
derived from the dependences represented by the depend objects specified on the \code{depend} 
