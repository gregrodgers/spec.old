...
global flush. If \plc{memory-order-clause} is present and is \code{seq\_cst},
each synchronizable implicit flush is also a global flush.
...
or \code{seq\_cst} clause is present.  A synchronizable acquire flush
...
flush operation has the acquire flush property. If \plc{memory-order-clause}
is present, the sync-set is empty.

If list items are specified on the \code{flush} construct, the sync-set of the
flush operation is empty. Otherwise, the sync-set contains all variables that
are flushed.
