...
Section \ref{subsec:OpenMP Memory Consistency}. The behavior of synchronizing
flush operations that are implied at various other locations in an OpenMP
program are defined in Section \ref{subsec:implicit flushes}.
...
\def\reductionflag{
If a reduction is being performed along with the region synchronization,
\plc{region_flags} will also include \code{ompt_sync_region_reduction}.
}

\def\omptSyncRegionTools#1#2{
...
\code{ompt_sync_region_#1}
in its \plc{kind} argument.
#2
The value \code{ompt_scope_begin} or \code{ompt_scope_end}
will be provided in its \plc{endpoint} argument, as appropriate.
...
\def\omptSyncRegion#1#2{
\events
\omptSyncRegionEvents{#1}
\tools
\omptSyncRegionTools{#1}{#2}
}
...
The \plc{implicit-barrier-reduction-begin} event occurs before a task 
begins to combine one or more of its reduction results with those of other tasks and
an \plc{implicit-barrier-reduction-end} event occurs after the task 
finishes combining reduction results with those of other tasks.
The interval defined by these events may not contain a task scheduling point.
...
This callback has the type signature
\code{ompt_callback_sync_region_t}.  
The callback receives \code{ompt_sync_region_barrier}
in its \plc{region_kind} argument.

If the barrier performs one or more reductions,
a thread will dispatch an \code{ompt_callback_sync_region_t} callback for 
each occurrence of an \plc{implicit-barrier-reduction-begin} and \plc{implicit-barrier-reduction-end} 
event in that thread.
The callback occurs in the implicit task executing in a parallel region.
This callback has the type signature \code{ompt_callback_sync_region_t}.  
The callback receives \code{ompt_sync_region_reduction} in its \plc{region_kind} argument.

The value \code{ompt_scope_begin} or \code{ompt_scope_end}
will be provided in the \plc{endpoint} argument, as appropriate, of each
\code{ompt_callback_sync_region_t} callback dispatched.
...
\omptSyncRegion{taskwait}{\reductionflag}
...
\omptSyncRegion{taskgroup}{\reductionflag}
...
A \code{flush} region that corresponds to a \code{flush} directive without a list and
...
\begin{itemize}
    \item When two different threads execute a \code{critical} region that
        corresponds to a \code{critical} construct that has the same name, the
    ...
    \item When executing a \code{taskwait} region that corresponds to a
        \code{taskwait} construct on which a \code{depend} clause is not
        present, the behavior is as if each thread that executes a remaining
        child task performs a release flush upon completion of the child task
        that synchronizes with an acquire flush performed in the
        \code{taskwait} region.

    \item When executing a \code{taskgroup} region, the behavior is as if each
	thread that executes a remaining descendant task performs a release flush
    ...
    \item When executing a \code{parallel} region, the behavior is as if the
        release flush performed by the master thread on entry to the
        \code{parallel} region synchronizes with the acquire flush performed
        on entry to each implicit task that is assigned to a different thread.

    \item When generating an explicit task that begins execution on a
