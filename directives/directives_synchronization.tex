...
for each occurrence of an \plc{implicit-barrier-reduction-begin} and \plc{implicit-barrier-reduction-end} 
event in that thread.  The callback occurs in the implicit task executing in a parallel region.
...
\events
\omptSyncRegionEvents{taskgroup}

The \plc{taskgroup-reduction-begin} event occurs before a task 
begins to contribute to a \code{task_reduction} and 
a \plc{taskgroup-reduction-end} event occurs after the task 
finishes combining to a \code{task_reduction}.
The interval defined by these events may not contain a task scheduling point.

\tools

\omptSyncRegionTools{taskgroup}

A thread dispatches a registered \code{ompt_callback_reduction}
for each occurrence of an \plc{taskgroup-reduction-begin} and \plc{taskgroup-reduction-end} 
event in that thread.  The callback occurs in the implicit task executing in a parallel region.
This callback has the type signature \code{ompt_callback_sync_region_t}.  
The callback receives \code{ompt_sync_region_reduction} in its \plc{kind} argument and
\code{ompt_scope_begin} or \code{ompt_scope_end} as its
