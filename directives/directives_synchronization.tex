...
encounters the \code{master} construct on entry to the master region.
...
\code{ompt_state_wait_barrier_implicit_parallel} results in unspecified behavior.
...
\item \plc{x} and \plc{v} (as applicable) are both \plc{l-value} expressions 
      with scalar type.
\item \plc{x} must not have an atomic type.
...
\item \plc{binop}, \plc{binop}\code{=}, \code{++}, and \code{-}\code{-} are not
...
\item \plc{hint-expression} is a constant expression that evaluates to a 
...
\code{seq_cst} clause is specified then the strong flush on entry to the
atomic operation is also a release flush.  If the \code{read} or \code{capture} clause
...
is also an acquire flush.  Therefore, if \plc{memory-order-clause} is specified and is not
...
avoid data races, all accesses of the locations designated by \plc{x}
...
\end{ompfPragma}
\begin{samepage} %$ Close off misinterpreted dollar sign/math symbol
where \plc{memory-order-clause} is one of the following:
...
implement a \code{flush} with a list by ignoring the list, and treating it the
same as a \code{flush} without a list.
...
the following two examples indicates that the accesses to \code{a} and
\code{b} are atomic write and atomic read operations. Otherwise both examples
would contain data races and automatically result in unspecified behavior. The
\plc{flush} operations are strong flushes that are applied to the specified
flush lists
...
\begin{itemize}
\item When a thread executes an \code{atomic} region for which the
      corresponding construct has the \code{release}, \code{acq_rel}, or
      \code{seq_cst} clause and specifies an atomic operation that starts a
...
\item When a thread executes an \code{atomic} region for which the
      corresponding construct has the \code{acquire}, \code{acq_rel}, or
      \code{seq_cst} clause and specifies an atomic operation that reads a
...
\item When a thread executes a \code{taskwait} region that does not result in
      the creation of a dependent task and the task that encounters the
      corresponding \code{taskwait} construct has at least one child task, the
      behavior is as if each thread that executes a child task that is
      generated before the \code{taskwait} region performs a release flush upon
...
\summary
The \code{depobj} construct initializes, updates or destroys an OpenMP depend 
...
A \code{depobj} construct with an \code{update} clause present changes the 
...
\item A \code{depend} clause on a \code{depobj} construct must not have 
      \code{source} or \code{sink} as \plc{dependence-type}.
\item An \code{update} clause on a \code{depobj} construct must not have
...
\code{inout}, \code{mutexinoutset} or \code{inoutset}. When the \plc{dependence-type} is 
...
The storage location of a list item matches the storage location of another
...
in a \code{depend} clause with an \code{out}, \code{inout}, \code{mutexinoutset}, or \code{inoutset}
...
\code{inout}, \code{mutexinoutset}, or \code{inoutset} \plc{dependence-type} on a construct from 
...
item appearing in a \code{depend} clause with an \code{in}, \code{out}, \code{inout}, or \code{inoutset}
\plc{dependence-type} on a construct from which a sibling task 
...
\plc{dependence-type} on a task-generating construct matches
...
For the \code{inoutset} \plc{dependence-type}, if the storage location of
at least one of the list items matches the storage location of a list
item appearing in a \code{depend} clause with an \code{in}, \code{out}, \code{inout}, or \code{mutexinoutset}
\plc{dependence-type} on a construct from which a sibling task
was previously generated, then the generated task will be a dependent task
of that sibling task.
...
The list items that appear in the \code{depend} clause may include array
sections or the \code{omp_all_memory} reserved locator.
...
\begin{ccppspecific}
The list items that appear in a \code{depend} clause may use shape-operators.
\end{ccppspecific}
...
\begin{note}
An iteration vector \plc{vec} that does not indicate a lexicographically 
earlier iteration may cause a deadlock.
\end{note}

\events

The \plc{task-dependences} event occurs in a thread that encounters a
task generating construct or a \code{taskwait} construct with a \code{depend} 
clause immediately after the \plc{task-create} event for the new task or 
the \plc{taskwait-begin} event.
...
\item List items,
      ...
      used in \code{depend} clauses of the same task or sibling
      tasks
...
\item The \code{omp_all_memory} reserved locator can only be used in a
      \code{depend} clause with an \code{out} or \code{inout} \plc{dependence-type}.
...
\begin{ccppspecific}
\item List items used in \code{depend} clauses with the \code{depobj} 
      dependence type must be expressions of the \code{omp_depend_t} type.
  \item List items that are expressions of the \code{omp_depend_t} type
      can only be used in \code{depend} clauses with the \code{depobj} dependence type.
\end{ccppspecific}
...
\item For a \plc{vec} element of \code{sink} \plc{dependence-type} of the form
      ...
      the expression 
      $($~x\textsubscript{i}~$-$~lb\textsubscript{i}~$)$~$+$~d\textsubscript{i} or
      ...
      encounter the \code{ordered} construct must be computable without overflow
      in the type that would be used by \plc{std::distance} applied to variables 
      of the type of x\textsubscript{i}.
\end{cppspecific}
...
\item \code{depobj} construct, see
\specref{subsubsec:depobj Construct}.
...
provided by the programmer to locks (by using the \code{omp_init_lock_with_hint} 
or \code{omp_init_nest_lock_with_hint} functions to initialize the lock), and to
\code{atomic} and \code{critical} directives by using the \code{hint} clause. The 
...
The C/C++ header file (\code{omp.h}) and the Fortran include file (\code{omp_lib.h}) 
and/or Fortran~90 module file (\code{omp_lib}) define the valid hint constants.
...
The hints can be combined by using the \code{+} or \code{|} operators in C/C++ 
or the \code{+} operator in Fortran. Combining \code{omp_sync_hint_none} with 
any other hint is equivalent to specifying the other hint.

The intended meaning of each hint is:

\begin{itemize}
\item \code{omp_sync_hint_uncontended}: low contention is expected in this 
...
\item \code{omp_sync_hint_contended}: high contention is expected in this 
      operation, that is, many threads are expected to perform the 
      operation simultaneously in a manner that requires synchronization;
\item \code{omp_sync_hint_speculative}: the programmer suggests that the 
      operation should be implemented using speculative techniques such 
      as transactional memory; and
\item \code{omp_sync_hint_nonspeculative}: the programmer suggests that 
...
The \code{omp_sync_hint_t} and \code{omp_lock_hint_t} enumeration types and 
...
\begin{itemize}
\item The hints \code{omp_sync_hint_uncontended} and \code{omp_sync_hint_contended} 
      cannot be combined.
\item The hints \code{omp_sync_hint_nonspeculative} and 
      \code{omp_sync_hint_speculative} cannot be combined.
\end{itemize}

The restrictions for combining multiple values of \code{omp_sync_hint} apply equally 
to the corresponding values of \code{omp_lock_hint}, and expressions that mix the 
two types.
...
\item \code{omp_init_lock_with_hint} and \code{omp_init_nest_lock_with_hint}, see
