...
\descr
Only the master thread of the team executing the binding \code{parallel} region
...
\descr
The region associated to a \code{critical} construct is executed as if only a single thread at a time among all threads in the
...
\restrictions
\begin{itemize}
\item Unless the effect is as if \code{hint}(\code{omp_sync_hint_none}) was specified, the \code{critical} construct must specify a name.
...
\item \plc{hint-expression} is a constant integer expression that evaluates to a valid synchronization
hint.
\end{itemize}
...
\item Where \plc{hint-expression} is a constant expression that evaluates to a scalar value with kind
 \code{omp_sync_hint_kind} and a value that is a valid synchronization hint.
...
A \code{depobj} construct with a \code{depend} clause present sets the state of \plc{depobj} to initialized. The \plc{depobj} is initialized 
to represent the dependence specified by the \code{depend} clause.

A \code{depobj} construct with a \code{destroy} clause present changes the state of the \plc{depobj} to uninitialized.
...
\restrictions
\begin{itemize}
 \item A \code{depend} clause on a \code{depobj} construct must not have \code{source} or \code{sink} as \plc{dependence-type}.
...
where \plc{n} is the value specified by the \code{ordered} clause in the worksharing-loop
directive, x\textsubscript{\plc{i}} denotes the loop iteration variable of the \plc{i}-th
nested loop associated with the worksharing-loop directive, and d\textsubscript{\plc{i}} is a
...
\item Array sections cannot be specified in \code{depend} clauses with no \plc{dependence-type} specified.
\item List items used in \code{depend} clauses with no \plc{dependence-type} specified must be depend objects in the initialized state.
...
\item List items used in \code{depend} clauses with no \plc{dependence-type} specified must be integer expressions of the \code{omp_depend_kind} \plc{kind}.
...
clause. The effect of a hint is implementation defined but cannot 
