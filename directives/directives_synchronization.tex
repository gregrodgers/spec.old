...
\code{depend} clause on a \code{taskwait} construct.
 \item If the \plc{dependence-type} of a \code{depend} clause is \code{depobj}, then the 
 \code{mutexinoutset} \plc{dependence-type} may not appear in the \code{depend} clause 
 of the \code{depobj} constructs that inialitized the depend objects.
\end{itemize}
...
items when \plc{dependence-type} is \code{in}, \code{out}, \code{inout}, or \code{mutexinoutset}. When the \code{depend} clause \plc{dependence-type} \plc{depobj} is specified, the task dependences are derived from the \code{depend} clause specified in the \code{depobj} constructs that initalized the depend objects specified on the \code{depend} clause as if the \code{depend} clauses of the \code{depobj} constructs were specified in the current construct. 
...
\begin{ccppspecific}
\item List items used in \code{depend} clauses with the \code{depobj} \plc{dependence-type} specified must be expressions of the \code{omp_depend_t} type.
...
\item List items that appear in \code{depend} clauses must not have an incomplete C/C++ type or be a reference to an incomplete type.
\end{ccppspecific}
