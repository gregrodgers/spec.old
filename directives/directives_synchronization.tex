...
which can optionally be used to optimize the implementation. 
The presence of a \code{hint} clause does not affect the isolation guarantees provided by the \code{atomic} construct.
...
\begin{itemize}
\item When a \code{hint} is present at the construct, \code{atomic} constructs which
...
\code{omp\_init\_nest\_lock\_with\_hint} functions to initailize the lock), and to
...
are synonyms for each other which are provided to avoid any user confusion caused by using a hint which
explicitly includes the word "lock" when dealing with a \code{atomic} construct whose implementation is not
expected to use locks. The user is free to choose whichever spelling
of the names she prefers.
