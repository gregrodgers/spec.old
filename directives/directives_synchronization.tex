...
a scalar value with kind \code{omp\_synch\_hint\_kind} and  a value
...
\code{hint} clause is specified, the effect is as if \code{hint(omp\_synch\_hint\_none)}
had been specified.
...
\begin{itemize}
\item When a \plc{hint} is provided all \code{atomic} constructs which
may be simultaneously accessing the storage locations designated by
\plc{x} are required to have hints which evaluate to the same value. 
\end{itemize}
...
typedef omp\_synch\_hint\_t omp\_lock\_hint\_t;
\end{codepar}
\end{ccppspecific}
...
Combining \code{omp\_synch\_hint\_none} with any other hint is equivalent to specifying the other hint.
The following restrictions apply to combined hints; violating these 
restrictions results in unspecified behavior:
\begin{itemize}
\item the hints \code{omp\_synch\_hint\_uncontended} and \code{omp\_synch\_hint\_contended} cannot be combined,
\item the hints \code{omp\_synch\_hint\_nonspeculative} and \code{omp\_synch\_hint\_speculative} cannot be combined.
\end{itemize}
The rules for combining \code{omp\_synch\_hint}s apply equally to the
corresponding \code{omp\_lock\_hint}s, and expressions mixing the two types.

\begin{note}
The \code{omp\_synch\_hint\_t} and \code{omp\_lock\_hint\_t} enumerations (and the equivalent types in Fortran) 
...
\code{omp\_synch\_hint\_t} type and the \code{omp\_synch\_hint\_kind} kind.  
Implementers are advised to add implementation-defined hints starting from 
the most significant bit of the \code{omp\_synch\_hint\_t} type and
\code{omp\_synch\_hint\_kind} kind and to include the name of the 
