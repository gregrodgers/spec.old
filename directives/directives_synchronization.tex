...
\summary
The \code{depobj} construct allows to initalize, update and destroy depend objects that represent data dependences.
...
\end{ompcPragma}
\end{samepage}
where \plc{depobj} is an expression of type \code{omp_depend_t}.

where \plc{clause} is one of the following{}:

\begin{indentedcodelist}
depend(\plc{[depend-modifiers}:\plc{]dependence-type }:\plc{ locator})
destroy
...
\end{ompfPragma}
where \plc{depobj} is an integer expression of the \code{omp_depobj_kind} \plc{kind}.
...
\restrictions
\begin{itemize}
 \item A \code{depend} clause on a \code{depobj} construct must specify the \plc{dependence-type}.
...
items when \plc{dependence-type} is \code{in}, \code{out}, \code{inout}, or \code{mutexinoutset}. When the \code{depend} clause has no \plc{depedence-type} specified, the task dependences are derived from the \code{depend} clause specified in the \code{depobj} constructs that initalized the depend objects specified on the \code{depend} clause as if the \code{depend} clauses of the \code{depobj} constructs were specified in the current construct. 
...
\item Array sections cannot be specified in \code{depend} clauses wit no \plc{dependence-type} specified.

\begin{ccppspecific}
\item List items used in \code{depend} clauses with no \plc{dependence-type} specified must be expressions of the \code{omp_depend_t} type.
...
\item List items used in \code{depend} clauses with no \plc{dependence-type} specified must be integer expressions of the \code{omp_depobj_kind} \plc{kind}.
