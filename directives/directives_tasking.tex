...
affinity(\plc{[aff-modifier [}, \plc{aff-modifier]}:\plc{]list})
detach(\plc{event-handler})
...
where \plc{event-handler} is variable of the \code{omp_event_t *} type.
...
where \plc{event-handler} is a variable of the \code{omp_event_kind} integer \plc{kind}
...
when the execution code of the associated structured block is completed.
...
If a \code{detach} clause is present on a \code{task} construct a new event of type \code{omp_event_t}, \plc{detach-event}, is created. The \plc{detach-event} is connected to the completion of the associated \code{task} region. The original \plc{event-handler} will be updated to point to the \plc{detach-event} event before the task data environment is created. The \plc{event-handler} will be considered as if it was specified on a \code{firstprivate} clause. Note that the use of a variable in a \code{detach} clause expression of a \code{task} construct causes an
implicit reference to the variable in all enclosing constructs

If no \code{detach} clause is present on a \code{task} construct the generated \code{task} is completed when the execution of its associated structured block is completed. If a \code{detach} clause is present on a \code{task} construct the task is completed when the execution of its associated structured block is completed and the \plc{detach-event} is fulfilled. In any case, the task data environment is destroyed at the end of the associated structured block. 
...
region includes a task scheduling point at the end of its associated structured block.
...
task. The task is recommended to execute closely to the location of the \plc{list} items. 
A program that relies on the task execution location
being determined by this \plc{list} may have unspecified behavior.
...
In the dispatched callback, \code{(task_type & ompt_task_explicit)} always
evaluates to \plc{true}.
If the task is an undeferred task, then \code{(task_type & ompt_task_undeferred)} evaluates to \plc{true}.
If the task is a final task, \code{(task_type & ompt_task_final)} evaluates to \plc{true}.
If the task is an untied task, \code{(task_type & ompt_task_untied)} evaluates to \plc{true}.
If the task is a mergeable task, \code{(task_type & ompt_task_mergeable)} evaluates to \plc{true}.
If the task is a merged task, \code{(task_type & ompt_task_merged)} evaluates to \plc{true}.
...
have canonical loop form (see \specref{sec:Canonical Loop Form}).
\end{fortranspecific}
...
\code{taskloop} construct and does not enclose the intervening code is zero
