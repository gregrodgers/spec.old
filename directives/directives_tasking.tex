...
affinity(\plc{list}[:\plc{iterators-definition}])
detach(\plc{event-handler})

\end{indentedcodelist}

where \plc{event-handler} is expression of the \code{const omp_event_t *} type.
...
\item If a \code{detach} clause appears on the directive, then neither a \code{final} clause nor a \code{mergeable} clause can appear on the same directive.
