...
allocate(\plc{[allocator }:\plc{] list})    %% LOTL like depend and affinity
...
where \plc{event-handler} is an integer variable of \code{omp_event_kind} \plc{kind}
...
If a \code{detach} clause is present on a \code{task} construct a new event of type \code{omp_event_t} (\code{omp_event_kind} \plc{kind}, for Fortran), 
...
\plc{event-handler} will be updated to point to the \plc{allow-completion-event}
event before the task data environment is created. The \plc{event-handler} will 
be considered as if it was specified on a \code{firstprivate} clause. Note that 
...
where \plc{clause} is one of the following:
\begin{indentedcodelist}
if(\plc{[} taskloop :\plc{] scalar-expression})  %%LOTL "scalar-expression"
shared(\plc{list})
private(\plc{list})
firstprivate(\plc{list})
...
mergeable
nogroup
allocate(\plc{[allocator }:\plc{] list})    %% now LOTL like depend and affinity
\end{indentedcodelist}
...
where \plc{clause} is one of the following:
\begin{indentedcodelist}
if(\plc{[} taskloop :\plc{] scalar-logical-expression})  %%LOTL "scalar-expression"
shared(\plc{list})
private(\plc{list})
firstprivate(\plc{list})
lastprivate(\plc{list})
...
the generated tasks use a \plc{priority-value} as if it was
