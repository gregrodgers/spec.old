...
%% There should be a space between ] and list. LOLT like depend and affinity
...
If a \code{detach} clause is present on a \code{task} construct a new event of type \code{omp_event_t},
...
In the dispatched callback, \code{(\plc{flag} & ompt_task_explicit)} always
evaluates to \plc{true}.
If the task is an undeferred task, then \code{(\plc{flag} & ompt_task_undeferred)} evaluates to \plc{true}.
If the task is a final task, \code{(\plc{flag} & ompt_task_final)} evaluates to \plc{true}.
If the task is an untied task, \code{(\plc{flag} & ompt_task_untied)} evaluates to \plc{true}.
If the task is a mergeable task, \code{(\plc{flag} & ompt_task_mergeable)} evaluates to \plc{true}.
If the task is a merged task, \code{(\plc{flag} & ompt_task_merged)} evaluates to \plc{true}.
