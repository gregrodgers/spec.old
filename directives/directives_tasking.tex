...
where \plc{event-handle} is a variable of the \code{omp_event_kind} integer \plc{kind}
...
If a \code{detach} clause is present on a \code{task} construct a new event of type \code{omp_event_t}, \plc{allow-completion-event}, is created. The \plc{allow-completion-event} is connected to the completion of the associated \code{task} region. The original \plc{event-handle} will be updated to point to the \plc{allow-completion-event} event before the task data environment is created. The \plc{event-handle} will be considered as if it was specified on a \code{firstprivate} clause. Note that the use of a variable in a \code{detach} clause expression of a \code{task} construct causes an implicit reference to the variable in all enclosing constructs.
...
suspend the current task region, for which execution cannot be resumed until 
the execution of the \plc{structured-block} associated with the generated task is completed.
The use of a variable in an \code{if} clause expression
