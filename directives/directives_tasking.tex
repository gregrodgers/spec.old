...
\code{ompt_callback_task_create_t}. The callback receives \code{ompt_task_initial} 
as its \plc{type} argument. The implicit parallel region does not dispatch a 
\code{ompt_callback_parallel_begin} callback; however, the implicit parallel 
region can be initialized within this \code{ompt_callback_task_create} callback.
...
\item \code{ompt_callback_task_create_t},
see \specref{sec:ompt_callback_task_create_t}.
\end{itemize}
...
\item Scheduling of new tied tasks is constrained by the set of task regions 
      ...
      \code{barrier} region. If this set is empty, any new tied task may be 
