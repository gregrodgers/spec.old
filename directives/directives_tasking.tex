...
where \plc{event-handle} is a variable of the \code{omp_event_handle_t} type.
...
where \plc{event-handle} is an integer variable of \code{omp_event_handle_kind} \plc{kind}.
\end{fortranspecific}
...
\plc{allow-completion-event} is created. The 
\plc{allow-completion-event} is connected to the completion of the associated 
\code{task} region. The original \plc{event-handle} will be updated to represent 
the \plc{allow-completion-event} event before the task data environment is 
...
\code{task} is completed when the execution of its associated 
\plc{structured-block} is completed. If a \code{detach} clause is present on a 
\code{task} construct the task is completed when the execution of its associated 
\plc{structured-block} is completed and the \plc{allow-completion-event} 
is fulfilled.
...
evaluated in the context outside of the \code{task} construct, and no ordering 
...
\code{ompt_callback_task_create_t} and the \plc{flags} argument indicates the 
...
\tablefirsthead{%
\hline
\textsf{\textbf{Operation}} & \textsf{\textbf{Evaluates to true}}\\
\hline\\[-3ex]
}
\tablehead{%
...
\tablelasttail{\hline}
\tablecaption{\code{ompt_callback_task_create} callback flags evaluation\label{tab:task create callback flags evaluation}}
\begin{supertabular}{p{2.1in}p{3in}}
{\scode{(}\plc{flags}~\scode{&}~\scode{ompt_task_explicit}\scode{)}}     &   Always in the dispatched callback \\
{\scode{(}\plc{flags}~\scode{&}~\scode{ompt_task_undeferred}\scode{)}}   &   If the task is an undeferred task \\
{\scode{(}\plc{flags}~\scode{&}~\scode{ompt_task_final}\scode{)}}        &   If the task is a final task \\
{\scode{(}\plc{flags}~\scode{&}~\scode{ompt_task_untied}\scode{)}}       &   If the task is an untied task \\
{\scode{(}\plc{flags}~\scode{&}~\scode{ompt_task_mergeable}\scode{)}}    &   If the task is a mergeable task \\
{\scode{(}\plc{flags}~\scode{&}~\scode{ompt_task_merged}\scode{)}}       &   If the task is a merged task  \\
\end{supertabular}
...
clause with the same reduction operator and list items is present on the \code{simd} construct.
...
\item At most one \code{if} clause with the \code{taskloop}
      \plc{directive-name-modifier} can appear on the directive.
...
callback, \code{(}\plc{flag}~\code{&}~\code{ompt_task_initial)} always evaluates to
\plc{true}.
...
\item during the generation of a \code{target} region;
...
\item in the \code{omp_target_memcpy_async} routine;

\item in the \code{omp_target_memcpy_rect} routine;

\item in the \code{omp_target_memcpy_rect_async} routine.
\end{itemize}
