...
prior to the \plc{initial-task-begin} event.
...
for each occurrence of a \plc{initial-task-begin} in that thread.
...
for each occurence of a \plc{initial-task-end} event in that thread. 
The callback occurs in the context of the initial task.  The callback 
has type signature \code{ompt_callback_implicit_task_t}.
