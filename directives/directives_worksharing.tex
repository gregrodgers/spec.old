...
\begin{supertabular}{ p{0.8in} p{4.5in}}
    ...
    type of {\splc{var}} that are loop invariant with respect to the
    outermost associated loop or are one of the
    following (where {\splc{var-outer}} is {\splc{var}} from an outer
    associated loop, and {\splc{a1}} and {\splc{a2}} are integer literals or
    variables that are loop invariant with respect to the outermost loop): \\
    & {\splc{var-outer}} \\
...
\begin{itemize}
    \item For any associated loop where \plc{b} or \plc{lb} is not loop invariant with
        respect to the outermost loop, let \plc{c} be the coefficient on
        \plc{var} in the expression resulting from subtracting \plc{lb} from
        \plc{b}, and let
        \plc{incr-outer} be the \plc{incr} expression of the
        outer loop referred to by \plc{var-outer}. The result of \plc{c}
        multiplied by \plc{incr-outer} must be evenly divisible by \plc{incr}.
...
\item The \code{schedule} clause must not appear on the loop directive if the
    associated loop(s) form a non-rectangular loop nest.

\item The \code{ordered} clause must not appear on the loop directive if the
    associated loop(s) form a non-rectangular loop nest.
