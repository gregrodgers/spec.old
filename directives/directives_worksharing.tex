...
\begin{supertabular}{ p{0.8in} p{4.5in}}
    ...
    type of {\splc{var}} that are either loop invariant with respect to the
    outermost associated loop or are one of the
    following (where {\splc{var-outer}} is {\splc{var}} from an outer
    associated loop, and {\splc{a1}} and {\splc{a2}} are integer literal
    constants):\\
    & {\splc{var-outer}} \\
    ...
    to the outermost loop in the loop nest.\\
\end{supertabular}
\medskip
...
\begin{itemize}
    ...
    \item If the associated loops form a non-rectangular loop nest, the
        \plc{incr} expression for any associated loop must be an integer
        literal constant.
