...
\code{MANDATORY \textnormal{|} PORTABLE \textnormal{|} DISABLED}

The \code{MANDATORY} value specifies that a target construct or a device memory routine must execute on a target device. If the target construct cannot execute on its target device, a warning is issued and the program execution aborts. Target constructs are exempt from this behavior when an if-clause is present and the if-clause expression evaluates to false. 

The \code{DISABLED} value specifies that a target construct or a device memory routine must execute on the host. The behavior with this environment value is equivalent to an if clause present on all target constructs, where each of these if clause expressions evaluate to false.

The \code{PORTABLE} value specifies that a target construct or a device memory routine may execute on a target device. Examples where a target construct may not execute on a target device includes executing a program on a machine that has no accelerators. 
