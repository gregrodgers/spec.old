...
that contains a colon-separated list of tool libraries that the runtime searches to find
one appropriate for use on a device where an OpenMP implementation is being initialized.

\item \code{OMP\_ALLOCATOR} sets the \plc{def-allocator-var} ICV that specifies the default memory
allocator on the host device.

\item \code{OMP\_TARGET\_ALLOCATOR} sets the \plc{target-def-allocator-var} ICV that specifies the default memory
allocator on the target construct.
\end{itemize}
...
allocator for allocation calls, directives and clauses that use default allocation other than those affected by the \plc{target-def-allocator-var} ICV.
The value of this environment variable is a predefined allocator. The value of this environment variable is not case sensitive.
...
\section{\code{OMP\_TARGET\_ALLOCATOR}}
\index{OMP\_TARGET\_ALLOCATOR@{\code{OMP\_TARGET\_ALLOCATOR}}}
\index{environment variables!OMP\_TARGET\_ALLOCATOR@{\code{OMP\_TARGET\_ALLOCATOR}}}
\label{sec:OMP_TARGET_ALLOCATOR}

\code{OMP\_TARGET\_ALLOCATOR} sets the \plc{target-def-allocator-var} ICV that specifies the default 
allocator on \code{target} constructs clauses that use default allocation.
The value of this environment variable is a predefined allocator. The value of this environment variable is not case sensitive.
