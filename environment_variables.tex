...
\section{\code{OMP\_DEBUG}}
\index{OMP\_DEBUG@{\code{OMP\_DEBUG}}}
\index{environment variables!OMP\_DEBUG@{\code{OMP\_DEBUG}}}
\label{sec:OMP_DEBUG}

The \code{OMP\_DEBUG} environment variable sets the
\plc{third-party-tool-var} ICV which controls whether an OpenMP runtime
...
or \code{disabled}.
If \code{OMP\_DEBUG} is set to any value other than \code{enabled}
or \code{disabled}, the behavior is unspecified.
If \code{OMP\_DEBUG} is not defined, the default value for
\plc{third-party-tool-var} is \code{disabled}.

Example:
\begin{boxedcode}
\% setenv OMP\_DEBUG enabled
\end{boxedcode}

\crossreferences
\begin{itemize}
\item \plc{third-party-tool-var} ICV,
see \specref{sec:Internal Control Variables}.
\item \code{omp\_debug\_enable}, see \specref{sec:ompd:omp_debug_enable}
\item Tool Interface, see \specref{chap:ToolsSupport}
\item Enabling the Runtime for OMPD, see \specref{sec:ompd:enabling-ompd}.
\end{itemize}
