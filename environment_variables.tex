...
setting the initial value of the \plc{max-active-levels-var} ICV. The value of
this environment variable must be \code{true} or \code{false}. If the
...
If the \code{OMP_MAX_ACTIVE_LEVELS} and \code{OMP_NESTED} environment
variables are both set, \plc{max-active-levels-var} will be initialized
according to the environment variable which results in the smaller initial
value; the other environment variable will have no effect.
