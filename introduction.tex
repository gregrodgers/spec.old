...
When a thread encounters an \code{independent} construct, an implementation may
choose to distribute, workshare, and/or SIMD vectorize the iterations of the
loop if it would be legal for the programmer to have explicitly used such
execution at this point in the code. If the implementation chooses to
distribute, workshare, and/or SIMD vectorize the iterations of the loop, the
data sharing and binding rules for the loop will be as if the programmer had
explicitly specified the parallelism used to execute the loop.  When an
\code{independent} construct is encountered, an implementation must guarantee
that the iterations of the associated loops are executed exactly once. When
encountering an independent construct an implementation will not generate new
thread teams, but may use any parallelism currently available to it. An
implementation may choose to perform optimizations or automatic parallelization
schemes that are outside of the scope of OpenMP.
%JLARKIN The last sentence is arguably superfluous, but I feel it is important
%to state because of the nature of this directive.
