...
OpenMP. If the implementation chooses to distribute, workshare, and/or SIMD
vectorize the iterations of the loop, the data sharing and binding rules for
the loop will be as if the programmer had explicitly specified the parallelism
used to execute the loop.  When an \code{independent} construct is encountered,
an implementation must guarantee that the iterations of the associated loops
are executed exactly once. When encountering an \code{independent} construct an
implementation will not generate a new league of thread teams, but may use any
thread teams currently available to it. 
