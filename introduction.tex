...
\glossaryterm{OMPT}
\glossarydefstart
An interface that enables a first-party tool to monitor the execution of an OpenMP program.
\glossarydefend
	
\glossaryterm{OMPD}
\glossarydefstart
An interface that enables third-party tools to inspect the OpenMP state of a program that has begun execution.
\glossarydefend
...
A opaque reference provided by an OMPD library implementation to a third-party tool. A handle uniquely identifies an abstraction.
...
A handle that refers to an instance of an OpenMP parallel region. 
...
A handle that refers to an instance of an OpenMP task region. 
