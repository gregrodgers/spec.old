...
\glossarydefstart
For C/C+, an ordering that exists between two operations executed by the same
thread, as defined by their sequenced before relation.

For Fortran, an ordering that exists between two operation executed by the
same thread, as defined by the program's execution sequence.
\glossarydefend
...
% the first must be a release flush and the second must be an acquire flush it
% synchronizes with.
% \end{enumerate}
% 
...
% 
% Additionally, if two operations have a happens before order according to the
% base language, they are treated as if they have a happens before order for the
% purposes of determining memory consistency guarantees in OpenMP.
...
\item \plc{X} synchronizes with \plc{Y}.
...
variable by an operation that happens before all operations occurring in the
program.

\subsection{OpenMP Memory Consistency}
\label{subsec:OpenMP Memory Consistency}
Given the happens before order defined in \specref{subsec:happens-before}
...
%    as if in that happens before order, as seen by all threads.
