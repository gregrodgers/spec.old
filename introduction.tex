...
\glossarydefstart
A tool that executes as a separate process from that which it is monitoring.
\glossarydefend
...
\glossaryterm{OMPD}
\glossarydefstart
A programing interface that allows third-party tools to inspect the state of OpenMP programs.
\glossarydefend

\glossaryterm{OMPD Library}
\glossarydefstart
A dynamic link library (DLL) that implements the OMPD interface.
\glossarydefend

\glossaryterm{Process}
\glossarydefstart
A collection of one or more threads and address spaces.
...
A process may be a ``live'' operating system process, or a core file.
\glossarydefend
...
A collection of logical, virtual, or physical memory address ranges containing code, stack, and data. 
The memory address ranges within an address space need not be contiguous. An address space 
has associated with it a collection of image files that have been loaded into it.
\glossarydefend

\glossaryterm{Image File}
\glossarydefstart
An executable or shared library file that is loaded into an address space and that provides symbolic 
debug information to a debugger.
\glossarydefend

\glossaryterm{Target Architecture}
\glossarydefstart
...
A combination of the processor (CPU or GPU) and the Application Binary Interface (ABI) used by 
threads and address spaces. A process may contain threads and address spaces for multiple 
target architectures.
\glossarydefend

\glossaryterm{OMPD Handle}
\glossarydefstart
...
An identifier of an OpenMP entity that is valid during the execution of an OpenMP program. OMPD 
handles are created by an OMPD library upon requests from a third-party tool and are opaque to 
the third-party tool. Handles remain valid until they are released by third-party tools.
\glossarydefend
...
%	OMPD API calls.}
An OMPD handle that identifies a portion of an instance of an OpenMP program that is running on 
a host device or a target device.
\glossarydefend
...
An OMPD handle that identifies an OpenMP thread. A handle is valid for the life time of the 
...
An OMPD handle that identifies an OpenMP parallel region. The handle is guaranteed to be valid if 
at least one thread in the parallel region is paused, or if a thread in a nested parallel region is 
paused.
\glossarydefend
...
An OMPD handle that identifies an OpenMP task region. The handle is valid for the life time of the 
task region. The handle is guaranteed to be valid if all threads in the task team are paused.
\glossarydefend

\glossaryterm{Debugger Contexts}
\glossarydefstart
...
An identifier of a process, address space, or thread that resides in a third-party tool. Contexts are 
opaque to an OMPD library, and are defined by a third-party tool.
\glossarydefend
...
An identifier of a third-party object for a portion of an instance of an OpenMP program that is 
running on a host or target device. An address space is contained within a process, and has an 
associated target architecture. The address space context must be valid for the life time of its
associated address space handle.
\glossarydefend
...
An identifier of a third-party tool object for a thread. The third-party tool owns and initializes the 
thread context. An OMPD library implementation can assume that the thread context is valid
for as long as the third-party tool is holding any references to thread handles that may contain the 
thread context.
\glossarydefend

\glossaryterm{Operating System Thread Identifier}
\glossarydefstart
...
An identifier of an object in a third-party tool that allows the third-party tool and an OMPD library
to map a thread handle to and from a thread context. The operating system-specific information is 
platform dependent, and therefore is not defined explicitly in this API.
\glossarydefend
...
the OpenMP API includes two tool interfaces: OMPD and OMPD.
...
The OMPD interface is intended for \emph{third-party} tools
that run outside the OpenMP program as a separate process.
The basic model is that an OpenMP implementor provides an OMPD plugin
shared library that can be used by a third-party tool to access
OpenMP-related information about a program using that implementation
of OpenMP.
OMPD defines the following:

\begin{itemize}
\item
  an interface an OMPD plugin shared library exports which a
  third-party tool can use to access OpenMP information about
  the OpenMP program it is examining;
\item
  a callback-interface that the third-party tools provides to the
  OMPD plugin that it can use to access the OpenMP program;
\item
  a small number of symbols that must be defined in an OpenMP program
  to help the third-party tool find the correct OMPD plugin to use
  for that OpenMP impementation, and to facilitate notification
  of events.
\end{itemize}
