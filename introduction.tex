...
\begin{itemize}
\item An implementation must not reorder with respect to a write flush any
...
\item An implementation must not reorder with respect to a read flush any
...
\item An implementation must not reorder with respect to eachother a write
    flush and a read flush that are applied to the same variable.
\end{itemize}
...
a shared variable since its last write flush of that variable, then when it
executes another write flush of the variable, the completion of the flush
...
between two write flushes of that variable, the write flush ensures that
it is the value of the last write that propagates to memory.  A read flush
...
value in the temporary view. When a thread executes a read flush, no later
...
\item The variable is flushed, with an read flush, by the second thread.   
