...
\glossarydefstart
The set of variables that a given \emph{flush} operation is applied to.
\glossarydefend

\glossaryterm{flush property}
\glossarydefstart
Properties that determine the manner in which a \emph{flush} operation enforces
memory consistency. These properties are: release, acquire, and global.

\begin{quote}
COMMENT: A given \emph{flush} operation can have one or more \emph{flush properties}.
\end{quote}
\glossarydefend

\glossaryterm{release flush}
\glossarydefstart
A \emph{flush} operation that has the release \emph{flush property}.
\glossarydefend

\glossaryterm{acquire flush}
\glossarydefstart
A \emph{flush} operation that has the acquire \emph{flush property}.
\glossarydefend

\glossaryterm{global flush}
\glossarydefstart
A \emph{flush} operation that has the global \emph{flush property}.
\glossarydefend
...
\glossarydefstart
A \emph{flush} operation with a non-empty \emph{sync-set}.
\glossarydefend

\glossaryterm{non-synchronizable flush}
\glossarydefstart
A \emph{flush} operation with an empty \emph{sync-set}.
\glossarydefend
...
containing a set of synchronization variables that may be used to synchronize
it with another flush operation.  The flush operation may
restrict reordering of memory operations that an implementation might
otherwise do. For a given release flush, an implementation must not reorder
after it any prior code for a memory operation or acquire flush on a variable
in its flush-set, and it must also not reorder before it any subsequent writes
to a variable in its sync-set.  Similarly, for a given acquire flush, an
implementation must not reorder before it any subsequent code for a memory
operation or release flush on a variable in its flush-set, and it must also
not reorder after it any prior reads from a variable in its sync-set. All
flushes occurring within an \code{atomic} region must be performed along with
the atomic memory operation as a single atomic action.

If a thread has performed a write to its temporary view of
a shared variable since its last release flush of that variable, then when it
executes another release flush of the variable, the completion of the flush
ensures the written value propagates to memory so that it can be read by
another thread.  If a thread performs multiple writes to the same variable
between two release flushes of that variable, the release flush ensures that
it is the value of the last write that propagates to memory.  If an acquire flush
of a variable executed by a thread causes its temporary view of the variable
to be discarded, so that if its next memory operation for that variable is a
read, then the thread will read from memory when it may again capture the
value in the temporary view. When a thread executes an acquire flush, no later
memory operation by that thread for a variable involved in that flush is
allowed to start until the flush completes.  

If a flush operation is a global flush applied to a given variable, its
completion will be ordered in some sequential manner with respect to all other
global flushes applied to the same variable. A global flush also imposes
stronger ordering requirements for memory operations performed on variables in
its flush-set, as described in the following section.

Flush operations enable a program to provide a guarantee of consistency
between a thread’s temporary view and memory. Therefore, flush operations can
be used to guarantee that a value written to a variable by one thread may be
read by a second thread. To accomplish this, the programmer must ensure that
the second thread has not written to the variable since its last flush of the
variable, and that the following sequence of events happens in the specified
order: 

\begin{enumerate}
\item The value is written to the variable by the first thread.   

\item The variable is flushed, with a release flush, by the first thread.    

\item The variable is flushed, with an acquire flush, by the second thread.   
...
A synchronizable release flush may synchronize with a synchronizable acquire
flush using a variable that is in the sync-sets of both flushes.
...
\begin{note}
The conditions in which a release flush synchronizes with an acquire flush
are intended to match the conditions for when release operations
synchronize with acquire operations in C11/C++11. A read-modify-write atomic
modification includes all atomic update or atomic captured update operations
specified with the \code{atomic} construct, as described in
\specref{subsec:atomic Construct}.
\end{note}

If there exists a sequence of operations executed in a program where the
following conditions are satisfied, each operation in the sequence
\emph{happens before} all subsequent operations in the sequence:

\begin{enumerate}
\item Any two operations performed by the same thread are ordered in the
sequence according to that thread’s program order.

\item Any two consecutive operations are operating on the same variable.

\item If any two consecutive operations are performed by different threads,
the first must be a release flush, the second must be an acquire flush it
synchronizes with, and both operations must flush a variable operated on by the
initial operation in the sequence.
\end{enumerate}

A variable with an initial value is treated as if the value is
stored to the variable by an operation that happens before all operations
occurring in the program. 

\begin{note}
The happens before relation as defined here differs from the definition of
happens before in C11/C++11 in two respects. First, operations that are performed by
the same thread but are not ordered through some intervening sequence of data
dependences do not have a happens before order. A thread may observe any two
such operations completing out of program order. Second, the happens before
relation need not be transitive, due to the presence of synchronizing flushes
with explicit flush-sets. However, the happens before relation in any program
which does not use such flushes will be transitive (i.e. if X happens before Y
and Y happens before Z, then X happens before Z).
\end{note}

\subsection{OpenMP Memory Consistency}
\label{subsec:OpenMP Memory Consistency}
Given the happens before relation defined in \specref{subsec:happens-before}
and the restrictions in \specref{subsec:The Flush Operation} on reordering
with respect to flush operations, OpenMP guarantees the following:

\begin{itemize}
\item If the intersection of the flush-sets of two global flushes performed by
    two different threads is non-empty, then the two flushes must be completed
    as if in some sequential order, seen by all threads.

\item If two operations access, modify, or, with a global flush, flush the
    same variable and one happens before the other, then they must be
    completed as if in that happens before order, as seen by all threads. 

\item Any two atomic memory operations from different \code{atomic} regions
    must be completed as if in the same order as the global flushes implied in
    their respective regions, as seen by all threads.
\end{itemize}
