...
\glossaryterm{Debugging Library}
\glossarydefstart
A dynamic link library (DLL) that allows a third-party tool to inspect the state of an OpenMP 
runtime.
\glossarydefend
...
An identifier of an OpenMP entity that is valid during the execution of an OpenMP program. Tool 
handles are created by debugging libraries upon requests from third-party tools and are opaque to 
third-party tools. Handles remain valid until they are released by third-party tools.
\glossarydefend
...
%	OMPD API calls.}
A tool handle that identifies a portion of an instance of an OpenMP program that is running on a 
host device or a target device.
\glossarydefend
...
A tool handle that identifies an OpenMP thread. A handle is valid for the life time of the 
...
A tool handle that identifies an OpenMP parallel region. The handle is guaranteed to be valid if at 
least one thread in the parallel region is paused, or if a thread in a nested parallel region is paused.
\glossarydefend
...
A tool handle that identifies an OpenMP task region. The handle is valid for the life time of the task 
region. The handle is guaranteed to be valid if all threads in the task team are paused.
\glossarydefend
...
opaque to a debugging library, and are defined by a third-party tool.
\glossarydefend
...
thread context. A debugging library implementation can assume that the thread context is valid
...
An identifier of an object in a third-party tool that allows the third-party tool and a debugging library
