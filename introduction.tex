...
\begin{itemize}
    \item \emph{strong}:  guarantees a completion ordering between memory operations
        on different threads that all threads agree upon;
    \item \emph{release}: makes modifications by the executing thread visible
        to other threads;
    \item \emph{acquire}: makes modifications by other threads visible to the
        executing thread.
\end{itemize}

\begin{quote}
COMMENT: A given \emph{flush} operation can have one or more \emph{flush
properties}, but must have at least the release or acquire flush property.
\end{quote}
\glossarydefend

\glossaryterm{strongly-ordered flush}
\glossarydefstart
A \emph{flush} operation that has the \emph{strong flush property}.
\glossarydefend

\glossaryterm{weakly-ordered flush}
\glossarydefstart
...
acquire flush; accordingly, it restricts reordering of memory operations
performed by the same thread that an implementation might otherwise do as
follows:

\begin{itemize}
\item An implementation must not reorder with respect to a release flush any
    prior memory operation that reads or writes to a variable in
    its flush-set or any subsequent memory operation that writes to
    a variable in its sync-set.

\item An implementation must not reorder with respect to an acquire flush any
    subsequent memory operation that reads or writes to a variable in
    its flush-set or any prior memory operation that reads from
    a variable in its sync-set.

\item An implementation must not reorder with respect to eachother a release
    flush and an acquire flush that are applied to the same variable.
\end{itemize}

A strongly-ordered flush imposes stronger ordering requirements across threads
compared to a weakly-ordered flush, as described in
\specref{subsec:OpenMP Memory Consistency}.
...
variable, and that the following sequence of events are completed in the
specified order according to the completion order guarantees defined in
\specref{subsec:OpenMP Memory Consistency}: 
...
\begin{itemize}
\item If the intersection of the flush-sets of two strongly-ordered flushes
...
\item If two operations performed by the same thread access, modify, or, with
    a strongly-ordered flush, flush the same variable, then they must be
...
\item Any two atomic memory operations from different \code{atomic} regions
    must be completed as if in the same order as the strongly-ordered flushes
