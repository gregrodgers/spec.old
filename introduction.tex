...
memory consistency. These properties are: global, release, and acquire.
...
must be a release flush, an acquire flush, or both a release flush and an
acquire flush; accordingly, it restricts reordering of memory operations that
an implementation might otherwise do as follows:

\begin{itemize}

\item An implementation must not reorder after a release flush any prior code
    that reads or write to a variable in its flush-set, and it must also not
    reorder before a release flush any subsequent code that writes to a
    variable in its sync-set.

\item An implementation must not reorder before an acquire flush any
    subsequent code that reads or writes to a variable in its flush-set, and
    it must also not reorder after an acquire flush any prior code that reads
    from a variable in its sync-set.

\item An implementation must not reorder before an acquire flush any
    subsequent release flush applied to a variable in its flush-set.
...
it is the value of the last write that propagates to memory.  An acquire flush
...
its flush-set, as described in \specref{subsec:OpenMP Memory Consistency}.
...
happens before in C++11 and C11 in two respects. First, operations that are performed by
...
which does not use such flushes will be transitive such that, for any
operations \emph{X}, \emph{Y}, and \emph{Z}, if \emph{X} happens before
\emph{Y} and \emph{Y} happens before \emph{Z}, then \emph{X} happens before
\emph{Z}.
\end{note}
...
\item If the intersection of the flush-sets of two global flushes is non-empty
    and one happens before the other, then the two flushes must be completed
    as if in that happens before order, seen by all threads.

\item If two operations performed by the same thread access, modify, or, with
    a global flush, flush the same variable, then they must be completed as if
    in that thread's program order, as seen by all threads. 
