...
\glossaryterm{synchronizing flush}
\glossarydefstart
A \emph{flush} operation with a non-empty \emph{sync-set}.
\glossarydefend

\glossaryterm{non-synchronizing flush}
\glossarydefstart
...
\emph{flush-set}. Every flush operation also has an associated \emph{sync-set}
that is either the same as its flush-set, consists of a single variable from
its flush-set, or is empty. The flush operation may restrict reordering of
memory operations that an implementation might otherwise do. For a given
release flush, an implementation must not reorder after it any prior code for
a memory operation or acquire flush on a variable in its flush-set, and it
must also not reorder before it any subsequent writes to a variable in its
sync-set.  Similarly, for a given acquire flush, an implementation must not
reorder before it any subsequent code for a memory operation or release flush
on a variable in its flush-set, and it must also not reorder after it any
prior reads from a variable in its sync-set. All flushes occurring
within an \code{atomic} region must be performed along with the atomic memory
operation as a single atomic action.
...
Synchronization may only occur between a release flush and an acquire flush
through a synchronization variable that is in the sync-sets of both flushes.
...
Additionally, a variable with an initial value is treated as if the value is
stored to the variable by an operation that happens before all operations
occurring in the program. 

Given the happens before relation as defined above and the restrictions in
\specref{subsec:The Flush Operation} on reordering with respect to flush
operations, OpenMP guarantees the following:
