...
used to execute the iterations of the associated loops.  When a
\code{concurrent} construct is encountered,
an implementation must guarantee that the logical iterations of the associated loops
are executed exactly once. When encountering an \code{concurrent} construct an
implementation will not generate a new league of thread teams, but may use any
thread teams currently available on which to execute. 
