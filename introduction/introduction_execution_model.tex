...
An OpenMP program begins as a single thread of execution, called an initial thread. This 
initial thread executes sequentially, as if the code encountered is part of an implicit task region, called an
%% An implicit parallel region is a scope, it cannot "generate" anything.
initial task region, that is formed as an implicit parallel region surrounding the whole
program.
...
another device. Threads cannot migrate from one device to another device. 
%%Reverse offloading is available now.
The execution model is host-centric such that the execution begins on the host device 
and \code{target} constructs may offload to target devices. Within a \code{target} 
region \code{target} constructs may reverse offload back to the parent device (with restrictions).
...
When a \code{loop} construct is encountered, the iterations of the loop
associated with the construct are executed in the context of its encountering
thread(s), as determined according to its binding region. If the \code{loop}
region binds to a \code{teams} region, the region is encountered by the set
of master threads that execute the \code{teams} region. If the \code{loop}
region binds to a \code{parallel} region, the region is encountered by the team
of threads executing the \code{parallel} region. Otherwise, the region is
encountered by a single thread.

If the \code{loop} region binds to a \code{teams} region, the encountering
threads may continue execution after the \code{loop} region without waiting
for all iterations to complete; the iterations are guaranteed to complete
before the end of \code{teams} region.  Otherwise, all iterations must
complete before the encountering thread(s) continue execution after the
\code{loop} region. All threads that encounter the \code{loop} construct may
...
%%"statements" are not synchronize; synchronization is performed with IO FLUSHES and OpenMP.
