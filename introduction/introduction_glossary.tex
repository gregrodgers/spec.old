...
\glossarydefstart
A \emph{combined construct} that is composed of a \code{target} construct along with another construct.
\glossarydefend
...
part of the \emph{region} of the \emph{encountering thread}.  However, an \emph{explicit task region}
corresponding to a \emph{task generating construct} is not part of the \emph{region} of the \emph{encountering thread} unless it is an
...
%% SUGGESTION
%% A machine instruction capable of executing a single operation concurrently on multiple data elements.
...
%% SUGGESTION
%% A software or hardware mechanism which performan a single operation of the SIMD instruction.
...
\glossarydefstart
A representation of storage resources from which \emph{memory} can be allocated or deallocated.  More than one memory space may exist.
\glossarydefend
...
\glossarydefstart
%% cancellation of a "region" is not correct. (?)
An action that cancels (that is, aborts) the execution of an OpenMP \emph{region} and causes executing
...
\glossarydefstart
A \emph{task} generated by an instance of an initial thread of an \emph{implicit parallel region} or generated when a \code{parallel}
...
\glossarydefstart
For C/C++, an expression that is an array but not an array element 
and that appears as the array to which a given array item refers.

For Fortran, a variable that is an array and that appears as the array 
to which a given array item refers.
\glossarydefend

\glossaryterm{named pointer}
\glossarydefstart
For C/C++, an lvalue expression that is a pointer and that appears as 
a pointer to the array to which a given array item implicitly refers. 
...
%%%SUGGESTION:
%%A pointer variable in a device data environment which is assigned to the address of
%%an array section, as a consequene of a \code{map} clause. 
\glossarydefend
\bigskip
...
\begin{quote}
	COMMENT: A handle cannot be used by the tool in an OMPD call if any
	ancestor of the handle has been released, except for OMPD calls 
