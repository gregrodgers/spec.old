...
\glossaryterm{structured block sequence}
\glossarydefstart
A \emph{structured block}, or, for C/C++, a sequence of two or more
executable statements that together have a single entry at the top and a
...
\glossarydefstart
For C/C++, a \pcode{\#pragma}, for C++, an attribute specifier and, for Fortran, a comment, that
...
must be applied to a function as permitted by the base language.  For Fortran, a declarative
directive must appear after any \code{USE} statement, any \code{IMPORT} statement and
any \code{IMPLICIT} statement in a declarative context.
\glossarydefend
...
\glossaryterm{explicit region}
\glossarydefstart
A \emph{region} that corresponds to either a \emph{construct} of the same name
or a library routine call that explicitly appears in the program.
\glossarydefend
...
\emph{explicit region nested} between them.
\glossarydefend
...
\glossarydefstart
An operation that is specified by an \code{atomic} construct or is implicitly
performed by the implementation and that atomically accesses and/or modifies a
...
\plc{allow-completion-event} that is created for the \emph{task} is fulfilled.
