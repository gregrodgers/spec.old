...
\glossaryterm{thread number}
\glossarydefstart
A number that the OpenMP runtime assigns to an OpenMP thread. 
...
\glossaryterm{place}
\glossarydefstart
Unordered set of \emph{processors} on a device that is treated by the execution environment
as a location unit.
\glossarydefend
...
generating \emph{task} \emph{region}. That is, its generating \emph{task region}
is suspended until execution of structured block associated with the 
...
\glossaryterm{OMPT interface state}
\glossarydefstart
Indicates what interactions between a first-party tool and the OpenMP 
implementation are permitted.
\glossarydefend

\glossaryterm{OMPT active}
\glossarydefstart
An OMPT interface state in which the OpenMP implementation is prepared to
accept runtime calls from a first party tool and dispatches any registered 
callbacks and in which a first-party tool can invoke OMPT runtime entry 
points if not otherwise restricted.
\glossarydefend

\glossaryterm{OMPT pending}
\glossarydefstart
An OMPT interface state in which the OpenMP implementation can only call 
functions to initialize a first party tool and in which a first-party tool 
cannot invoke OMPT runtime entry points.
\glossarydefend

\glossaryterm{OMPT inactive}
\glossarydefstart
An OMPT interface state in which the OpenMP implementation will not make 
any callbacks and in which a first-party tool cannot invoke OMPT runtime 
entry points.
\glossarydefend
...
\glossaryterm{native thread identifier}
\glossarydefstart
