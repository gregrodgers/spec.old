...
The host device, and target devices that an implementation may support, have attached storage resources where program variables are stored. These resources can be of
different kinds and of different traits. A memory space in an OpenMP program represents a set of these resources. Memory spaces have different traits that define them and a single resource may be exposed as multiple memory spaces with different traits. In any device at least one memory space is guaranteed to exist.
