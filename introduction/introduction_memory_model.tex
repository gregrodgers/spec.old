...
An OpenMP program can use a \emph{memory allocator} to allocate \emph{memory} where variables are stored. This \emph{memory} is allocated on the storage resources of the \emph{memory space} associated with a memory allocator. Memory allocators are also used to deallocate previously allocated \emph{memory}. When an OpenMP memory allocator is not used variables may be allocated in any storage resource.
...
% "executed" should be closer to "strong flush" and we use use our flush-set definition
...
memory to all other threads, and that thread’s temporary view of flush-set
is discarded.
...
\item If an acquire flush is performed by an implicit \code{flush} region, an
    atomic operation, provided by the implementation and which reads an internal
