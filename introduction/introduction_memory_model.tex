...
A private variable in a task region that subsequently generates an inner nested \code{parallel}
...
When an original variable is mapped to a device data environment and the corresponding variable is not present in the device data environment, a new corresponding variable (of the same type and size as the original variable) is created in the device data environment. Conversely the original variable becomes the new variable's
...
unspecified behavior unless the \code{unified_shared_memory} requirement clause
...
%% Using definition from 1st sentence in last paragraph for first sentence in this paragraph.
...
%% MAYBE for previous original sentence use (?): 
%% "When an OpenMP memory allocator is not used in the allocation of a variable,
%%  the storage resource used for allocation of the variable is not prescribed by OpenMP."
...
read from memory.  The OpenMP flush operations enforce consistency between a thread's temporary view of memory and memory, as well as between multiple threads' view of memory.

%% Seems like a subsubsection is needed ?
...
variable, and that the following sequence of events are completed in this specific
order:
...
\item If a release flush is performed in an implicit \code{flush} region, some atomic 
...
\item If an acquire flush is performed in an implicit \code{flush} region, some  
