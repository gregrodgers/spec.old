...
unspecified behavior unless the \code{unified\_shared\_memory} requirement clause
is specified on a \code{requires} directive for the program.
%% \code{unified_address} requirement in 45:18 should be \code{unified_address} requirement clause
%%  as defined by 44:1
...
%% Using definition from 1st sentence in last paragraph for the first sentence in this paragraph.

An OpenMP program can use a \emph{memory allocator} to allocate \emph{memory} in which to store variables. This \emph{memory} will be allocated from the storage resources of the \emph{memory space} associated with the memory allocator. Memory allocators are also used to deallocate previously allocated \emph{memory}. When an OpenMP memory allocator is not used variables may be allocated in any storage resource.
%% MAYBE: When an OpenMP memory allocators is not used, the storage resource used for allocation is not prescribed by OpenMP.
...
%% it may not be a "capture again" if all memory operation before were only writes.
...
of a flush-set executed by a thread is defined as the point at which all writes to
the flush-set performed by the thread before the strong flush are visible in 
memory to all other threads, and at which that threadʼs temporary view of the flush-set
is discarded.
...
% Although 100% technically correct, the expression 
% "consistency between two synchronizing threads' view of memory",
% slows the read down, a more natural 
% "consistency between the view of memory for two synchronizing threads"
% flows much better (Just my opinion-- puts consistency closer to view).
...
% should this be an acquire flush (see line 25:24).  If not, maybe this
% description should be expanded to explain why this is specific to
% a release flush and not generically an acquire, release  or even a strong flush.
...
%% LOTL states we should use "provided by the implementation" Bronis wanted proof:
%% 36:16-17 A compliant implementation must provide an option or interface
%% 43:13  The requires directive specifies the features an implementation must provide
%% *******45:32  atomic constructs that must be provided by an implementation
%% 421:11 that an OpenMP implementation provides to
%% 421:15 ...Implementations can provide
%% 517:2  An OpenMP implementation provides a pointer to a lookup routine
\item If a release flush is performed by an implicit \code{flush} region, an 
    atomic operation, provided by the implementation and performed on an internal
...
%% LOTL states we should use "provided by the implementation"
\item If an acquire flush is performed by an implicit \code{flush} region, an
    atomic operation, provided by the implementation and that reads an internal
...
%% by 27:6-9 it isn't "written", but more generally modified.  Otherwise "written" should appear in 27:6-9
