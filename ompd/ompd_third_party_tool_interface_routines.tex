...
OMPD API version that the OMPD library supports.
...
The OMPD API version number is equal to the value of the \code{_OPENMP} macro defined 
in the associated OpenMP implementation, if the C preprocessor is supported. 
If the associated OpenMP implementation compiles Fortran codes without the use 
of a C preprocessor, the OMPD API version number is equal to the value of the 
Fortran integer parameter \code{openmp_version}.
...
string for the OMPD library version.
...
version string of the OMPD library vendor, implementation, internal version, 
date, or any other information that may be useful to a tool user or vendor.
...
\descr
An OpenMP implementation must support all ICVs listed in Section
\ref{subsec:ICV Descriptions}. An OpenMP implementation may
...
The ICVs \plc{num-procs-var}, \plc{thread-num-var}, \plc{final-task-var},
\plc{implicit-task-var}, and \plc{team-size-var} will also be available
with an \plc{ompd-} prefix.
...
ID of the next ICV in the enumeration. On return, the \plc{next_icv_name} argument 
