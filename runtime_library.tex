...
\item The type \code{omp_memspace_handle_t} must be an implementation defined enum type with at least a value for each predefined memory space in \tabref{tab:Predefined Memory Spaces}.

\item The type \code{omp_allocator_handle_t} must be an implementation defined enum type with at least the value \code{OMP_NULL_ALLOCATOR} that should be equal to zero and a value for each predefined memory allocator in \tabref{tab:Predefined Allocators}.
