...
a definition of each required data types listed below. In addition, each 
...
\item A global variable of type \code{const omp_memspace_t *} for each 
      predefined memory space in \tabref{tab:Predefined Memory Spaces}; and
\item A global variable of type \code{const omp_allocator_t *} for each 
      predefined memory allocator in \tabref{tab:Predefined Allocators}.
\end{itemize}
...
\item An \code{integer} \code{parameter} variable of kind 
      \code{omp_memspace_kind} for each predefined memory space 
      in \tabref{tab:Predefined Memory Spaces};
\item An \code{integer} \code{parameter} variable of kind 
      \code{omp_allocator_kind} for each predefined memory allocator 
      in \tabref{tab:Predefined Allocators};
\item The \code{integer} \code{parameter} \code{openmp_version} with 
      a value \plc{yyyymm} where \plc{yyyy} and \plc{mm} are the year 
      and month designations of the version of the OpenMP Fortran
      API that the implementation supports; this value matches that 
      of the C preprocessor macro \code{_OPENMP}, when a macro 
      preprocessor is supported (see \specref{sec:Conditional Compilation});
\item The \code{integer} \code{parameter} \code{omp_pause_kind}; and
