...
queryable runtime states, and is divided into the following sections:
...
\item Lock routines that can be used to synchronize access to 
      data (\specref{sec:Lock Routines}).
...
Throughout this chapter, \plc{true} and \plc{false} are used as 
...
definitions of required data types listed below. In addition, each set of 
definitions may specify other implementation specific values.
...
\item The type \code{omp_uintptr_t} which is an unsigned integer type capable of
  holding a pointer on any device.

\item The type \code{omp_pause_resource_t}.

\item The type \code{omp_event_t}.
...
\item The \code{integer} \code{parameter} \code{omp_event_kind}.
