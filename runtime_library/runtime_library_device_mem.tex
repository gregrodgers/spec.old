...
Task dependencies are expressed with zero or more \code{omp_depend_t}
...
\begin{itemize}
\begin{ccppspecific}
\item passes in a count of zero and value of \code{NULL} for
\plc{depobj_count} and \plc{depobj_list}, respectively,
\end{ccppspecific}

\begin{fortranspecific}
\item passes in a count of zero for \plc{depobj_count} and a zero-sized array for \plc{depobj_list}, or
\item passes in a count of zero for \plc{depobj_count} and \plc{depobj_list} is not present.
\end{fortranspecific}
\end{itemize}

If the value for \plc{depobj_count} is zero and 
\plc{depobj_list} is not \code{NULL} (or not present or not a
zero-sized array if present in Fortran), \plc{depobj_list} is
ignored.
