...
Unless \code{unified_address} clause appears on a \code{requires} directive in
the compilation unit, pointer arithmetic on the device address returned by
\code{omp_target_alloc} is only supported on the device specified by
\plc{device_num}.
