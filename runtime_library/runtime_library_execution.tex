...
\plc{kind}. It can be any of the standard schedule kinds or any other implementation 
specific one. For the schedule kinds \code{static}, \code{dynamic}, and
\code{guided} the \plc{chunk_size} is set to the value of the second 
...
If it is between 0 and one less than \code{omp_get_num_devices()} then 
...
memory routines and not in the \code{device} clause. When called from 
within a \code{target} region the effect of this routine is unspecified.
