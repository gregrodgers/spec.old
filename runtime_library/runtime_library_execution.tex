...
\effect
The effect of this routine is to set the value of the \plc{run-sched-var} ICV of 
the current task to the values specified in the two arguments. The schedule is 
set to the schedule kind specified by the first argument \plc{kind}. It can be 
any of the standard schedule kinds or any other implementation specific one. For 
the schedule kinds \code{static}, \code{dynamic}, and \code{guided} the 
\plc{chunk_size} is set to the value of the second argument, or to the default
\plc{chunk_size} if the value of the second argument is less than 1; for the 
schedule kind \code{auto} the second argument has no meaning; for implementation 
specific schedule kinds, the values and associated meanings of the second 
