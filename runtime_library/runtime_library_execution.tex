...
\plc{kind}. It can be any of the standard schedule types or any other implementation 
...
argument is less than 1; for the schedule type \code{auto} the second 
...
to be used. It can be any of the standard schedule types as defined in
\specref{subsec:omp_set_schedule}, or any implementation specific
schedule type. If the first argument returns \code{static}, \code{dynamic}, 
...
\begin{itemize}
\item The routine may only be called in the sequential part when 
      no pending tasks are waiting for execution. Calling the routine 
