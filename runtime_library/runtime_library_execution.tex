...
\subsection{\hcode{omp_get_max_parallel_nesting}}
\index{omp_get_max_parallel_nesting@{\code{omp_get_max_parallel_nesting}}}
\label{subsec:omp_get_max_parallel_nesting}

\summary
The \code{omp_get_max_parallel_nesting} routine return the maximum
number of nesting levels supported by the implementation.

\format
\begin{ccppspecific}
\begin{ompcFunction}
int omp_get_max_parallel_nesting(void);
\end{ompcFunction}
\end{ccppspecific}

\begin{fortranspecific}
\begin{ompfFunction}
integer function omp_get_max_parallel_nesting()
\end{ompfFunction}
\end{fortranspecific}

\binding
The binding task set for an \code{omp_get_max_parallel_nesting}
region is the generating task.

\effect
The \code{omp_get_max_parallel_nesting} routine returns the maximum
number of nesting levels supported by the implementation.  If the implementation
does not support nesting, the number of levels returned is 1 (one).
...
\item \code{omp_get_max_parallel_nesting} routine, see
\specref{subsec:omp_get_max_parallel_nesting}.
