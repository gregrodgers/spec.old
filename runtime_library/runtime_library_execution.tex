...
which may be relinquished across the call.
 ...
 Soft pause enables to resume processing OpenMP  regions more quickly 
 but may relinquish fewer resources. 
An OpenMP implementation can reclaim resources during a subsequent OpenMP region.
 \end{note}
...
If the tool is not allowed to interact with the specified device, then the runtime must call
the tool finalizer for that device.

\restrictions
OpenMP pause routine has the following restriction:
...
If the tool is not allowed to interact with a given device, then the runtime must call
the tool finalizer for that device.
