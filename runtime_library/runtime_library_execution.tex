...
\effect
The effect of \code{omp_set_affinity_format} routine is to set the value of
...
If the return value is larger than \plc{size} (for C/C++) or {\pcode{len(\plc{buffer})}}
...
\effect
The \code{omp_capture_affinity} routine returns the number of characters required to hold
...
\plc{size} (for C/C++) or {\pcode{len(\plc{buffer})}} (for Fortran)
...
If the return value is larger than \plc{size} (for C/C++) or {\pcode{len(\plc{buffer})}} (for Fortran), the
...
The \code{omp_pause_resource} routine enables the runtime to 
...
\begin{ccppspecific}
\begin{ompcFunction}
int omp_pause_resource(int device, omp_pause_resource_t kind);
\end{ompcFunction}
\end{ccppspecific}
\end{samepage}
...
\begin{fortranspecific}
\begin{ompfFunction}
integer function omp_get_max_task_priority(device, kind)
integer device
...
The first argument passed to this routine indicates which device is paused. The device parameter
...
The second argument passed to this routine can be one of the valid OpenMP pause kind, 
...
\begin{fortranspecific}
\begin{ompfFunction}
integer (kind=omp_pause_resource_kind, parameter :: omp_pause_soft = 0
integer (kind=omp_pause_resource_kind), parameter :: omp_pause_hard = 1
\end{ompfFunction}
\end{fortranspecific}
...
The \code{omp_pause_resource} routine enables the runtime to relinquish resources 
used by OpenMP on the specified device.

The \code{omp_pause_hard} value indicates that OpenMP state is not guaranteed to 
...
for that device as well as data present on the device as a result of a declare target construct.
 Hard pause may also relinquish any data associated with a \code{threadprivate} directive.

The \code{omp_pause_soft} value indicates that OpenMP state is guaranteed to persist across 
the call, with the exception of any data associated with a \code{threadprivate} directive 
which may be relinquished across the call.

 \begin{note}
 Hard pause may relinquish more resources but may resume processing OpenMP constructs 
 and routines more slowly. 
 Soft pause enables to resume processing OpenMP  constructs and routines more quickly 
 but may relinquish fewer resources. 
An OpenMP implementation may reclaim resources during a subsequent OpenMP region.
 \end{note}
 
The routine returns zero in case of success, and nonzero otherwise.

\restrictions
OpenMP pause routine has the following restrictions:
...
\begin{itemize}
\item \code{omp_pause_resource_all} library routine, see
...
The \code{omp_pause_resource_all} routine enables the runtime to 
...
\begin{fortranspecific}
\begin{ompfFunction}
integer function omp_get_max_task_priority(kind)
integer (kind=omp_pause_resource_kind) kind
\end{ompfFunction}
\end{fortranspecific}
...
The \code{omp_pause_resource_all} routine enables the runtime to relinquish resources 
...
\begin{itemize}
\item \code{omp_pause_resource} library routine, see
