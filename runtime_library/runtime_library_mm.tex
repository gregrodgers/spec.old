...
integer(kind=omp_alloctratit_val_kind), &
  parameter :: omp_atv_true = 0
integer(kind=omp_alloctratit_val_kind), &
  parameter :: omp_atv_false = 1
integer(kind=omp_alloctratit_val_kind), &
...
If the \plc{memspace} is \scode{omp_default_mem_space} and the traits argument is an empty set this routine will always return a handler to an allocator. Otherwise if an allocator based on the requirements cannot be created then the special value \scode{OMP_NULL_ALLOCATOR} is returned.
