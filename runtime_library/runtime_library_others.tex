...
This section describes routines that support OpenMP depend objects that can be used to supply user computed dependeces to \code{depend} clauses. OpenMP depend objects must be accessed only through routines described in this section or through the \code{depend} clause; programs that otherwise access OpenMP depend objects are non-conforming.
...
\subsection{\hcode{omp_init_depend}}
\index{omp_init_depend@{\code{omp_init_depend}}}
...
\begin{ccppspecific}
\begin{ompcFunction}
void omp_init_depend(omp_depend_t *depobj, omp_depend_flags_t flags, void *base_address, size_t length);
\end{ompcFunction}
\end{ccppspecific}
...
The effect of this routine is to initialize the depend object in the first argument to represent a dependence to the storage location from $\plc{base_address}$ to $\plc{base_address}+\plc{length}-1$ and of the dependence type specified in the \plc{flags} argument.
...
\summary
This routine ensure that the OpenMP depend object is uninitialized.
...
\subsection{\hcode{omp_adjust_depend}}
\index{omp_adjust_depend@{\code{omp_adjust_depend}}}
\label{subsec:omp_adjust_depend}
\summary

This routine allows to change the dependence type of an OpenMP depend object.
...
The effect of this routine is to change the dependence type of the dependence respresented by \plc{depbobj} to the one specified by \plc{flags}.
