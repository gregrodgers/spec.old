...
\section{Depend Object Routines}
\index{depend object routines}
\index{depend object}
\label{sec:Depend Object Routines}

This section describes routines that support OpenMP depend objects that can be used to supply user computed dependences to \code{depend} clauses. OpenMP depend objects must be accessed only through routines described in this section or through the \code{depend} clause; programs that otherwise access OpenMP depend objects are non-conforming.
...
The binding thread set for all depend object routine regions is the encountering thread.

\subsection{\hcode{omp_init_depend_contig}}
\index{omp_init_depend_contig@{\code{omp_init_depend_contig}}}
\label{subsec:omp_init_depend}
\summary
This routine initializes a depend object.

\format
\begin{ccppspecific}
\begin{ompcFunction}
void omp_init_depend_contig(omp_depend_t *depobj, omp_depend_flags_t flags, void *base_address, size_t length);
\end{ompcFunction}
\end{ccppspecific}

\begin{fortranspecific}
\begin{ompfFunction}
subroutine omp_init_depend(depobj, flags, var? )
integer (kind=omp_depend_kind) \plc{depobj}
integer (kind=omp_depend_flags_kind) \plc{flags}
\end{ompfFunction}
\end{fortranspecific}

\constraints

A  program that accesses a depend object that is not in the uninitialized state through this routine is non-conforming.

If the \plc{length} argument does not evaluate to an integer greater than zero the behavior of this routine is unspecified.

\effect

The effect of this routine is to initialize the depend object in the first argument to represent a dependence on the storage location from $\plc{base_address}$ to $\plc{base_address}+\plc{length}-1$ and of the dependence type specified in the \plc{flags} argument.
...
\subsection{\hcode{omp_destroy_depend}}
\index{omp_destroy_depend@{\code{omp_destroy_depend}}}
\label{subsec:omp_destroy_depend}
\summary
This routine ensures that the OpenMP depend object is uninitialized.

\format
\begin{ccppspecific}
\begin{ompcFunction}
void omp_destroy_depend(omp_depend_t *depobj);
\end{ompcFunction}
\end{ccppspecific}

\begin{fortranspecific}
\begin{ompfFunction}
subroutine omp_init_depend(depobj)
integer (kind=omp_depend_kind) \plc{depobj}
\end{ompfFunction}
\end{fortranspecific}
...
A program that accesses a depend object that is not in the initialized state through this routine is non-conforming.

\effect

The effect of this routine is to change the state of \plc{depobj} to uninitialized.
...
\subsection{\hcode{omp_adjust_depend_type}}
\index{omp_adjust_depend_type@{\code{omp_adjust_depend_type}}}
\label{subsec:omp_adjust_depend_type}
\summary

This routine changes the dependence type of an OpenMP depend object.

\format
\begin{ccppspecific}
\begin{ompcFunction}
void omp_adjust_depend(omp_depend_t *depobj, omp_depend_flags_t flags);
\end{ompcFunction}
\end{ccppspecific}

\begin{fortranspecific}
\begin{ompfFunction}
subroutine omp_adjust_depend(depobj, flags)
integer (kind=omp_depend_kind) \plc{depobj}
integer (kind=omp_depend_flags_kind) \plc{flags}
\end{ompfFunction}
\end{fortranspecific}
...
The effect of this routine is to change the dependence type of the dependence respresented by \plc{depobj} to the one specified by \plc{flags}.
