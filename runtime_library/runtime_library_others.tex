...
the \plc{task-fulfill} event occurs in a thread when it executes the 
...
callback with \code{ompt_task_fulfill} as its  \plc{prior_task_status}
argument and \code{NULL} as its \plc{next_task_data} argument while
the argument \plc{prior_task_data} binds to the detached task for each 
occurrence of a \plc{task-fulfill} event. This callback has type signature 
