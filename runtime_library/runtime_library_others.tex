...
\events
The \plc{lock-acquire} event occurs in a thread that executes a 
...
The \plc{lock-acquired} event occurs in a thread that executes a 
...
lock and executes a \code{omp_set_nest_lock} region. THe event occurs after 
...
\events
The \plc{lock-release} or \plc{nest-lock-release} event occurs in a thread
that executes a \code{omp_unset_lock} or \code{omp_unset_nest_lock} region
after it releases the associated lock but before it finishes the region.

The \plc{nest-lock-held} event occurs in a thread that executes a 
...
These callback have the type signature \code{ompt_callback_mutex_t}
and occur in the task that encounters the routine.
