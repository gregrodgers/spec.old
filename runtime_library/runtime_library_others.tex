...
\events
The \plc{lock-init} event occurs in the thread that executes an 
\code{omp_init_lock} region after initialization of the lock, but 
before finishing the region. The \plc{nest-lock-init} event occurs 
in the thread that executes an \code{omp_init_nest_lock} region
...
lock, but before finishing the region. The \plc{nest-lock-init_with_hint} 
event occurs in the thread that executes an \code{omp_init_nest_lock} 
region after initialization of the lock, but before finishing the region.

\tools
A thread dispatches a registered \code{ompt_callback_lock_init} 
callback with the function's \plc{hint} argument as the \plc{hint} argument 
and \code{ompt_mutex_lock} as the \plc{kind} argument for each 
occurrence of a \plc{lock-init} event in that thread. Similarly, 
a thread dispatches a registered \code{ompt_callback_lock_init} 
callback with the function's \plc{hint} argument as the \plc{hint} argument 
...
\events
The \plc{lock-destroy} event occurs in the thread that executes an 
\code{omp_destroy_lock} region before finishing the region. The 
\plc{nest-lock-destroy_with_hint} event occurs in the thread that 
executes an \code{omp_destroy_nest_lock} region before finishing the region.
...
\events
The \plc{lock-acquire} event occurs in the thread that executes a 
\code{omp_set_lock} region before the associated lock is requested.
The \plc{nest-lock-acquire} event occurs in the thread that executes
a \code{omp_set_nest_lock} region before the associated lock is requested.

The \plc{lock-acquired} event occurs in the thread that executes a 
\code{omp_set_lock} or \code{omp_set_nest_lock} region after it acquires 
the associated lock, if the thread did not already own the lock, but 
before finishing the region. The \plc{nest-lock-acquired} event occurs 
in the thread that executes a \code{omp_set_lock} or \code{omp_set_nest_lock} 
region after it acquires the associated lock, if the thread did not 
already own the lock, but before finishing the region.

The \plc{nest-lock-owned} event occurs in the thread that executes
a \code{omp_set_nest_lock} region when the thread already owns the lock,
...
This section describes routines that support OpenMP event objects. OpenMP event objects must be accessed only through routines described in this section or through the \code{detach} clause of the \code{task} construct; programs that otherwise access OpenMP event objects are non-conforming.
...
integer (kind=omp_event_kind) \plc{event_handle}
\end{ompfFunction}
\end{fortranspecific}
...
A program that calls this routine with an event handle that was not created by the \code{detach} clause is non-conforming.
