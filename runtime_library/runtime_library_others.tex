...
section or through the \code{detach} clause of the \code{task} construct; 
...
If the OpenMP event object was created by a \code{detach} clause on a task,
the \plc{task-fulfill} event occurs in a thread when the thread executes the 
\code{omp_fulfill_event} routine, before the effect is realized.

\tools

A thread dispatches a registered \code{ompt_callback_task_schedule}
callback for each occurrence of a \plc{task-fulfill} event. 
This callback has the type signature \code{ompt_callback_task_schedule_t}.
The argument \plc{prior_task_status} is set to \code{ompt_task_fulfill}.
The argument \plc{prior_task_data} binds to the detached task.
The argument \plc{next_task_data} is \code{NULL}.
