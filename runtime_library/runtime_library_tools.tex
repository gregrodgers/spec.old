...
\descr
An OpenMP program may use \code{omp_control_tool} to pass commands 
to a tool.Using \code{omp_control_tool}, an application can request 
that a tool start or restart data collection when a code region of 
interest is encountered, pause data collection when leaving the region 
of interest, flush any data that it has collected so far, or end data 
collection. Additionally, \code{omp_control_tool} can be used to pass
