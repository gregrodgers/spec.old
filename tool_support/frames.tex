...
\subsubsubsection{\code{ompt_frame_t}}
\index{frames}
\label{sec:ompt_frame_t}
\label{subsubsubsec:ompt_frame_t}

\summary
The \code{ompt_frame_t} type describes procedure frame information for an OpenMP task.
...
Each \code{ompt_frame_t} object is associated with the task to which the procedure frames belong.
Each non-merged initial, implicit, explicit, or target task with one or more frames on
the stack of a native thread has an associated \code{ompt_frame_t} object.
...
\plc{enter_frame} for the \code{ompt_frame_t} object
...
\code{ompt_frame_flag_t} enumeration type.

The lifetime of an \code{ompt_frame_t} begins when a task is created
...
a task's lifetime. 
A pointer to an \code{ompt_frame_t} object is passed to
some callbacks; a pointer to the \code{ompt_frame_t} object of a task
...
A pointer to an \code{ompt_frame_t} object that a tool retrieved
...
%\begin{table}
%\begin{center}
%\caption{States of an \code{ompt_frame_t} Object\label{tab:frame}}
...
%an \code{ompt_frame_t} object may be observed and their meaning.
%In the presence of nested parallelism, a tool may
%observe a sequence of \code{ompt_frame_t} objects for a thread.
%Appendix~\ref{chap:frames} illustrates
%use of \code{ompt_frame_t} objects with nested parallelism.
...
Tools must be prepared to observe and handle \code{ompt_frame_t}
objects observed just prior to when their field values will be set or
cleared.
\end{note}


\subsubsubsection{\code{ompt_frame_flag_t}}
\summary
The \code{ompt_frame_flag_t} enumeration type defines valid frame information
flags.
...
The value \code{ompt_frame_application} of the \code{ompt_frame_flag_t} type
...
The value \code{ompt_frame_cfa} of the \code{ompt_frame_flag_t} type
indicates that a frame address is specifying a \plc{canonical frame address}.
The value \code{ompt_frame_framepointer} of the \code{ompt_frame_flag_t} type
indicates that a frame address is providing the value of the frame pointer
register.
The value \code{ompt_frame_stackaddress} of the \code{ompt_frame_flag_t} type
