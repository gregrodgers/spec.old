...
frames on the stack has not suspended, the value of \plc{enter_frame} 
...
the \plc{enter_frame_flags} field respectively indicates whether the 
...
of values in the \code{ompt_frame_flag_t} enumeration type.
...
a task's lifetime. A pointer to an \code{ompt_frame_t} object is passed 
...
\subsection{\hcode{ompt_frame_flag_t}}
...
for the particular frame pointer. The value \code{ompt_frame_cfa} of the 
\code{ompt_frame_flag_t} type indicates that a frame address is specifying 
a \plc{canonical frame address}. The value \code{ompt_frame_framepointer} 
of the \code{ompt_frame_flag_t} type indicates that a frame address is 
providing the value of the frame pointer register. The value 
\code{ompt_frame_stackaddress} of the \code{ompt_frame_flag_t} type
