...
\begin{enumerate}
\item
  the environment variable \code{OMP\_DEBUG} is set to \code{on}
\item
...
\subsection{Finding the OMPD plugin}
...
This symbol must have external, \code{C}, linkage.
Conceptually, \code{ompd\_dll\_locations\_valid} has the signature
\code{void ompd\_dll\_locations\_valid~(~void~);}.
If \code{ompd\_dll\_locations} is NULL, a third-party tool like a debugger
...
Note that \code{ompd\_dll\_locations\_valid} need not be a function,
