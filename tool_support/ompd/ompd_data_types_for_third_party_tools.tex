...
Recommended values of \code{ompd_device_t} are defined in the \code{ompd_types.h}
...
the interface via the \code{ompd_thread_id_t} type, which is a pointer to where
...
\code{ompd_types.h} header file, which is available on 
...
\item \code{ompd_rc_unavailable} is returned wheninformation is not 
...
\item \code{ompd_rc_incompatible} is returned when this OMPD library 
