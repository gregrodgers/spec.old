\ompdsection{Runtime Entry Points for OMPD}
\label{sec:ompd:runtime-entry-points-for-ompd}

Most of the third-party tool's OpenMP-related activity on an OpenMP
...
be accessible to the third-party tool.

\subsection{Identifying the Matching OMPD}
...
\subsection{Indicating that \code{ompd\_dll\_locations} is Valid}
...
To avoid a third-party having to poll \code{ompd\_dll\_locations}
...
the third-party tool can be notified when the vector of names is
ready to be read.

If \code{ompd\_dll\_locations} vector is statically allocated
and initialized, the third-party tool should not need to use
...
and no demangling or other transformations are required by a third-party
tool before looking up its address in the OpenMP program.
...
\subsection{Enabling Support for OMPD at Runtime}
\label{sec:ompd:omp_debug_enable}
\index{ompd\_debug\_enable@{\code{omp\_debug\_enable}}}

\summary
Instructs the OpenMP runtime to collect whatever information
is necessary for supporting access by third-party tools using OMPD.

\vbox{
\cspecificstart
\begin{boxedcode}
void omp_debug_enable ( void );
\end{boxedcode}
\cspecificend
}
...
environment to set the \code{OMP\_DEBUG} variable so as to enable
data collection by the runtime for OMPD.
\code{omp\_debug\_enable} allows a target itself to turn on data collection.
\code{omp\_debug\_enable} may be called by the main executable, or any of the
...
\restrictions
If it is called, then \code{omp\_debug\_enable} must be called before
the OpenMP program executes its first OMP construct.
...
\item
  Enabling the Runtime for OMPD, \specref{sec:ompd:enabling-ompd}
\end{itemize}

\subsection{Event notification}
\label{sec:ompd:event-notification}

Neither a third-party tool nor an OpenMP runtime system know what
...
To help a third-party tool control the execution of an OpenMP program
...
\label{sec:ompd:ompd_bp_parallel_begin}
...
The third-party tool can find the entry point of the user code that
...
\begin{itemize}
\item
  \code{ompd\_get\_thread\_handle}, \specref{sec:ompd:ompd_get_thread_handle}
\item
  \code{ompd\_get\_top\_parallel\_region}, \specref{sec:ompd:ompd_get_top_parallel_region}
\item
  \code{ompd\_get\_parallel\_function}, \specref{sec:ompd:ompd_get_parallel_function}
\item
  \code{ompd\_get\_num\_threads}, \specref{sec:ompd:ompd_get_num_threads}
\item
  \code{ompd\_get\_top\_task\_region}, \specref{sec:ompd:ompd_get_top_task_region}
\item
  \code{ompd\_get\_task\_frame}, \specref{sec:ompd:ompd_get_task_region}
\end{itemize}
...
\item
  \code{ompd\_get\_top\_task\_region}, \specref{sec:ompd:ompd_get_top_task_region}
\item
  \code{ompd\_get\_task\_frame}, \specref{sec:ompd:ompd_get_task_frame}
\end{itemize}
...
the third-party tool can map the operating thread to an OpenMP handle using
\code{ompd\_get\_thread\_handle}.
...
\item
  \code{ompd\_get\_top\_task\_region}, \specref{sec:ompd:ompd_get_top_task_region}
\item
  \code{ompd\_get\_task\_function}, \specref{sec:ompd:ompd_get_task_function}
\end{itemize}

\subsubsection{Ending Task Regions}
\label{sec:ompd:ompd_bp_task_end}
