...
After passing \code{ompd_bp_parallel_end}, any \plc{parallel_handle} acquired for this
...
After passing \code{ompd_bp_task_end}, any \plc{task_handle} acquired for this
...
\summary
The OpenMP runtime must execute through
\code{ompd_bp_thread_begin} at the \plc{thread-begin} and the \plc{implicit-thread-begin} event.
...
to flow through \code{ompd_bp_thread_begin}.
This should occur before the thread executes any OpenMP region's work.

\restrictions

\code{ompd_bp_thread_begin} has external \code{C} linkage, and no
demangling or other transformations are required by a tool
to look up its address in the OpenMP program.

Conceptually \code{ompd_bp_thread_begin} has the type signature
given above.
However, it does not need to be a function, but can be a labeled location
in the runtime code.
...
\summary
The OpenMP runtime must execute through
\code{ompd_bp_thread_end} at the \plc{thread-end} and the \plc{implicit-thread-end} event.
...
to flow through \code{ompd_bp_thread_end}.
This should occur after the thread executes any OpenMP region's work.

After passing \code{ompd_bp_thread_end}, any \plc{thread_handle} acquired for this thread 
is invalid and should be released.

\restrictions

\code{ompd_bp_thread_end} has external \code{C} linkage, and no
demangling or other transformations are required by a tool
to look up its address in the OpenMP program.

Conceptually \code{ompd_bp_thread_end} has the type signature
given above.
However, it does not need to be a function, but can be a labeled location
in the runtime code.
...
to flow through \code{ompd_bp_device_begin}.
This should occur before any OpenMP region's work executes on the device.

\restrictions

\code{ompd_bp_device_begin} has external \code{C} linkage, and no
demangling or other transformations are required by a tool
to look up its address in the OpenMP program.

Conceptually \code{ompd_bp_device_begin} has the type signature
given above.
However, it does not need to be a function, but can be a labeled location
in the runtime code.
...
When terminating an OpenMP thread, the runtime must allow execution
to flow through \code{ompd_bp_device_end}.
This should occur after the thread executes any OpenMP region's work.

After passing \code{ompd_bp_device_end}, any \plc{address_space_handle} acquired for this
device is invalid and should be released.

\restrictions

\code{ompd_bp_device_end} has external \code{C} linkage, and no
demangling or other transformations are required by a tool
to look up its address in the OpenMP program.

Conceptually \code{ompd_bp_device_end} has the type signature
given above.
However, it does not need to be a function, but can be a labeled location
in the runtime code.
