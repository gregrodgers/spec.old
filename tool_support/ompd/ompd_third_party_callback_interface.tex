...
callback the tool provide to look up the addresses of symbols in an OpenMP program.
...
address space of the process, core file, or device. The use of a NULL 
\plc{address_space_context} results in unspecified behavior.
...
storage~(TLS) addresses. If \plc{thread_context} is not NULL then the thread 
t o which \plc{thread_context} refers must be associated either to the process 
or to the device that corresponds to the \plc{address_space_context} argument.
...
variable, function, or untyped label. The symbol must be defined and can have 
...
If \plc{file_name} is not NULL then the tool first tries to find the symbol in 
...
The callback returns the address of the symbol in the location to which
the \plc{symbol_addr} argument points.
