...
callbacks that tool provides. These callbacks must also be used for allocating 
...
The OMPD library may need to access memory in the OpenMP program. The 
\plc{symbol_addr_lookup} field points to a callback that the OMPD library can 
use to find the address of a global or thread local storage symbol. The 
