...
\plc{nbytes} argument is the number of bytes to be transfered. The 
...
tool and OMPD library use and that the OpenMP program uses.
...
function to emit output, such as logging or debug information. If the tool 
does not want to allow the OMPD library to emit output, the tool can provide 
to the OMPD library a \code{NULL} value for the 
\code{ompd_callback_print_string_fn_t} callback function. Note that the OMPD 
library is prohibited from writing to file descriptors that it did not open.
...
The input \plc{category} parameter is the category of the string to be printed. 
The value of \plc{category} is implementation defined.
