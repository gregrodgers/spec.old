...
\label{ompd:ompd_memory_alloc_fn_t}
...
\descr
The OMPD plugin may call the \code{ompd\_memory\_alloc\_fn\_t} callback function to allocate memory.
...
\label{ompd:ompd_memory_free_fn_t}
...
\descr
The OMPD plugin calls the \code{ompd\_memory\_free\_fn\_t} callback function to 
deallocate memory obtained from a prior call to the \code{ompd\_memory\_alloc\_fn\_t} 
callback function.
...
\label{ompd:ompd_get_thread_context_for_thread_id_fn_t}
...
\index{ompd\_get\_thread\_context\_for\_thread_id\_fn\_t@{\code{ompd\_get\_thread\_context\_for\_\discretionary{}{}{}thread_id\_fn\_t}}}
...
\cspecificstart
\begin{boxedcode}
typedef ompd\_rc\_t (*ompd\_get\_thread\_context\_for\_thread_id\_fn\_t) (
  \code{ompd\_address\_space\_context\_t}  *\plc{address\_space\_context},
  \code{ompd\_thread_id\_kind\_t}           \plc{kind},
  \code{ompd\_size\_t}                    \plc{sizeof\_thread_id},
...
The input arguments \plc{kind},  \plc{sizeof\_thread_id}, and \plc{thread_id} represent an operating 
...
\label{ompd:ompd_get_address_space_context_for_thread_fn_t}
% the use of \discretionary below overrides the character used for
% hyphenation when an optional linebreak is used.
% if \- were used, we'd get a "-" at the end of the line being
% broken.  We probably don't wantthat, but instead want just a plain
...
\label{ompd:ompd_get_process_context_for_address_space_context}
% the use of \discretionary below overrides the character used for
% hyphenation when an optional linebreak is used.
% if \- were used, we'd get a "-" at the end of the line being
% broken.  We probably don't wantthat, but instead want just a plain
...
\item
  \code{ompd\_address\_space\_context\_t}, \specref{ompd:ompd_address_space_context_t}
\item
  \code{ompd\_device\_type\_sizes\_t}, \specref{ompd:ompd_device_type_sizes_t}
\item
  \code{ompd\_rc\_t}, \specref{ompd:ompd_rc_t}
\item
  \code{ompd\_callbacks\_t}, \specref{ompd:ompd_callbacks_t}
...
\label{ompd:ompd_symbol_addr_fn_t}
...
\label{ompd:ompd_memory_read_fn_t}
...
(see~\specref{ompd:ompd_device_host_fn_t}) to interpret the
data returned.
...
\item
  \code{ompd\_rc\_t}, \specref{ompd:ompd_rc_t}
\item
  \code{ompd\_device\_host\_fn\_t}, \specref{ompd:ompd_device_host_fn_t}
\item
  \code{ompd\_callbacks\_t}, \specref{ompd:ompd_callbacks_t}
\end{itemize}

\subsubsubsection{Writing Memory}
\label{ompd:ompd_memory_write_fn_t}
...
It is the responsibility of the OMPD plugin to arrange for
any transformations such as byte-swapping that may be necessary
(see~\specref{ompd:ompd_device_host_fn_t})
...
\label{ompd:ompd_device_host_fn_t}
...
\label{ompd:ompd_print_string_fn_t}
...
typedef struct \{
  ...
  ompd\_get\_thread\_context\_for\_thread_id\_fn\_t
                         \plc{get\_thread\_context\_for\_thread_id};
  ompd\_get\_address\_space\_context\_for\_thread\_fn\_t
...
\plc{get\_thread\_context\_for\_thread_id} is a pointer to a function
...
\begin{itemize}
\item
  \code{ompd\_memory\_alloc\_fn\_t}, \specref{ompd:ompd_memory_alloc_fn_t}
\item
  \code{ompd\_memory\_free\_fn\_t}, \specref{ompd:ompd_memory_free_fn_t}
\item
  \code{ompd\_print\_string\_fn\_t}, \specref{ompd:ompd_print_string_fn_t}
\item
  \code{ompd\_sizeof\_fn\_t}, \specref{ompd:ompd_sizeof_fn_t}
\item
  \code{ompd\_symbol\_addr\_fn\_t}, \specref{ompd:ompd_symbol_addr_fn_t}
\item
  \code{ompd\_memory\_read\_fn\_t}, \specref{ompd:ompd_memory_read_fn_t}
\item
  \code{ompd\_memory\_write\_fn\_t}, \specref{ompd:ompd_memory_write_fn_t}
\item
  \code{ompd\_device\_host\_fn\_t}, \specref{ompd:ompd_device_host_fn_t}
\item
  \code{ompd\_get\_thread\_context\_for\_thread_id\_fn\_t}, \specref{ompd:ompd_get_thread_context_for_thread_id_fn_t}
\item
  \code{ompd\_get\_address\_space\_context\_for\_thread\_fn\_t}, \specref{ompd:ompd_get_address_space_context_for_thread_fn_t}
\item
  \code{ompd\_get\_process\_context\_for\_address\_space\_context\_fn\_t}, \specref{ompd:ompd_get_process_context_for_address_space_context}
\end{itemize}
