...
\summary
The \code\hcode{ompd_get_task_frame} the frame pointers of a task.
...
return, the location pointed to which the \plc{more_enums} argument points
...
value that the call returned in the \plc{next_state} output argument.
On return, the \plc{next_state} argument points to an integer with the 
value of the next state in the enumeration. On return, the \plc{next_state_name} 
argument points to a character string that describes the next state. On
return, the \plc{more_enums} argument points to an integer with a value 
