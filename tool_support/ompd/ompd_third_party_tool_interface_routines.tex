...
\argdesc
The input argument \plc{parallel_handle} is an opaque handle for a parallel region and 
selects the parallel region to operate on.
On return the output argument \plc{val} is set to the value that 
\code{omp_get_num_threads} would return when
...
\descr
An OpenMP implementation might support ICVs in addition to the variables listed in 
Table~\ref{tab:ICV Initial Values}. 
...
to the locations pointed to by the 
\plc{next}, \plc{next_icv}, and \plc{scope} output arguments respectively.
...
value returned by the call in the \plc{next} output argument.

The output argument \plc{next} is a pointer to an integer where
the call will return the value of the next state in the
enumeration.
...
If the ICV cannot represented by an integer type value, the function returns 
...
\constraints
Any string returned through the argument \plc{icv_string} must be freed by the tool.
