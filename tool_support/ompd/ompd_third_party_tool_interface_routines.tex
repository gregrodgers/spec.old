...
\label{ompd:ompd_get_top_parallel_region}
\summary
...
\label{ompd:ompd_get_top_task_region}
\summary
...
\label{ompd:ompd_get_generating_ancestor_task_region}
\summary
...
\label{ompd:ompd_get_scheduling_ancestor_task_region}
\summary
...
ompd\_rc\_t ompd\_get\_thread\_handle(
  ...
  ompd\_thread_id\_kind\_t           \plc{kind},
  ompd\_size\_t                    \plc{sizeof\_thread_id}, 
...
The operating system ID \plc{*thread_id} is guaranteed to be valid for the duration of the call. If the 
...
\begin{itemize}
	\item \code{ompd\_address\_space\_handle\_t} type, see \specref{ompd:ompd_address_space_handle_t}.
	\item \code{ompd\_thread_id\_kind\_t} type, see \specref{ompd:ompd_thread_id_kind_t}.
	\item \code{ompd\_size\_t} type, see \specref{ompd:ompd_size_t}.
	\item \code{ompd\_thread\_handle\_t} type, see \specref{ompd:ompd_thread_handle_t}.
\end{itemize}

\ompdcallbacksignature{\code{ompd\_get\_thread_id}}
...
\cspecificstart
\begin{boxedcode}
ompd\_rc\_t ompd\_get\_thread_id(
  ompd\_thread\_handle\_t *\plc{thread\_handle}, 
  ompd\_thread_id\_kind\_t \plc{kind}, 
  ompd\_size\_t \plc{sizeof\_thread_id}, 
  void  *\plc{thread_id} 
...
\descr
\code{ompd\_get\_thread_id} performs the mapping between an OMPD
thread handle and a thread identifier.

\argdesc

The input argument \plc{thread\_handle} is an opaque thread handle.
The input argument  \plc{kind} represents the operating system-specific thread
identifier. The input argument \plc{sizeof\_thread\_id} represents the size of the 
thread identifier. The output argument \plc{thread\_id} is a buffer that represents an operating 
system thread identifier.
