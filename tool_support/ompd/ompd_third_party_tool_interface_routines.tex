...
to obtain a pointer to the parallel handle for an OpenMP thread's current 
parallel region.
...
argument is set to a handle for the parallel region that the asociatd thread 
...
to obtain a pointer to the parallel handle for an enclosing parallel region.
...
encloses the parallel region specified by \code{parallel_handle}. This 
...
is returned in \plc{*enclosing_parallel_handle}. After the call, the 
tool owns the handle; the tool must release the handle when it is no 
longer required by calling \code{ompd_release_parallel_handle}.
...
pointer to the parallel handle for the parallel region that encloses a task region.
...
points. The tool owns that parallel handle, which it must release by calling 
