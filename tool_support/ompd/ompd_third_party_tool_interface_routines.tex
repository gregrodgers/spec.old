...
library by calling \code{ompd_get_api_version} (see~\specref{subsubsubsec:ompd_get_api_version}). If the tool supports the version returned by \code{ompd_get_api_version},
...
of the OMPD API supported by the library.  If the tool does not support the version returned by \code{ompd_get_api_version}, it may attempt to call \code{ompd_initialize} with
a different version.
...
The tool provides the OMPD library with a set of callback functions in the \plc{callbacks} input argument
...
\argdesc
The latest version number is returned in to the location pointed to by the \plc{version} output 
argument.
...
\descr
The tool may call this function to obtain a pointer to a descriptive version string of the OMPD API 
version.
...
the \plc{context}. Incompatibility is signaled by a return value of \code{ompd_rc_incompatible}.
...
\begin{itemize}
    ...
    \specref{subsubsec:ompd_address_space_context_t}.
  	\item \code{ompd_rc_t} type, see \specref{subsubsec:ompd_rc_t}.
	\item \code{ompd_release_address_space_handle} type, see 
	\specref{subsubsubsec:ompd_release_address_space_handle}.
\end{itemize}
...
Upon return, the output argument \plc{omp_version} will contain the version of the OpenMP 
...
The input argument \plc{address_space} is an opaque handle provided by the tool
to reference the address space of the OpenMP process or device.
A pointer to a descriptive version string will be placed into the *\plc{string} output 
argument.
...
The input argument \plc{thread_num} selects the thread of the team to be returned.
...
%The input argument \code{thread_num} selects the thread of the team to be returned.
%On return the output argument \plc{thread_handle} is an opaque handle for the selected thread.
%
%\crossreferences
%\begin{itemize}
...
identifier \plc{thread_id} is an OpenMP thread.
%If the function returns \code{ompd_rc_ok},
If so, the function returns \code{ompd_rc_ok} and \plc{*thread_handle}
...
The native thread identifier \plc{*thread_id} is guaranteed to be valid for the duration of the call. If the
...
On success, \code{ompd_thread_handle_compare} returns in \plc{*cmp_value} a signed
integer value that indicates how the underlying threads compare: a value less than, equal to, or
greater than 0 indicates that the thread corresponding to \plc{thread_handle_1} is, respectively,
...
The input argument  \plc{kind} represents the native thread
identifier. The input argument \plc{sizeof_thread_id} represents the size of the
native thread identifier. 
The output argument \plc{thread_id} is a buffer that represents a native thread identifier.
...
\begin{itemize}
	...
	\item \code{ompd_parallel_handle_t} type, see \specref{subsubsec:ompd_parallel_handle_t}.
	\item \code{ompd_rc_t} type, see \specref{subsubsec:ompd_rc_t}.
	\item \code{ompd_release_parallel_handle} call, see 
	\specref{subsubsubsec:ompd_release_parallel_handle}.
\end{itemize}
...
\plc{*enclosing_parallel_handle}. After the call the handle is owned by the tool, which must
...
A pointer to the parallel regions handle is returned in \plc{*task_parallel_handle}.
...
On success, \code{ompd_parallel_handle_compare} returns in \plc{*cmp_value} a signed
integer value that indicates how the underlying parallel regions compare: a value less than, equal
to, or greater than 0 indicates that the region corresponding to \plc{parallel_handle_1} is,
respectively, less than, equal to, or greater than that corresponding to \plc{parallel_handle_2}.
