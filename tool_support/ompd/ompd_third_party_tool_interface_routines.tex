...
%On success this should return \code{ompd\_rc\_ok}; \code{ompd\_rc\_bad\_input} indicates that 
%the 
%argument is invalid. Other errors could be reported by \code{ompd\_rc\_error}.
A descriptive string describing the OMPD library is returned by the function 
...
\summary
The  \code{ompd\_get\_top\_parallel\_region} operation enables the tool to obtain a pointer to 
...
ompd\_rc\_t ompd\_get\_task\_frame (
  ompd\_task\_handle\_t  *\plc{task\_handle}, 
  ompd\_address\_t  *\plc{exit\_runtime\_addr},
  ompd\_address\_t *\plc{reenter\_runtime\_addr}
);
\end{boxedcode}
\cspecificend
...
ompd\_rc\_t ompd\_get\_state (
  ompd\_thread\_handle\_t *\plc{thread\_handle},
  ompd\_state\_t *\plc{state},
  ompd\_wait\_id\_t    *\plc{wait\_id}
);
\end{boxedcode}
\cspecificend
...
%The input argument \plc{thread\_handle} is a thread handle. On return, the output argument
