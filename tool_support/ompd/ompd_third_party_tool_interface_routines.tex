...
Once loaded, the tool can determine the version of the OMPD API supported 
by the library by calling \code{ompd_get_api_version} (see
\specref{subsubsubsec:ompd_get_api_version}). If the tool supports the 
version returned by \code{ompd_get_api_version}, the tool starts the 
initialization by calling \code{ompd_initialize} (see
\specref{subsubsubsec:ompd_initialize}) using the version of the OMPD API 
supported by the library.  If the tool does not support the version returned 
by \code{ompd_get_api_version}, it may attempt to call \code{ompd_initialize} 
