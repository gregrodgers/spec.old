...
\descr
An OpenMP implementation  maintains an \code{ompt_frame_t} object for every implicit or explicit task.
For the task identified by \plc{task_handle}, \code{ompd_get_task_frame} extracts
the \plc{enter_frame} and \plc{exit_frame} fields of the task's \code{ompt_frame_t} object.
...
\code{ompt_frame_flag_t}. This argument specifies the meaning of the 
...
\begin{itemize}
	...
	\specref{subsubsubsec:ompd_frame_info_t}.
\end{itemize}
...
\code{ompt_state_undefined} as the value of the input argument \plc{current_state}.  Subsequent
