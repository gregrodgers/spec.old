...
The input argument \plc{parallel_handle} is an opaque handle for a parallel region
and selects the parallel region to operate on.
The input argument \code{thread_num} selects the implicit task of the team to be 
returned.
The selected implicit task would return \code{thread_num} from a call of the 
\code{omp_get_thread_num()} routine.
On return the output argument \plc{task_handle} is an opaque handle for the selected 
implicit task.

\restrictions
The value of \code{thread_num} must be a non-negative integer smaller than the
team size provided as the \plc{ompd-team-size-var} from \code{ompd_get_icv_from_scope}.
