...
\code{ompt_start_tool_result_t} structure contains pointers to tool initialization 
and finalization callbacks as well as a tool data word that an OpenMP implementation 
must pass by reference to these callbacks.

A tool may use the \plc{omp_version} argument to determine if it is compatible with 
the OMPT interface that the OpenMP implementation provides.

A tool may return \code{NULL} from \code{ompt_start_tool} to indicate that it
will not use the OMPT interface in a particular execution.
