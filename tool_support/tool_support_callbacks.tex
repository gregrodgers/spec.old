...
that are dispatched when a native thread is created.
...
that are dispatched when a native thread is destroyed.
...
that are dispatched when a \code{parallel} or \code{teams} region starts.
...
\summary
The \code{ompt_callback_parallel_begin_t} type is used for callbacks 
that are dispatched when a \code{parallel} or \code{teams} region ends.
...
\summary
The \code{ompt_callback_master} callback is used to dispatch
\plc{master-begin} and \plc{master-end} events.
...
\summary
The \code{ompt_callback_task_create} callback is used to dispatch
\plc{task-create} and \plc{initial-task-create} events.
...
The argument \plc{flags} indicates the kind of the task: initial,
explicit or target.
Values for \plc{flags} are a disjunction of elements in the enum
\code{ompt_task_flag_t}.

The argument \plc{has_dependences} indicates whether the created
...
\summary
The \code{ompt_callback_task_dependence} callback is used to dispatch
\plc{task-dependence} events.
...
\summary
The \code{ompt_callback_task_schedule} callback is used to dispatch
\plc{task-schedule} events.
...
This argument will be \code{NULL} if the callback is dispatched for a
\plc{task-fulfill} event.
...
\summary
The \code{ompt_callback_implicit_task} callback is used to dispatch
\plc{initial-task-begin}, \plc{initial-task-end}, \plc{implicit-task-begin}
and \plc{implicit-task-end} events.
...
For initial tasks, that are not closely nested in a \code{teams} 
construct, this argument is \code{1}.
...
The argument \plc{flags} indicates the kind of the task: initial or
implicit.
...
\summary
The \code{ompt_callback_sync_region} callback is used to dispatch
\plc{explicit-barrier-begin}, \plc{explicit-barrier-end},
\plc{implicit-barrier-begin}, \plc{implicit-barrier-end},
\plc{taskwait-begin}, \plc{taskwait-end},
\plc{taskgroup-begin}, \plc{taskgroup-end},
\plc{reduction-begin} and \plc{reduction-end} events.

The \code{ompt_callback_sync_region_wait} callback is used to dispatch
\plc{barrier-wait-begin}, \plc{barrier-wait-end},
\plc{implicit-barrier-wait-begin}, \plc{implicit-barrier-wait-end},
\plc{taskwait-wait-begin}, \plc{taskwait-wait-end},
\plc{taskgroup-wait-begin} and \plc{taskgroup-wait-end} events
...
\summary
The \code{ompt_callback_lock_init} callback is used to dispatch
\plc{lock-init} and \plc{nest-lock-init} events.

The \code{ompt_callback_mutex_acquire} callback is used to dispatch
\plc{critical-acquire}, \plc{atomic-acquire}, \plc{ordered-acquire},
\plc{lock-acquire}, \plc{nest-lock-acquire},
\plc{lock-test} and \plc{nest-lock-test} events.
...
\summary
The \code{ompt_callback_mutex_acquired} callback is used to dispatch
\plc{critical-acquired}, \plc{atomic-acquired},
\plc{ordered-acquired},  \plc{lock-acquired},
\plc{nest-lock-acquired}, \plc{lock-test-acquired} and
\plc{nest-lock-test-acquired} events.

The \code{ompt_callback_mutex_released} callback is used to dispatch
\plc{critical-release}, \plc{atomic-release},
\plc{ordered-release}, \plc{lock-release} and
\plc{nest-lock-release} events.

The \code{ompt_callback_lock_destroy} callback is used to dispatch
\plc{lock-destroy} and \plc{nest-lock-destroy} events.
...
\summary
The \code{ompt_callback_nest_lock} callback is used to dispatch
\plc{nest-lock-owned} and \plc{nest-lock-held} events.
...
\summary
The \code{ompt_callback_work} callback is used to dispatch
\plc{sections-begin}, \plc{sections-end},
\plc{single-begin}, \plc{single-end},
\plc{workshare-begin}, \plc{workshare-end},
\plc{ws-loop-begin}, \plc{ws-loop-end},
\plc{distribute-begin}, \plc{distribute-end},
\plc{taskloop-begin} and \plc{taskloop-end} events.
...
\summary
The \code{ompt_callback_flush} callback is used to dispatch
\plc{flush} events.
...
The binding of argument \plc{thread_data} is the executing thread.
...
\summary
The \code{ompt_callback_dispatch} callback is used to dispatch
\plc{section-begin}, \plc{taskloop-iteration-begin} and
\plc{ws-loop-iteration-begin} events.
...
\summary
The \code{ompt_callback_target} callback is used to dispatch
\plc{target-data-begin}, \plc{target-data-end},
\plc{target-enter-data-begin}, \plc{target-enter-data-end},
\plc{target-exit-begin}, \plc{target-exit-end},
\plc{target-begin}, \plc{target-end},
\plc{target-update-begin} and \plc{target-update-end} events.
...
The \code{ompt_callback_device_load} callback is used to dispatch
\plc{device-load} events.
...
A value of \code{ompt_addr_none} indicates that a virtual address in the file is not 
available.
...
A value of \code{ompt_addr_none} indicates that a host code address is not available.

The argument \plc{device_addr} indicates where the device code has been loaded in device memory.
A value of \code{ompt_addr_none} indicates that a device code address is not available.
...
The \code{ompt_callback_device_unload} callback is used to dispatch
\plc{device-unload} events.
...
\summary
The \code{ompt_callback_device_load} callback is used to dispatch
\plc{target-data-op}, \plc{target-data-associate}, \plc{target-data-disassociate},
\plc{target-data-allocation} and \plc{target-data-free} events.
...
\summary
The \code{ompt_callback_target_map} callback is used to dispatch
\plc{target-map} events.
...
\summary
The \code{ompt_callback_target_submit} callback is used to dispatch
\plc{target-submit} events.
...
\summary
The \code{ompt_callback_control_tool} callback is used to dispatch
\plc{tool-control} events.
...
\code{omp_control_tool_t} in Section~\ref{sec:control_tool}.
...
\summary
The \code{ompt_callback_cancel} callback is used to dispatch
\plc{cancellation}, \plc{cancel} and \plc{discarded-task} events.
...
The binding of argument \plc{task_data} is the task
...
The \code{ompt_callback_device_initialize} callback is used to dispatch
\plc{device-initialize} events.
...
The \code{ompt_callback_device_finalize} callback is used to dispatch
\plc{device-initialize} events.
