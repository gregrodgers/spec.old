...
\restrictions
The \code{ompt_start_too_result_t} type has the following restriction:
...
\plc{start_time} (\plc{end_time}), the value of the \plc{start_time} (\code{end_time})
...
\code{ompt_time_none} refers to an uknown or unspecified time and is is defined as 
...
\code{ompt_task_yield} indicates that the task encountered a \code{taskyield} 
construct. The value \code{ompt_task_cancel} indicates that the task was canceled 
when it encountered an active cancellation point. The value \code{ompt_task_detach} 
indicates the complete execution of a task with \code{detach} clause, if it is 
waiting for a \plc{completion-event} to finish. The value \code{ompt_task_fulfill} 
indicates that the \plc{allow-completion-event} of the task was fulfilled. The value 
\code{ompt_task_switch} is used for all other cases that a task was switched.
