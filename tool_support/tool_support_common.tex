...
\item \code{ompt_start_tool_result_t}, 
...
\item \code{ompt_enumerate_states_t}, 
...
\item \code{ompt_get_place_proc_ids_t}, 
...
\item \code{ompt_get_parallel_info_t}, 
...
%% Table~\ref{table:result_set} shows the possible return codes for
%The possible return codes for \code{ompt_set_callback} correspond to
%the \code{ompt_set_result_t} enumeration type. 
If the \code{ompt_set_callback} 
runtime entry point is called outside a tool initializer, registration of 
supported callbacks may fail with a return value of \code{ompt_set_error}.
...
\item \code{ompt_get_record_native}, 
see \specref{sec:ompt_get_record_native_t}.

\item \code{ompt_get_record_abstract}, 
...
\code{ompt_set_trace_native} runtime entry point return.
...
Only the \code{ompt_set_callback} runtime entry point can use the
return value \code{ompt_set_sometimes_paired}; only the 
\code{ompt_set_trace_ompt} and the \code{ompt_set_trace_native} runtime
 entry point can use the return value \code{ompt_set_impossible}.
