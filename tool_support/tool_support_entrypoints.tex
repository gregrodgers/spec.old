...
The \plc{etype} argument indicates the event or events to which the invocation
of \code{ompt_set_trace_ompt} applies. If the value of \plc{etype} is 0 then
the invocation applies to all events. If \plc{etype} is positive value then 
the invocation applies to the event in \code{ompt_callbacks_t} that matches 
that value.
...
a value of 0 for \plc{enable} indicates that recording of events should be disabled.
