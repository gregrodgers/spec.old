...
\crossreferences
\begin{itemize}
\item \code{ompt_state_t}, see \specref{sec:thread-states}.
\end{itemize}
...
%%%%% BRONIS:Alternative to using set_result table
% The return value of \code{ompt_set_callback}, which corresponds to a value of
% the \code{ompt_set_result_t} enumeration type, may indicate several possible 
...
outcomes, as Table~\ref{table:result_set} shows. Callback registration 
may fail if it is called outside the initializer for the callback interface, 
which is indicated by a return value of 0. Otherwise, the return value indicates 
if \emph{dispatching} a callback event leads to the invocation of the callback. 
A return value of 1 indicates that the callback will never be invoked at runtime. 
A return value of 2 indicates that the callback will be invoked at runtime for 
an implementation-defined subset of associated event occurrences. A return value 
of $3$ indicates the same result as a return value of 2, and, in addition, that 
a callback with an \plc{endpoint} value of \code{ompt_scope_begin} will be invoked 
if and only if the same callback with an \plc{endpoint} value of 
\code{ompt_scope_end} will also be invoked sometime in the future. A return value 
of 4 indicates that the callback will always be invoked when an associated 
event occurs.
...
\tablelasttail{\hline}
\tablecaption{Return Codes for \code{ompt_set_callback}\label{table:result_set}}
\begin{supertabular}{p{2.0in} p{3.0in}}
0 & Error\\
1 & Never invoked\\
2 & Invoked for a subset of occurences\\
3 & Invoked for a subset of occurences, pairedt\\
4 & Always inovked\\
\end{supertabular}
...
\crossreferences
\begin{itemize}
\item \code{omp_get_wtime}, see \specref{subsec:omp_get_wtime}.
...
\code{ompt_set_trace_ompt}. If the invocation of \code{ompt_set_trace_ompt} 
applies to all events then the return code is 3 if tracing is possible for 
one or more events but not for others.

\crossreferences
\begin{itemize}
\item \code{ompt_callbacks_t}, see \specref{sec:ompt_callbacks_t}.
...
values from type \code{ompt_native_mon_flag_t}. Table~\ref{table:record_set} 
shows the possible return codes for \code{ompt_set_trace_native}. If an 
invocation of \code{ompt_set_trace_native} is used to enable or to disable 
...
\item \code{ompt_buffer_cursor_t}, see \specref{sec:ompt_buffer_cursor_t}.
\end{itemize}
...
\crossreferences
\begin{itemize}
\item \code{ompt_record_t}, see \specref{sec:ompt_record_t}.

\item \code{ompt_buffer_t}, see \specref{sec:ompt_buffer_t}.
...
\crossreferences
\begin{itemize}
\item \code{ompt_record_ompt_t}, see \specref{sec:ompt_record_ompt_t}.
...
\crossreferences
\begin{itemize}
\item \code{ompt_record_abstract_t}, see \specref{sec:ompt_record_abstract_t}.
\end{itemize}
