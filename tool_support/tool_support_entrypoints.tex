...
\omptruntimeentrypoint{\code{ompt_enumerate_states_t}}
...
The \callbackarg{} \plc{current_state} must be a thread state
...
The \callbackarg{} \plc{next_state} is a pointer to an integer where
the entry point will return the value of the next state in the
enumeration.

The \callbackarg{} \plc{next_state_name} is a pointer to a
...
has previously been assigned to \code{ompt_enumerate_states_fn}.
...
\omptruntimeentrypoint{\code{ompt_enumerate_mutex_impls_t}}
...
employs.
The value \code{ompt_mutex_impl_unknown} is reserved to indicate an invalid
implementation.
...
The \callbackarg{} \plc{current_impl} must be a mutex implementation
...
tool should pass \code{ompt_mutex_impl_unknown} as the first
argument of the enumerate mutex kinds runtime entry point.  Subsequent
invocations of the runtime entry point by the tool should pass the
value assigned to the variable passed by reference as the second
argument to the previous call.

The \callbackarg{} \plc{next_impl} is a pointer to an integer where
the entry point will return the value of the next mutex implementation
in the enumeration.

The \callbackarg{} \plc{next_impl_name} is a pointer to a character
...
has previously been assigned to \code{ompt_enumerate_mutex_impls_fn}.
\vspace{2ex}
...
\omptruntimeentrypoint{\code{ompt_set_callback_t}}
...
\omptruntimeentrypoint{\code{ompt_get_callback_t}}
...
The \callbackarg{} \plc{callback} returns a pointer to the callback being inspected.
...
\omptruntimeentrypoint{\code{ompt_get_thread_data_t}}
...
\omptruntimeentrypoint{\code{ompt_get_num_procs_t}}
...
\omptruntimeentrypoint{\code{ompt_get_num_places_t}}
...
\omptruntimeentrypoint{\code{ompt_get_place_proc_ids_t}}
...
The \callbackarg{} \plc{place_num} specifies the place being
queried.

The \callbackarg{} \plc{ids_size} indicates the size of the result
...
\omptruntimeentrypoint{\code{ompt_get_place_num_t}}
...
\omptruntimeentrypoint{\code{ompt_get_partition_place_nums_t}}
...
The \callbackarg{} \plc{place_nums_size} indicates the size of the result
array specified by \callbackarg{} \plc{place_nums}.

The \callbackarg{} \plc{place_nums} is an array where the routine can return
...
\omptruntimeentrypoint{\code{ompt_get_proc_id_t}}
...
\omptruntimeentrypoint{\code{ompt_get_state_t}}
...
The \callbackarg{} \plc{wait_id} is a pointer to an opaque handle
...
\omptruntimeentrypoint{\code{ompt_get_parallel_info_t}}
...
The \callbackarg{} \plc{ancestor_level} specifies the parallel region
...
\constraints
While \callbackarg{} \plc{ancestor_level} is passed by
...
\omptruntimeentrypoint{\code{ompt_get_task_info_t}}
...
The \callbackarg{} \plc{ancestor_level} specifies the task region
of interest to a tool by its ancestor level.  Ancestor level 0 refers
to the active task; information about
ancestor tasks found in the current execution context may be queried at
higher ancestor levels.

The \callbackarg{} \plc{type} returns the task type 
if the argument is not \code{NULL}.

The \callbackarg{} \plc{task_data} returns the task data
if the argument is not \code{NULL}.

The \callbackarg{} \plc{task_frame} returns the task frame
pointer 
if the argument is not \code{NULL}.

The \callbackarg{} \plc{parallel_data} returns the parallel data
if the argument is not \code{NULL}.

The \callbackarg{} \plc{thread_num} returns the thread number
if the argument is not \code{NULL}.
...
\omptruntimeentrypoint{\code{ompt_get_target_info_t}}
...
\omptruntimeentrypoint{\code{ompt_get_num_devices_t}}
...
\omptruntimeentrypoint{\code{ompt_get_unique_id_t}}
...
\omptruntimeentrypoint{\code{ompt_get_device_num_procs_t}}
...
\omptruntimeentrypoint{\code{ompt_get_device_time_t}}
...
\omptruntimeentrypoint{\code{ompt_translate_time_t}}
...
\omptruntimeentrypoint{\code{ompt_set_trace_ompt_t}}
...
\omptruntimeentrypoint{\code{ompt_set_trace_native_t}}
...
\omptruntimeentrypoint{\code{ompt_start_trace_t}}
...
\omptruntimeentrypoint{\code{ompt_pause_trace_t}}
...
The \callbackarg{} \plc{begin_pause} indicates whether to pause or
resume tracing.
...
\omptruntimeentrypoint{\code{ompt_stop_trace_t}}
...
\omptruntimeentrypoint{\code{ompt_advance_buffer_cursor_t}}
...
The \callbackarg{} \plc{next} returns the next value of a opaque buffer cursor.
...
\omptruntimeentrypoint{\code{ompt_get_record_type_t}}
...
\omptruntimeentrypoint{\code{ompt_get_record_ompt_t}}
...
\omptruntimeentrypoint{\code{ompt_get_record_native_t}}
...
The \callbackarg{} \plc{host_op_id} is a pointer to an identifier
...
\omptruntimeentrypoint{\code{ompt_get_record_abstract_t}}
...
The \callbackarg{} \plc{native_record} is a pointer to a native trace
record.
...
\omptruntimeentrypoint{\code{ompt_function_lookup_t}}
...
\argdesc
The \callbackarg{} \plc{interface_function_name} is a C string
